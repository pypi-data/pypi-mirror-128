...
class CommunityClient(AminoHttpClient):
    def __init__(self, comId: int, session: ClientSession = None, 
    ...
    async def upload_media(self, file: BinaryIO, fileType: str) -> str:
        response = await self.post_content(f"/x{self.comId}/s/media/upload", file.read(), fileType)
    ...
    async def get_online_users(self, start: int = 0, size: int = 25):
        ...
        return (await response.json())["userProfileList"]
