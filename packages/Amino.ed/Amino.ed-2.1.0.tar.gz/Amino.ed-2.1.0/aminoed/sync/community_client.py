...
class CommunityClient(AminoHttpClient):
    def __init__(self, comId: int, session: Session = None, 
    ...
    def __aenter__(self) -> "CommunityClient":
        return self
    ...
    def upload_media(self, file: BinaryIO, fileType: str) -> str:
        response = self.post_content(f"/x{self.comId}/s/media/upload", file.read(), fileType)
    ...
    def edit_profile(self, nickname: str = None, content: str = None, icon: BinaryIO = None, chatRequestPrivilege: str = None, 
    ...
    def edit_chat(self, chatId: str, doNotDisturb: bool = None, pinChat: bool = None, 
        ...
        if backgroundImage:
            ...
            response = self.post(f"/x{self.comId}/s/chat/thread/{chatId}/member/{self.userId}/background", data)
            responses.append(response.status_code)
        
        if pinChat is True: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/pin", data)).status_code)
        if pinChat is False: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/unpin", data)).status_code)

        if viewOnly is True: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/view-only/enable", data)).status_code)
        if viewOnly is False: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/view-only/disable", data)).status_code)

        if canInvite is True: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/members-can-invite/enable", data)).status_code)
        if canInvite is False: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/members-can-invite/disable", data)).status_code)

        if canTip is True: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/tipping-perm-status/enable", data)).status_code)
        if canTip is False: responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}/tipping-perm-status/disable", data)).status_code)

        responses.append((self.post(f"/x{self.comId}/s/chat/thread/{chatId}", data)).status_code)
    ...
    def get_chat_thread(self, chatId: str) -> Thread:
        ...
        return Thread(loads(response.json())["thread"])

    def get_chat_messages(self, chatId: str, size: int = 25, pageToken: str = None) -> GetMessages:
    ...
    def get_recent_blogs(self, pageToken: str = None, start: int = 0, size: int = 25) -> GetBlogs:
    ...
    def get_live_layer(self) -> dict:
        ...
        return (response.text())["liveLayerList"]
    ...
    def generate_chat_bubble(self, bubble: BinaryIO = None, templateId: str = "949156e1-cc43-49f0-b9cf-3bbbb606ad6e") -> ChatBubble:
        response = self.post(f"/x{self.comId}/s/chat/chat-bubble/templates/{templateId}/generate", bubble)
        return ChatBubble(**(response.text)["chatBubble"])
    
    def edit_chat_bubble(self, bubbleId: str, bubble: BinaryIO) -> ChatBubble:
        response = self.post(f"/x{self.comId}/s/chat/chat-bubble/{bubbleId}", bubble)
        return ChatBubble(**(response.text)["chatBubble"])
