...
class SystemTests(unittest.TestCase):
    def test_mod_varx(self):
        assert(vc.check_vars('d',{"a":1, "b":2},modname=tmod,output=False))

    def test_mod_varx(self):
        assert(vc.check_vars('x',sp.symbols("x"),modname=tmod,output=False))

    def test_mod_vary(self):
        assert(vc.check_vars('y',sp.Array([1,2,sp.symbols("x")]),modname=tmod,output=False))

    def test_mod_varz(self):
        assert(vc.check_vars('z',sp.Matrix([[1,2,3],[1,3,2],[3,1,2]]),modname=tmod,output=False))
