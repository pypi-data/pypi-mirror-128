...
import logging
import os
# import six (commented out line 54,55,56 in Command class - __init__)
import socket
import subprocess
import time
import warnings
...
class Emulator(object):
    ...
    def __init__(self, visible=False, timeout=120, app=None, _sp=None):
        """
    ...
    def send_string(self, tosend, ypos=None, xpos=None):
        ...
        # throws AttributeError: 'str' object has no attribute 'decode'
    ...
    def move_up(self):
        self.exec_command(b'Up')

    def move_down(self):
        self.exec_command(b'Down')

    def move_left(self):
        self.exec_command(b'Left')

    def move_right(self):
        self.exec_command(b'Right')

    def clear_keyboard_lock(self):
        self.exec_command(b'Reset')

    def backspace(self):
        self.exec_command(b'BackSpace')

    def backtab(self):
        self.exec_command(b'BackTab')

    def wc3270_terminal(self):
        self.exec_command(b'Escape')

    def home(self):
        """
            Move's cursor to the first field of current screen
        """
        self.exec_command(b'Home')

    def show_keypad(self):
        self.exec_command(b'Keypad')

    def execute_macro(self, macro):
        self.exec_command(b'Macro({0})'.format(macro))

    def newline(self):
        """
            Move cursor to first field in next row
        """
        self.exec_command(b'Newline')

    def next_word(self):
        """
            Move cursor to next word
        """
        self.exec_command(b'NextWord')

    def pause(self, pauseTime):
        """
            Set to zero to use the termial pause command
            Wait for 350ms
        """
        #set to zero to use the termial pause command
        if pauseTime == 0:
    ...
    def previous_word(self):
        ...
        """
        self.exec_command(b'PreviousWord')
    ...
    def print_text(self):
        ...
        """
        self.exec_command(b'ignore')
        #self.exec_command(b' PrintText([html|rtf,][modi,][caption,<caption>,][replace|append,]file,<filename>)')

    def show_status(self):
        self.exec_command(b'Show(Status)')

    def read_file(self, fileName):
        """
            Read actions from file
        """
        self.exec_command(b'Source({0})'.format(fileName))
    ...
    def gateway(self, xpos, ypos, text):
        while not self.string_found(xpos, ypos, text):
            time.sleep(gate)