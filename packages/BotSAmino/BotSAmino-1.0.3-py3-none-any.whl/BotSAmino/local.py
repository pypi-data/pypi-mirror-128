...
class Local:
    ...
    def join_chat(self, chatId: str = None, asWeb: bool = False):
        if asWeb:
            data = {"ndcId": f"x{self.comId}", "threadId": chatId}
    ...
    def get_wall_comments(self, userId: str, sorting: str, start: int = 0, size: int = 25):
        ...
        else: raise TypeError("Errorر")  # Not me typed this its (a7rf)
    ...
    def user_active_time(self):
        try:
            req = requests.post("https://aminoapps.com/api/community/stats/web-user-active-time", json={"ndcId": self.comId}, headers=self.web_headers)
            try:
                if req.json()["code"] != 200: return CheckExceptions(req.json())
                else: Json(req.json())
            except: return CheckExceptions(req.json())
    ...
    def send_active_obj(self, startTime: int = None, endTime: int = None, optInAdsFlags: int = 2147483647, tz: int = -timezone // 1000, timers: list = None, timestamp: int = int(timestamp() * 1000)):
        ...
        sig = base64.b64encode(b"\x22" + hmac.new(bytes.fromhex("307c3c8cd389e69dc298d951341f88419a8377f4"), data.encode(), sha1).digest()).decode()
        headers = self.headers
        headers["NDC-MSG-SIG"] = sig
        req = requests.post(f"{self._api}/x{self.comId}/s/community/stats/user-active-time", headers=headers, data=data)
        if "OK" not in req.json()["api:message"]: return CheckExceptions(req.json())
