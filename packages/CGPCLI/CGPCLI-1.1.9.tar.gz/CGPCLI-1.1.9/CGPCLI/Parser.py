...
def parse_to_python_object(string):
    ...
    def parse_date(src):
        ...
        if match is not None:
            ...
            yield datetime.strptime(date, '#T%d-%m-%Y_%H:%M:%S'), src.lstrip()

    xml_regex = re.compile(r"(<.+?>.*</.+?>)(.*)", re.DOTALL)
    
    def parse_xml(src):
        match = xml_regex.match(src)
        if match is not None:
            xml, src = match.groups()
            yield ET.fromstring(xml), src.lstrip()
    ...
    def parse_datablock(src):
        ...
        if match is not None:
            ...
            yield sert, src.lstrip()
    
    password_regex = re.compile(r"\"?Password\"?\s*=\s*\"(.*?)\";(?=\s*(?:}|\"?[a-zA-Z0-9]))(.*)", re.DOTALL)        

    def parse_password(src):
        match = password_regex.match(src)
        if match is not None:
            pwd, src = match.groups()
            yield {'Password': pwd}, src.lstrip()
    
    quoted_regex = re.compile(r"\"(.*?(?<!\\))\"(.*)", re.DOTALL)
    unquoted_regex = re.compile(r"([\w.@#-]+)(.*)", re.DOTALL)
    multistring_regex = re.compile(r"\"(.*?)\"((?:\s+\"(?:.*?)\"){1,})(.*)", re.DOTALL)

    def parse_string(src):
        match = multistring_regex.match(src)
        if match is not None:
            string, mstrings,  src = match.groups()
            while mstrings:
                for s, mstrings in parse_string(mstrings.lstrip()):
                  string += s
                  break
