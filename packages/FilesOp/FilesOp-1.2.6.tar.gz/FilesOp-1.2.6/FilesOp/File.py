...
class FFile:
	...
	def append_json(self, content, to_key) -> None:
		...
		if self.__file_exists():
			self.read_json()
			lista = list()
			if to_key in self.json_:
				lista.append(self.json_[to_key])
			else:
				lista = []
			lista.append(content)
			self.json_[to_key] = lista
			with open(self.nomeFile, "w") as fa:
				fa.write(json.dumps(self.json_))
		else:
			self.create_file(json.dumps({to_key: content}))

if __name__ == "__main__":
	test = FFile("prova.json")