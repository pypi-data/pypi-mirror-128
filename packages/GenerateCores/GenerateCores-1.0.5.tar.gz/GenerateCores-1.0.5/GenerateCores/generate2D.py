...
def is_separable(outcat1, outcat2):
...
def make_2Dclumps(n, path1, header_path=None, size_y=100, size_x=100, number=10000):
    ...
    path2 = os.path.join(path1, '2D_nClumps_%03d' % n)  # 次级目录
    ...
    while 1:
        ...
        while i < n:
            ...
            if i != 0:  # 从数据块中加入第二个核开始，判断当前的核能否加入到数据块
                ...
                outcat2 = np.array(
                    [[x0, y0, size1, size2]])
                is_join = is_separable(outcat1, outcat2)
