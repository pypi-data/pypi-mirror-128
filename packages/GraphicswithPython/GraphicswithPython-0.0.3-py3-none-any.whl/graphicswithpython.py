...
def midpointcircle(radius:int, xcenter:int, ycenter:int, midpointtype:str, color:tuple):
	...
	while (x < y):
		...
		if selectedType == 5:
			dottedandline(xcenter, ycenter,x,y)
...
def dottedandline(xcenter, ycenter, x, y):
	for i in range(0, x):
		if i%2 == 1:
			...
			putpixel( xcenter+x, ycenter+y, color("white"))
			putpixel( xcenter+y, ycenter+x, color("white"))
			putpixel( xcenter-y, ycenter-x, color("white"))
			putpixel( xcenter-x, ycenter-y, color("white"))
