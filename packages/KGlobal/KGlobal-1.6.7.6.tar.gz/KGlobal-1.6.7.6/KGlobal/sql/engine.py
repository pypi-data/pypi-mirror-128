...
class SQLEngineClass(BaseSQLEngineClass, PickleMixIn):
    ...
    def connect(self, test_conn=False):
        ...
        if self.engine_type == 'alchemy':
            ...
            engine = None
            connect_args = {'timeout': self.__conn_timeout, 'connect_timeout': self.__conn_timeout,
                            'options': '-c statement_timeout=%s' % self.__query_timeout}

            try:
                engine = create_engine2(
                    self.__sql_config.gen_conn_str(), connect_args=connect_args,
                    autocommit=self.__sql_config.autocommit)
                engine.commit()
            except Error as e:
                close_engine(engine)
        ...
        if test_conn:
            ...
            if not engine:
                raise ValueError("'engine' failed to create connection {%s}" % self.__sql_config.gen_conn_str())

            conn_engine, spid = self.__validate_engine(engine)
    ...
    def stream_execute(self, buffer=1000):
        ...
        :var kwargs: kwargs from sql_execute command
        """
    ...
    def sql_execute(self, query_str, execute=False, queue_cursor=False, new_engine=False, file_path=None,
                    file_replace=False, file_params={}, **kwargs):
        """
        ...
        :param file_path: File path where data is stored at for csv file
        :param file_replace: (Optional) [True/False] Replace csv file if exists
        ...
        if new_engine:
            return self.__execute_sql(engine, spid, keep_engine_alive, query_str, execute, queue_cursor, file_path,
                                      file_replace, file_params, **kwargs)
        else:
    ...
    def __execute_sql(self, engine, spid, keep_engine_alive, query_str, execute=False, queue_cursor=False,
                      file_path=None, file_replace=False, file_params={}, **kwargs):
        from ..sql.cursor import SQLCursor

        if queue_cursor:
            params = dict(query_str=query_str, execute=execute, handlers=self.__sql_handlers, file_path=file_path,
                          file_replace=file_replace, file_params=file_params)
            ...
            for handler, buffer in self.__sql_handlers:
                ...
                try:
                    ...
                    cursor.execute(query_str=query_str, execute=execute, handler=handler, buffer=buffer,
                                   file_path=file_path, file_replace=file_replace, file_params=file_params, **kwargs)
                    cursor.join()
            ...
            try:
                cursor.start()
                cursor.execute(query_str=query_str, execute=execute, file_path=file_path, file_replace=file_replace,
                               file_params=file_params, **kwargs)
                cursor.join()
            except:
                cursor.close()
                log.debug(format_exc())
