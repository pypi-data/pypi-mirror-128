    ...
    def generate_mini_map(self, m: KLMap) -> QPixmap:
        ...
        for y in range(m.h):
            for x in range(m.w):
                ...
                p.fillRect(
                    x * MINI_TILE_SIZE, y * MINI_TILE_SIZE, MINI_TILE_SIZE - 1, MINI_TILE_SIZE - 1, QBrush(color)
                )
                ...
                if rpid == pid:
                    p.drawLine(
                        (x + 1) * MINI_TILE_SIZE - 1,
                        y * MINI_TILE_SIZE,
                        (x + 1) * MINI_TILE_SIZE - 1,
                        (y + 1) * MINI_TILE_SIZE - 2,
                    )
                if dpid == pid:
                    p.drawLine(
                        x * MINI_TILE_SIZE,
                        (y + 1) * MINI_TILE_SIZE - 1,
                        (x + 1) * MINI_TILE_SIZE - 2,
                        (y + 1) * MINI_TILE_SIZE - 1,
                    )
                if rdpid == pid:
