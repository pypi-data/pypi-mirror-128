...
def range_check(
    ...
    :returns: A CheckResult object, containing tree_data key/values:
      * histogram: filled 1D histogram of the quantity defined by the expression parameter
...
def range_check_nd(
    ...
    if length_expr < 2 or length_expr > 4:
...
def range_check_bkg_subtracted(
    ...
    :returns: A CheckResult object, containing tree_data key/values:
      * histograms: a list of filled 1D histograms
                    The first is the control variable used to perform the subtraction
                    The second is the background-subtracted result
      * hist_signal: filled 1D histogram of events in the signal window
      * hist_background: filled 1D histogram of events in the background window
    """
        ...
        with uproot.open(filepath) as f:
            for key, obj in f.items(cycle=False):
                ...
                if key not in result.tree_data:
                    ...
                    result.tree_data[key]["hist_signal"] = Hist(axis1)
                    result.tree_data[key]["hist_background"] = Hist(axis1)
                ...
                result.tree_data[key]["hist_signal"].fill(test_array_sig)
                result.tree_data[key]["hist_background"].fill(test_array_bkg)
    ...
    if result.passed:
        for key in result.tree_data:
            ...
            if (result.tree_data[key]["hist_signal"].sum() == 0) or (
                result.tree_data[key]["hist_background"].sum() == 0
            ):
                result.passed = False
                result.messages += [
            ...
            hsub = (
                result.tree_data[key]["hist_signal"]
                + (-1 * alpha) * result.tree_data[key]["hist_background"]
            )
