...
class Maceio():
    ...
    def __init__(self, engineText, schema, pool_size=5, max_overflow=2, echo=False):
        """
        Constructor method
        ...
        self.__type = {
            ...
            'float': Float,
            'bool': Boolean,
            'dict': JSON if 'postgresql' in self.engineText else Text,
            'tuple': JSON if 'postgresql' in self.engineText else Text,
            'list': JSON if 'postgresql' in self.engineText else Text,
        }
    ...
    def save(self, table, data, conflicts=[], verify=True, primaries=[]):
        ...
        if isinstance(data, list):
            ...
            for item in data:
                cols, cNameCols, element = self.__generateColumnsAndData(item, primaries=primaries)

                columns.extend(cols)
                nameCols.extend(cNameCols)
                elements.append(element)
        else:
            if len(data.keys()) == 0:
                raise Exception('Não há valores a serem inseridos.')

            columns, nameCols, element = self.__generateColumnsAndData(data, primaries=primaries)
            elements.append(element)
    ...
    def __insert(self, table, data, name_index_unique=None):
        """
        ...
        try:
            ...
            for d in data:
                try:
                    ...
                    updateFields = self.__generateUpdateConflicts(d, insert_stmt)
    ...
    def __addTable(self, table, columns, conflicts, verify=True, primaries=[]):
        ...
        try:
            if primaries:
                table = Table(table, self.meta, *columns)
            else:
    ...
    def __generateColumnsAndData(self, data, nodes=3, level=1, parentName=None, primaries=None):
        """
            ...
            if type(e[1]).__name__ == 'dict' or self.__testJson(e[1]):
                if level <= nodes:
                    if self.__testJson(e[1]):
                        ...
                        nameCols.append(name)
                        dataDict.update({name: json.loads(e[1])})
                    else:
                        cols, cNameCols, datas = self.__generateColumnsAndData(
                            e[1], nodes, level + 1, name, primaries)
                ...
                value = e[1]
                if type(value).__name__ == 'list' and 'postgresql' not in self.engineText:
