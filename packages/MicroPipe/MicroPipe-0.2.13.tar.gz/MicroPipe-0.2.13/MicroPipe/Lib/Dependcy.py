...
def Prokka_Commandline( Contig,Genius,Spieces,Strain,Center,Prefix,OutPut,Plasmid,Evalue,Gencode=0 ):
	raw_commandline =   config_hash["Tools"]["prokka"]+ ' --rfam  --prefix %(Prefix)s --outdir %(output)s --evalue %(e_value)s  --genus %(Genius)s --strain %(Strain)s  --cpus 64 --compliant --force --quiet --centre %(Centre)s   --locustag %(Prefix)s --rnammer --gcode %(Code)s'%(
	    {
	        "Centre":Center,
	        "Prefix":Prefix,
	        "output":OutPut,
