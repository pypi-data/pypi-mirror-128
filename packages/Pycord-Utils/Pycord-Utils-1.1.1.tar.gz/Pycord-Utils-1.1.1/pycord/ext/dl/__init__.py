...
def _real_main(argv=None):
    ...
    if opts.batchfile is not None:
        try:
            if opts.batchfile == "-":
                ...
                batchfd = io.open(
                    expand_path(opts.batchfile), "r", encoding="utf-8", errors="ignore"
                )
    ...
    if opts.list_extractors:
        for ie in list_extractors(opts.age_limit):
            write_string(
                ie.IE_NAME + (" (CURRENTLY BROKEN)" if not ie._WORKING else "") + "\n",
                out=sys.stdout,
            )
    ...
    if opts.metafromtitle:
        postprocessors.append(
            {"key": "MetadataFromTitle", "titleformat": opts.metafromtitle}
        )
    if opts.extractaudio:
        postprocessors.append(
            {
