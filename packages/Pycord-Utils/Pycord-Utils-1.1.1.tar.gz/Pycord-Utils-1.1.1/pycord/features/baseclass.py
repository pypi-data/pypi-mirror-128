...
class Feature(commands.Cog):
    """
    Baseclass defining feature components of the pycord cog.
    """
    ...
    async def cog_check(
        ...
        if not await ctx.bot.is_owner(ctx.author):
            raise commands.NotOwner("You must own this bot to use pycord.")
        return True

    @contextlib.contextmanager
    def submit(self, ctx: commands.Context):
        """
        A context-manager that submits the current task to pycord's task list
