...
"""
pycord.features.filesystem
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The pycord filesystem-related commands.
...
from pycord.hljs import get_language, guess_file_traits
from pycord.paginators import PaginatorInterface, WrappedFilePaginator, use_file_check
...
class FilesystemFeature(Feature):
    ...
    @Feature.Command(parent="pyc", name="cat")
    async def pyc_cat(
    ...
    @Feature.Command(parent="pyc", name="curl")
    async def pyc_curl(self, ctx: commands.Context, url: str):
        """
        Download and display a text file from the internet.

        This command is similar to pyc cat, but accepts a URL.
        """
