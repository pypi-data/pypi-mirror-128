...
"""
pycord.features.invocation
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The pycord command invocation related commands.
...
class InvocationFeature(Feature):
        ...
        for override in overrides:
            if isinstance(override, discord.User):
                ...
                if ctx.guild:
                    ...
                    #  the command more compatible with chaining, e.g. `pyc in .. pyc su ..`
                    target_member = None
    ...
    async def pyc_repeat(
        ...
        You can use this in conjunction with `pyc sudo` to bypass this.
        """
    ...
    @Feature.Command(parent="pyc", name="debug", aliases=["dbg"])
    async def pyc_debug(self, ctx: commands.Context, *, command_string: str):
        """
