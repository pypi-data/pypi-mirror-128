...
"""
pycord.features.python
~~~~~~~~~~~~~~~~~~~~~~~~

The pycord Python evaluation/execution commands.
...
from pycord.codeblocks import codeblock_converter
...
class PythonFeature(Feature):
    ...
    @Feature.Command(parent="pyc", name="retain")
    async def pyc_retain(self, ctx: commands.Context, *, toggle: bool = None):
        """
    ...
    async def pyc_python_result_handling(
        self, ctx: commands.Context, result
    ):  # pylint: disable=too-many-return-statements
        """
        Determines what is done with a result when it comes out of pyc py.
    ...
    @Feature.Command(parent="pyc", name="py", aliases=["python"])
    async def pyc_python(self, ctx: commands.Context, *, argument: codeblock_converter):
        """
            ...
            async with ReplResponseReactor(ctx.message):
                with self.submit(ctx):
                    ...
                    async for send, result in AsyncSender(executor):
                        ...
                        send(await self.pyc_python_result_handling(ctx, result))
    ...
    @Feature.Command(parent="pyc", name="dis", aliases=["disassemble"])
    async def pyc_disassemble(
        self, ctx: commands.Context, *, argument: codeblock_converter
    ):
        """
