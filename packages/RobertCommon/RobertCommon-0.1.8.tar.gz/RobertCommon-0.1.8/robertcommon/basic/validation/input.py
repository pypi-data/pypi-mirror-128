...
def _check_in(name: str, value: In_T, error_type: type,
    ...
    if value not in allowed_values:
        raise error_type(f"{_escape(name)} is not valid.")
    return value
...
def _check_type(name: str, value: In_T, error_type: type,
    ...
    try:
        if not isinstance(value, allowed_types):
            raise error_type(
                f"{_escape(name)} is not valid {allowed_types}: {value}.")
    except TypeError:
        raise error_type(f"allowed_types is not a valid type "
                         f"or tuple of types: {allowed_types}")
    return value
