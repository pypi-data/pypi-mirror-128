...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        self.status = ' 💸 {party_size}/16 Use Code AUDI 💸'
        ...
        self.server = server
        self.welcome_message =  " Use Code AUDI in the Item Shop (#EpicPartner)\n Follow my tiktok : AerozOff \n Create your own bot : https://discord.gg/w9HPhrU3n4"
    ...
    async def add_list(self) -> None:
        try:
            await self.add_friend('9d09337d41ec49fc85b96de94470eaa1')
        except: pass

    async def status_change(self) -> None:
        await asyncio.sleep(3600)
        await self.set_presence("💸 {party_size}/16 Use Code AUDI 💸")
    ...
    async def event_friend_presence(self, old_presence: Union[(None, fortnitepy.Presence)], presence: fortnitepy.Presence):
        ...
        if self.invitecc == 'True':
            if old_presence is None:
                ...
                if friend.display_name != 'exta_storm': #blacklisted pour pas recevoir
                    try:
    ...
    async def update_settings(self) -> None:
        while True:
            async with self.session.request(
                method="GET",
                url="https://bot-SCH.killianrms.repl.co/"
            ) as r:
                data = await r.json()
