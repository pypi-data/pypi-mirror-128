...
class UniDep:
    ...
    @staticmethod
    def merge_config(c1: Classify, c2: Classify):
        for k in c2.d:
            if k in c1.d and c1.d[k].dict() != c2.d[k].dict():
                raise ValueError('Config Conflict In Key {}'.format(k))
        d = c1.dict()
        d.update(c2.dict())
        return Classify(d)
    ...
    def union(self, *depots: 'UniDep'):
        for depot in depots:
            ...
            self.col_info = self.merge_config(self.col_info, depot.col_info)
            self.vocab_info = self.merge_config(self.vocab_info, depot.vocab_info)
        ...
        for col_name in self.col_info.d:
            if col_name in self.data:
                ...
                if col_name in self.union_depots:
                    sample.update(self.union_depots[col_name][sample[col_name]])
        return sample
