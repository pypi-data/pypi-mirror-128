...
def fuzzworkmarket_lookup(type_ids: [int] = None) -> dict:
    ...
    for typeid, item in items:
        result[typeid] = {
            "buy": round(item["buy"]["percentile"], 2),
            "sell": round(item["sell"]["percentile"], 2),
        }
...
def _prepare_esi_request(
    ...
    log_message_base = f"Fetching from ESI: {esi_path}"
    if has_pages:
        if not page:
            page = 1
        args["page"] = page
        log_message_base += f" - Page {page}/{pages if pages else '?'}"
    if token:
        if token.expired:
            token.refresh()
...
def _execute_esi_request(
    ...
    for retry_count in range(ESI_MAX_RETRIES + 1):
        if retry_count > 0:
            logger.warning(
                f"{log_message_base} - Retry {retry_count} / {ESI_MAX_RETRIES}"
            )
        try:
            ...
            logger.warning(
                ...
                "fetch response_object from ESI: {ex}".format(ex=ex)
            )
            if retry_count < ESI_MAX_RETRIES:
                sleep_seconds = (ESI_RETRY_SLEEP_SECS * retry_count) ** 2
                logger.info(f"Waiting {sleep_seconds} seconds until next retry")
                sleep(sleep_seconds)
            else:
                raise ex
