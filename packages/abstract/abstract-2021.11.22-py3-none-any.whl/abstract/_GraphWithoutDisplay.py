...
class GraphWithoutDisplay(BasicGraph):
	...
	def stylize(self):
		if self._global_node_style_overwrite is not None:
			for node in self.nodes:
				node.style = self._global_node_style_overwrite
		...
		if self._global_node_style_overwrite is not None:
			for node in self.nodes:
				node.style.complement(self._global_node_style_overwrite)
	...
	def __graph__(self):
		...
		return {
			...
			'graph_node_style': self._global_node_style_overwrite,
			'graph_edge_style': self._global_edge_style_overwrite,
