...
class PreprocessorBase(BaseClass):
    ...
    this is the way that ``add_preprocess`` method works:
    ...
    def add_image_preprocess(self, generator):
        ...
        Notes:
            Note that you have to do preprocessing on image (``x``), keep label-preprocessing (if any) for ``self.add_label_preprocess``.
            Note that you have to use exactly the same logic that you define in ``self.image_preprocess`` method. Consider using ``map`` method for ``tf.data.Dataset``, or {``map``function/``for`` loop} for vanilla ``Python generator``.

        Args:
            generator: a ``Python generator``/``tf.data.Dataset`` which yields a single data-point ``(x, y, sample_weight)`` or ``(x, y, data_id)`` if it is ``test_data_generator``
    ...
    def batchify(self, generator, n_data_points):
        ...
        Notes:
            Note that you have to return ``n_iter`` in a correct way, consider the last batch of different size if ``batch_size`` does not fit into ``n_data_points``.
        ...
        Returns:
            batched_generator: A repeated ``generator``/``tf.data.Dataset`` which yields a batch of data for each iteration: ``(x_batch, y_batch, sample_weights)`` or ``(x_batch, y_batch, data_ids)`` for test data gen.
                shapes:
