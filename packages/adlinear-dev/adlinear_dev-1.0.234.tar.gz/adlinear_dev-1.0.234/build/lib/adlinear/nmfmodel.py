...
def scree_plot(mat: Union[np.ndarray, pd.DataFrame],
               ncmin: int, ncmax: int):
    columns = ["Error", "Volume", "Err_Vol_Ratio", "Stability"]
    ncomp = ncmin
    ...
    for ncomp in range(ncmin, ncmax+1):
        ...
        line.loc[ncomp, "Error"] = nmfmodel.get_precision()
        line.loc[ncomp, "Volume"] = nmfmodel.get_volume()
        line.loc[ncomp, "Err_Vol_Ratio"] = line.loc[ncomp, "Error"] / line.loc[ncomp, "Volume"]
        line.loc[ncomp, "Stability"] = nmfmodel.get_stability()
