...
def generate_vectors_bouquet(vpivot, bouquet_size: int, min_corr: float, positive: bool = True) -> pd.DataFrame:

    vpivot = pd.DataFrame(vpivot)
    df_bouquet = pd.DataFrame(index=vpivot.index, columns=range(bouquet_size))
...
def generate_clusters_centroids(ndim: int, max_corr: float):
