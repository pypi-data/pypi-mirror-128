...
def get_release_list(base_url, mac_id, mac_token, host, offset=30):
            ...
            with open(file) as parse_file:
                for line in parse_file:
                    if not "ERRORS" in line:
                        ...
                        print(f'Error line found in {file} ')
...
@main.command()
@click.option('--item', '-i', default='client', type=click.Choice(['logs', 'plugins', 'signed']))
def validate(item):
    ...
    if item == "logs":
        ...
        excel_log_file_name = os.path.expanduser('~\\Desktop\\install_log_history.xlsx')
        html_log_filename = os.path.expanduser('~\\Desktop\\install_log_history.html')

        log_df.to_html(html_log_filename, header=True)
        log_df.to_excel(excel_log_file_name, header=True)
    else:
        print(f'The {item} validation is not yet implemented')
