...
class Consumer(threading.Thread):
    ...
    def _run_message_callback(self, delivery_tag, body, properties):
        try:
        ...
        except BaseDeterministicError as e:
            logger.info('Deterministic error %s', e)
            ...
            self._connection.ioloop.add_callback_threadsafe(cb)
        except BaseEnvironmentalError as e:
            logger.info('Environmental error %s', e)
            logger.info('Adding unacknowledge callback %s', delivery_tag)
            logger.info('Requeueing message # %s', delivery_tag)
            cb = functools.partial(self._reject_message, delivery_tag, True, body)
            self._connection.ioloop.add_callback_threadsafe(cb)
        except BaseUnknownError as e:
            logger.info('Unknown error %s', e)
            logger.info('Adding unacknowledge callback %s', delivery_tag)
            logger.info('Requeueing message # %s', delivery_tag)
