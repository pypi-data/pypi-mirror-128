...
class Anaml:
    ...
    def preview_feature(
            self,
            feature: Union[List[Feature], Feature],
            *,
    ...
    def preview_feature_and_plot(
        ...
        Args:
            ...
            snapshot_date: Date to filter feature preview on.
        """
        feature_stats = self.preview_feature(feature, entity, snapshot_date)
        for fs in feature_stats:
            self._build_feature_plots(fs)
        self._warn_empty_feature_stats([
            fs.featureName
            for fs in feature_stats if isinstance(fs, EmptySummaryStatistics)
        ])
