...
@click.group(
    ...
    help="Manages configurations such as cluster environments and compute configs that help you run jobs and clusters on Anyscale.",
)
def config_cli() -> None:
    pass
...
@config_cli.command(
    name="create-cluster-env", help="Creates a cluster env from a cluster env yaml.",
)
@click.argument("cluster-env-yaml-file", type=click.File("rb"), required=True)
@click.option(
...
@config_cli.command(
    ...
    help="Creates a compute config from a compute config yaml.",
)
@click.argument("compute-config-yaml-file", type=click.File("rb"), required=True)
@click.option(
...
@click.option(
    ...
    "-c",
    type=str,
    help="Name of the cluster-compute",
    required=False,
    default=None,
)
@click.option(
...
def setup_dev_ray(
    ...
    ConfigController().setup_dev_ray(
        working_dir, cluster_name, cluster_env, cluster_compute, timeout
    )
