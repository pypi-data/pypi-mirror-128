...
class ProjectController:
    ...
    def clone(self, project_name: str, owner: Optional[str] = None) -> None:
        message = (
            ...
            "Please use `anyscale init [project_id]` to connect to an existing project."
        )
    ...
    def list(self, name: str, json_format: bool, any_creator: bool):
        paging_count = 20
    ...
    def _format_project_dict(self, project: Project):
        return {
            "name": project.name,
            "id": project.id,
            "url": f"{shared_anyscale_conf.ANYSCALE_HOST}/projects/{project.id}",
