    ...
    def launch_cluster(
        ...
        if clusters:
            ...
            if not cluster_environment_build_id or not cluster_compute_id:
                project_message = (
                    f"Project {project_id}" if project_id else "the default Project"
                )
                raise Exception(
                    (
                        "Cluster named {cluster_name} does not exist in {project_message}. "
