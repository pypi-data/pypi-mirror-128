...
from anyscale.sdk.anyscale_client import ComputeTemplateQuery, CreateComputeTemplate


def test_get_default_cluster_compute_id():
    ...
    mock_register_template = Mock(return_value="mock_compute_template_id")
    with patch.multiple(
        ...
        assert (
            get_default_cluster_compute_id(
                "mock_cloud_name", mock_api_client, mock_anyscale_api_client
            )
...
def test_register_compute_template():
    ...
    assert (
        register_compute_template("mock_template_obj", mock_api_client)
        == "mock_template_id"
    )
...
def test_get_cluster_compute_id_from_name():
    ...
    assert get_cluster_compute_id_from_name("mock_name", mock_api_client) == "mock_id"

    mock_api_client.search_compute_templates_api_v2_compute_templates_search_post.assert_called_once_with(
        ComputeTemplateQuery(
            orgwide=True, name={"equals": "mock_name"}, include_anonymous=True,
        )
    )
