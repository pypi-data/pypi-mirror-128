...
from anyscale.cluster_env import (
    get_build_id_from_cluster_env_identifier,
    get_cluster_env_id_from_name,
    list_builds,
)
...
def test_get_build_id_from_cluster_env_identifier():
    ...
    mock_get_cluster_env_id_from_name = Mock()
    mock_list_builds = Mock(return_value=[Mock(revision=3, id="mock_build_id")])
    with patch.multiple(
        "anyscale.cluster_env",
        get_cluster_env_id_from_name=mock_get_cluster_env_id_from_name,
...
def test_get_cluster_env_id_from_name():
    ...
    assert (
        get_cluster_env_id_from_name("my_cluster_env", mock_anyscale_api_client)
        == "my_cluster_env_id"
    )
    ...
    with pytest.raises(click.ClickException):
        assert get_cluster_env_id_from_name("my_cluster_env", mock_anyscale_api_client)
