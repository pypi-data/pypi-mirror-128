...
@cluster_compute_cli.command(
    name="get", help=("Get details about cluster compute configuration."), hidden=True,
)
...
def get(cluster_compute_name: Optional[str], id: Optional[str]) -> None:
    cluster_compute_controller = ClusterComputeController()
    cluster_compute_controller.get(
        cluster_compute_name=cluster_compute_name, cluster_compute_id=id,
    )
