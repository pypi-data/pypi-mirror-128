...
from aos_prov.command_vm import new_vm, start_vm, forward_provisioning_ports, delete_provisioning_ports, install_vbox_sdk
...
def main():
    ...
    try:
        ...
        if args.which == _COMMAND_UNIT_CREATE:
            ...
            cloud_api = CloudAPI(uc, args.register_port)
            cloud_api.check_cloud_access()
            vm_port = new_vm(args.name, args.disk)
            start_vm(args.name)
            time.sleep(10)
            run_provision(f'127.0.0.1:{vm_port}', cloud_api, reconnect_times=20)
            delete_provisioning_ports(args.name)
