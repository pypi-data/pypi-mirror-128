...
class SoftwareManagement2ServiceProxy(UpnpServiceProxy, LoadableExtension):
    """
        This is a code generated proxy class to the 'SoftwareManagement2' service.
    """
    ...
    def action_GetEUInfo(self, EUID, *, extract_returns=True, aspects:Aspects=DEFAULT_ASPECTS):
        """
            ...
            :returns: "EUName", "EUVersion", "EURequestedState", "EURunningState"
        """
        arguments = {
            "EUID": EUID,
        }
        ...
        rtn_args = out_params
        if extract_returns:
            rtn_args = [out_params[k] for k in ("EUName", "EUVersion", "EURequestedState", "EURunningState",)]
            if len(rtn_args) == 1:
                rtn_args = rtn_args[0]
