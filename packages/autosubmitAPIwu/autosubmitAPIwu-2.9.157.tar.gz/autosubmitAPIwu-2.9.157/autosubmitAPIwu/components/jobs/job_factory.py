...
class Job:
  ...
  def __init__(self):
    ...
    self.qos = "" # type : str
    self.parents_names = set() # type : List[str]
    self.children_names = set() # type : List[str]
  ...
  def do_print(self):
    # type : () -> None
    print("Job {0} \n Date {5} \n Section {1} \n Qos {2} \n Children: {3} \n Platform {4}. ".format(
      self.name, self.section, self.qos, self.children_names, self.platform, self.date))
  ...
  def from_old_job_data(cls, job_data):
    ...
    job.queue_time = job_data.queuing_time() # type : int
    job.run_time = job_data.running_time() # type : int
    job.energy = job_data.energy # type : int
    job.submit = job_data.submit # type : int
    job.start = job_data.start # type : int
    job.finish = job_data.finish # type : int
    job.ncpus = job_data.ncpus # type : int
    return job
