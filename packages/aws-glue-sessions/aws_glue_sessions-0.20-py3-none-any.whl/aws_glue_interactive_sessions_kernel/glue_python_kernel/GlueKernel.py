...
class GlueKernel(IPythonKernel):
    ...
    def configure(self, configs_json):
        ...
        try:
            ...
            if 'glue_role_arn' in configs:
                self.set_glue_role_arn(configs.get('glue_role_arn'))
                updated_configs['glue_role_arn'] = configs.get('glue_role_arn')
    ...
    async def _print_startup_text(self):
        ...
        self._send_output("For more information on available magic commands, please type %help in any new cell.\n")
        ...
        if latest_kernel_version != current_kernel_version:
            self._send_output(f"It looks like there is a newer version of the kernel available. The latest version is {latest_kernel_version} and you have {current_kernel_version} installed.")
            self._send_output("Please run `pip install --upgrade aws-glue-sessions` to upgrade your kernel")
