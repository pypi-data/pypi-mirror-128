...
fleet = SpotEventPluginFleet(stack, "SpotEventPluginFleet",
    ...
    vpc_subnets={
        "subnet_group_name": "SpotEventPluginFleetSubnets"
    }
)

ConfigureSpotEventPlugin(stack, "ConfigureSpotEventPlugin",
    vpc=vpc,
    spot_fleets=[fleet
    ]
)
...
render_queue = RenderQueue(stack, "RenderQueue",
    ...
    vpc_subnets_alb=vpc.select_subnets(subnet_group_name="RenderQueueALBSubnets")
)
```
...
```python
# Example automatically generated without compilation. See https://github.com/aws/jsii/issues/826
secrets_management_credentials = Secret.from_secret_complete_arn(self, "DeadlineSecretsManagementCredentials", "yourSecretArn")
...
worker_instance_fleet = WorkerInstanceFleet(stack, "WorkerInstanceFleet",
    # ...
    vpc=vpc,

    # Select the dedicated WorkerInstanceFleet subnets to put the worker nodes in
    vpc_subnets={
        "subnet_group_name": "WorkerInstanceFleetSubnets"
    }
)
```
'''
