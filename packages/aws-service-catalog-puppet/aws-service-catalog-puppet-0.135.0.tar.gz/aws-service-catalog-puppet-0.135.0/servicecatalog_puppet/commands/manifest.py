...
def expand(f, puppet_account_id, single_account, subset=None):
    ...
    manifest_accounts_all = [
        a.get("account_id") for a in new_manifest.get("accounts", [])
    ]
    ...
    dumped = dumped.replace(
        "${AWS::ManifestAccountsAll}", ",".join(manifest_accounts_all)
    )
    dumped = dumped.replace(
        "${AWS::ManifestAccountsSpokes}", ",".join(manifest_accounts_excluding)
    )
