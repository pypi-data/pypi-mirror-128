...
from azure.ml.component._util._exceptions import _assert_frame_package_name
...
@track(_get_logger, activity_type=_PUBLIC_API, activity_name="dsl_generate_package")
        ...
        .. remarks::
            ...
            components: single or list of glob string which specify a set of components. Example values:
                ...
                * components from local yaml
                    ``file:components/**/module_spec.yaml``
                * components from feeds. For feed concept, please see: https://aka.ms/azuremlsharing.
                    1. All assets in feed
                        ``azureml://feeds/azureml``
                    2. components which name contains 'Convert'
                        ``azureml://feeds/azureml/components/*Convert*``
...
def _resolve_source_directory():
    """Resolve source directory as last customer frame's module file position."""
    try:
        ...
        for frame, last_frame in zip(frame_list, frame_list[1:]):
            if _assert_frame_package_name(pattern, frame.frame) and \
                ...
                return Path(os.path.dirname(module.__file__)).absolute() if module else os.getcwd()
    except Exception:
