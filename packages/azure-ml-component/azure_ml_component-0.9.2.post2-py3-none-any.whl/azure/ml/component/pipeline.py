...
class Pipeline(Component, Visible, WorkspaceTelemetryMixin):
    ...
    def __init__(self, nodes: List[Union[Component, 'Pipeline']],
        ...
        _init_params = self._refine_and_validate_params(_init_params)
    ...
    def _refine_and_validate_params(self, params):
        """Extract enum and convert group class type to group attr dict."""
        ...
        for k, v in params.items():
            if isinstance(v, Enum):
                v = v.value
            params[k] = _Group._customer_class_value_to_attr_dict(v)
        # Validate group parameter attribute
        for k, annotation in self._definition.parameters.items():
            if k not in params or not isinstance(annotation, _Group):
                continue
            v = params[k]
    ...
    def set_inputs(self, *args, **kwargs) -> 'Component':
        ...
        for idx, node in enumerate(self.nodes):
            ...
            _kwargs = self._definition._convert_component_kwargs(kwargs, idx, _for_build=True)
            if len(_kwargs) > 0:
    ...
    def _expand_default_pipeline_parameters(self, pipeline_parameters):
        ...
        # Extract Enum value and handle group
        return self._refine_and_validate_params(pipeline_parameters)
        ...
        if unregister_component_definition:
            ...
            for node in nodes:
                if node._definition in unregister_component_definition and not node._runsettings._workspace:
                    ...
                    node._update_component_params_and_runsettings(workspace, node_pipeline_parameter)
