...
class _DataFrameReader:
    ...
    def to_pandas_dataframe(self,
        ...
        def to_pandas_preppy():
            ...
            random_id = uuid4()
            intermediate_path = Path(os.path.join(
                tempfile.gettempdir(), str(random_id)))
            try:
                ...
                activity_data = Dataflow._dataflow_to_anonymous_activity_data(
                    dataflow_to_execute)
        ...
        def to_pandas_feather():
            ...
            dataflow._raise_if_missing_secrets()
            activity_data = dataflow_to_execute._dataflow_to_anonymous_activity_data(
                dataflow_to_execute)
            dataflow._engine_api.execute_anonymous_activity(
        ...
        def to_pandas_arrow():
            ...
            def callback(batches, stream_cols):
                nonlocal record_batches
                nonlocal stream_columns
                record_batches = batches
                stream_columns = stream_cols
            ...
            activity_data = rslex_dflow._dataflow_to_anonymous_activity_data(
                rslex_dflow)
            ...
            for i in range(0, len(record_batches)):
                incoming_dfs[i] = pyarrow.Table.from_batches(
                    [record_batches[i]])

            return self.complete_incoming_dataframe(random_id, stream_columns=stream_columns)
    ...
    def complete_incoming_dataframe(self, dataframe_id: str, stream_columns=None) -> 'pandas.DataFrame':
        import pyarrow
        import pandas as pd
        ...
        partitions_dfs = \
            [partitions_dfs[key] for key in sorted(
                partitions_dfs.keys()) if partitions_dfs[key].num_rows > 0]
            ...
            for partition in partitions_dfs:
                ...
                if partition.num_columns != expected_count:
                    raise make_schema_error(
                        'partition had different number of columns.', expected_names, found_names)
                        ...
                        if column_type == pyarrow.bool_():
                            ...
                            if isinstance(column, pyarrow.ChunkedArray):
                                ...
                                partition = partition.add_column(
                                    j, fields[j], pyarrow.chunked_array(typed_chunks))
                            else:
        ...
        def set_stream_columns(df):
            if stream_columns is not None:
                for (path, values) in stream_columns:
                    if (len(path)) == 1:
                        # We only support top-level stream columns for now since there's no way to create a nested
                        # StreamInfo in rslex.
                        df[path[0]] = values
        ...
        with tracer.start_as_current_span('_DataFrameReader.complete_incoming_dataframe', trace.get_current_span()):
            ...
            import pyarrow
            df = pyarrow.concat_tables(
                partitions_dfs, promote=True).to_pandas(use_threads=True)
            set_stream_columns(df)
            return df
...
def ensure_dataframe_reader_handlers(requests_channel):
    ...
    requests_channel.register_handler(
        'get_dataframe_partition_data', process_get_data)
    requests_channel.register_handler(
