...
#Extract Beauty_cosmetics
			...
			tokens3 = message.text.lower().split()
			print("tokens3")
			print(tokens3)


			for w in group_types :
				types_index.append(tokens3.index(w))
			for w in group_vars:
				vars_index.append(tokens3.index(w))
			...
			tokens2 =nltk.word_tokenize(message.text.lower())
			print("PART OF SPEECH")
			print(nltk.pos_tag(tokens2))
			pos_tag = nltk.pos_tag(tokens2)
			pos_and = -1
			for p in pos_tag :
				if p[1] == "CC"  :
			...
			entity_category = self.convert_to_category(dict_entities_category)
			message.set("products_categories", entity_category, add_to_output=True)

			entity_variation = self.convert_to_variation(dict_entities_variations)
			message.set("products_variations", entity_variation, add_to_output=True)
