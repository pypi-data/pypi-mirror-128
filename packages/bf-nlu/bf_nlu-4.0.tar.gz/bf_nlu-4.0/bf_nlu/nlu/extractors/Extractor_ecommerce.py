...
class Extractor_ecommerce(Component):
	...
	def _init_(self, component_config=None):
		super(Extractor_ecommerce, self)._init_(component_config)
	...
	def convert_to_size(self, value):
		"""Convert model output into the bf_nlu NLU compatible output format.""" 
	...
	def convert_to_category(self, dict):
		"""Convert model output into the bf_nlu NLU compatible output format.""" 
		entities = []
...
#Extract Beauty_cosmetics
			...
			if group_types != [] :
				for w in group_types :
					ind = re.findall(w,text)
					if ind == []:
						print("type not found")
					else :
						mot = ind[0].split()[0]
						types_index.append(tokens3.index(mot))

			if group_vars != [] :
				for w in group_vars:
					print("w")
					print(w)
					ind2 = re.findall(w,text)
					if ind2 == []:
						print("type not found")
					else :
						mot2 = ind2[0].split()[0]
						vars_index.append(tokens3.index(mot2))
			...
			for j  in range(len(dict_variations))  :
				...
				for i in range(len(pos_tag)):
					...
					print("ookkkk")
					print(dict_variations[j]["word"])
					print(pos_tag[i])

					if dict_variations[j]["word"] in pos_tag[i] :
						print("yes")
						
						dict_variations[j]["TAG"] = pos_tag[i][1]
					...
					for j in range(len(dict_variations)) :
						...
						if dict_variations[j]['INDEX'] != -1 :
							if dict_variations[j]['INDEX'] < dict_categories[i]['INDEX']   :
								print("avant index")
								print(dict_variations[j])
								if dict_variations[j]['TAG'] == "JJ":
									print("JJ")
									
									
									products.append({ dict_categories[i]['TYPE'] : dict_categories[i]['CATEGORY'] ,  dict_variations[j]["TYPE"] : dict_variations[j]["VARIATION"]} )
									dict_variations[j] = {"word": "", "INDEX" : -1  , "VARIATION" : "" , "TYPE" : "" }
										...
										if dict_variations[j]['INDEX'] != -1 :
											print("-1")
											print(dict_variations[j]['INDEX'])
											if dict_variations[j]['INDEX'] < pos_and and dict_categories[i]["INDEX"] < pos_and :
												print('avant and')
											...
											if dict_variations[j]['INDEX'] > pos_and and dict_categories[i]["INDEX"] > pos_and :
												print("apres")
												if i < len(products) - 1  :
													print(products[i][dict_variations[j]["TYPE"]])
													print(dict_variations[j]["VARIATION"])
													products[i][dict_variations[j]["TYPE"]] = dict_variations[j]["VARIATION"]
													print("ok")
													dict_variations[j] = {"word": "", "INDEX" :  -1 , "VARIATION" : "" , "TYPE" : "" }
												else :
								...
								if pos_and != -1 :
									print("AND")
									if dict_variations[j]['INDEX'] != -1 :
										if dict_variations[j]['INDEX'] < pos_and : 
											print("apres and")
											
											products[i][dict_variations[j]["TYPE"]] = dict_variations[j]["VARIATION"]
											dict_variations[j] = {"word": "", "INDEX" :  -1 , "VARIATION" : "" , "TYPE" : "" }
										elif len(dict_categories) == i+1 :
												print("apres and")
			...
			entity_category = self.convert_to_category(products)
