...
class FirstSeatDeclarerSuit(FirstSeat):
    ...
    def _select_suit(self) -> Suit:
        ...
        for suit in SUITS:
            can_lead_to_dummy = (player.touching_honours(player.dummys_hand, suit) and
                                    player.unplayed_cards[suit])
            if player.touching_honours(player.hand, suit) or can_lead_to_dummy:
                return log(inspect.stack(), Suit(suit))
    ...
    def _select_lead_card(self, suit: Suit) -> Card:
        ...
        if not partner_long and partners_cards and player.is_winner(partners_cards[0]):
            return log(inspect.stack(), my_cards[-1])
        ...
        if player.is_winner(my_cards[0]) and not player.partners_unplayed_cards[suit.name]:
    ...
    def _get_winning_suit_list(self) -> List[Suit]:
        ...
        print(colored(f'{winning_suits}', MODULE_COLOUR))
    ...
    def _can_draw_trumps(self) -> bool:
        ...
        if not player.trump_cards:
            return_and_print(False, 'can_draw_trumps a', show_return)

        if self._ruff_in_short_hand():
            return_and_print(False, 'can_draw_trumps b', show_return)
        ...
        if len(declarers_trumps) + len(dummys_trumps) <= 7 and max(len(declarers_trumps), len(dummys_trumps)) == 4:
            return_and_print(False, 'can_draw_trumps c', show_return)
        ...
        if len(declarers_trumps) + len(dummys_trumps) >= 9:  # TODO too simplistic - determine losers first
            return_and_print(True, 'can_draw_trumps d', show_return)
        ...
        if short_hand.shape[3] <= 2:
            ...
            if len(short_hand.cards_by_suit[player.trump_suit.name]) > len(opponents_trumps):
                return_and_print(True, 'can_draw_trumps e', show_return)
            ...
            if ((ace in player.total_unplayed_cards[suit.name] and not ace in suit_cards) and
                ...
                return_and_print(False, 'can_draw_trumps f', show_return)
        return_and_print(True, 'can_draw_trumps g', show_return)
            ...
            if (short_cards and
                ...
                if ((Card('A', suit) in long_cards and
                        ...
                        if not player.dashboard.threats[suit]:
                            return False
                        # if not player.is_winner(player.unplayed_cards[suit][-1]):
