...
class Player():
    ...
    def is_winner(self, card: Card, trick: Trick=None) -> bool:
    ...
    def touching_honours(self, hand: Hand, suit: str) -> Union[Card, None]:
        """Return the top of touching_honours if there is one or None otherwise."""
        cards = Cards(hand.unplayed_cards).by_suit[suit]
        for index, card in enumerate(cards[:-1]):
            if card.is_honour and card.value == cards[-1].value + 1:
                return card
        return None
