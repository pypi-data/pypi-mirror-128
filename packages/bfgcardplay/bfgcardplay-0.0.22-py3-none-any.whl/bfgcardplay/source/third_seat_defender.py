...
from .defender_play import deduce_partner_void_in_trumps
...
class ThirdSeatDefender(ThirdSeat):
    ...
    def selected_card(self) -> Card:
        ...
        if len(cards) == 2:
            ...
            if leading_card.value < cards[0].value:
                # if not opponents_cards or leading_card.value > opponents_cards[0].value:
                return log(inspect.stack(), cards[0])
        ...
        if player.is_winner(cards[0], trick):
            return log(inspect.stack(), cards[0])

        # signal attitude
        if not manager.like_dislike(player.seat, suit_name):
    ...
    def _winning_card(self) -> Union[Card, None]:
        ...
        if player.is_winner(cards[0], trick) and player.defenders_tricks >= MAXIMUM_TRICKS - player.declarers_target:
            return log(inspect.stack(), cards[0])
