...
def checkOverlappingRuns(procs, mtx_dir):

    tmp_hits = collect_files(mtx_dir, 'tmp_hits.txt')
    hit_sets = {}
    for f in tmp_hits:
        rows = re.sub(r'tmp_hits.txt$', 'mtx.tmp0', f)
        if not os.path.isfile(rows):
            os.remove(f)
        else:
            b = int(os.path.basename(f).replace('sub.','').replace('.tmp_hits.txt',''))
            with open(f, 'r') as raw:
                hits = set([int(x.rstrip()) for x in raw])
            hit_sets[b] = hits

    hit_keys, todel = list(hit_sets.keys()), {}
    for i0, key0 in enumerate(hit_keys): # could be improved to imply overlap
        hit0 = hit_sets[key0]
        for i1, key1 in enumerate(hit_keys[i0 + 1:]):
            hit1 = hit_sets[key1]
            if hit0.intersection(hit1):
                print('\tOVERLAP IDENTIFIED', flush = True)
                ...
                if del_hit in procs:
                    ...
                    for f in del_files:
                        if os.path.isfile(f):
                            os.remove(f)
                    del procs[del_hit]
                    print('\t\tOVERLAP REMOVED ' + str(del_hit), flush = True)
        if del_key in procs:
            ...
            for f in del_files:
                if os.path.isfile(f):
                    os.remove(f)
            print('\t\tOVERLAP REMOVED ' + str(del_key), flush = True)
            del procs[del_key]
...
#                        todel.append(i1)
                ...
                procs = checkOverlappingRuns(procs, mtx_dir) # only worry when bottlenecked
