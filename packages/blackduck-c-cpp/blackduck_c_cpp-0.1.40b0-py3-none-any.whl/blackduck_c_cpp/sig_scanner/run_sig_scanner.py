...
class SigScanner:
    ...
    def __init__(self, cov_base_path, hub_api, files_notdetect_from_pkgmgr, project_name, project_version_name,
                 ...
                 use_offline_files, os_dist, additional_sig_scan_args, scan_interval, json_splitter_limit,
    ...
    def tar_files(self):
        ...
        logging.info("Number of source files in sig tar file is {}".format(len(files_to_tar)))
        ...
        logging.info("Number of files in sig tar file is {}".format(len(files_to_tar)))
    ...
    def find_any_sig_scanner(self):
        ...
        """
        try:
            self.scan_cli_directory = os.path.basename(sorted(glob.glob(os.path.join(self.scan_cli_home, 'scan.cli-*')), reverse=True)[0])
    ...
    def build_sig_scanner_command(self):
        ...
        sig_scanner_command += self.sig_scan_tar_output_directory
    ...
    def parse_additional_args(self):
        ...
        while additional_sig_scan_args_lst:
            ...
            if item == delim:
                ...
                additional_sig_scan_args_str_with_quotes += '{} '.format(parameter)  # parameter does not need to be escaped, will never contain spaces
    ...
    def upload_sig_bdio_results(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            with open(jsonld_headers_path, 'rb') as f:
                ...
                response = requests.post(endpoint, headers=headers, data=f, verify=not self.insecure)
            ...
            logging.info("Headers file uploaded successfully, proceeding to upload remaining scans using URL {}".format(
                location))
    ...
    def run(self):
        ...
        # tar the files or get the offline tar file
        if not self.use_offline_files:
            self.tar_files()
        if self.use_offline_files:
            if not (self.get_offline_tar_file() or self.find_dryrun_files()):
                return
