...
class CoverityInstall:
    ...
    def hub_api_tool_token(self):
        ...
        self.tool_token_response = self.hub.execute_post('{}/api/tool-token'.format(self.hub.config['baseurl']), data)
    ...
    def run(self):
        ...
        if int(vers_result[0]) >= 2021 and int(vers_result[1]) >= 10:
            ...
            util.error_and_exit(
                "Automatic update of coverity not supported in hub version {}. please provide coverity_root in yaml file".format(
                    hub_version))
