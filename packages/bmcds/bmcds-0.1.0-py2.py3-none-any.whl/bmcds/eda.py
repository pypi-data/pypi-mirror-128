...
def report_df(dataframe, sort_col = 'datatypes', fill_value = '-'):
    ...
    desc_num = dataframe.select_dtypes(include = ['number', 'datetime']).describe(datetime_is_numeric=True).T

    desc_obj = dataframe.select_dtypes(include = ['object', 'category']).describe(datetime_is_numeric=True).T
