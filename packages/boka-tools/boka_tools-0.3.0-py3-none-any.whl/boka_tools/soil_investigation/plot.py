...
PAPER_SIZES = dict(A3=[16.5, 11.75], A4=[8.25, 11.75],)  # A3 Landscape  # A4 Portrait
...
def plot_si_results(plot_props):
    """Function to plot si results in an gINT like manner
    ...
    if 'logo' in plot_props.layout.keys():
        if plot_props.layout.logo is not None:
            ax_logo.imshow(
    ...
    for plot in plot_props.plots.plots:
        ax = fig_layout.axes.axs_plots[plot.ax]
        kwargs = plot.kwargs
        plot_type = plot.type

        if 'x_data' in plot.keys():
            args = (plot.x_data, y_data)
        elif 'args' in plot.keys():
            args = tuple(plot.args)
        else:
            raise ValueError('No x-data passed??')
