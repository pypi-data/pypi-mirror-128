        ...
        try:
            ...
            for _ in range(10000):
                ...
                if result.find('"shortName":"{}"'.format(symbol)) > 0 and result.find('"id":"') > 0:
                    ...
                    return price_data['M'][0]['A'][0]['id']
        except Exception as e:
            logger.critical('func=Utils.get_price_id, symbol={}, msg={}'.format(symbol, e))
            raise e
    ...
    def get_latest_price(cls, symbol):
        try:
            ...
            for _ in range(10000):
                ...
                if result.find('"shortName":"{}"'.format(symbol)) > 0 and result.find('"id":"') > 0:
                    price_data = json.loads(result)
                    return price_data['M'][0]['A'][0]['price']
        except Exception as e:
            logger.critical('func=Utils.get_latest_price, symbol={}, msg={}'.format(symbol, e))
            raise e
...
class Quantfury:
        ...
        try:
            ...
            logger.critical(
                ...
                'error_message={}, msg={}'.format(
                    ...
                    reason, error_code, error_message, e))
            raise e
    ...
    def update_position(self, position_id, symbol, price_value, quote_order_quantity, order_type):
        """
        Update a position
        Parameters
        ----------
    ...
    def list_orders(self):
        """
        List open positions, active limit orders in the account
        Returns
        -------
        dict
    ...
    @classmethod
    def filter_order(cls, list_orders_response, order_id):
        orders = [i for i in list_orders_response['data']['activeLimitOrders'] if order_id == i['id']]
        if orders:
            return orders[0]


def test_oco_order(username, password, symbol):
    ...
    print(order_id)
    list_orders_response = quantfury.list_orders()
    if len(list_orders_response['data']['activeLimitOrders']) == 0:
        raise ValueError('No active orders')

    order_id_listed = list_orders_response['data']['activeLimitOrders'][0]['id']
    ...
    list_orders_response = quantfury.list_orders()
    order = quantfury.filter_order(list_orders_response, order_id)
...
if __name__ == '__main__':
    ...
    test_oco_order(username, password, symbol)
