    ...
    def close_with_earnings(self, client, target_price):
        ...
        try:
            ...
            target_json_result = client.update_position(
                self.api_position_id, self.symbol, target_price + 50,
                self.limit_amount_system, QuantfuryOrderType.target_order.value)
            ...
            stop_json_result = client.update_position(
                self.api_position_id, self.symbol, target_price - 50,
                self.limit_amount_system, QuantfuryOrderType.stop_order.value)
    ...
    def sync_closed_positions(cls, client):
        ...
        for close_position in closed_positions:
            stored_position = session.query(QuantfuryPositions).filter_by(
                api_position_id=close_position['api_position_id'], status=QuantfuryOrderStatus.open).first()
            if stored_position:
