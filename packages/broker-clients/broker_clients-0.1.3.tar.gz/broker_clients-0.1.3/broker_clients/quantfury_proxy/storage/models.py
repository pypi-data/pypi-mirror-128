...
class QuantfuryPositions(Base):
    ...
    status = Column(Enum(QuantfuryOrderStatus), nullable=False, default=False)  # hold, open, close, cancel
    ...
    def close_with_earnings(self, client, target_price):
        stage = 'self.fix_decimals'
        try:
            # TODO un hardcode target price quotes
            ...
            if symbol_metadata:
                ...
                upper_target_price = symbol_metadata.fix_price_precision(target_price+50)
                lower_target_price = symbol_metadata.fix_price_precision(target_price-50)
            stage = 'API.update_target_position'

            api_order_id = client.active_order_in_position(self.symbol, QuantfuryOrderType.target_order.value)
            if api_order_id:
                target_json_result = client.update_position(
                    api_order_id, self.symbol, upper_target_price,
                    limit_amount_system, QuantfuryOrderType.target_order.value)
            else:
                target_json_result = client.create_position(
                    self.api_position_id, self.symbol, upper_target_price,
                    limit_amount_system, QuantfuryOrderType.target_order.value)

            code = target_json_result.get('code', None)
            if code != 'Success':
                raise ConnectionError('First Update failed')

            stage = 'API.update_stop_loss_position'
            api_order_id = client.active_order_in_position(self.symbol, QuantfuryOrderType.stop_order.value)
            if api_order_id:
                stop_json_result = client.update_position(
                    api_order_id, self.symbol, lower_target_price,
                    limit_amount_system, QuantfuryOrderType.stop_order.value)
                code = stop_json_result.get('code', None)
            else:
                stop_json_result = client.create_position(
                    self.api_position_id, self.symbol, lower_target_price,
                    limit_amount_system, QuantfuryOrderType.stop_order.value)
                code = stop_json_result.get('code', None)

            if code != 'Success':
                raise ConnectionError('First Update failed')
            stage = 'SQL.update_quantfury_positions'
            ...
            logger.info('func=QuantfuryPositions.close_with_earnings, trade_id={}, balance_id={}, '
                        'lower_target_price={}, upper_target_price={}'.format(
                            self.trade.trade_id, self.trade.balance.balance_id, target_price-50, target_price+50))
        except Exception as e:
            logger.critical('func=QuantfuryPositions.close_with_earnings, trade_id={}, balance_id={}, '
                            'target_price={}, stage={}, msg={}'.format(
                                self.trade.trade_id, self.trade.balance.balance_id, target_price, stage, e))
    ...
    def match_limit_with_position(cls, limit, open_positions):
        for position in open_positions:
            position['open_price'] = Decimal(position['open_price'])
            if limit.symbol == position['short_name'] and \
                    cls.is_equal_price(limit.limit_price, position['open_price'], limit.symbol) and \
    ...
    def sync_closed_positions(cls, client):
        ...
        for close_position in closed_positions:
            stored_position = session.query(QuantfuryPositions).filter_by(
                api_position_id=close_position['api_position_id'], status=QuantfuryOrderStatus.target).first()
            if stored_position:
                stored_position.update(close_position)
                stored_position.status = QuantfuryOrderStatus.close
                session.commit()
                logger.info(
                    'func=QuantfuryPositions.sync_closed_positions, trade_id={}, close_price={}, '
                    'profit_and_loss_instrument={}'.format(stored_position.trade_id, close_position['close_price'],
                                                           close_position['profit_and_loss_instrument']))
