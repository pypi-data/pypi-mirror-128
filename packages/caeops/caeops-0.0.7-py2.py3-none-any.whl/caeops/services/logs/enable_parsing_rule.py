...
## Examples
---
The following `logs enable-parsing-rule` example enables the parsing rule for the given logs source
...
def _enable(payload: dict, tenant_id: str):
    """
    Enables a given parsing rule for the given log source
...
def enable_parsing_rule(payload):
    """
    Runs the enable parsing rule command for logs
    ...
    try:
        response = _enable(payload, tenant_id)
