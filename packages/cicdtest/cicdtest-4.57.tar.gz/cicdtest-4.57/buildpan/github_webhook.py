...
def github(project_id, path, token, username, repo_name, provider):
    ...
    try:
        ...
        try:
            ...
            print(fetch_log)
            print(header)
            # requests.post(fetch_log + "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=Exception: webhook already exists for repository - '+repo_name+'&status=failed&operation=webhook', data=header)
            
            requests.post(fetch_log, data=header)
            installer(project_id, repo_name, username)
