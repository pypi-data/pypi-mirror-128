...
class _CrawlerObject():
    ...
    def __build__(self, objectType):
        ...
        subtitle_dest_list = [self.subtitle_data_dir]
        if objectType == "raw":
            destination_list = raw_dest_list
        if objectType == "channel":
            destination_list = channel_dest_list
        if objectType == "subtitle":
            destination_list = subtitle_dest_list
    ...
    def _fetch_vars(self):
        ...
        self.subtitle_data_dir = \
            self.RAW_PARENT_PATH + config.get("rawfilepath", "video_subtitle_dir")
    # Try update the API
