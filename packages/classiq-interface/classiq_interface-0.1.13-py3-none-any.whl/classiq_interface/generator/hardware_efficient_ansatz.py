...
class AnsatzInputs(Enum):
    IN = "IN"


AnsatzOutputs = function_params.DefaultOutputEnum
...
class HardwareEfficientAnsatz(function_params.FunctionParams):
    ...
    one_qubit_gates: Union[str, List[str], None] = pydantic.Field(
        default=["x", "ry"],
        description='List of gates for the one qubit gates layer, e.g. ["x", "ry"]',
    )
    two_qubit_gates: Union[str, List[str], None] = pydantic.Field(
        default=["cx"],
        description='List of gates for the two qubit gates entangling layer, e.g. ["cx", "cry"]',
    )

    _input_enum: Type[Enum] = pydantic.PrivateAttr(default=AnsatzInputs)
    _output_enum: Type[Enum] = pydantic.PrivateAttr(default=AnsatzOutputs)
