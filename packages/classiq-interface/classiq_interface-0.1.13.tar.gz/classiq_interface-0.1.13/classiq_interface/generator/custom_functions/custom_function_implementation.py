...
from classiq_interface.generator.custom_functions.custom_register import CustomRegister

CustomRegistersStrictType: type = Tuple[CustomRegister, ...]
CustomRegistersType: type = Union[CustomRegister, CustomRegistersStrictType]
...
class CustomFunctionImplementation(pydantic.BaseModel):
    ...
    input_registers: CustomRegistersType = pydantic.Field(
        default=tuple(),
        description="A tuple of input registers to the custom implementation",
    )

    output_registers: CustomRegistersType = pydantic.Field(
        description="A tuple of output registers of the custom implementation",
    )

    zero_input_registers: CustomRegistersType = pydantic.Field(
        default=tuple(),
        description="A tuple of zero input registers to the custom implementation",
    )

    auxiliary_registers: CustomRegistersType = pydantic.Field(
        default=tuple(),
    ...
    def validate_all_registers_are_tuples(
        cls,
        registers: CustomRegistersType,
    ):
        if isinstance(registers, CustomRegister):
            return (registers,)
        return registers
    ...
    def validate_ranges_of_all_registers(cls, values):
        input_registers: CustomRegistersStrictType = values.get("input_registers")
        output_registers: CustomRegistersStrictType = values.get("output_registers")
        zero_input_registers: CustomRegistersStrictType = values.get(
            "zero_input_registers"
        )
        auxiliary_registers: CustomRegistersStrictType = values.get(
            "auxiliary_registers"
        )
    ...
    def validate_output_registers(
        cls, output_registers: CustomRegistersStrictType, values
    ):
    ...
    def validate_io_registers_are_distinct(
        cls,
        io_registers: CustomRegistersStrictType,
    ):
