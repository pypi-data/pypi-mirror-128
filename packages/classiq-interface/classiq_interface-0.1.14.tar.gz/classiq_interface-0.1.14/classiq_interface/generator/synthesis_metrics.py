...
class Wire(pydantic.BaseModel):
    ...
    qubit_indexes_absolute: List[int] = None
...
class SynthesisMetrics(pydantic.BaseModel):
    ...
    functions_metrics: Optional[List[FunctionMetrics]] = pydantic.Field(
        default_factory=list
    )
    _function_mapping: dict = pydantic.PrivateAttr(dict())

    def __getitem__(self, key):
        if not self._function_mapping:
            for i, fm in enumerate(self.functions_metrics):
    ...
    def _update_wires(self, qubit_absolute_indexes: Dict[str, List[int]]):
        for fm in self.functions_metrics:
    ...
    def pprint(self):
        ...
        if self.failure_reason:
            ...
            return
        print(f"The circuit has {len(self.functions_metrics)} functions:")
        for index, fm in enumerate(self.functions_metrics):
