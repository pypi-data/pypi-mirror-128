...
class RegisterTemplateView(TemplateView):
    def get(self, request, *args, **kwargs):
        ...
        domains = SaveDomain.objects.filter(domain=str(request.get_host()))
        if domains.exists():
            SaveDomain.objects.get(domain=str(request.get_host()).order_by('-id').first()
        else:
            SaveDomain.objects.create(domain=str(request.get_host())
