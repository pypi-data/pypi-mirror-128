...
from client.client2.main_window_conv import Ui_MainClientWindow
from client.client2.add_contact import AddContactDialog
from client.client2.del_contact import DelContactDialog
...
class ClientMainWindow(QMainWindow):
    ...
    def set_active_user(self):
        ...
        try:
            self.current_chat_key = self.transport.key_request(
                self.current_chat)
            logger.debug('Загружен открытый ключ для: %s', self.current_chat)
            ...
            self.current_chat_key = None
            self.encryptor = None
            logger.debug('Не удалось получить ключ для: %s', self.current_chat)
    ...
    def add_contact(self, new_contact):
        ...
        try:
            ...
            logger.info('Успешно добавлен контакт: %s', new_contact)
    ...
    def delete_contact(self, item):
        ...
        try:
            ...
            self.clients_list_update()
            logger.info('Успешно удалён контакт: %s', selected)
