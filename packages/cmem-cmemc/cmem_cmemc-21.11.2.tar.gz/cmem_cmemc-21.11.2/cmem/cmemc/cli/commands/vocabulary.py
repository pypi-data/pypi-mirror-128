...
def _get_vocabulary_metadata_from_file(file):
    ...
    if len(vann_data) == 0:
        raise ValueError(
            "There is no vann namespace defined "
...
@click.pass_obj
def import_command(app, file, replace):
    ...
    """
    # fetch meta data
    meta_data = _get_vocabulary_metadata_from_file(file)
    iri = meta_data["iri"]
