    ...
    def forward(self, graph):
        ...
        for i in range(self.num_layers):
            if i > 0:
                ...
                h = self.layers[i](graph, h, idx, self.hidden_size / self.sample_size)
            else:
                idx = torch.randint(self.in_feats, (self.sample_size, )).to(h.device)
                h = self.layers[i](graph, h, idx, self.in_feats / self.sample_size)
