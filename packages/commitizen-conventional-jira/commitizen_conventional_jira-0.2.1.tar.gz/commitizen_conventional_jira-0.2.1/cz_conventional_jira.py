...
class ConventionalJiraCz(ConventionalCommitsCz):
    def schema_pattern(self) -> Pattern[AnyStr]:
        ...
        jira_prefix = conf.settings.get('jira_prefix', default=None)
        jira_issues_required = conf.settings.get('jira_issues_required', default=False)
        jira_no_ticket_key = conf.settings.get('jira_no_ticket_key', default=None)
        if jira_prefix:
