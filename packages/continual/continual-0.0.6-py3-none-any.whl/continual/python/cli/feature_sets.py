...
def get_data(
    ...
    typer.secho(
        "Successfully saved data for featureset %s to %s" % (feature_set, str(path)),
        fg="green",
    )
...
def export_yaml(
    ...
    if project is None:
        project = utils.get_default_project()
    fs = c.feature_sets.get(feature_set)
    dict = fs.schema.to_dict()
    yaml_text = yaml.dump(dict)
    if path == "print":
        typer.secho(
        ...
        path = "%s/%s.yaml" % (path, feature_set)
        f = open(path, "w")
        f.write(yaml_text)
        f.close()
        typer.secho(
...
def diff(
    ...
    right_feature_set: str = typer.Option(None, help="Right feature set."),
    left_yaml: str = typer.Option(None, help="YAML location."),
    left_project: str = typer.Option(None, help="Left project."),
    left_feature_set: str = typer.Option(None, help="Left feature set"),
):
    """Diff feature sets.
