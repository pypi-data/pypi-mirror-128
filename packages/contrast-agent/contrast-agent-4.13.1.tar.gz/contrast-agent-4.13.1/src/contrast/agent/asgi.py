...
async def _get_body_non_destructive(request):
    # TODO: PYT-1779 - investigate this solution
    """
    ...
    which uses `call_next` for each successive middleware. This will almost definitely
    not work as intended in the general ASGI case.

    When supporting other ASGI frameworks that don't use starlette, we will probably
    have to update this method.
    """
    body = await request.body()

    async def replaced_receive():
        # note: by replacing the original receive method, we guarantee that the
        # `disconnect` event will never be received. This appears not to be much of an
