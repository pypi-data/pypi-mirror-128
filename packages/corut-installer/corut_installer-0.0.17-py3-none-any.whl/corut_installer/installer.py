...
class _Installer:
    ...
    def __run_command(command, password=None):
        ...
        try:
            ...
            logger.error(msg=f'Subprocess Popen Error:{error}')
        logger.info(msg='#' * 99)
    ...
    def __install_env(self, path):
        if path:
            ...
            self.__run_command(f'{executable_base} -m pip install -U virtualenv {self.__proxy}')
    ...
    def __install_python_packages(self, packages):
        for package in self.__check_command(packages):
            self.__run_command(f'{self.executable} -m pip install -U {package} {self.__proxy}')
