...
from gateways.common.cs18_api_responses import (AccountStatusResponse, BlueprintFileResponse, BlueprintResponse,
                                                ...
                                                TerraformModuleDescriptorResponse, SpaceTerraformModuleResponse)
...
class Colony:
    ...
    def add_space_terraform_module(
            self,
            module_alias: str,
            module_name: str,
            cloud_account_name: str,
            compute_service_name: str,
            region: str,
            inputs: List[SpaceTerraformModuleInput],
        ...
        request = {
            ...
            "inputs": inputs if not inputs else Utils.get_as_json(inputs)
        }
        response = self.session.post(url=method_url, json=request)
        GatewayUtils.handle_response(response=response, return_codes=[200])

    def update_space_terraform_module(
            self,
            module_alias: str,
            cloud_account_name: str,
            compute_service_name: str,
        ...
        response = self.session.put(url=method_url, json=request)
        GatewayUtils.handle_response(response=response, return_codes=[200])
