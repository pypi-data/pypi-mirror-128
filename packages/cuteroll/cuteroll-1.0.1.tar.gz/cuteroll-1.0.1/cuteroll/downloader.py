...
class crunchyroll:
    ...
    def __get_request(self, stream_id):
        ...
        endpoint = "https://beta-api.crunchyroll.com/cms/v2{}/videos/{}/streams".format(
            self.config.get("configuration").get("token").get("bucket"), stream_id
        )
    ...
    def url(self, stream_id):
        ...
        (video_url, subtitles_url, audio_language) = extractor.download_url(
            r, self.config
        )
    ...
    def download(self, stream_id):
        ...
        (metadata, cover, thumbnail, output, path) = extractor.get_metadata(
            type, id, self.config
        )
        ...
        if self.config.get("preferences").get("download").get("subtitles"):
            ...
            subtitle = converter.Subtitles(
                ...
                self.config.get("preferences").get("subtitles").get("language"),
            )
        ...
        if self.config.get("preferences").get("image").get("cover") or self.config.get(
            "preferences"
        ).get("video").get("attached_picture"):
            ...
            if extension == "mkv" or extension == "mp4":
                ...
                if extension == "mkv":
                    if self.config.get("preferences").get("download").get("subtitles"):
                        ...
                        if subtitles.get("ass") and os.path.exists(
                            "{}.ass".format(subtitles_path)
                        ):
                            command += ["-i", '"{}.ass"'.format(subtitles_path)]
                            index += 1
                            subs.append(index)
                        if subtitles.get("vtt") and os.path.exists(
                            "{}.vtt".format(subtitles_path)
                        ):
                            command += ["-i", '"{}.vtt"'.format(subtitles_path)]
                            index += 1
                            subs.append(index)
                        if subtitles.get("srt") and os.path.exists(
                            "{}.srt".format(subtitles_path)
                        ):
                ...
                if (
                    self.config.get("preferences").get("video").get("attached_picture")
                    and extension == "mp4"
                ):
                ...
                command += [
                    '"{}"'.format(
                        os.path.join(path, "{}.{}".format(output, extension))
                    ),
                    "-y",
                ]
                ...
                if os.path.exists(
                    ...
                    utils.print_msg(
                        "[debug] Download resolution: [{}]".format(
                            self.config.get("preferences")
                            .get("video")
                            .get("resolution")
                        ),
    ...
    def download_season(self, season_id, playlist_episode):
        ...
        endpoint = "https://beta-api.crunchyroll.com/cms/v2{}/episodes".format(
            self.config.get("configuration").get("token").get("bucket")
        )
        r = requests.get(endpoint, params=params).json()
        if utils.get_error(r):
            sys.exit(0)
        ...
        if not playlist_id:
            ...
            for i in range(len(playlist_id)):
                utils.print_msg(
                    "[debug] Download playlist: {}/{}".format(i + 1, len(playlist_id)),
                    0,
                )
