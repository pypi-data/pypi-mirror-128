...
HandDetectorYOLOX::HandDetectorYOLOX(const char* param_buffer,
...
HandDetectorYOLOX::HandDetectorYOLOX(const std::string& param_file,
...
#if __ANDROID__
HandDetectorYOLOX::HandDetectorYOLOX(AAssetManager* mgr,
...
void HandDetectorYOLOX::Preprocess(const cv::Mat& image, ncnn::Mat& net_input) {
...
int HandDetectorYOLOX::Predict(const cv::Mat& image,
  ...
  for (int i = 0; i < count; i++) {
    hands[i] = proposals[picked[i]];

    // adjust offset to original unpadded
    float x0 = (hands[i].x) / scale_;
    float y0 = (hands[i].y) / scale_;
    float x1 = (hands[i].x + hands[i].w) / scale_;
    float y1 = (hands[i].y + hands[i].h) / scale_;
