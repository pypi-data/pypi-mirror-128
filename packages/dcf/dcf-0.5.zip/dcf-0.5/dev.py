...
from dcf import DiscountFactorCurve, ZeroRateCurve, CashRateCurve, ShortRateCurve, dyn_scheme
from dcf import MarginalDefaultProbabilityCurve, MarginalSurvivalProbabilityCurve
...
if 1:
    ...
    principal = FixedCashFlowList([start_date], [-notional], origin=start_date)  # first schedule date is excluded
    redemption = FixedCashFlowList(payment_dates, plan, origin=start_date)  # first schedule date is excluded
...
if 0:
    def plot_cast(curve, x=None):
        ...
        y = [curve.__class__._get_storage_value(curve.cast(ZeroRateCurve), _) for _ in x]
        ax.plot(z, y, label='zero rate')

        y = [curve.__class__._get_storage_value(curve.cast(DiscountFactorCurve), _) for _ in x]
        ax.plot(z, y, label='discount factor')

        y = [curve.__class__._get_storage_value(curve.cast(ShortRateCurve), _) for _ in x]
        ax.plot(z, y, label='short rate')

        y = [curve.__class__._get_storage_value(curve.cast(CashRateCurve, forward_tenor='1m'), _) for _ in x]
        ax.plot(z, y, label='cash rate 1m ')

        y = [curve.__class__._get_storage_value(curve.cast(CashRateCurve, forward_tenor='3m'), _) for _ in x]
