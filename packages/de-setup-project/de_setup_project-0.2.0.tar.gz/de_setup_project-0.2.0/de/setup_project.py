...
def _compile_setup_kwargs(pev: PevType):
    ...
    if 'extras_require' not in kwargs:
        kwargs['extras_require'] = {'docs': pev['docs_require'],
                                    'tests': pev['tests_require'],
                                    'dev': pev['docs_require'] + pev['tests_require']}                  # type: ignore
...
def _load_requirements(pev: PevType):
    ...
    :param pev:                 dict of project environment variables; with the following required project env vars:
                                REQ_FILE_NAME, REQ_TEST_FILE_NAME, namespace_name, package_name, project_path.
                                the project env vars that get overwritten in `pev` by this function are:
    ...
    pev['install_require'] = min_require = _package_list(os.path.join(project_path, pev_str(pev, 'REQ_FILE_NAME')))
    prefix = f'{namespace_name}_'
    pev['portions_packages'] = [_ for _ in min_require if _.startswith(prefix)]

    pev['tests_require'] = tests_require = _package_list(os.path.join(project_path, pev_str(pev, 'REQ_TEST_FILE_NAME')))
    pev['docs_require'] = [_ for _ in tests_require if _.startswith('sphinx')]
    ...
    if not from_setup and os.path.isfile(setup_file):
        with in_wd(project_path):
            ...
            if isinstance(pev, dict):                                                       # PevType type
                if cae:
                    _init_app_configs(pev, cae)             # overwrite constants with cae app config vars
                    # interim/temporary commenting out TODO
