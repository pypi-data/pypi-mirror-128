...
def readActions(format_inhalt_list, messanger="Telegram", asset_url=""):
    ...
    for format, inhalt in format_inhalt_list:
        ...
        if format == "Textnachricht":
            ...
            if match:
                action["file"] = os.path.join(asset_url, match.group('file'))
                action["title"] = match.group('name')
                action["performer"] = match.group('performer')
            else:
                action["file"] = os.path.join(asset_url, "platzhalter.mp3")
                action["title"] = inhalt
                action["performer"] = "🤖"
            ...
            if match:
                ...
                action["file"] = os.path.join(asset_url, "platzhalter.png")
