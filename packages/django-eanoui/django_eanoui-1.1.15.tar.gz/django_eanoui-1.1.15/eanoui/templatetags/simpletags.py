...
@register.simple_tag
def get_version():
    return eanoui.get_version()


@register.simple_tag
def get_app_info():
    return format_table({"version": eanoui.get_version()})
...
def menus(context, _get_config=None):
    ...
    # 如果有配置自定义menu就读取自定义的，没有就调用系统的
    if config and 'menus' in config:
        # 如果'system_keep'!=True，则清空系统的菜单
        if not config.get('system_keep', None):
            data = []
        ...
        # 根据权限配置删除菜单
            ...
            # 获取权限列表
            groups_permissions = context.request.user.get_group_permissions()
            for i in temp:
                # 检查1级菜单
                if 'codename' not in i \
                        or (True in [(True if i['codename'] in g_p else False)
                                     for g_p in groups_permissions]) == False:
                ...
                if 'models' in i:
                    ...
                    for k in i.get('models'):
                        # check level 2 menu
                        if 'codename' not in i \
                            or 'codename' not in k \
                            or (i['codename']+'.'+(k['codename']) not in groups_permissions \
                                and i['codename']+'.view_'+(k['codename']) not in groups_permissions \
                                and i['codename']+'.add_'+(k['codename']) not in groups_permissions \
                                and i['codename']+'.change_'+(k['codename']) not in groups_permissions \
                                and i['codename']+'.delete_'+(k['codename']) not in groups_permissions \
                                and k['codename'] not in groups_permissions):
                            if not context.request.user.is_superuser:
                                delete_models.append(k)
                                continue
                        k['breadcrumbs'] = [{
                            'name': i.get('name'),
                            'icon': i.get('icon')
                    ...
                    for del_m in delete_models:
                        i['models'].remove(del_m)
                else:
                    i['breadcrumbs'] = [{
                        'name': i.get('name'),
                        'icon': i.get('icon')
