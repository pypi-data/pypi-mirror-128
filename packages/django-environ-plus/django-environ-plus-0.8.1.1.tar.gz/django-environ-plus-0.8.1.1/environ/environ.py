        ...
        if url.query:
            ...
            for k, v in parse_qs(url.query).items():
                ...
                if k.upper() in cls._CACHE_BASE_OPTIONS:
                    ...
                    config_behaviors.update(
                        {k.upper().replace(cls._CACHE_BEHAVIOR_PREFIX, '').lower(): _cast_int(v[0])})
                else:
                    config_options.update(opt)

            if len(config_behaviors.keys()) > 0:
                config_options.update({'behaviors': config_behaviors})
            config['OPTIONS'] = config_options
