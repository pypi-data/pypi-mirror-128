...
class Filter:
    ...
    def __init__(self, field_name=None, lookup_expr=None, *, label=None,
                 method=None, distinct=False, exclude=False, 
                 default=NOT_PROVIDED, null_skip=True, **kwargs):
    ...
    def get_value(self, value, exists):
        return value if exists or self.default == NOT_PROVIDED else self.default

    def skip(self, value, exists):
        return self.null_skip and (
            value in EMPTY_VALUES or not exists
        )

    def filter(self, qs, value, exists):
        if self.distinct:
            qs = qs.distinct()
...
class ChoiceFilter(Filter):
    ...
    def filter(self, qs, value, exists):
        if value != self.null_value:
            return super().filter(qs, value, exists)
...
class FilterMethod:
    ...
    def __call__(self, qs, value, exists):
        return self.method(qs, self.f.field_name, value, exists)
