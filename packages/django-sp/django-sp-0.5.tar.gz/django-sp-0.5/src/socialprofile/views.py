...
# from .models import SocialProfile, Group
...
class SocialProfileEditView(SocialProfileView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if username:
            ...
            raise Http404  # Case where user gets to this view anonymously for non-existent user
        sp_form = SocialProfileForm(self.request.POST, instance=user)
        print(f"Iamhere: maybe")
        ...
        if sp_form.is_valid():
            try:
                ...
                messages.add_message(
                    self.request, messages.INFO, _("Your profile has been updated.")
                )
                ...
                messages.add_message(
                    self.request,
                    messages.INFO,
                    _("ERROR: Your profile has NOT been updated! [" + str(e) + "]"),
                )
                return self.render_to_response({"sp_form": sp_form})
        else:
            messages.add_message(
