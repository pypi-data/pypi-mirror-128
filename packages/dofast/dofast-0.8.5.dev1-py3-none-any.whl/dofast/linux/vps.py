...
class VPS:
    ...
    def vnstat(self) -> str:
        ...
        js = json.loads(js)
        bit_shift = 20 if js['vnstatversion'].startswith('1.') else 30
        js = next((e for e in js['interfaces'] if e['name'] == self.interface),
                  {})
        if not js:
            return ''
        daykey = 'days' if 'days' in js['traffic'] else 'day'
