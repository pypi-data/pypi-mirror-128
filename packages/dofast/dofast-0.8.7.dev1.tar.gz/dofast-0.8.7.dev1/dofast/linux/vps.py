...
class VPS:
    ...
    def vnstat_(self) -> List[Tuple[str]]:
        ...
        total = rx + tx
        avg = total / max(_id, 1)
        return [('TX', tx), ('RX', rx), ('TOTAL', total), ('AVG', avg)]
...
class Monitor:
    ...
    @cf.utils.retry(tries=3, delay=5)
    def run(self) -> None:
        vps_info = self.vps.info()
        image_path = self.record.draw_from_list(vps_info)
        if image_path:
            self.post_telegram_image(image_path)
        else:
            cf.error('No image generated')
