...
import eavesdropper.pkg_data.mitmproxy.command
import eavesdropper.pkg_data.mitmproxy.flow
import eavesdropper.pkg_data.mitmproxy.master
...
class CompletionState(typing.NamedTuple):
    completer: Completer
    parsed: typing.Sequence[eavesdropper.pkg_data.mitmproxy.command.ParseResult]


class CommandBuffer:
    def __init__(self, master: eavesdropper.pkg_data.mitmproxy.master.Master, start: str = "") -> None:
        self.master = master
        self.text = start
    ...
    def render(self):
        ...
        if not parts:
            ...
            for p in parts:
                if p.valid:
                    if p.type == eavesdropper.pkg_data.mitmproxy.types.Cmd:
            ...
            if remaining:
                if parts[-1].type != eavesdropper.pkg_data.mitmproxy.types.Space:
                    ret.append(("text", " "))
    ...
    def cycle_completion(self, forward: bool = True) -> None:
        if not self.completion:
            ...
            if parts and parts[-1].type != eavesdropper.pkg_data.mitmproxy.types.Space:
                ...
                return
            ct = eavesdropper.pkg_data.mitmproxy.types.CommandTypes.get(type_to_complete, None)
            if ct:
...
class CommandEdit(urwid.WidgetWrap):
    ...
    def __init__(self, master: eavesdropper.pkg_data.mitmproxy.master.Master, text: str) -> None:
