...
it = iterwalk(modroot_x, events=["start","end"], tags=[rdfxml.uri_to_tag('rm_module:childBindings'),rdfxml.uri_to_tag("rm_module:Binding")] )
level = 0
headinglevel = []
for event,el in it():
    print( f"{event=} {el.tag=} {headinglevel=}" )
    summary = ""
    ...
    if el.tag==rdfxml.uri_to_tag("rm_module:Binding"):
        isheading = ( rdfxml.xml_find_element( el, './rm_module:isHeading' ) is not None )
        print( f"{isheading=}" )
        ba_u = rdfxml.xmlrdf_get_resource_uri( el, './rm_module:boundArtifact' )
#        print( f"{ba_u=} {c.app.baseurl=}" )
...
#        print( f"{ba=}" )
        if event=="start":
            if isheading:
#                headinglevel[-1][0] += 1
                headinglevel.append( [1,0] )
            if not isheading:
                headinglevel[-1][1] += 1
            # check for heading and increase
            print( f"{'    '*level}", end="" )
            if isheading:
            ...
            print( f"  {summary}" )
        if event == "end":
            # check for heading and reduce
            if isheading:
                # remove a heading level and incresa the previous sublevel
                headinglevel.pop()
                if len(headinglevel)>0:
#                    headinglevel[-1][0] += 1
                    headinglevel[-1][1] += 1
            else:
#                headinglevel[-1][0] += 1
