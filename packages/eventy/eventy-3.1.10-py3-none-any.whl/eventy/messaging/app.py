...
class EventyApp:
    ...
    def run(self, keep_alive=False) -> None:
        ...
        def run_loop():
            while True:
                if self._cancelled:
                    logger.info(f"Eventy App has been cancelled. Will terminate App.")
                    finish_reason = "cancelled"
                    break
                elif not keep_alive and not current_thread.is_alive():
                    logger.info(f"Eventy App calling thread is not alive. Will terminate App.")
    ...
    def __del__(self):
        logger.info(f"Deleting Eventy App.")
        self.cancel()
