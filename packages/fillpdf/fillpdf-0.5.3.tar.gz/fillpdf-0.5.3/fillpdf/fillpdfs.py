                ...
                if annotation[SUBTYPE_KEY] == WIDGET_SUBTYPE_KEY:
                    if annotation[ANNOT_FIELD_KEY]:
                        ...
                        if annotation[ANNOT_VAL_KEY]:
                            ...
                            try:
                                if type(annotation[ANNOT_VAL_KEY]) == objects.pdfstring.PdfString:
                                    data_dict[key] = objects.PdfString.decode(annotation[ANNOT_VAL_KEY])
                                elif type(annotation[ANNOT_VAL_KEY]) == objects.pdfname.BasePdfName:
                                ...
                                if annotation['/AP']:
                                    ...
                                    for each in annotation['/Kids']:
                                        ...
                                        if f'/{data_dict[key]}' == export:
                                            val_str = objects.pdfname.BasePdfName(f'/{data_dict[key]}')
                                        else:
                                            val_str = objects.pdfname.BasePdfName(f'/Off')
                                    ...
                                    if data_dict[key] not in options:
                                        ...
                                        if set(keys).intersection(set(temp_dict.values())):
                                            annotation.update(PdfDict(V=objects.pdfname.BasePdfName(f'/{data_dict[key]}')))
                            else:
                                # button field i.e. a checkbox
                                target.update( PdfDict( V=PdfName(data_dict[key]) , AS=PdfName(data_dict[key]) ))
                                if target[ANNOT_FIELD_KIDS_KEY]:
                                    target[ANNOT_FIELD_KIDS_KEY][0].update( PdfDict( V=PdfName(data_dict[key]) , AS=PdfName(data_dict[key]) ))
                        elif target[ANNOT_FORM_type] == ANNOT_FORM_combo:
                            # Drop Down Combo Box
                            export = None
                            options = annotation[ANNOT_FORM_options]
                            if len(options) > 0:
                                if type(options[0]) == objects.pdfarray.PdfArray:
                                    options = list(options)
                                    options = [objects.pdfstring.PdfString.decode(x[0]) for x in options]
                                if type(options[0]) == objects.pdfstring.PdfString:
                                    options = [objects.pdfstring.PdfString.decode(x) for x in options]
                            if type(data_dict[key]) == list:
                                export = []
                                for each in options:
                                    if each in data_dict[key]:
                                        export.append(objects.pdfstring.PdfString.encode(each))
                                if export is None:
                                    if data_dict[key] != "None"  and data_dict[key] != "":
                                        raise KeyError(f"{data_dict[key]} Not An Option For {annotation[ANNOT_FIELD_KEY]}, Options are {options}")
                                pdfstr = objects.pdfarray.PdfArray(export)
                            else:
                                for each in options:
                                ...
                                if export is None:
                                    if data_dict[key] != "None" and data_dict[key] != "":
                                        raise KeyError(f"{data_dict[key]} Not An Option For {annotation[ANNOT_FIELD_KEY]}, Options are {options}")
                                pdfstr = objects.pdfstring.PdfString.encode(data_dict[key])
                            annotation.update(PdfDict(V=pdfstr, AS=pdfstr))
                        elif target[ANNOT_FORM_type] == ANNOT_FORM_text:
                            # regular text field
                            target.update( PdfDict( V=data_dict[key], AP=data_dict[key]) )
                            if target[ANNOT_FIELD_KIDS_KEY]:
                                target[ANNOT_FIELD_KIDS_KEY][0].update( PdfDict( V=data_dict[key], AP=data_dict[key]) )
                if flatten == True:
                    annotation.update(PdfDict(Ff=1))
    template_pdf.Root.AcroForm.update(PdfDict(NeedAppearances=PdfObject('true')))
    PdfWriter().write(output_pdf_path, template_pdf)