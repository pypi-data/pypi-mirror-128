...
def create_cookie_cutter(extra_content):
...
def write_step_to_file(directory, step_name, func, req_file=None):
    ...
    # Split, remove first 2 lines and rejoin
    # find where function begins
    pattern = f"def {step_name}(self):"
    lines = find_pattern_list_split(lines, pattern)

    # Filename
    filename = f"{step_name}.py"

    # Check path
    path = f"{directory}/pipeline/{step_name}"
    if not os.path.exists(path):
        os.makedirs(path)

    if req_file is not None:
        copyfile(req_file, f"{path}/requirements.txt")

    # Create init file
    open(f"{path}/__init__.py", "w").close()

    filehandle = open(f"{path}/{filename}", "w")
...
def write_to_yaml(data: dict, directory: str, file_name: str):
    ...
    with open(f"{path}/{file_name}.yml", "w") as outfile:
...
def verify_args(engine, number_engines, ac_type):
...
def get_config() -> dict:
    for path in Path(os.getcwd()).rglob("*config.yml"):
