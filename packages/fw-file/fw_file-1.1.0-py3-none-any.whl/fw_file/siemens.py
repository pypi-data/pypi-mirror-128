...
DAT_TYPE_RE = r"(?:at|b|d|f?l|s|t|u[ci])?(?=[A-Z])"
DAT_FIELD_RE = fr'\s*<Param.+"{DAT_TYPE_RE}(?P<key>.*?)">{{\s*"?(?P<value>.*?)"?\s*}}'
DAT_ARRAY_RE = fr'\s*<ParamArray\."{DAT_TYPE_RE}(?P<key>.*?)">'
DAT_VALUE_RE = r"\s*\{\s*(?P<value>.*?)\s*\}"
...
def load_dat(file: t.BinaryIO) -> t.Dict[str, t.Any]:
    """Return fields parsed from a Siemens MR RAW (.dat) file."""
    first_line = file.readline()
    if not first_line.endswith(DAT_HEADER):
    ...
    for line_bytes in file:
        ...
        dat_value = re.match(DAT_VALUE_RE, line)
        if dat_field:
            key, value = dat_field.groups()
            fields[key] = parse_yaml_value(value)
        elif dat_array:
            array_key = dat_array.group("key")
        elif array_key and dat_value:
            fields[array_key] = parse_yaml_value(dat_value.group("value"))
            array_key = None
    fields = {DAT_FIELD_MAP.get(k, k): v for k, v in fields.items()}
...
def get_ptd_dcm_pos(file: t.BinaryIO) -> t.Tuple[int, int]:
...
def load_rda(file: t.BinaryIO) -> t.Dict[str, t.Any]:
