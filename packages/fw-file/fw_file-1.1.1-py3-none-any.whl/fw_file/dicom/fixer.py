...
def invalid_float_callback(
    ...
    **kwargs,
    ...
    if callable(data_element_callback):
        raw_elem = data_element_callback(raw_elem, **kwargs)
    # IS (integer string), can't check if it's integer, instead check either if
    # the value can be parsed as an integer
    if raw_elem.VR == "IS":
        # Based on IS valuerep in pydicom
        # https://github.com/pydicom/pydicom/blob/2.2.X/pydicom/valuerep.py#L751
        try:
            val = int(raw_elem.value)
        except ValueError:
            val = int(float(raw_elem.value))
        if isinstance(raw_elem.value, (float, Decimal, str, bytes)) and val != float(
            raw_elem.value
        ):
            log.warning(
                f"Float value found for Integer String VR, ({raw_elem.tag}) "
                "casting to int, MAY RESULT IN LOSS OF PRECISION"
            )
        # Top level data_element_callback calls `convert_value` which expects
        # value to be in bytes.
        raw_elem = raw_elem._replace(value=f"{val}".encode())
    return raw_elem
