...
class MetaExtractor:  # pylint: disable=too-few-public-methods
    ...
    def extract(self, data: dict) -> MetaData:
        ...
        for field, file_default in getattr(data, "default_meta", {}).items():
            setdefault(field, file_default)  # pragma: no cover
        ...
        for prefix in ("session", "acquisition"):
            ...
            if meta.get(ts_field):
                meta[tz_field] = meta[ts_field].tzinfo.key
                meta[ts_field] = meta[ts_field].isoformat(timespec="milliseconds")
