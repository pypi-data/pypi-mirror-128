...
class Package:
  ...
  def write_to_existing_db(self, dbfilename, timestamp: Optional[float] = None, init_schema=False):
    conn = sqlite3.connect(dbfilename)
    cursor = conn.cursor()
    if timestamp is None:
      timestamp = time.time()
  ...
  def write_to_db(self, cursor, timestamp: float, id_gen, init_schema=True):
    if init_schema:
