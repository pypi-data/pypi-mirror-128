...
class SysfsBase(object):
    ...
    def setup(self):                                # open or reopen attr files
        ...
        for attr in self.attributes:
            ...
            mode = [None, 'w', 'r', 'w+'][perm >> 1]
            if not mode:
                continue
            if attr in self._file:
                self._file[attr].close()
            self._file[attr] = open(fn, mode + 'b', buffering=0)
    ...
    def fileno(self, attr=None):
        if attr is None:
            attr = (self.attributes or [None])[0]
        if attr not in self._file:
            return -1
        return self._file[attr].fileno()

    def _read(self, attr, intbase=0):
        # value may be string or integer or hex
        if attr not in self._file:
            return 0 if intbase else 'none'
        with self._read_lock:
            self._file[attr].seek(0)
            value = self._file[attr].read().strip()
        return int(value, intbase) if intbase else value

    def _write(self, attr, data):
        if attr not in self._file:
            return 0
        binary = str(data).encode('latin')
        with self._write_lock:
            self._file[attr].seek(0)
            self._file[attr].write(binary)
        return len(binary)
...
class SysfsGPIO(SysfsBase):
    ...
    def __init__(self, pin):
        self.pin = int(pin)
        super().__init__('/sys/class/gpio/gpio{:d}'.format(self.pin))
        self.export = True
    ...
    edge = property(
        lambda self: self._read('edge'),
        lambda self, data: self._write('edge', data))
    value = property(
        lambda self: self._read('value', 10),
        lambda self, data: self._write('value', data))
    direction = property(
        lambda self: self._read('direction'),
        lambda self, data: self._write('direction', data))
    active_low = property(
        lambda self: self._read('active_low', 10),
        lambda self, data: self._write('active_low', data))

    @property
    def export(self): return os.path.exists(self.path)
...
class SysfsLED(SysfsBase):
    attributes = ('brightness', 'max_brightness', 'trigger')

    def __init__(self, name):
        self.name = str(name)
        super().__init__('/sys/class/leds/%s' % self.name)
        self.setup()
        self.max_brightness = self._read('max_brightness', 10) or 255

    def __repr__(self):
        return '<led({}) brightness: {}/{} at {}>'.format(
            self.name, self.brightness,
            self.max_brightness, hex(self.__hash__()))

    trigger = property(
        lambda self: self._read('trigger'),
        lambda self, data: self._write('trigger', data))
    brightness = property(
        lambda self: self._read('brightness', 10),
        lambda self, data: self._write(
            'brightness', min(int(data), self.max_brightness)))
