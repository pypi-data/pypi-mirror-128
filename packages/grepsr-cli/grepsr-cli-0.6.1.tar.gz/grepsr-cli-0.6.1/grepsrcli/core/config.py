...
def _generate_autocomplete_script(crawler_paths):
    ...
    for auto_script in ['autocomplete',  'autocomplete_zsh']:
        ...
        if(platform.system() == 'Linux'):
            # check if autocomplete has already been sourced in .bashrc file
            with open('~/.bashrc') as rc_file:
                rc_file = Template(file.read())
                if("source $HOME/.grepsr/autocomplete.sh" in rc_file):
                    print("Already added !!")
                else:
                    system(
                        '''echo "source $HOME/.grepsr/autocomplete.sh" >>  ~/.bashrc''')
        elif (platform.system() == 'Darwin'):
            with open('~/.zshrc') as rc_file:
                rc_file = Template(file.read())
                if("source $HOME/.grepsr/autocomplete_zsh.sh" in rc_file):
                    print("Already added !!")
                else:
                    system(
