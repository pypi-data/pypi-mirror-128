...
class Guide:
    ...
    previous start dimensions are transfered to the next module in each step.
    """
    ...
    def add_to_instrument(self, instrument, instrument_parameters, sample_dimensions):
        """
        Adds McStasScript objects describing this guide to a instrument
        ...
        previous_start_dimensions = sample_dimensions
...
class Project:
    ...
    def write(self, cluster=None):
        ...
        moderator = {"moderator_name": self.moderator.get_name(),
                     ...
                     "moderator_units": scan.moderator_parameters.get_unit_dict(only_scanned=True)}

        sample = {"sample_name": type(self.sample).__name__,
                  "sample_scan": scan.sample_parameters.get_scan_dict(),
                  "sample_units": scan.sample_parameters.get_unit_dict(only_scanned=True)}

        analysis_moderators = []
        for mod in self.analysis_moderators:
            analysis_moderators.append(mod.get_name())

        scan_states = []
        scan.reset_configuration()
        while scan.next_state():
            state_info = {"scan_state": scan.state_string()}
            state_info["sample_scan"] = scan.get_sample_state_dict()
            state_info["moderator_scan"] = scan.get_moderator_state_dict()

            scan_states.append(state_info)

        overview = {"guide_names": guide_names,
                    "sample": sample,
                    "moderator": moderator,
                    "analysis_moderators": analysis_moderators,
