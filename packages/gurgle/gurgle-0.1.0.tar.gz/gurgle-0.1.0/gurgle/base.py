...
# TODO: Make identity_func "identifiable". If we use the following one,
#  we can use == to detect it's use,
# TODO: ... but there may be a way to annotate, register,
#  or type any identity function so it can be detected.
def identity_func(x):
    return x
...
class CallableModel:
    ...
    def __dir__(self):
        return list(
            set(dir(self.__class__)).union(self.model.__dir__())
        )  # to forward dir to delegated stream
...
def enable_incremental_learning_on_call(
    call_method_str='transform', fv_is_a_single_number=False, warm_up=0, batch_size=0
):
