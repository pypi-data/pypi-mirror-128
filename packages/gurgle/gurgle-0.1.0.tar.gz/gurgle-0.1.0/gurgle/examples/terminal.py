"""Example using gurgle tools to do online audio outlier learning with terminal output"""
...
ascii_levels_fv_to_str = partial(
    multipart_fv_str, num_to_viz_str=ascii_levels, fv_num_sep=DFLT.fv_num_sep
)

raw_fv_to_str = partial(
    multipart_fv_str, num_to_viz_str=str, fv_num_sep=DFLT.fv_num_sep
)
...
def chk_to_fv_viz(chk: np.ndarray, chk_to_fv=dflt_chk_to_fv, fv_to_str=dflt_fv_to_str):
...
def make_and_consume_iterator(
    iterator_func, *interator_func_args, **interator_func_kwargs
):
...
def launch(
    ...
    **kwargs
    ...
    print('Starting...')
    with WfChunks(
        input_device_index=input_device_index, sr=sr, chk_size=chk_size, **kwargs
    ) as wf_chunks:
        try:
...
wf_to_spectr = Pipe(wf_to_chks, iterize(chk_to_spectr), iterize(bands_sp))

from gurgle.sp_gurgle import ResidueGurgle, ScaledIncrementalPCA
