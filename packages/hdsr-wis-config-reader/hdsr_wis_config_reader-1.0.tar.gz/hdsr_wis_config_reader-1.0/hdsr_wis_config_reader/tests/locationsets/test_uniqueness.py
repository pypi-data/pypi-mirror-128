...
from hdsr_wis_config_reader.tests.fixtures import locsets
...
def test_general_location_set_dict(locsets):
    for locset in locsets:
        assert locset.general_location_sets_dict == general_location_set_dict


def _attribs(locset: location_sets.base.LocationSetBase):
    return locset.get_validation_attributes(int_pars=None)


def _meta(locset: location_sets.base.LocationSetBase, general_location_set_dict: List[Dict]):
    return [loc_set for loc_set in general_location_set_dict if loc_set["id"] == locset.fews_name][0]["csvFile"]


def test_unique_validation_attributes(locsets):
    hoofdloc, subloc, wsloc, mswloc, psloc = locsets
    assert _attribs(hoofdloc) and _attribs(subloc) and _attribs(wsloc)
    assert _attribs(mswloc) == [] and _attribs(psloc) == []
    assert _attribs(hoofdloc) != _attribs(subloc) != _attribs(mswloc)


def test_unique_csvfile_meta(locsets):
    hoofd, sub, ws, msw, ps = locsets
    ...
    assert hoofd.csv_file_meta == _meta(locset=hoofd, general_location_set_dict=general_location_set_dict)
    assert sub.csv_file_meta == _meta(locset=sub, general_location_set_dict=general_location_set_dict)
    assert ws.csv_file_meta == _meta(locset=ws, general_location_set_dict=general_location_set_dict)
    assert ps.csv_file_meta == _meta(locset=ps, general_location_set_dict=general_location_set_dict)
    assert msw.csv_file_meta == _meta(locset=msw, general_location_set_dict=general_location_set_dict)


def test_unique_attribute_files(locsets):
    hoofd, sub, ws, msw, ps = locsets
