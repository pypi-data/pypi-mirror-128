...
class TopdeskConnector:
	...
	def get_uploaded_dataset_value(self, template_id: str, field_id: str):
		"""
		Verkrijg de waarde van een uploaded dataset field. Dit is nuttig voor bijvoorbeeld het ophalen van het merk van een asset die is genoteerd in een dropdown.

		Args:
			template_id: ID van de dataset waarin de field zich begeeft. Bijvoorbeeld Merk.
			field_id: ID van de geselecteerde waarde.

		Returns: str: textwaarde van field.
		...
		field = self.make_request('GET', 'assetmgmt/assets/', params=params)
		if field and field.get('dataSet', None):
			return field.get('dataSet')[0].get('text', None)
		else:
			raise Exception('Template bevat geen dataset.')
