...
class Client:
    @staticmethod
    def get_client(host, redis_client: XRedis, mq_manager: MQManager, async_mq_mode=True, cache_path='../cache', custom_async_loop=None):
        client = Client(host, redis_client, mq_manager, async_mq_mode)
        return client
    
    def __init__(self, host, redis_client: XRedis, mq_manager: MQManager, async_mq_mode=True, cache_path='../cache', custom_async_loop=None) -> None:
        self.host = host
    ...
    def put_from_cloud(self, url, bucket, bucket_name, timeout=10, file_ttl=-1):
        # save to cache
    ...
    async def put_from_cloud_async(self, url, bucket, bucket_name, timeout, file_ttl, mq=None):
        # save to cache
        task_id = 'tmp_file_{}_{}'.format(bucket, bucket_name)
