    ...
    with cache_engine.lock(key, ttl=30) as lock:
        if lock:
            try:
                ...
                # ret, e = await file_manager.download(tmp_url, file_path, use_cache=True, force_v1=True)
...
async def file_transfer_get(bucket, bucket_name):
    key = 'file_transfer_get_{}_{}'.format(bucket, bucket_name)
    if await cache_engine.get_cache_async(key, return_obj=False) is not None:
        return
    ...
    with cache_engine.lock(key, ttl=30) as lock:
        if lock:
            ...
            ret, e = hmfs_client.put_to_cloud(file_path, bucket, bucket_name) #await file_manager.upload_file(file_path,'tmp_file_' + bucket, bucket_name, False, backend='qiniu')
            if ret:
                cache_engine.cache('tmp_file_{}_{}'.format(bucket, bucket_name), {
                    'ret': True,
                    'url': e
