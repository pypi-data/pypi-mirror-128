...
class Session(_Session):
    ...
    def request(self, method, url, *args, **kwargs):
        ...
        logger.info("Sending HTTP %s request: %s", method, response.request.url)
        logger.info("Received HTTP %s response: %s (status code: %d)", method, response.request.url, response.status_code)
        return response
