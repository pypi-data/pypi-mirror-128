...
class AuthService(object):
    ...
    def _get_jwt(self) -> None:
        ...
        if self.pat is None:
            ...
            try:
                resp = requests.request("POST", f"{self.host}/auth/v1/pat/exchange/jwt", headers=headers)
            except requests.exceptions.ConnectionError:
                raise HossException(f"Cannot reach Hoss auth service for server '{self.host}'. "
    ...
    def get_user(self, user_name):
        resp = requests.request("GET", f"{self.host}/auth/v1/user/{user_name}", headers=self.headers())
        if not resp.ok:
    ...
    def get_group(self, group_name):
        resp = requests.request("GET", f"{self.host}/auth/v1/group/{group_name}", headers=self.headers())
        if not resp.ok:
            if "not found" in resp.text:
                raise NotFoundException()
    ...
    def create_group(self, group_name, description="No description provided"):
        ...
        resp = requests.request("POST", f"{self.host}/auth/v1/group", json=data, headers=self.headers())
        if not resp.ok:
    ...
    def delete_group(self, group_name):
        resp = requests.request("DELETE", f"{self.host}/auth/v1/group/{group_name}", headers=self.headers())
        if not resp.ok:
            print(f"{resp.status_code} {resp.reason}: {resp.text}")
            raise HossException("Could not delete group: " + resp.text)

    def add_user_to_group(self, group_name, username) -> None:
        resp = requests.request("PUT", f"{self.host}/auth/v1/group/{group_name}/user/{username}",
    ...
    def remove_user_from_group(self, group_name, username) -> None:
        resp = requests.request("DELETE", f"{self.host}/auth/v1/group/{group_name}/user/{username}", headers=self.headers())
        if not resp.ok:
            print(f"{resp.status_code} {resp.reason}: {resp.text}")
