...
singularity exec /softs/applications/containers/mafft/mafft_7.307_v1.0.simg mafft --{} --thread {} --quiet {} > {}
"""
               ).format(high_accuracy_method,
                        cores,
                        path_input,
                        path_output)
    else:
...
singularity exec /softs/applications/containers/mafft/mafft_7.307_v1.0.simg mafft --thread {} --quiet {} > {}
"""
               ).format(cores, path_input, path_output)
...
def hpc_tcoffee(path_input, path_output, seq_type="dna", pairwise=False, debug=False):
    ...
    cmd = ("""
module load singularity/3.8.3 &
singularity exec /softs/applications/containers/t_coffee/tcoffee_13.39.0_v1.0.simg t_coffee -in=S{} -outfile {} -type {}
"""
           ).format(path_input, path_output, seq_type)
