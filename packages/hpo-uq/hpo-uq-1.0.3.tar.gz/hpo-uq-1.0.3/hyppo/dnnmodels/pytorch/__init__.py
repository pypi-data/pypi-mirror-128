...
def train(itrial     = 0,
    ...
    logging.info('-'*40)
    test_loss = evaluate(device, data['test'], model, criterion, accuracy, **kwargs)
...
def evaluate(device, data, model, criterion, accuracy=False, update=False, **kwargs):
    model.eval()
    start_time = time.time()
...
#     plt.plot(torch.stack(y_pred).detach().numpy().squeeze(),color='orange')
#     plt.savefig('pred')
    loss = criterion(torch.stack(y_pred).to(device), y_real.to(device))
    logging.info('\t\tLoss : {:>11.5f}'.format(loss))
    if accuracy:
        logging.info('\t\tAcc. : {:>11.5f} %'.format(100*acc/len(data.sampler)))
    logging.info('\t\tTime : {:>11.5f} s'.format(time.time()-start_time))
    return loss
