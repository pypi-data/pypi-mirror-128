...
#  
#  ############################################################################################
#  def run_uq(data, criterion, model, state, hyperprms, loss, update, uq={}, **kwargs):
#      trained_models = []
#      for state_path in state:
#          state_model = model
#          state_model.load_state_dict(torch.load(state_path), strict=False)
#          trained_models.append(state_model)
#      data_shape = shape_and_size(data['test'], **kwargs)
#      data_variance = uq.get('data_noise',0.0)
#      weights = uq.get('uq_weights', [0.5,0.5])
#      #update  = uq['update']
...
#          logging.warning('Running uncertainty quantification with dropout rate 0 is not optimal.')
#          logging.warning('Dropout rate set to 5% for Monte Carlo dropout UQ analysis as a default.')
#      pred_mean, total_var, confidence_interval = uq_quantifer(data, trained_models, data_shape, criterion, data_variance, weights,update)
