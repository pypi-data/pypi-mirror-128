...
def train_evaluation(x_sc, samples, trainer, model, dist, output='out', data={}, uq={}, **kwargs):
    ...
    results['loss'] = [model_results['loss'] for model_results in all_results]
    results['models'] = [model_results['models'] for model_results in all_results]
    ...
    if dist['rank']==0:
        ...
        if 'uq_on' in uq.keys() and uq['uq_on']==True:
            ...
            logging.info('\tOuter Loss : {:>11.5f}'.format(numpy.mean(results['loss'])))
