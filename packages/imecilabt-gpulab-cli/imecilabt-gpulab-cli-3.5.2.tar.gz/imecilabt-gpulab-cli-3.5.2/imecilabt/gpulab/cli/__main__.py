...
def localhost_mode_cli(ctx):
    """GPULab client version 3.5.2 (in LOCALHOST_MODE!)
        ...
        if raw:
            ...
            for e in events:
                ...
                if event.type == JobEventType.STATUS_CHANGE:
                    print('{} STATE CHANGED -> {}'.format(_date_to_str(event.time), event.new_status.name))
                else:
