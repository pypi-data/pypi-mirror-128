...
def _get_token_file_name():
    return os.path.join(posixpath.expanduser("~"), ".infinstor", "token")
...
def get_id_token(region):
    ...
    # we have so many copies of tokenfile writing code in our codebase, there is a chance that not all copied write idToken.  handle this with 'if not id_token' below.
    if ((token_time + (30 * 60)) < time_now) or not id_token:
        logger.error(f'get_id_token(): InfinStor token has expired or id_token not found in tokenfile: Calling renew: id_token found in token file={id_token == None} token_time={token_time}; (token_time + (30 * 60)={token_time + (30 * 60)}; time_now={time_now}')
        renew_token(region, _get_token_file_name(), refresh_token, client_id, service)
...
def get_token(region, tokfile, force_renew):
    ...
    if (force_renew == True):
        logger.error("get_token(): Forcing renewal of infinstor token")
    ...
    if ((token_time + (30 * 60)) < time_now):
        logger.error(f'get_token(): InfinStor token has expired. Calling renew: token_time={token_time}; (token_time + (30 * 60)={(token_time + (30 * 60))}; time_now={time_now}')
        renew_token(region, tokfile, refresh_token, client_id, service)
        token, refresh_token, token_time, client_id, service, token_type, id_token = read_token_file(tokfile)
        return token, service
    else:
