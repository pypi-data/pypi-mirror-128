...
def circ_diff(a,b,r=360):
    ...
    for k in np.arange(len(a)):
        ...
        if diff[k] < -r//2:
            ...
            diff[k] = b[k] - a[k] - r
    return diff
def find_vm_sd(channel_responses,stim_max=180,plotting=False):
    # fits to von Mises distribution
    f = sp.interpolate.interp1d(np.arange(len(channel_responses)),channel_responses)
    yReal = f(np.arange(0,len(channel_responses)-1,(len(channel_responses)-1)/stim_max))
    yReal = np.roll(yReal,stim_max-np.argmax(yReal))
    
    losses = [np.abs(np.sum(yReal - make_gaussian_iter(stim_max//2,sd,stim_max))) for sd in np.arange(10,180)]

    if plotting:
        plt.plot(yReal)
        plt.plot(make_gaussian_iter(stim_max//2,np.arange(10,180)[np.argmin(losses)],stim_max))
