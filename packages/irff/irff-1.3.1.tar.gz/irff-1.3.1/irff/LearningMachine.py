...
from .dft.siesta import siesta_opt 
from .dft.qe import qeopt # qemd,
...
class LearningMachine(object):
  ...
  def __init__(self,initConfig='poscar.gen',
               ...
               step=20000,md_step=20,MaxMDstep=200,MinMDstep=50,colFrame=10,
               mom_step=100,
  ...
  def run(self):
      ...
      while running:
          ...
          e_aimd,eml_,dEmax_,d2Emax_,LabelDataLog = self.aimd(atoms,cwd,run_dir,dft_step,gen,learnWay)
          ...
          while (accu<=self.accCriteria or loss>=self.lossCriteria) and training:        
                ...
                loss,accu,accMax,p,zpe,tstep = self.trainer(dataset=trajs_,
                                                     ...
                                                     spv_bo=self.spv_bo,bolo=self.bolo,boup=self.boup,
  ...
  def aimd(self,atoms,cwd,run_dir,tstep,gen,learnWay):
      mkdir(run_dir)
      chdir(cwd+'/'+run_dir)
      ...
      if learnWay==1:
         if self.dft=='siesta':
            ...
            #               gen=cwd+'/'+gen,
            #               kpts=self.kpts)         ## geomentry optimize
...
if __name__ == '__main__':
   ...
   lm = LearningMachine(initConfig='c14.gen',
                     ...
                     T=500,
                     angmax=20.0,
                     lossCriteria=20.0,lossTole=30.0,
                     CheckZmat=False,
                     # regularize=True,
                     ...
                     bo_univeral_nn=None,#['O-O'],
                     be_univeral_nn=None,#['O-O'],
                     mf_univeral_nn=None,#['C'],
                     spv_be=False,bore={'others':0.22},
                     spv_ang=False,
                     vdw_univeral_nn=None,vdwnn=True,
                     weight={# 'no2-l':20.0,
                             'others':2.0},
