...
class MPNN(ReaxFF):
  def __init__(self,libfile='ffield',dataset={},
               ...
               boup={'others':[(1.5,1.0,1.0,1.0)]},
               bolo={'others':[(1.5,0.0,0.0,0.0)]},
               bom={'others':1.2},
               pim={'others':10.0},
               spv_bm=False,
               spv_be=False,
               ...
               messages=1,
               TwoBodyOnly=False,
               be_univeral_nn=None,bo_layer=[6,1],
      ...
      for bd in self.bonds: 
          ...
          if self.nbd[bd]>0:
             ...
             if self.spv_be:
                ...
                self.diffe_[bd]  = tf.reduce_sum(input_tensor=tf.nn.relu((bore_+self.EBD[bd])*fe))
                ...
                l_atol  = tf.add(self.diffe[bd]*self.lambda_bd,l_atol)    ## penalty iterm for bond order

             if self.spv_bo:
                if (bd in self.boup) or (bdr in self.boup):
                   bd_ = bd if bd in self.boup else bdr
                   for boup_ in self.boup[bd_]:
                       r_,bosi_u,bopi_u,bopp_u = boup_  
                       fu      = tf.where(tf.greater(self.rbd[bd],r_),1.0,0.0) ##### 
                       diff_si = tf.reduce_sum(input_tensor=tf.nn.relu((self.bop_si[bd] - bosi_u)*fu))
                       diff_pi = tf.reduce_sum(input_tensor=tf.nn.relu((self.bop_pi[bd] - bopi_u)*fu))
                       diff_pp = tf.reduce_sum(input_tensor=tf.nn.relu((self.bop_pp[bd] - bopp_u)*fu))
                       l_atol  = tf.add((diff_si+diff_pi+diff_pp)*self.lambda_bd,l_atol) 

                if (bd in self.bolo) or (bdr in self.bolo):
                   bd_ = bd if bd in self.bolo else bdr
                   for bolo_ in self.bolo[bd_]:
                       r_,bosi_l,bopi_l,bopp_l = bolo_
                       fl      = tf.where(tf.less_equal(self.rbd[bd],r_),1.0,0.0) #####
                       diff_si = tf.reduce_sum(input_tensor=tf.nn.relu((bosi_l - self.bop_si[bd])*fl))
                       diff_pi = tf.reduce_sum(input_tensor=tf.nn.relu((bopi_l - self.bop_pi[bd])*fl))
                       diff_pp = tf.reduce_sum(input_tensor=tf.nn.relu((bopp_l - self.bop_pp[bd])*fl))
                       l_atol  = tf.add((diff_si+diff_pi+diff_pp)*self.lambda_bd,l_atol)  ## penalty iterm for bop

             if self.EnergyFunction == 3:
                fesi = tf.where(tf.less_equal(self.bo0[bd],self.botol),1.0,0.0) ##### bo <= 0.0 that e = 0.0
                self.diffesi[bd]  = tf.reduce_sum(tf.nn.relu(self.esi[bd]*fesi))
