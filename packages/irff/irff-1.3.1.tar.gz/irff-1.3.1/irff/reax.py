...
class ReaxFF(object):
  def __init__(self,libfile='ffield',dataset={},
               dft='ase',atoms=None,
               cons=['val','vale','lp3','cutoff','hbtol'],# 'acut'
  ...
  def clip_parameters(self):
      ...
      for k in self.v:
          ...
          if key == 'n.u.':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0000999,0.002)
          elif key=='hbtol':
             self.p[k] = tf.clip_by_value(self.v[k],0.0001,0.0002)
          elif key=='cutoff':
             self.p[k] = tf.clip_by_value(self.v[k],0.009999,0.012)
          elif key=='ovun5':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,999.0*self.unit)
          elif key=='ovun1':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,10.0)
          elif key in ['boc4','boc5','vdw1']:
             self.p[k] = tf.clip_by_value(self.v[k],0.00000001,100.0)
          elif key == 'boc3':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,50.0)
          elif key == 'gammaw':
             self.p[k] = tf.clip_by_value(self.v[k],0.5,19.0)
          elif key == 'gamma':
             self.p[k] = tf.clip_by_value(self.v[k],0.001,6.0)
          elif key == 'pen1':
             self.p[k] = tf.clip_by_value(self.v[k],-69.0*self.unit,199.0*self.unit)
          elif key == 'Devdw':
             D_= 600.0 if self.vdwnn else 0.8
             self.p[k] = tf.clip_by_value(self.v[k],0.01*self.unit,D_*self.unit)
          elif key == 'Dehb':
             self.p[k] = tf.clip_by_value(self.v[k],-3.0*self.unit,3.0*self.unit)
          elif key == 'Desi':
             if self.nn:
                self.p[k] = tf.clip_by_value(self.v[k],1.0*self.unit,999.0*self.unit)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],100.0*self.unit,999.0*self.unit)
          elif key== 'Depi':
             self.p[k] = tf.clip_by_value(self.v[k],0.000*self.unit,999.0*self.unit)
          elif key== 'Depp':
             self.p[k] = tf.clip_by_value(self.v[k], 0.000*self.unit,999.0*self.unit)
          elif key=='tor1':
             self.p[k] = tf.clip_by_value(self.v[k],-39.9900,-0.0)
          elif key== 'ovun2':
             ...
             if self.nn:
                if key =='bo1':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.5,-0.25)
                elif key =='bo3':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.5,-0.25)
                elif key =='bo5':
                   self.p[k] = tf.clip_by_value(self.v[k],-0.5,-0.25)
                # self.p[k] = tf.clip_by_value(self.v[k],-0.35,-0.25)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],-50.0,-0.0001)
          elif key in ['bo2','bo4','bo6']:
             if self.nn:
                if key =='bo2':
                   self.p[k] = tf.clip_by_value(self.v[k],6.0,10.0)
                elif key =='bo4':
                   self.p[k] = tf.clip_by_value(self.v[k],5.5,10.0)
                elif key =='bo6':
                   self.p[k] = tf.clip_by_value(self.v[k],5.0,10.0)
                # self.p[k] = tf.clip_by_value(self.v[k],4.5,5.0)
             else:
                self.p[k] = tf.clip_by_value(self.v[k],0.0,50.0)
          elif key == 'boc1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.001,20.0)
          elif key== 'lp1':
             self.p[k] = tf.clip_by_value(self.v[k],1.0,50.0)
          elif key== 'lp2':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,99.0*self.unit)
          elif key== 'lp3':
             self.p[k] = tf.clip_by_value(self.v[k],1.0,75.0001)
          elif key == 'be1':
             ...
             self.p[k] = tf.clip_by_value(self.v[k],0.0,20.0)
          elif key=='val3':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,8.0)
          elif key=='val1':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,200.0*self.unit)
          elif key=='val2':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,8.0)
          elif key=='val9':
             self.p[k] = tf.clip_by_value(self.v[k],0.0,8.0)
  ...
  def set_parameters(self,opt=[],libfile=None):
      ...
      for k in self.p_:
          ...
          if key == 'zpe':
             continue
          if key != 'n.u.':
             if key in self.cons or k in self.cons:
                if key in self.punit:
                   self.v[k] = tf.constant(np.float32(self.unit*self.p_[k]),name=k)
                else:
                   self.v[k] = tf.constant(np.float32(self.p_[k]),name=k)
  ...
  def run(self,learning_rate=1.0e-4,method='AdamOptimizer',
               step=2000,print_step=10,writelib=100,close_session=True):
      ...
      for i in range(step+1):
          ...
          if self.optmol:
             loss_ = loss - latol - ME_*self.lambda_me
          else:
             loss_ = loss - latol
...
def test_reax(dataset=None,batch_size=200,dft='siesta'):
