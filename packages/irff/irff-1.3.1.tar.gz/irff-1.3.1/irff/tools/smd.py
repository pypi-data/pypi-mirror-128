...
def md(ncpu=40,T=100,us='F',tstep=5000,dt=1.0,gen='poscar.gen',index=-1):
...
def mmd(ncpu=12,T=300,us='F',tstep=100,dt=1.0,gen='strucs.traj'):
...
def opt(ncpu=20,T=2500,us='F',tstep=2001,dt=1.0,P=6.0,coord='zmatrix',
        gen='poscar.gen'):
    if exists('siesta.MDE') or exists('siesta.MD_CAR'):
       system('rm siesta.MDE siesta.MD_CAR')
    A = read(gen,index=-1)
    A = press_mol(A)
    print('\n-  running siesta opt ...')
    siesta_opt(A,ncpu=ncpu,us=us,P=P,coord=coord)
...
def nvt(T=350,time_step=0.1,tot_step=10000,gen='siesta.traj',mode='w'):
    ''' a gulp MD run '''
    A = read(gen,index=-1)
...
if __name__ == '__main__':
   ...
   argh.add_commands(parser, [md,mde,mmd,cmd,opt,traj,nvt,pm])
