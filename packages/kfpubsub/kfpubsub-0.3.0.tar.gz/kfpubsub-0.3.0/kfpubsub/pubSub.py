...
class PubSub(object):
    ...
    def __init__(
        ...
        if enable_kafka:
            ...
            self.kafka_emit_prefix = kafka_emit_prefix
            self.kafka_receive_prefix = kafka_receive_prefix
    ...
    def create_kafka_topics(self):
        ...
        for topic in self.kafka_topics:
            for topic_prefix in self.kafka_receive_prefix:
    ...
    def emit(self, event, message, emit=False):
        formatted_event = "%s%s" % (self.kafka_emit_prefix, event)
        # Convert dict to string
        message_str = json.dumps(message)
        logger.debug(" [x] Sent %r:%r" % (formatted_event, message_str))
        ...
        self.connection.publish(channel=formatted_event, message=message_str)
        self.pubsub.close()
    ...
    def receive_kafka(self, listeners):
        ...
        for k, v in listeners.items():
            for prefix in self.kafka_receive_prefix:
