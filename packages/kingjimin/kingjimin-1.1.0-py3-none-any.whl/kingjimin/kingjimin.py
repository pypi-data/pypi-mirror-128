...
class Error():
    ...
    def status_msg(self):
        ...
        if self.status_code == 400:
            ...
            return f"Internal Server Error({self.status_code})! Please contact to sanggeon.park@nota.ai"
        else:
            return f'Unknown Error. Original error message - {self. df_msg}'

class DeviceFarmGateway():
    def __init__(self, addr, port, poll_time=5):
        self.address = f"http://{addr}:{port}/"
        self.timeout = poll_time

    def _node_model_compatibility(self, model_uuid, node_uuid):
        node, error = self.get_node_info(node_uuid)
        model, error = self.get_model_info(model_uuid)
        if model.type not in node.available_model_types:
            return False
        else:
            return True
    ...
    @dispatch(str)
    def get_node_info(self, node_uuid: str):
        '''
    ...
    @dispatch(Node)
    def get_node_tasks(self, node: Node):
        return self.get_node_tasks(node.uuid)

    @dispatch(str)
    def get_node_tasks(self, node_uuid: str):
        '''
    ...
    def get_model_info(self, model_uuid: str):
        '''
    ...
    def upload_model(self, model_path: str, model_type: str):
        ...
        if model_type not in support_type:
            msg = f"Unsupported model type(received {model_type}). We can receive (list of str/list of Node/str/Node) types\n"
    ...
    @dispatch(Model)
    def download_model(self, model: Model):
        return self.download_model(model.uuid)

    @dispatch(str)
    def download_model(self, model_uuid: str):
        '''
        ...
        for node in node_list:
            if type(node) is Node:
                if not self._node_model_compatibility(model_uuid, node.uuid):
                    msg = f"Can't run model on this node. Please check available nodes."
                    return None, Error(proc_msg = msg)
                else: 
    ...
    @dispatch(Model)
    def all_benchmarks_on_model(self, model: Model):
        return self.all_benchmarks_on_model(model.uuid)

    @dispatch(str)
    def all_benchmarks_on_model(self, model_uuid: str):
        '''
    ...
    @dispatch(Model, str)
    def benchmarks_on_model(self, model: Model, bench_uuid: str):
        return self.benchmarks_on_model(model.uuid, bench_uuid)
        
    @dispatch(str, str)
    def benchmarks_on_model(self, model_uuid: str, bench_uuid: str):
        '''
        model_uuid에 해당하는 Model의 벤치마크 정보들을 가져온다.
        model_uuid: Benchmark 정보들을 가져올 Model의 uuid
        Return:
