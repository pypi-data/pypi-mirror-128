...
class Inference_Result(BaseData):
    ...
    def __init__(self, inf_data):
        ...
        mem_data = inf_data.get("memory_footprints", None)
        if mem_data.get("cpu", None) is not None:
            self.memory_footprints = Memory_Footprint_CGPU(mem_data)
        else:
            self.memory_footprints = Memory_Footprint(mem_data)
...
class Memory_Footprint_CGPU(BaseData):
    def __init__(self, mem_data):
