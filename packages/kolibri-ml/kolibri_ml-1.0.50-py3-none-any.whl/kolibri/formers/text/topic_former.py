...
class TextTopicFormer():
    ...
    def __init__(self,output_folder, data, target, configs={}):
        ...
        if data is not None:
            ...
            #
            # trainer.fit([v for v in self.data[self.target].values if v is not None], None)
            self.trainer=self._build(output_folder, progress)
            self.topics={"Topic_"+str(i):str(t) for i, t in enumerate(self._model.model.get_top_topics_words())}
            self.topics=pd.DataFrame(self.topics)
            if self.verbose:
                clear_output()
                progress.value+=1
...
#                    common_words=[(w, c) for w, c in self._model.indexer.token2count.items()][:100]
            ...
            #     if topic_num is None:
            #         logger.warning("topic_num set to None. Plot generated at corpus level.")
            #         sentiments = self.data[self.target].map(
            ...
            #
            #         if display_format == "streamlit":
            #             sentiments = sentiments[self.target].iplot(
            #                 kind="hist",
            #                 bins=50,
            #                 xTitle="polarity",
            #                 linecolor="black",
            ...
            #             "SubProcess predict() called =================================="
            #         )
            #         assigned_df = self.model_interpreter.predict(model, verbose=False)
            #         logger.info(
            #             "SubProcess predict() end =================================="
            #         )
            #         filtered_df = assigned_df.loc[
            #             assigned_df["Dominant_Topic"] == topic_num
            #             ]
            #         sentiments = filtered_df[self.target].map(
            #             lambda text: TextBlob(text).sentiment.polarity
            #         )
            #         sentiments = pd.DataFrame(sentiments)
            #         logger.info("Rendering Visual")
