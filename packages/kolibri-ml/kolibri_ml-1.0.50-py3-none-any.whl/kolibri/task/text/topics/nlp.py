...
def setup(
    ...
    try:
        ...
        sys.exit(
            "(Type Error): spacy english model_type is not yet downloaded. See the documentation of setup to see installation guide."
        )
...
def create_model(
    ...
    This function trains a given topic model_type. All the available models
    ...
    model_type: str, default = None
        Models available in the model_type library (ID - Name):
    ...
    multi_core: bool, default = False
        True would utilize all CPU cores to parallelize and speed up model_type training.
        Ignored when ``model_type`` is not 'lda'.
    ...
    **kwargs: 
    ...
    logger.info(
        """create_model(model_type={}, multi_core={}, num_topics={}, verbose={}, system={})""".format(
    ...
    if logging_param and system:
        ...
        with mlflow.start_run(run_name=topic_model_name) as run:
            ...
            # Log model_type and related artifacts
            if model_name_short == "nmf":
                ...
                mlflow.log_artifact("model_type.pkl")
                size_bytes = Path("model_type.pkl").stat().st_size
                os.remove("model_type.pkl")

            elif model_name_short == "lda":
                model.save("model")
                mlflow.log_artifact("model")
                mlflow.log_artifact("model_type.expElogbeta.npy")
                mlflow.log_artifact("model_type.id2word")
                mlflow.log_artifact("model_type.state")
                size_bytes = (
                    Path("model").stat().st_size
                    + Path("model_type.id2word").stat().st_size
                    + Path("model_type.state").stat().st_size
                )
                os.remove("model")
                os.remove("model_type.expElogbeta.npy")
                os.remove("model_type.id2word")
                os.remove("model_type.state")

            elif model_name_short == "lsi":
                model.save("model")
                mlflow.log_artifact("model")
                mlflow.log_artifact("model_type.projection")
                size_bytes = (
                    Path("model").stat().st_size
                    + Path("model_type.projection").stat().st_size
                )
                os.remove("model")
                os.remove("model_type.projection")
...
def assign_model(model, verbose=True):
    ...
    logger.info(
        """assign_model(model_type={}, verbose={})""".format(str(model), str(verbose))
    )
    ...
    logger.info("Determining model_type type")

    # determine model_type type
    ...
    logger.info("model_type type: " + str(mod_type))
...
def plot_model(model=None, plot="frequency", topic_num=None, save=False, system=True, display_format = None):
    ...
    logger.info(
        """plot_model(model_type={}, plot={}, topic_num={}, save={}, system={})""".format(
    ...
    # plots without topic model_type
    if model is None:
        not_allowed_wm = ["tsne", "topic_model", "topic_distribution"]
        if plot in not_allowed_wm:
            sys.exit(
                "(Type Error): Model parameter Missing. Plot not supported without specific model_type passed in as Model param."
            )
...
def tune_model(
    ...
    >>> from pycaret.nlp import *
    >>> exp_name = setup(data = kiva, target = 'en')        
    >>> tuned_lda = tune_model(model_type = 'lda', supervised_target = 'status')
...
def save_model(model, model_name, verbose=True, **kwargs):
    ...
    """
    This function saves the trained model_type object into the current active
    ...
    **kwargs: 
    ...
    Returns:
        Tuple of the model_type object and the filename.
    ...
    logger.info(
        """save_model(model_type={}, model_name={}, verbose={})""".format(
...
def load_model(model_name, verbose=True):
    ...
    """
    This function loads a previously saved model_type.
