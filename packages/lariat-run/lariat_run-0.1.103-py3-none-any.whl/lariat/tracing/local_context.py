...
from .transport import _make_request
...
class LocalContext(object):
    ...
    def add_new_span_to_parent_tree(self, new_trace=None):
        try:
            ...
            log.error("failed to add span to parent tree: %s", e)
    ...
    def add_span_to_trace_buffer(self, span_record):
        try:
            ...
            log.error("failed to log span: %s", e)
    ...
    def add_event_to_event_buffer(self, event_group_id, name, meta):
        try:
            ...
            self.event_buffer[event_group_id].append({"event_id": random.getrandbits(32), "name": name, "meta": meta})
            if len(self.event_buffer) >= TracerConfig.event_buffer_size and not self._dry_run:
