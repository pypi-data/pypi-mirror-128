...
class GaussianFitter:
    ...
    def fit2_eqsigma(self, x, y, pmin=(1e-6, -500, 1), pmax=(np.inf, 500, 500), p1_init=(1, 0, 5), nmc=0, dposmax=500):
        ...
        if nmc <= 0:
            ...
            p2_opt = least_squares(res_gauss2_eqsigma, p2_init, args=(x, y), bounds=(p2_min, p2_max))
            return p2_opt
        else:
            # random initialization for p2 + Monte Carlo
            p2_mc = np.zeros((nmc, 5))
            cost_mc = np.zeros((nmc))
            p2_min = tuple([*pmin, *pmin[:2]])
            p2_max = tuple([*pmax, *pmax[:2]])
            for imc in range(nmc):
                p2_init = [p1_opt.x[0], p1_opt.x[1], p1_opt.x[2] / 2, p1_opt.x[0],
                           p1_opt.x[1] + np.random.uniform(-1, 1) * dposmax]
