...
def Dataset(name="wdbc"):
    return idict() >> let(dataset, name=name)


def File(name="iris.arff") -> idict:
    """
    >>> File().show(colored=False)
    {
        ...
        "_id": "7V_bdb0596bdac3a55eaa5e738d23b88294dfaea",
        "_ids": {
            "df": "7V_bdb0596bdac3a55eaa5e738d23b88294dfaea"
        }
    }
    """
    split = name.split(".")
    ...
    ext = split[-1]
    if ext == "arff":
        return idict() >> let(arff2df, name=name)
    elif ext == "csv":
        return idict() >> let(csv2df, name=name)


def dataset(name="wdbc"):
    X, y = fetch_openml(name, return_X_y=True)
    return {"X": X, "y": y, "_history": ..., "_code": ...}


dataset.metadata = {"id": "dataset...........................lazyds", "name": "arff2df", "description": "Arff loader"}


def arff2df(name=None):
    """
    >>> from idict import let
    >>> d = idict() >> let(arff2df, name="iris")
    >>> d.show(colored=False)
    {
        "df": "→(name)",
        "_id": "rmYX.pQabv7SubMPVFzu6S3lrew25w6ds5WwYZxh",
        "_ids": {
            "df": "rmYX.pQabv7SubMPVFzu6S3lrew25w6ds5WwYZxh"
        }
    }
    """
    with open(name + ".arff") as f:
        df = a2p.load(f)
    return {"df": df, "_history": ..., "_code": ...}


arff2df.metadata = {"id": "arff2df...........................lazyds", "name": "arff2df", "description": "ARFF loader"}


def csv2df(name=None):
    """
    >>> from idict import let
    >>> d = idict() >> let(csv2df, name="iris")
    >>> d.show(colored=False)
    {
        "df": "→(name)",
        "_id": "ZemIq.VYGd60iRWc5Oe.1ayF4JeIRMwxqw3jz5wi",
        "_ids": {
            "df": "ZemIq.VYGd60iRWc5Oe.1ayF4JeIRMwxqw3jz5wi"
        }
    }
    """
    return {"df": read_csv(name + ".arff"), "_history": ..., "_code": ...}


csv2df.metadata = {"id": "csv2df............................lazyds", "name": "csv2df", "description": "CSV loader"}


def df2arff(field=None, **kwargs):  # TODO: change Noine to a def value "df" and correct fosh
    """
    >>> from idict import Ø, let
    >>> d = File("iris.arff") >> let(arff, field="df")
    >>> d.arff
    ...
    """
    return {"arff": a2p.dumps(kwargs[field]), "_history": ..., "_code": ...}


df2arff.metadata = {"id": "df2arff...........................lazyds", "name": "df2arff",
                    "description": "DF to ARFF converter"}
...
df2np.metadata = {
    "id": "df2np.............................lazyds",
    "name": "df2np",
    "description": "DF to numpy converter"
}
