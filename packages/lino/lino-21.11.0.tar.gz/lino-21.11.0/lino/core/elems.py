...
class StoryElement(LightWeightContainer):
    ...
    tables with `display_mode` 'story'.
...
def create_layout_element(lh, name, **kw):
    ...
    if isinstance(de, type) and issubclass(de, tables.AbstractTable):
        ...
        if isinstance(lh.layout, FormLayout):
            ...
            elif de.display_mode == constants.DISPLAY_MODE_STORY:
                e = StoryElement(lh, de, name, **kw)
                lh.add_data_elem(e.field)
                return e

            elif de.display_mode == constants.DISPLAY_MODE_HTML:
                # if de.editable:
                #     a = de.insert_action
                #     if a is not None:
            ...
            elif de.display_mode == constants.DISPLAY_MODE_SUMMARY:
