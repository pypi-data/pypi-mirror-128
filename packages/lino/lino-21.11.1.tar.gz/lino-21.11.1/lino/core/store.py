...
class PreviewTextStoreField(StoreField):
    ...
    def value2list(self, ar, v, l, row):
        if isinstance(row, PhantomRow):
            ...
            l.append(getattr(row, self.name))

    def value2dict(self, ar, k, v, d, row):
        if not isinstance(row, PhantomRow):
            d[self.name] = getattr(row, self.name)
            d[self.name+ "_short_preview"] = getattr(row, self.name+ "_short_preview")
            d[self.name+ "_full_preview"] = getattr(row, self.name+ "_full_preview")
