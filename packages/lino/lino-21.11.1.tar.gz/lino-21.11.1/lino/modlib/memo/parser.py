...
class Parser(object):
    ...
    def register_command(self, cmdname, func):
        ...
        `func` is the command handler.  It must be a callable which will be
    ...
    def register_usage_manager(self, cmdname, func):
        self.usage_managers[cmdname] = func
    ...
    def register_django_model(
            self, name, model, cmd=None, rnd=None, title=str, manage_usage=None):
        """
        ...
        - `manage_usage` a function that takes a list of db objects used by the
        command as an argument and can be used to modify other related database
        records.
        """
...
All remaining arguments are used as the text of the link.
    ...
    def cmd_match_func(self, ar, usages):
        ...
        def func(matchobj):
            ...
            try:
                return self.format_value(cmdh(ar, params, cmd, usages))
            except Exception as e:
                # logger.warning(e)
                # don't log an exception because that might cause lots of
                # emails to the admins.
    ...
    def parse(self, src, ar=None, context=None):
        ...
        src = COMMAND_REGEX.sub(self.cmd_match_func(ar, usages), src)

        for cmdname, cmdusages in usages.items():
            self.usage_managers.get(cmdname)(ar, cmdusages)
