...
class Addressable(object):
    ...
    def get_address_html(self, *args, **kwargs):
        ...
        See usage examples in :doc:`/topics/utils`.

        In case no argument is given, it returns an empty paragraph of the html
        form "<p/>". Optional attributes for the enclosing tag, usually a `<p>`,
        can be given as keyword arguments. If `min_height` is specified, make
        sure that the string contains at least that many lines. Adds as many
        empty lines (``<br/>``) as needed.  This is useful in a template which
        wants to get a given height for every address. Any arguments are
        forwarded to :meth:`lines2p <etgen.html.lines2p>` which is used to pack
        the address lines into a paragraph (see there for more examples).
    ...
    def has_address(self):
        ...
        Usage examples in :doc:`/topics/utils`.
        """
