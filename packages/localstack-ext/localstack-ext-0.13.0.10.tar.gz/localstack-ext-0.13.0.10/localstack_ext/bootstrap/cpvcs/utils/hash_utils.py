...
def random_hash()->Ftyek:
 return FtyeA(random.getrandbits(160))
def compute_file_hash(file_path:Ftyek,accum=FtyeH)->Optional[Ftyek]:
 try:
  with Ftyeo(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:FtyeW,state_archive:Ftyek)->Ftyek:
 ...
 m.update(Ftyek(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->Ftyek:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=Ftyeb(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with Ftyeo(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except FtyeS as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if Ftyea(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(Ftyek(cpvcs_node.revision_number).encode("utf-8"))
 elif Ftyea(cpvcs_node,Version):
  m.update(Ftyek(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
