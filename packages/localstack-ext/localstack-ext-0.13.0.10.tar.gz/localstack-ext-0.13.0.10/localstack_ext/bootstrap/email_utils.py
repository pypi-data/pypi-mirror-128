...
SENT_EMAILS=[]
EMAIL_BLACKLIST=UghTx()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",UghTI(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=UghTs,from_name=UghTs,smtp_host=UghTs,smtp_user=UghTs,smtp_pass=UghTs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if UghTk(recipients,UghTq)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=UghTs):
 ...
 result["From"]=formataddr((UghTI(Header(from_name,"utf-8")),from_email))
 return result
