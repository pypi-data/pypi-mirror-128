...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[MRlye],MRlye],api_name:MRlye):
 if not(is_debian()and in_docker()):
  ...
  return
 if MRlyh(packages,MRlye):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=MRlyE)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=MRlyE)
def check_or_install(command:MRlye,packages:Union[List[MRlye],MRlye],api:MRlye):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=MRlyE):
  try:
