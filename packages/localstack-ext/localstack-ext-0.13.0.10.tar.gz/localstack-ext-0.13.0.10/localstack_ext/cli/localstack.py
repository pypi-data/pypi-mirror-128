...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return Mimvn if e else MimvN
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->MimvR:
...
@click.option("--revert",is_flag=Mimvn,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:MimvX):
...
def _cpvcs_initialized(pod_name:MimvU)->MimvX:
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_init(name:MimvU):
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=Mimvn,default=MimvN)
def cmd_cpvcs_delete(name:MimvU,remote:MimvX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_register_remote(name:MimvU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=Mimvn)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=Mimvn)
def cmd_cpvcs_rename(name:MimvU,new_name:MimvU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_commit(message:MimvU,name:MimvU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=Mimvn,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_push(squash:MimvX,message:MimvU,name:MimvU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=MimvT,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=Mimvn,default=MimvN,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_inject(version:MimvT,reset:MimvX,name:MimvU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=Mimvn,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=Mimvn,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:MimvU,inject:MimvX,reset:MimvX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=Mimvn,default=MimvN)
def cmd_cpvcs_list_pods(remote:MimvX):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_versions(name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=Mimvn,type=MimvT)
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_version_info(version:MimvT,name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=MimvT,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_version_metamodel(version:MimvT,name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=Mimvn,type=MimvT,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=Mimvn,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=Mimvn,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=MimvN,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_set_version(version:MimvT,inject:MimvX,reset:MimvX,commit_before:MimvX,name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_commits(version:MimvT,name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=Mimvn)
@click.option("--commit","-c",required=Mimvn)
@click.option("-n","--name",help="Name of the cloud pod",required=Mimvn)
def cmd_cpvcs_commit_diff(version:MimvT,commit:MimvT,name:MimvU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(MimvR)
  status.stop()
...
def print_table(columns:Dict[MimvU,MimvU],rows:List[Dict[MimvU,Any]],formatters:Dict[MimvU,Callable[[Any],MimvU]]=MimvR):
 from rich.table import Table
 if formatters is MimvR:
  formatters=MimvP()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  MimvH(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->MimvR:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->MimvR:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->MimvR:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->MimvR:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
