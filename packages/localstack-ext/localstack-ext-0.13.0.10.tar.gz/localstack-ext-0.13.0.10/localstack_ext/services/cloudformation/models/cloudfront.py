...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or hRlLB
 @hRlLS
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return hRlLn(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @hRlLa
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if hRlLH(origins,hRlLQ)else origins.get("Items")
  origins_to_create=hRlLC([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if hRlLH(aliases,hRlLQ)else aliases.get("Items")
  aliases_to_create=hRlLC(aliases)
  for distr in distrs:
   origins=hRlLC([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=hRlLC(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",hRlLY)
   config["Enabled"]=hRlLo(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if hRlLH(custom_errors,hRlLJ)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=hRlLo(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":hRlLY}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=hRlLo(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=hRlLB):
    parent=config if parent is hRlLB else parent
    items=parent.get(key)or[]
    if hRlLH(items,hRlLJ)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":hRlLm(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if hRlLH(cache_behavs,hRlLJ)else cache_behavs)
