...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=tTrwD,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if tTrwy(perm["IpProtocol"])!=tTrwy(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",tTrwD)
   vpc_id=tTrwD
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or tTrwD
    vpc_id=groups and groups[0].get("VpcId")or tTrwD
   dst_group_id=result.pop("DestinationSecurityGroupId",tTrwD)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=tTrwy(result.get("IpProtocol"))
   description=result.pop("Description",tTrwD)
   cidr_ipv6=result.pop("CidrIpv6",tTrwD)
