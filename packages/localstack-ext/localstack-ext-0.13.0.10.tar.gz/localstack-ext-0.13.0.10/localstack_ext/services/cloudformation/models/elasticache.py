...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=NnHVW(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not NnHVt
   return result
  except NnHVQ:
   props=self.props
   ...
   if parts[0]in candidates and NnHVJ(parts)>1:
