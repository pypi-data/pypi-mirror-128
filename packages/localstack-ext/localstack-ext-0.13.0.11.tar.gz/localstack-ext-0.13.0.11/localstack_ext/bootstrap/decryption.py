...
class DecryptionHandler:
 decryption_key:FTquw
 def __init__(self,decryption_key:FTquw):
  self.decryption_key=decryption_key
 def decrypt(self,content)->FTquw:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=FTquV):
  if path and not FTqui(path,FTqup):
   path=FTqup(FTqud(path,"_path",[]))
  if not path:
   return FTquV
...
def patch_traceback_lines():
 if FTqud(traceback.FrameSummary,"_ls_patch_applied",FTquV):
  return
 @FTquk
 def line(self):
  try:
 ...
 FTquB(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=FTqux
def patch_inspect_findsource():
 if FTqud(inspect,"_ls_patch_applied",FTquV):
  return
 ...
 inspect._ls_patch_applied=FTqux
