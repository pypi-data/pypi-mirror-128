...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SvtGD()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SvtGo(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SvtGC,from_name=SvtGC,smtp_host=SvtGC,smtp_user=SvtGC,smtp_pass=SvtGC,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SvtGN(recipients,SvtGp)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SvtGC):
 ...
 result["From"]=formataddr((SvtGo(Header(from_name,"utf-8")),from_email))
 return result
