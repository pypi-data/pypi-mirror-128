...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[uiaQb],uiaQb],api_name:uiaQb):
 if not(is_debian()and in_docker()):
  ...
  return
 if uiaQe(packages,uiaQb):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=uiaQw)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=uiaQw)
def check_or_install(command:uiaQb,packages:Union[List[uiaQb],uiaQb],api:uiaQb):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=uiaQw):
  try:
