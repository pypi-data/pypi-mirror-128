...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return MVQDk if e else MVQDK
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->MVQDG:
...
@click.option("--revert",is_flag=MVQDk,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:MVQDq):
...
def _cpvcs_initialized(pod_name:MVQDs)->MVQDq:
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_init(name:MVQDs):
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=MVQDk,default=MVQDK)
def cmd_cpvcs_delete(name:MVQDs,remote:MVQDq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_register_remote(name:MVQDs):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=MVQDk)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=MVQDk)
def cmd_cpvcs_rename(name:MVQDs,new_name:MVQDs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_commit(message:MVQDs,name:MVQDs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=MVQDk,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_push(squash:MVQDq,message:MVQDs,name:MVQDs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=MVQDW,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=MVQDk,default=MVQDK,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_inject(version:MVQDW,reset:MVQDq,name:MVQDs):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=MVQDk,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=MVQDk,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:MVQDs,inject:MVQDq,reset:MVQDq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=MVQDk,default=MVQDK)
def cmd_cpvcs_list_pods(remote:MVQDq):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_versions(name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=MVQDk,type=MVQDW)
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_version_info(version:MVQDW,name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=MVQDW,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_version_metamodel(version:MVQDW,name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=MVQDk,type=MVQDW,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=MVQDk,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=MVQDk,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=MVQDK,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_set_version(version:MVQDW,inject:MVQDq,reset:MVQDq,commit_before:MVQDq,name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_commits(version:MVQDW,name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=MVQDk)
@click.option("--commit","-c",required=MVQDk)
@click.option("-n","--name",help="Name of the cloud pod",required=MVQDk)
def cmd_cpvcs_commit_diff(version:MVQDW,commit:MVQDW,name:MVQDs):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(MVQDG)
  status.stop()
...
def print_table(columns:Dict[MVQDs,MVQDs],rows:List[Dict[MVQDs,Any]],formatters:Dict[MVQDs,Callable[[Any],MVQDs]]=MVQDG):
 from rich.table import Table
 if formatters is MVQDG:
  formatters=MVQDa()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  MVQDL(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->MVQDG:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->MVQDG:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->MVQDG:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->MVQDG:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
