...
def api_key_configured()->vpIrV:
 return vpIrs if os.environ.get("LOCALSTACK_API_KEY")else vpIrf
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if vpIrG([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
