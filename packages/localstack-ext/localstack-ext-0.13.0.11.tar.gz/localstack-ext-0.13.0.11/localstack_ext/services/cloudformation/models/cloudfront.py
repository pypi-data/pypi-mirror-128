...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or wOcLU
 @wOcLa
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return wOcLs(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @wOcLq
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if wOcLm(origins,wOcLA)else origins.get("Items")
  origins_to_create=wOcLN([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if wOcLm(aliases,wOcLA)else aliases.get("Items")
  aliases_to_create=wOcLN(aliases)
  for distr in distrs:
   origins=wOcLN([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=wOcLN(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",wOcLd)
   config["Enabled"]=wOcLi(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if wOcLm(custom_errors,wOcLW)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=wOcLi(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":wOcLd}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=wOcLi(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=wOcLU):
    parent=config if parent is wOcLU else parent
    items=parent.get(key)or[]
    if wOcLm(items,wOcLW)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":wOcLl(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if wOcLm(cache_behavs,wOcLW)else cache_behavs)
