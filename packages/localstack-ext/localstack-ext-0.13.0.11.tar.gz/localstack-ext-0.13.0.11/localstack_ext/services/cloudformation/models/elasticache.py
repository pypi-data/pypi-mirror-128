...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=anCJv(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not anCJB
   return result
  except anCJP:
   props=self.props
   ...
   if parts[0]in candidates and anCJz(parts)>1:
