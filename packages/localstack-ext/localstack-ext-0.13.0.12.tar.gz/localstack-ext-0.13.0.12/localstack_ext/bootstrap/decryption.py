...
class DecryptionHandler:
 decryption_key:tMXim
 def __init__(self,decryption_key:tMXim):
  self.decryption_key=decryption_key
 def decrypt(self,content)->tMXim:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=tMXiR):
  if path and not tMXiQ(path,tMXiY):
   path=tMXiY(tMXil(path,"_path",[]))
  if not path:
   return tMXiR
...
def patch_traceback_lines():
 if tMXil(traceback.FrameSummary,"_ls_patch_applied",tMXiR):
  return
 @tMXiU
 def line(self):
  try:
 ...
 tMXio(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=tMXiq
def patch_inspect_findsource():
 if tMXil(inspect,"_ls_patch_applied",tMXiR):
  return
 ...
 inspect._ls_patch_applied=tMXiq
