...
SENT_EMAILS=[]
EMAIL_BLACKLIST=YItXi()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",YItXh(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=YItXv,from_name=YItXv,smtp_host=YItXv,smtp_user=YItXv,smtp_pass=YItXv,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if YItXn(recipients,YItXK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=YItXv):
 ...
 result["From"]=formataddr((YItXh(Header(from_name,"utf-8")),from_email))
 return result
