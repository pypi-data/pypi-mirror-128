...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[wtiEN],wtiEN],api_name:wtiEN):
 if not(is_debian()and in_docker()):
  ...
  return
 if wtiET(packages,wtiEN):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=wtiEW)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=wtiEW)
def check_or_install(command:wtiEN,packages:Union[List[wtiEN],wtiEN],api:wtiEN):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=wtiEW):
  try:
