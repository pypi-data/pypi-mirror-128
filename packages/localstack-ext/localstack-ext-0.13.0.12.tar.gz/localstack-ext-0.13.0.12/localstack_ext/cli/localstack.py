...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return EXGmx if e else EXGmy
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->EXGmk:
...
@click.option("--revert",is_flag=EXGmx,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:EXGmz):
...
def _cpvcs_initialized(pod_name:EXGma)->EXGmz:
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_init(name:EXGma):
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=EXGmx,default=EXGmy)
def cmd_cpvcs_delete(name:EXGma,remote:EXGmz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_register_remote(name:EXGma):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=EXGmx)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=EXGmx)
def cmd_cpvcs_rename(name:EXGma,new_name:EXGma):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_commit(message:EXGma,name:EXGma):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=EXGmx,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_push(squash:EXGmz,message:EXGma,name:EXGma):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=EXGmb,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=EXGmx,default=EXGmy,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_inject(version:EXGmb,reset:EXGmz,name:EXGma):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=EXGmx,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=EXGmx,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:EXGma,inject:EXGmz,reset:EXGmz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=EXGmx,default=EXGmy)
def cmd_cpvcs_list_pods(remote:EXGmz):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_versions(name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=EXGmx,type=EXGmb)
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_version_info(version:EXGmb,name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=EXGmb,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_version_metamodel(version:EXGmb,name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=EXGmx,type=EXGmb,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=EXGmx,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=EXGmx,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=EXGmy,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_set_version(version:EXGmb,inject:EXGmz,reset:EXGmz,commit_before:EXGmz,name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_commits(version:EXGmb,name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=EXGmx)
@click.option("--commit","-c",required=EXGmx)
@click.option("-n","--name",help="Name of the cloud pod",required=EXGmx)
def cmd_cpvcs_commit_diff(version:EXGmb,commit:EXGmb,name:EXGma):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(EXGmk)
  status.stop()
...
def print_table(columns:Dict[EXGma,EXGma],rows:List[Dict[EXGma,Any]],formatters:Dict[EXGma,Callable[[Any],EXGma]]=EXGmk):
 from rich.table import Table
 if formatters is EXGmk:
  formatters=EXGmB()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  EXGme(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->EXGmk:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->EXGmk:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->EXGmk:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->EXGmk:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
