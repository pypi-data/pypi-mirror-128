...
from localstack.utils.common import is_mac_os
...
def api_key_configured()->twuLT:
 return twuLD if os.environ.get("LOCALSTACK_API_KEY")else twuLq
...
def start_ec2_daemon():
 try:
  if is_api_enabled("ec2")and is_mac_os():
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if twuLU([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
