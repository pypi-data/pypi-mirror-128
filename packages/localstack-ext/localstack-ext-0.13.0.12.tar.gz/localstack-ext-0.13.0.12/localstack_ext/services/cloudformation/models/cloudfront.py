...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or fEOSn
 @fEOSU
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return fEOSq(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @fEOSx
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if fEOSM(origins,fEOSX)else origins.get("Items")
  origins_to_create=fEOSR([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if fEOSM(aliases,fEOSX)else aliases.get("Items")
  aliases_to_create=fEOSR(aliases)
  for distr in distrs:
   origins=fEOSR([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=fEOSR(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",fEOSc)
   config["Enabled"]=fEOSd(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if fEOSM(custom_errors,fEOSK)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=fEOSd(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":fEOSc}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=fEOSd(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=fEOSn):
    parent=config if parent is fEOSn else parent
    items=parent.get(key)or[]
    if fEOSM(items,fEOSK)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":fEOSv(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if fEOSM(cache_behavs,fEOSK)else cache_behavs)
