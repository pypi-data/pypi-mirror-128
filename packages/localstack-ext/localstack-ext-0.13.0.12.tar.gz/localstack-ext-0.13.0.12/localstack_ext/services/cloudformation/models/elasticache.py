...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=QTjOq(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not QTjOb
   return result
  except QTjOc:
   props=self.props
   ...
   if parts[0]in candidates and QTjOF(parts)>1:
