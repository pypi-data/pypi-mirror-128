...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[fwlpt])[0]
 def get_physical_resource_id(self,attribute=fwlpt,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if fwlpW(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return fwlpQ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @fwlpv
 def add_defaults(resource,stack_name:fwlpN):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return fwlpQ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @fwlpV
