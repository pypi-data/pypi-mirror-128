...
SENT_EMAILS=[]
EMAIL_BLACKLIST=CzrdR()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",CzrdM(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=CzrdY,from_name=CzrdY,smtp_host=CzrdY,smtp_user=CzrdY,smtp_pass=CzrdY,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if CzrdE(recipients,Czrdc)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=CzrdY):
 ...
 result["From"]=formataddr((CzrdM(Header(from_name,"utf-8")),from_email))
 return result
