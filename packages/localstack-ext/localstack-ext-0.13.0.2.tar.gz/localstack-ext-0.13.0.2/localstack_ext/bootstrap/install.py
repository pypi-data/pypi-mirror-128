...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[pJfxK],pJfxK],api_name:pJfxK):
 if not(is_debian()and in_docker()):
  ...
  return
 if pJfxw(packages,pJfxK):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=pJfxc)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=pJfxc)
def check_or_install(command:pJfxK,packages:Union[List[pJfxK],pJfxK],api:pJfxK):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=pJfxc):
  try:
