...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return UhPwv if e else UhPws
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->UhPwJ:
...
@click.option("--revert",is_flag=UhPwv,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:UhPwK):
...
def _cpvcs_initialized(pod_name:UhPwF)->UhPwK:
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_init(name:UhPwF):
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=UhPwv,default=UhPws)
def cmd_cpvcs_delete(name:UhPwF,remote:UhPwK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_register_remote(name:UhPwF):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=UhPwv)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=UhPwv)
def cmd_cpvcs_rename(name:UhPwF,new_name:UhPwF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_commit(message:UhPwF,name:UhPwF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=UhPwv,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_push(squash:UhPwK,message:UhPwF,name:UhPwF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=UhPwC,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=UhPwv,default=UhPws,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_inject(version:UhPwC,reset:UhPwK,name:UhPwF):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=UhPwv,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=UhPwv,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:UhPwF,inject:UhPwK,reset:UhPwK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=UhPwv,default=UhPws)
def cmd_cpvcs_list_pods(remote:UhPwK):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_versions(name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=UhPwv,type=UhPwC)
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_version_info(version:UhPwC,name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=UhPwC,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_version_metamodel(version:UhPwC,name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=UhPwv,type=UhPwC,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=UhPwv,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=UhPwv,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=UhPws,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_set_version(version:UhPwC,inject:UhPwK,reset:UhPwK,commit_before:UhPwK,name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_commits(version:UhPwC,name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=UhPwv)
@click.option("--commit","-c",required=UhPwv)
@click.option("-n","--name",help="Name of the cloud pod",required=UhPwv)
def cmd_cpvcs_commit_diff(version:UhPwC,commit:UhPwC,name:UhPwF):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(UhPwJ)
  status.stop()
...
def print_table(columns:Dict[UhPwF,UhPwF],rows:List[Dict[UhPwF,Any]],formatters:Dict[UhPwF,Callable[[Any],UhPwF]]=UhPwJ):
 from rich.table import Table
 if formatters is UhPwJ:
  formatters=UhPwV()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  UhPwb(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->UhPwJ:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->UhPwJ:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->UhPwJ:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->UhPwJ:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
