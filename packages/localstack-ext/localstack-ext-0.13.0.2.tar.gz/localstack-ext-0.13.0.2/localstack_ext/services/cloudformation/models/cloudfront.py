...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or YxFUt
 @YxFUb
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YxFUq(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @YxFUW
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if YxFUE(origins,YxFUm)else origins.get("Items")
  origins_to_create=YxFUd([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if YxFUE(aliases,YxFUm)else aliases.get("Items")
  aliases_to_create=YxFUd(aliases)
  for distr in distrs:
   origins=YxFUd([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=YxFUd(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",YxFUp)
   config["Enabled"]=YxFUV(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if YxFUE(custom_errors,YxFUS)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=YxFUV(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":YxFUp}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=YxFUV(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=YxFUt):
    parent=config if parent is YxFUt else parent
    items=parent.get(key)or[]
    if YxFUE(items,YxFUS)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":YxFUA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if YxFUE(cache_behavs,YxFUS)else cache_behavs)
