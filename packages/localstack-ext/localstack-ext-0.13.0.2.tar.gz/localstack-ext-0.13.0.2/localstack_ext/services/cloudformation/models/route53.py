...
def canonicalize_name(name:FEolJ)->FEolJ:
 if name[-1]!=".":
  return f"{name}."
 return name
class Route53HostedZone(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(result or[FEolC])[0]
 def get_physical_resource_id(self,attribute=FEolC,**kwargs):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FEolN(Route53HostedZone,self).get_cfn_attribute(attribute_name)
 @FEoly
