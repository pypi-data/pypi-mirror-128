...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[DUgVS])[0]
 def get_physical_resource_id(self,attribute=DUgVS,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if DUgVX(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return DUgVJ(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @DUgVE
 def add_defaults(resource,stack_name:DUgVy):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return DUgVJ(SESReceiptRule,self).get_cfn_attribute(attribute)
 @DUgVl
