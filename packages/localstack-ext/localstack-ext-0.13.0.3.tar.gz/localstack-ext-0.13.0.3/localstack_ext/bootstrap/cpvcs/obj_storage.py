...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=aYdsK
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:aYdsD)->aYdsD:
  pass
 @aYdsn
 def get(cls,requested_file_locator:aYdsD):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=aYdsK
 @aYdsn
 def get(cls,state_file_locator:aYdsD,requested_storage:aYdsD,serializers:Dict[aYdsD,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[aYdsD,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:aYdsD)->aYdsD:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:aYdsD)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:aYdsD)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:aYdsD)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:aYdsD)->aYdsD:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[aYdsD]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:aYdsD,new_key:aYdsD,referenced_by_version:aYdsD=aYdsK):
  pass
 @abc.abstractmethod
 def version_exists(self,key:aYdsD)->aYdsc:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:aYdsD,key:aYdsD):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:aYdsD,new_key:aYdsD,base_path:aYdsD)->aYdsc:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[aYdsD]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[aYdsD,aYdsD],remote_location:aYdsD=aYdsK)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[aYdsD]:
  ...
  for cpvcs_obj in args:
   if aYdsj(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif aYdsj(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[aYdsD,aYdsD],remote_location:aYdsD=aYdsK)->List[Union[Revision,Version]]:
