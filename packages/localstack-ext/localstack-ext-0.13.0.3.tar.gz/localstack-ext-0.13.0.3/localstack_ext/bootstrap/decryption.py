...
class DecryptionHandler:
 decryption_key:GYDiy
 def __init__(self,decryption_key:GYDiy):
  self.decryption_key=decryption_key
 def decrypt(self,content)->GYDiy:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=GYDiH):
  if path and not GYDid(path,GYDim):
   path=GYDim(GYDit(path,"_path",[]))
  if not path:
   return GYDiH
...
def patch_traceback_lines():
 if GYDit(traceback.FrameSummary,"_ls_patch_applied",GYDiH):
  return
 @GYDiq
 def line(self):
  try:
 ...
 GYDiJ(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=GYDir
def patch_inspect_findsource():
 if GYDit(inspect,"_ls_patch_applied",GYDiH):
  return
 ...
 inspect._ls_patch_applied=GYDir
