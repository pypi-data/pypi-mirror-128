...
SENT_EMAILS=[]
EMAIL_BLACKLIST=SDPCu()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",SDPCB(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=SDPCp,from_name=SDPCp,smtp_host=SDPCp,smtp_user=SDPCp,smtp_pass=SDPCp,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if SDPCt(recipients,SDPCQ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=SDPCp):
 ...
 result["From"]=formataddr((SDPCB(Header(from_name,"utf-8")),from_email))
 return result
