...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[sAxhD],sAxhD],api_name:sAxhD):
 if not(is_debian()and in_docker()):
  ...
  return
 if sAxhH(packages,sAxhD):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=sAxhm)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=sAxhm)
def check_or_install(command:sAxhD,packages:Union[List[sAxhD],sAxhD],api:sAxhD):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=sAxhm):
  try:
