...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return QsMXg if e else QsMXl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->QsMXf:
...
@click.option("--revert",is_flag=QsMXg,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:QsMXE):
...
def _cpvcs_initialized(pod_name:QsMXj)->QsMXE:
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_init(name:QsMXj):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=QsMXg,default=QsMXl)
def cmd_cpvcs_delete(name:QsMXj,remote:QsMXE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_register_remote(name:QsMXj):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=QsMXg)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=QsMXg)
def cmd_cpvcs_rename(name:QsMXj,new_name:QsMXj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_commit(message:QsMXj,name:QsMXj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=QsMXg,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_push(squash:QsMXE,message:QsMXj,name:QsMXj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=QsMXJ,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=QsMXg,default=QsMXl,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_inject(version:QsMXJ,reset:QsMXE,name:QsMXj):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=QsMXg,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=QsMXg,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:QsMXj,inject:QsMXE,reset:QsMXE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=QsMXg,default=QsMXl)
def cmd_cpvcs_list_pods(remote:QsMXE):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_versions(name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QsMXg,type=QsMXJ)
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_version_info(version:QsMXJ,name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=QsMXJ,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_version_metamodel(version:QsMXJ,name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=QsMXg,type=QsMXJ,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=QsMXg,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=QsMXg,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=QsMXl,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_set_version(version:QsMXJ,inject:QsMXE,reset:QsMXE,commit_before:QsMXE,name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_commits(version:QsMXJ,name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=QsMXg)
@click.option("--commit","-c",required=QsMXg)
@click.option("-n","--name",help="Name of the cloud pod",required=QsMXg)
def cmd_cpvcs_commit_diff(version:QsMXJ,commit:QsMXJ,name:QsMXj):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(QsMXf)
  status.stop()
...
def print_table(columns:Dict[QsMXj,QsMXj],rows:List[Dict[QsMXj,Any]],formatters:Dict[QsMXj,Callable[[Any],QsMXj]]=QsMXf):
 from rich.table import Table
 if formatters is QsMXf:
  formatters=QsMXR()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  QsMXv(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->QsMXf:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->QsMXf:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->QsMXf:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->QsMXf:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
