...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or hKYRb
 @hKYRV
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return hKYRI(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @hKYRx
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if hKYRm(origins,hKYRC)else origins.get("Items")
  origins_to_create=hKYRn([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if hKYRm(aliases,hKYRC)else aliases.get("Items")
  aliases_to_create=hKYRn(aliases)
  for distr in distrs:
   origins=hKYRn([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=hKYRn(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",hKYRD)
   config["Enabled"]=hKYRt(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if hKYRm(custom_errors,hKYRk)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=hKYRt(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":hKYRD}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=hKYRt(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=hKYRb):
    parent=config if parent is hKYRb else parent
    items=parent.get(key)or[]
    if hKYRm(items,hKYRk)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":hKYRN(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if hKYRm(cache_behavs,hKYRk)else cache_behavs)
