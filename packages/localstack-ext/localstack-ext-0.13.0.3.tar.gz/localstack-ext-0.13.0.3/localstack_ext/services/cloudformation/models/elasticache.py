...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=agBHl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not agBHr
   return result
  except agBHM:
   props=self.props
   ...
   if parts[0]in candidates and agBHy(parts)>1:
