...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[OtcSC])[0]
 def get_physical_resource_id(self,attribute=OtcSC,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if OtcST(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return OtcSU(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @OtcSM
 def add_defaults(resource,stack_name:OtcSV):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return OtcSU(SESReceiptRule,self).get_cfn_attribute(attribute)
 @OtcSH
