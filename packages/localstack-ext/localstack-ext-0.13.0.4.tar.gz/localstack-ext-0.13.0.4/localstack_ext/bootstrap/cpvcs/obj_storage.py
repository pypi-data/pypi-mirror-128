...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=XWDPB
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:XWDPR)->XWDPR:
  pass
 @XWDPG
 def get(cls,requested_file_locator:XWDPR):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=XWDPB
 @XWDPG
 def get(cls,state_file_locator:XWDPR,requested_storage:XWDPR,serializers:Dict[XWDPR,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[XWDPR,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:XWDPR)->XWDPR:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:XWDPR)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:XWDPR)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:XWDPR)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:XWDPR)->XWDPR:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[XWDPR]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:XWDPR,new_key:XWDPR,referenced_by_version:XWDPR=XWDPB):
  pass
 @abc.abstractmethod
 def version_exists(self,key:XWDPR)->XWDPH:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:XWDPR,key:XWDPR):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:XWDPR,new_key:XWDPR,base_path:XWDPR)->XWDPH:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[XWDPR]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[XWDPR,XWDPR],remote_location:XWDPR=XWDPB)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[XWDPR]:
  ...
  for cpvcs_obj in args:
   if XWDPv(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif XWDPv(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[XWDPR,XWDPR],remote_location:XWDPR=XWDPB)->List[Union[Revision,Version]]:
