...
class DecryptionHandler:
 decryption_key:pTcxh
 def __init__(self,decryption_key:pTcxh):
  self.decryption_key=decryption_key
 def decrypt(self,content)->pTcxh:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=pTcxS):
  if path and not pTcxt(path,pTcxq):
   path=pTcxq(pTcxm(path,"_path",[]))
  if not path:
   return pTcxS
...
def patch_traceback_lines():
 if pTcxm(traceback.FrameSummary,"_ls_patch_applied",pTcxS):
  return
 @pTcxl
 def line(self):
  try:
 ...
 pTcxD(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=pTcxs
def patch_inspect_findsource():
 if pTcxm(inspect,"_ls_patch_applied",pTcxS):
  return
 ...
 inspect._ls_patch_applied=pTcxs
