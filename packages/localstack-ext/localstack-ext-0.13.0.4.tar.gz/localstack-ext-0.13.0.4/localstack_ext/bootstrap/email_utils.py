...
SENT_EMAILS=[]
EMAIL_BLACKLIST=xQRMG()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",xQRMt(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=xQRMs,from_name=xQRMs,smtp_host=xQRMs,smtp_user=xQRMs,smtp_pass=xQRMs,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if xQRMk(recipients,xQRME)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=xQRMs):
 ...
 result["From"]=formataddr((xQRMt(Header(from_name,"utf-8")),from_email))
 return result
