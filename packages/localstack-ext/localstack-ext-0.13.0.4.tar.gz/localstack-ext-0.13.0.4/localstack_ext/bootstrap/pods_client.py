...
class PodInfo:
 def __init__(self,name=TpGxH,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(TpGxO):
 BACKEND="_none_"
 def __init__(self,pod_name=TpGxH,config=TpGxH):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise TpGxb("Not implemented")
 def delete(self,remote:TpGxC)->TpGxC:
  raise TpGxb("Not implemented")
 def push(self,comment:TpGxS=TpGxH)->PodInfo:
  raise TpGxb("Not implemented")
 def pull(self,inject_version_state:TpGxC=TpGxf,reset_state_before:TpGxC=TpGxf,fetch_all:TpGxC=TpGxH):
  raise TpGxb("Not implemented")
 def commit(self,message:TpGxS=TpGxH):
  raise TpGxb("Not implemented")
 def inject(self,version:TpGxw,reset_state:TpGxC):
  raise TpGxb("Not implemented")
 def list_versions(self)->List[TpGxS]:
  raise TpGxb("Not implemented")
 def version_info(self,version:TpGxw):
  raise TpGxb("Not implemented")
 def version_metamodel(self,version:TpGxw):
  raise TpGxb("Not implemented")
 def set_version(self,version:TpGxw,inject_version_state:TpGxC,reset_state:TpGxC,commit_before:TpGxC):
  raise TpGxb("Not implemented")
 def list_version_commits(self,version:TpGxw)->List[TpGxS]:
  raise TpGxb("Not implemented")
 def get_commit_diff(self,version:TpGxw,commit:TpGxw)->TpGxS:
  raise TpGxb("Not implemented")
 def register_remote(self,pod_name:TpGxS)->TpGxC:
  raise TpGxb("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->TpGxC:
  raise TpGxb("Not implemented")
 def list_pods(self,fetch_remote:TpGxC)->Set[TpGxS]:
  raise TpGxb("Not implemented")
 ...
 def get(cls,pod_name,pre_config=TpGxH):
  ...
  raise TpGxb('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=TpGxf
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=TpGxa)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=TpGxa
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise TpGxb("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @TpGxX
 def get_state_zip_from_instance(get_content=TpGxf):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise TpGxb("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:TpGxS=TpGxH):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @TpGxX
 def parse_pod_name_from_qualifying_name(qualifying_name:TpGxS)->TpGxS:
 ...
 @TpGxX
 def _get_max_version_for_pod_from_platform(pod_name:TpGxS,auth_headers):
  ...
  remote_max_ver=TpGxw(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with TpGxs(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with TpGxs(meta_archive,"rb")as meta_archive_content:
   ...
   with TpGxs(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=TpGxH)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:TpGxS):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def push(self,comment:TpGxS=TpGxH)->PodInfo:
  ...
  return PodInfo()
 def pull(self,inject_version_state:TpGxC=TpGxf,reset_state_before:TpGxC=TpGxf,fetch_all:TpGxC=TpGxa):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=TpGxf):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(TpGxv(lambda ver:TpGxS(ver),TpGxi(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:TpGxw,reset_state:TpGxC):
  ...
  if not tmp_pod_path:
   ...
   return
  if reset_state:
   reset_local_state(reset_data_dir=TpGxa,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def list_pods(self,fetch_remote:TpGxC)->Set[TpGxS]:
  ...
  return result
 @TpGxK
 def upload_content(cls,presigned_url:TpGxS,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise TpGxb("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @TpGxX
 def create_platform_url(request:TpGxS)->TpGxS:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:TpGxS=TpGxH)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=TpGxa)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(TpGxq(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=TpGxq(zip_data_content)
  return result
 def pull(self,inject_version_state:TpGxC=TpGxf,reset_state_before:TpGxC=TpGxf,fetch_all:TpGxC=TpGxH):
  ...
  rm_rf(zip_path)
 @TpGxX
 def presigned_url(pod_name:TpGxS,mode:TpGxS)->TpGxS:
  ...
  if response.status_code>=400:
   raise TpGxb("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise TpGxb("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:TpGxS=TpGxH):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=TpGxa)
 ...
 def pull(self,inject_version_state:TpGxC=TpGxf,reset_state_before:TpGxC=TpGxf,fetch_all:TpGxC=TpGxH):
  ...
  if remote_ref not in remote_refs:
   raise TpGxb('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=TpGxg(self,"clone_dir",TpGxH)
...
class PodConfigManagerMeta(TpGxo):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=TpGxH
   for manager in cls.CHAIN:
    try:
     tmp=TpGxg(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif TpGxE(tmp,TpGxR)and TpGxE(result,TpGxR):
       result.extend(tmp)
    except TpGxb:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not TpGxH:
    return result
   raise TpGxb('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(TpGxO,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @TpGxK
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise TpGxb('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(TpGxO):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=TpGxa).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=TpGxd([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=TpGxf):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except TpGxb:
   if safe:
    return{}
   raise TpGxb('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(TpGxO):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise TpGxb("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise TpGxb("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:TpGxS,remote:TpGxC,pre_config:Dict[TpGxS,TpGxS])->TpGxC:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs)->TpGxC:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name)
 return result
def rename_pod(current_pod_name:TpGxS,new_pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:TpGxC,pre_config:Dict[TpGxS,TpGxS],**kwargs)->List[TpGxS]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],message:TpGxS=TpGxH,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:TpGxS,version:TpGxw,reset_state:TpGxC,pre_config:Dict[TpGxS,TpGxS],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs)->List[TpGxS]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:TpGxw,pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:TpGxw,pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:TpGxw,inject_version_state:TpGxC,reset_state:TpGxC,commit_before:TpGxC,pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS],**kwargs)->TpGxC:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:TpGxw,pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS])->List[TpGxS]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:TpGxw,commit:TpGxw,pod_name:TpGxS,pre_config:Dict[TpGxS,TpGxS])->TpGxS:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_state(pod_name,pre_config=TpGxH,squash_commits=TpGxf,comment=TpGxH,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=TpGxf,reset_state_before=TpGxf,**kwargs):
 pre_config=kwargs.get("pre_config",TpGxH)
 if not pod_name:
  raise TpGxb("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=TpGxf,exclude_from_reset:List[TpGxS]=TpGxH):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise TpGxb("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->TpGxS:
 try:
...
def get_persisted_resource_names(data_dir)->List[TpGxS]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and TpGxq(os.listdir(entry.path))>0:
    names.append(entry.name)
