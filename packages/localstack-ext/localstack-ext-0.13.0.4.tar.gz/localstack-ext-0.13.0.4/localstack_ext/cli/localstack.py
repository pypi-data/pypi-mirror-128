...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return rBEvz if e else rBEvy
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->rBEvi:
...
@click.option("--revert",is_flag=rBEvz,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:rBEvR):
...
def _cpvcs_initialized(pod_name:rBEvw)->rBEvR:
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_init(name:rBEvw):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=rBEvz,default=rBEvy)
def cmd_cpvcs_delete(name:rBEvw,remote:rBEvR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_register_remote(name:rBEvw):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=rBEvz)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=rBEvz)
def cmd_cpvcs_rename(name:rBEvw,new_name:rBEvw):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_commit(message:rBEvw,name:rBEvw):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=rBEvz,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_push(squash:rBEvR,message:rBEvw,name:rBEvw):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=rBEvp,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=rBEvz,default=rBEvy,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_inject(version:rBEvp,reset:rBEvR,name:rBEvw):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=rBEvz,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=rBEvz,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:rBEvw,inject:rBEvR,reset:rBEvR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=rBEvz,default=rBEvy)
def cmd_cpvcs_list_pods(remote:rBEvR):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_versions(name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rBEvz,type=rBEvp)
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_version_info(version:rBEvp,name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=rBEvp,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_version_metamodel(version:rBEvp,name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=rBEvz,type=rBEvp,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=rBEvz,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=rBEvz,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=rBEvy,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_set_version(version:rBEvp,inject:rBEvR,reset:rBEvR,commit_before:rBEvR,name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_commits(version:rBEvp,name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=rBEvz)
@click.option("--commit","-c",required=rBEvz)
@click.option("-n","--name",help="Name of the cloud pod",required=rBEvz)
def cmd_cpvcs_commit_diff(version:rBEvp,commit:rBEvp,name:rBEvw):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(rBEvi)
  status.stop()
...
def print_table(columns:Dict[rBEvw,rBEvw],rows:List[Dict[rBEvw,Any]],formatters:Dict[rBEvw,Callable[[Any],rBEvw]]=rBEvi):
 from rich.table import Table
 if formatters is rBEvi:
  formatters=rBEve()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  rBEvc(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->rBEvi:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->rBEvi:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->rBEvi:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->rBEvi:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
