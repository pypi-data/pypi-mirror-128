...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or sICea
 @sICev
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return sICeQ(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @sICet
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if sICeU(origins,sICeA)else origins.get("Items")
  origins_to_create=sICeE([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if sICeU(aliases,sICeA)else aliases.get("Items")
  aliases_to_create=sICeE(aliases)
  for distr in distrs:
   origins=sICeE([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=sICeE(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",sICeD)
   config["Enabled"]=sICeW(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if sICeU(custom_errors,sICeY)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=sICeW(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":sICeD}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=sICeW(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=sICea):
    parent=config if parent is sICea else parent
    items=parent.get(key)or[]
    if sICeU(items,sICeY)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":sICef(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if sICeU(cache_behavs,sICeY)else cache_behavs)
