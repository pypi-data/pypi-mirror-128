...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=IChUp(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not IChUw
   return result
  except IChUV:
   props=self.props
   ...
   if parts[0]in candidates and IChUy(parts)>1:
