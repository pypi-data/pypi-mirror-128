...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[hxYQq])[0]
 def get_physical_resource_id(self,attribute=hxYQq,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if hxYQP(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hxYQT(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @hxYQt
 def add_defaults(resource,stack_name:hxYQi):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return hxYQT(SESReceiptRule,self).get_cfn_attribute(attribute)
 @hxYQA
