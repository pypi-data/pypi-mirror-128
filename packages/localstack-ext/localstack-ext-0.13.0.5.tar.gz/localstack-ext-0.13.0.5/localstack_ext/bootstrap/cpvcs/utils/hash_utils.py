...
def random_hash()->cSWwi:
 return cSWws(random.getrandbits(160))
def compute_file_hash(file_path:cSWwi,accum=cSWwB)->Optional[cSWwi]:
 try:
  with cSWwQ(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:cSWwH,state_archive:cSWwi)->cSWwi:
 ...
 m.update(cSWwi(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_node_hash(cpvcs_node:CPVCSNode)->cSWwi:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=cSWwl(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with cSWwQ(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except cSWwT as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if cSWwb(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(cSWwi(cpvcs_node.revision_number).encode("utf-8"))
 elif cSWwb(cpvcs_node,Version):
  m.update(cSWwi(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
