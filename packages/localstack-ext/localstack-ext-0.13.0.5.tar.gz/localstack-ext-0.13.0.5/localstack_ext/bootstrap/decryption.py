...
class DecryptionHandler:
 decryption_key:fFwWk
 def __init__(self,decryption_key:fFwWk):
  self.decryption_key=decryption_key
 def decrypt(self,content)->fFwWk:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=fFwWA):
  if path and not fFwWs(path,fFwWQ):
   path=fFwWQ(fFwWp(path,"_path",[]))
  if not path:
   return fFwWA
...
def patch_traceback_lines():
 if fFwWp(traceback.FrameSummary,"_ls_patch_applied",fFwWA):
  return
 @fFwWn
 def line(self):
  try:
 ...
 fFwWR(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=fFwWa
def patch_inspect_findsource():
 if fFwWp(inspect,"_ls_patch_applied",fFwWA):
  return
 ...
 inspect._ls_patch_applied=fFwWa
