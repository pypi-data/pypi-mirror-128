...
SENT_EMAILS=[]
EMAIL_BLACKLIST=dGfsy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",dGfsi(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=dGfsH,from_name=dGfsH,smtp_host=dGfsH,smtp_user=dGfsH,smtp_pass=dGfsH,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if dGfsz(recipients,dGfsj)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=dGfsH):
 ...
 result["From"]=formataddr((dGfsi(Header(from_name,"utf-8")),from_email))
 return result
