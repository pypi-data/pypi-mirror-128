...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[UaoXj],UaoXj],api_name:UaoXj):
 if not(is_debian()and in_docker()):
  ...
  return
 if UaoXz(packages,UaoXj):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=UaoXp)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=UaoXp)
def check_or_install(command:UaoXj,packages:Union[List[UaoXj],UaoXj],api:UaoXj):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=UaoXp):
  try:
