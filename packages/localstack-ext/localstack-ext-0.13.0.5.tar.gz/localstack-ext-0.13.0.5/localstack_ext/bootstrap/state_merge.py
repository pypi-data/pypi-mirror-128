...
def _merge_helper(current,injecting,merge_strategy=iUBDM,visited:Set=iUBDM):
 if iUBDv(current,iUBDE)and iUBDv(injecting,iUBDE):
 ...
 for field_name,inj_field_value in inj_dict.items():
  ...
  if cur_field_value is not iUBDM:
   ...
   cur_dict[field_name]=inj_field_value
 return cur_dict
def merge_object_state(current,injecting,merge_strategy=iUBDM):
...
def handle_special_case(current,injecting):
 if iUBDv(injecting,Queue):
  current.queues[injecting.name]=injecting
  return iUBDC
 elif iUBDv(injecting,FakeBucket):
  current["global"].buckets[injecting.name]=injecting
  return iUBDC
def add_missing_attributes(obj:Any,safe:iUBDm=iUBDC,visited:Set=iUBDM):
 try:
  obj_dict=get_object_dict(obj)
  if obj_dict is iUBDM:
   return
  ...
  class_inst_attrs=infer_class_attributes(iUBDb(obj))
  for key,value in class_inst_attrs.items():
   if key not in obj_dict:
    LOG.debug("Add missing attribute '%s' to state object of type %s"%(key,iUBDb(obj)))
    obj_dict[key]=value
 except iUBDn as e:
  if not safe:
   raise
  LOG.warning("Unable to add missing attributes to persistence state object %s: %s",(obj,e))
def infer_class_attributes(clazz:Type)->Dict[iUBDY,Any]:
 if clazz in[iUBDE,iUBDp]or not inspect.isclass(clazz)or clazz.__name__=="function":
  return{}
 constructor=iUBDA(clazz,"__init__",iUBDM)
 if not constructor:
  return{}
 try:
  ...
  def get_default_arg_value(arg_name,arg_index=-1):
   ...
   num_non_default_args=iUBDX(sig_args.args or[])-iUBDX(arg_defaults)
   offset=arg_index-num_non_default_args
   if offset in iUBDI(iUBDX(arg_defaults)):
  ...
  args=[]
  kwargs={}
  for arg_idx in iUBDI(1,iUBDX(sig_args.args)):
  ...
  result=iUBDp(instance.__dict__)
  return result
 except iUBDn:
  return{}
def merge_sqllite_dbs(file_dest:iUBDY,file_src:iUBDY)->iUBDM:
 def _merge_table(table_name:iUBDY,cursor_a,cursor_b)->iUBDM:
  tmp_table_name=f"'{table_name}_new'"
  table_name=f"'{table_name}'"
  select_query=f"SELECT * FROM {table_name}"
  if table_name=="'cf'":
   return
  schema=iUBDk(iUBDl(lambda x:x[1],cursor_b.execute(f"PRAGMA table_info({table_name})")))
  params=f"({('?,' * len(schema))[:-1]})"
  insert_str=f"INSERT INTO {tmp_table_name} {str(schema)} values {params}"
  if table_name=="'dm'":
   excl_tables=(iUBDY(iUBDE(iUBDl(lambda x:x[0],cursor_a.execute(f"SELECT TableName FROM {table_name}")))).replace("[","(",1).replace("]",")",1))
 ...
 with contextlib.closing(sqlite3.connect(file_dest))as db_src,contextlib.closing(sqlite3.connect(file_src))as db_target:
  ...
  table_names=iUBDE(iUBDl(lambda x:x[0],cursor_dest.execute("SELECT name FROM sqlite_master WHERE type='table'")))
...
def merge_kinesis_state(path_dest:iUBDY,path_src:iUBDY)->iUBDm:
 ...
 if not os.path.isfile(tmp_dir_statefile):
  ...
  return iUBDx
 with iUBDj(datadir_statefile)as datadir_kinesis_file,iUBDj(tmp_dir_statefile)as tmp_dir_kinesis_file:
  ...
  if iUBDX(tmp_dir_streams)>0:
   ...
   with iUBDj(datadir_statefile,"w")as mutated_datadir_kinesis_file:
