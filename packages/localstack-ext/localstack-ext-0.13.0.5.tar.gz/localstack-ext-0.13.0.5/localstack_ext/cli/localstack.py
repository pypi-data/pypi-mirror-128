...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return nOSPi if e else nOSPD
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->nOSPo:
...
@click.option("--revert",is_flag=nOSPi,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:nOSPp):
...
def _cpvcs_initialized(pod_name:nOSPT)->nOSPp:
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_init(name:nOSPT):
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=nOSPi,default=nOSPD)
def cmd_cpvcs_delete(name:nOSPT,remote:nOSPp):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_register_remote(name:nOSPT):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=nOSPi)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=nOSPi)
def cmd_cpvcs_rename(name:nOSPT,new_name:nOSPT):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_commit(message:nOSPT,name:nOSPT):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=nOSPi,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_push(squash:nOSPp,message:nOSPT,name:nOSPT):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=nOSPN,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=nOSPi,default=nOSPD,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_inject(version:nOSPN,reset:nOSPp,name:nOSPT):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=nOSPi,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=nOSPi,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:nOSPT,inject:nOSPp,reset:nOSPp):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=nOSPi,default=nOSPD)
def cmd_cpvcs_list_pods(remote:nOSPp):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_versions(name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=nOSPi,type=nOSPN)
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_version_info(version:nOSPN,name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=nOSPN,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_version_metamodel(version:nOSPN,name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=nOSPi,type=nOSPN,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=nOSPi,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=nOSPi,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=nOSPD,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_set_version(version:nOSPN,inject:nOSPp,reset:nOSPp,commit_before:nOSPp,name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_commits(version:nOSPN,name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=nOSPi)
@click.option("--commit","-c",required=nOSPi)
@click.option("-n","--name",help="Name of the cloud pod",required=nOSPi)
def cmd_cpvcs_commit_diff(version:nOSPN,commit:nOSPN,name:nOSPT):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(nOSPo)
  status.stop()
...
def print_table(columns:Dict[nOSPT,nOSPT],rows:List[Dict[nOSPT,Any]],formatters:Dict[nOSPT,Callable[[Any],nOSPT]]=nOSPo):
 from rich.table import Table
 if formatters is nOSPo:
  formatters=nOSPU()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  nOSPJ(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->nOSPo:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->nOSPo:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->nOSPo:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->nOSPo:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
