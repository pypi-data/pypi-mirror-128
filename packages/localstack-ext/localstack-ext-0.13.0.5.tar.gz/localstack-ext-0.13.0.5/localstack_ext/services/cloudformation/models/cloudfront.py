...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or SyaVL
 @SyaVs
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return SyaVj(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @SyaVR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if SyaVJ(origins,SyaVh)else origins.get("Items")
  origins_to_create=SyaVA([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if SyaVJ(aliases,SyaVh)else aliases.get("Items")
  aliases_to_create=SyaVA(aliases)
  for distr in distrs:
   origins=SyaVA([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=SyaVA(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",SyaVx)
   config["Enabled"]=SyaVM(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if SyaVJ(custom_errors,SyaVN)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=SyaVM(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":SyaVx}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=SyaVM(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=SyaVL):
    parent=config if parent is SyaVL else parent
    items=parent.get(key)or[]
    if SyaVJ(items,SyaVN)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":SyaVg(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if SyaVJ(cache_behavs,SyaVN)else cache_behavs)
