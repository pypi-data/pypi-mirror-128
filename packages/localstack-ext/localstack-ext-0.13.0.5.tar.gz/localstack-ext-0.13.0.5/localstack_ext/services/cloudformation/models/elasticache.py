...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=sUGrb(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not sUGrf
   return result
  except sUGry:
   props=self.props
   ...
   if parts[0]in candidates and sUGrX(parts)>1:
