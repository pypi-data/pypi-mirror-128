...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[yrlhT])[0]
 def get_physical_resource_id(self,attribute=yrlhT,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if yrlhj(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return yrlhD(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @yrlhU
 def add_defaults(resource,stack_name:yrlhf):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return yrlhD(SESReceiptRule,self).get_cfn_attribute(attribute)
 @yrlhP
