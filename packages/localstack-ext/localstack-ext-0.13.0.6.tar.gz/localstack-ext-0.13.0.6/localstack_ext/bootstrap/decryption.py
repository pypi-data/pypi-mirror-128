...
class DecryptionHandler:
 decryption_key:mbqeH
 def __init__(self,decryption_key:mbqeH):
  self.decryption_key=decryption_key
 def decrypt(self,content)->mbqeH:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=mbqex):
  if path and not mbqeS(path,mbqeI):
   path=mbqeI(mbqeP(path,"_path",[]))
  if not path:
   return mbqex
...
def patch_traceback_lines():
 if mbqeP(traceback.FrameSummary,"_ls_patch_applied",mbqex):
  return
 @mbqeK
 def line(self):
  try:
 ...
 mbqeu(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=mbqei
def patch_inspect_findsource():
 if mbqeP(inspect,"_ls_patch_applied",mbqex):
  return
 ...
 inspect._ls_patch_applied=mbqei
