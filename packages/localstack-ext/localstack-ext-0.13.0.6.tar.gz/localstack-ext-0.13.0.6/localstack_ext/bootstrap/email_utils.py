...
SENT_EMAILS=[]
EMAIL_BLACKLIST=bVCDS()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",bVCDQ(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=bVCDu,from_name=bVCDu,smtp_host=bVCDu,smtp_user=bVCDu,smtp_pass=bVCDu,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if bVCDY(recipients,bVCDX)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=bVCDu):
 ...
 result["From"]=formataddr((bVCDQ(Header(from_name,"utf-8")),from_email))
 return result
