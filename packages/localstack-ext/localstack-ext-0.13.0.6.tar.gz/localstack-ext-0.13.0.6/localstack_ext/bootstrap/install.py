...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[hUweX],hUweX],api_name:hUweX):
 if not(is_debian()and in_docker()):
  ...
  return
 if hUweA(packages,hUweX):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=hUweb)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=hUweb)
def check_or_install(command:hUweX,packages:Union[List[hUweX],hUweX],api:hUweX):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=hUweb):
  try:
