...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return WMOBE if e else WMOBh
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->WMOBH:
...
@click.option("--revert",is_flag=WMOBE,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:WMOBL):
...
def _cpvcs_initialized(pod_name:WMOBA)->WMOBL:
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_init(name:WMOBA):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=WMOBE,default=WMOBh)
def cmd_cpvcs_delete(name:WMOBA,remote:WMOBL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_register_remote(name:WMOBA):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=WMOBE)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=WMOBE)
def cmd_cpvcs_rename(name:WMOBA,new_name:WMOBA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_commit(message:WMOBA,name:WMOBA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=WMOBE,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_push(squash:WMOBL,message:WMOBA,name:WMOBA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=WMOBq,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=WMOBE,default=WMOBh,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_inject(version:WMOBq,reset:WMOBL,name:WMOBA):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=WMOBE,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=WMOBE,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:WMOBA,inject:WMOBL,reset:WMOBL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=WMOBE,default=WMOBh)
def cmd_cpvcs_list_pods(remote:WMOBL):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_versions(name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WMOBE,type=WMOBq)
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_version_info(version:WMOBq,name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=WMOBq,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_version_metamodel(version:WMOBq,name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=WMOBE,type=WMOBq,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=WMOBE,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=WMOBE,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=WMOBh,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_set_version(version:WMOBq,inject:WMOBL,reset:WMOBL,commit_before:WMOBL,name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_commits(version:WMOBq,name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=WMOBE)
@click.option("--commit","-c",required=WMOBE)
@click.option("-n","--name",help="Name of the cloud pod",required=WMOBE)
def cmd_cpvcs_commit_diff(version:WMOBq,commit:WMOBq,name:WMOBA):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(WMOBH)
  status.stop()
...
def print_table(columns:Dict[WMOBA,WMOBA],rows:List[Dict[WMOBA,Any]],formatters:Dict[WMOBA,Callable[[Any],WMOBA]]=WMOBH):
 from rich.table import Table
 if formatters is WMOBH:
  formatters=WMOBo()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  WMOBN(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->WMOBH:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->WMOBH:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->WMOBH:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->WMOBH:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
