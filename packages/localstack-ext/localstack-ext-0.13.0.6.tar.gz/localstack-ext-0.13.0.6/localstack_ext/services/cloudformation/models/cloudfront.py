...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or FrSHf
 @FrSHD
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FrSHp(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @FrSHt
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if FrSHQ(origins,FrSHy)else origins.get("Items")
  origins_to_create=FrSHs([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if FrSHQ(aliases,FrSHy)else aliases.get("Items")
  aliases_to_create=FrSHs(aliases)
  for distr in distrs:
   origins=FrSHs([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=FrSHs(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",FrSHX)
   config["Enabled"]=FrSHI(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if FrSHQ(custom_errors,FrSHx)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=FrSHI(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":FrSHX}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=FrSHI(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=FrSHf):
    parent=config if parent is FrSHf else parent
    items=parent.get(key)or[]
    if FrSHQ(items,FrSHx)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":FrSHA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if FrSHQ(cache_behavs,FrSHx)else cache_behavs)
