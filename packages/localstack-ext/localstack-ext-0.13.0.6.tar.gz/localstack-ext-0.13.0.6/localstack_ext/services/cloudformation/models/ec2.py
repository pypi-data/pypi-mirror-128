...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=tYXpg,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if tYXpC(perm["IpProtocol"])!=tYXpC(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",tYXpg)
   vpc_id=tYXpg
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or tYXpg
    vpc_id=groups and groups[0].get("VpcId")or tYXpg
   dst_group_id=result.pop("DestinationSecurityGroupId",tYXpg)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=tYXpC(result.get("IpProtocol"))
   description=result.pop("Description",tYXpg)
   cidr_ipv6=result.pop("CidrIpv6",tYXpg)
