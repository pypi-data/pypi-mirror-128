...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=AmfSl(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not AmfSg
   return result
  except AmfSI:
   props=self.props
   ...
   if parts[0]in candidates and AmfSB(parts)>1:
