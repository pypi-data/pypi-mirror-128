...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[ObNQy])[0]
 def get_physical_resource_id(self,attribute=ObNQy,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if ObNQD(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ObNQC(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @ObNQi
 def add_defaults(resource,stack_name:ObNQk):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return ObNQC(SESReceiptRule,self).get_cfn_attribute(attribute)
 @ObNQT
