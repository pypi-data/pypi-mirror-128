...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=tRxIc
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:tRxIM)->tRxIM:
  pass
 @tRxIH
 def get(cls,requested_file_locator:tRxIM):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=tRxIc
 @tRxIH
 def get(cls,state_file_locator:tRxIM,requested_storage:tRxIM,serializers:Dict[tRxIM,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[tRxIM,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:tRxIM)->tRxIM:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:tRxIM)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:tRxIM)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:tRxIM)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:tRxIM)->tRxIM:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[tRxIM]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:tRxIM,new_key:tRxIM,referenced_by_version:tRxIM=tRxIc):
  pass
 @abc.abstractmethod
 def version_exists(self,key:tRxIM)->tRxIB:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:tRxIM,key:tRxIM):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:tRxIM,new_key:tRxIM,base_path:tRxIM)->tRxIB:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[tRxIM]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[tRxIM,tRxIM],remote_location:tRxIM=tRxIc)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[tRxIM]:
  ...
  for cpvcs_obj in args:
   if tRxIv(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif tRxIv(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[tRxIM,tRxIM],remote_location:tRxIM=tRxIc)->List[Union[Revision,Version]]:
