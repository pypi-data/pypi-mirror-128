...
class DecryptionHandler:
 decryption_key:leVsJ
 def __init__(self,decryption_key:leVsJ):
  self.decryption_key=decryption_key
 def decrypt(self,content)->leVsJ:
...
class EncryptedFileFinder(MetaPathFinder):
 ...
 def find_spec(self,fullname,path,target=leVsX):
  if path and not leVsh(path,leVsi):
   path=leVsi(leVsL(path,"_path",[]))
  if not path:
   return leVsX
...
def patch_traceback_lines():
 if leVsL(traceback.FrameSummary,"_ls_patch_applied",leVsX):
  return
 @leVsE
 def line(self):
  try:
 ...
 leVsv(traceback.FrameSummary,"line",line)
 traceback.FrameSummary._ls_patch_applied=leVsN
def patch_inspect_findsource():
 if leVsL(inspect,"_ls_patch_applied",leVsX):
  return
 ...
 inspect._ls_patch_applied=leVsN
