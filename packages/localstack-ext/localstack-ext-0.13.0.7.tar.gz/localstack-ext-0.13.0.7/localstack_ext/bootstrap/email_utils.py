...
SENT_EMAILS=[]
EMAIL_BLACKLIST=uhakP()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",uhakK(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=uhakF,from_name=uhakF,smtp_host=uhakF,smtp_user=uhakF,smtp_pass=uhakF,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if uhakl(recipients,uhakJ)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=uhakF):
 ...
 result["From"]=formataddr((uhakK(Header(from_name,"utf-8")),from_email))
 return result
