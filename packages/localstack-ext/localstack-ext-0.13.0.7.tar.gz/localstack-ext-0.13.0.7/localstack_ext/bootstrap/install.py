...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[UDxbT],UDxbT],api_name:UDxbT):
 if not(is_debian()and in_docker()):
  ...
  return
 if UDxbK(packages,UDxbT):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=UDxbh)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=UDxbh)
def check_or_install(command:UDxbT,packages:Union[List[UDxbT],UDxbT],api:UDxbT):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=UDxbh):
  try:
