...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return XGzot if e else XGzoe
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->XGzog:
...
@click.option("--revert",is_flag=XGzot,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:XGzoS):
...
def _cpvcs_initialized(pod_name:XGzoU)->XGzoS:
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_init(name:XGzoU):
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=XGzot,default=XGzoe)
def cmd_cpvcs_delete(name:XGzoU,remote:XGzoS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_register_remote(name:XGzoU):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=XGzot)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=XGzot)
def cmd_cpvcs_rename(name:XGzoU,new_name:XGzoU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_commit(message:XGzoU,name:XGzoU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=XGzot,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_push(squash:XGzoS,message:XGzoU,name:XGzoU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=XGzoM,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=XGzot,default=XGzoe,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_inject(version:XGzoM,reset:XGzoS,name:XGzoU):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=XGzot,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=XGzot,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:XGzoU,inject:XGzoS,reset:XGzoS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=XGzot,default=XGzoe)
def cmd_cpvcs_list_pods(remote:XGzoS):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_versions(name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=XGzot,type=XGzoM)
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_version_info(version:XGzoM,name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=XGzoM,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_version_metamodel(version:XGzoM,name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=XGzot,type=XGzoM,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=XGzot,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=XGzot,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=XGzoe,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_set_version(version:XGzoM,inject:XGzoS,reset:XGzoS,commit_before:XGzoS,name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_commits(version:XGzoM,name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=XGzot)
@click.option("--commit","-c",required=XGzot)
@click.option("-n","--name",help="Name of the cloud pod",required=XGzot)
def cmd_cpvcs_commit_diff(version:XGzoM,commit:XGzoM,name:XGzoU):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(XGzog)
  status.stop()
...
def print_table(columns:Dict[XGzoU,XGzoU],rows:List[Dict[XGzoU,Any]],formatters:Dict[XGzoU,Callable[[Any],XGzoU]]=XGzog):
 from rich.table import Table
 if formatters is XGzog:
  formatters=XGzoj()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  XGzom(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->XGzog:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->XGzog:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->XGzog:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->XGzog:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
