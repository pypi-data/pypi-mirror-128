...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Mhqik
 @Mhqit
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return MhqiR(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @MhqiV
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if Mhqix(origins,MhqiQ)else origins.get("Items")
  origins_to_create=MhqiU([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if Mhqix(aliases,MhqiQ)else aliases.get("Items")
  aliases_to_create=MhqiU(aliases)
  for distr in distrs:
   origins=MhqiU([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=MhqiU(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",MhqiH)
   config["Enabled"]=Mhqia(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if Mhqix(custom_errors,Mhqis)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Mhqia(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":MhqiH}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Mhqia(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Mhqik):
    parent=config if parent is Mhqik else parent
    items=parent.get(key)or[]
    if Mhqix(items,Mhqis)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":MhqiT(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if Mhqix(cache_behavs,Mhqis)else cache_behavs)
