...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=kFPrM(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not kFPrG
   return result
  except kFPrf:
   props=self.props
   ...
   if parts[0]in candidates and kFPrm(parts)>1:
