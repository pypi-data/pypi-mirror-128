...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[rvBlC])[0]
 def get_physical_resource_id(self,attribute=rvBlC,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if rvBlX(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return rvBlz(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @rvBlS
 def add_defaults(resource,stack_name:rvBlg):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return rvBlz(SESReceiptRule,self).get_cfn_attribute(attribute)
 @rvBlW
