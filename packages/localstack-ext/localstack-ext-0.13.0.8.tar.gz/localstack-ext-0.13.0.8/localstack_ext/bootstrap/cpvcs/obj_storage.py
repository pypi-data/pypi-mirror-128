...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=CAiwU
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:CAiwF)->CAiwF:
  pass
 @CAiwH
 def get(cls,requested_file_locator:CAiwF):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=CAiwU
 @CAiwH
 def get(cls,state_file_locator:CAiwF,requested_storage:CAiwF,serializers:Dict[CAiwF,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[CAiwF,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:CAiwF)->CAiwF:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:CAiwF)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:CAiwF)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:CAiwF)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:CAiwF)->CAiwF:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[CAiwF]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:CAiwF,new_key:CAiwF,referenced_by_version:CAiwF=CAiwU):
  pass
 @abc.abstractmethod
 def version_exists(self,key:CAiwF)->CAiwo:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:CAiwF,key:CAiwF):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:CAiwF,new_key:CAiwF,base_path:CAiwF)->CAiwo:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[CAiwF]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[CAiwF,CAiwF],remote_location:CAiwF=CAiwU)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[CAiwF]:
  ...
  for cpvcs_obj in args:
   if CAiwu(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif CAiwu(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[CAiwF,CAiwF],remote_location:CAiwF=CAiwU)->List[Union[Revision,Version]]:
