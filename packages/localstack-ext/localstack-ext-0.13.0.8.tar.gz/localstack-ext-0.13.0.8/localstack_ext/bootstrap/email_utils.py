...
SENT_EMAILS=[]
EMAIL_BLACKLIST=tHOBY()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",tHOBp(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=tHOBA,from_name=tHOBA,smtp_host=tHOBA,smtp_user=tHOBA,smtp_pass=tHOBA,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if tHOBv(recipients,tHOBm)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=tHOBA):
 ...
 result["From"]=formataddr((tHOBp(Header(from_name,"utf-8")),from_email))
 return result
