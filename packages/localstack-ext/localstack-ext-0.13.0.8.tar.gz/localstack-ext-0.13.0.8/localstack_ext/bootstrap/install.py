...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[zYkAU],zYkAU],api_name:zYkAU):
 if not(is_debian()and in_docker()):
  ...
  return
 if zYkAK(packages,zYkAU):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=zYkAh)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=zYkAh)
def check_or_install(command:zYkAU,packages:Union[List[zYkAU],zYkAU],api:zYkAU):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=zYkAh):
  try:
