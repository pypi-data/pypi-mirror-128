...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return BoLyh if e else BoLyk
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->BoLyT:
...
@click.option("--revert",is_flag=BoLyh,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:BoLyQ):
...
def _cpvcs_initialized(pod_name:BoLyl)->BoLyQ:
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_init(name:BoLyl):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=BoLyh,default=BoLyk)
def cmd_cpvcs_delete(name:BoLyl,remote:BoLyQ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_register_remote(name:BoLyl):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=BoLyh)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=BoLyh)
def cmd_cpvcs_rename(name:BoLyl,new_name:BoLyl):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_commit(message:BoLyl,name:BoLyl):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=BoLyh,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_push(squash:BoLyQ,message:BoLyl,name:BoLyl):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=BoLyM,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=BoLyh,default=BoLyk,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_inject(version:BoLyM,reset:BoLyQ,name:BoLyl):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=BoLyh,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=BoLyh,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:BoLyl,inject:BoLyQ,reset:BoLyQ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=BoLyh,default=BoLyk)
def cmd_cpvcs_list_pods(remote:BoLyQ):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_versions(name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BoLyh,type=BoLyM)
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_version_info(version:BoLyM,name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=BoLyM,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_version_metamodel(version:BoLyM,name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=BoLyh,type=BoLyM,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=BoLyh,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=BoLyh,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=BoLyk,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_set_version(version:BoLyM,inject:BoLyQ,reset:BoLyQ,commit_before:BoLyQ,name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_commits(version:BoLyM,name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=BoLyh)
@click.option("--commit","-c",required=BoLyh)
@click.option("-n","--name",help="Name of the cloud pod",required=BoLyh)
def cmd_cpvcs_commit_diff(version:BoLyM,commit:BoLyM,name:BoLyl):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(BoLyT)
  status.stop()
...
def print_table(columns:Dict[BoLyl,BoLyl],rows:List[Dict[BoLyl,Any]],formatters:Dict[BoLyl,Callable[[Any],BoLyl]]=BoLyT):
 from rich.table import Table
 if formatters is BoLyT:
  formatters=BoLys()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  BoLyS(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->BoLyT:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->BoLyT:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->BoLyT:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->BoLyT:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
