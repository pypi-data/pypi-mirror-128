...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or bewOl
 @bewOY
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return bewOU(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @bewOR
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if bewOW(origins,bewOS)else origins.get("Items")
  origins_to_create=bewOq([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if bewOW(aliases,bewOS)else aliases.get("Items")
  aliases_to_create=bewOq(aliases)
  for distr in distrs:
   origins=bewOq([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=bewOq(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",bewOy)
   config["Enabled"]=bewOX(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if bewOW(custom_errors,bewOz)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=bewOX(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":bewOy}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=bewOX(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=bewOl):
    parent=config if parent is bewOl else parent
    items=parent.get(key)or[]
    if bewOW(items,bewOz)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":bewOQ(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if bewOW(cache_behavs,bewOz)else cache_behavs)
