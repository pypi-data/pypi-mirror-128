...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=HTqBO(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not HTqBs
   return result
  except HTqBd:
   props=self.props
   ...
   if parts[0]in candidates and HTqBf(parts)>1:
