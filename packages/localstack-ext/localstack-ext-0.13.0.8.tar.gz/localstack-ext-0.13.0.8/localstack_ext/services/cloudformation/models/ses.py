...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[lnKQd])[0]
 def get_physical_resource_id(self,attribute=lnKQd,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if lnKQV(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lnKQA(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @lnKQO
 def add_defaults(resource,stack_name:lnKQT):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return lnKQA(SESReceiptRule,self).get_cfn_attribute(attribute)
 @lnKQj
