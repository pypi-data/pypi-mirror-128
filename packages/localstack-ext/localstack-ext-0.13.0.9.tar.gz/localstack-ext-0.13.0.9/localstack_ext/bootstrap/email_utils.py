...
SENT_EMAILS=[]
EMAIL_BLACKLIST=DyWoz()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",DyWoe(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=DyWoN,from_name=DyWoN,smtp_host=DyWoN,smtp_user=DyWoN,smtp_pass=DyWoN,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if DyWos(recipients,DyWot)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=DyWoN):
 ...
 result["From"]=formataddr((DyWoe(Header(from_name,"utf-8")),from_email))
 return result
