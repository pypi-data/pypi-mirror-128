...
def install_mqtt():
 ...
 return "mosquitto"
def install_package(packages:Union[List[vVtnx],vVtnx],api_name:vVtnx):
 if not(is_debian()and in_docker()):
  ...
  return
 if vVtnC(packages,vVtnx):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=vVtnH)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=vVtnH)
def check_or_install(command:vVtnx,packages:Union[List[vVtnx],vVtnx],api:vVtnx):
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=vVtnH):
  try:
 ...
 if not result:
  ...
  do_download(url,vVtnJ)
class ExtInstallerRepository(InstallerRepository):
 name="ext"
 def get_installer(self)->List[Installer]:
  return[("iot-rule-engine",install_iot_rule_engine),("postgres",install_postgres),("timescaledb",install_timescaledb),("redis",install_redis),("mqtt",install_mqtt)]
