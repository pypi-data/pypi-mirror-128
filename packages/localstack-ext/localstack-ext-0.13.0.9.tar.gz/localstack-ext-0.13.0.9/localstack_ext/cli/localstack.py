...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return SEnLt if e else SEnLs
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->SEnLk:
...
@click.option("--revert",is_flag=SEnLt,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:SEnLm):
...
def _cpvcs_initialized(pod_name:SEnLe)->SEnLm:
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_init(name:SEnLe):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=SEnLt,default=SEnLs)
def cmd_cpvcs_delete(name:SEnLe,remote:SEnLm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_register_remote(name:SEnLe):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=SEnLt)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=SEnLt)
def cmd_cpvcs_rename(name:SEnLe,new_name:SEnLe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_commit(message:SEnLe,name:SEnLe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=SEnLt,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_push(squash:SEnLm,message:SEnLe,name:SEnLe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=SEnLK,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=SEnLt,default=SEnLs,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_inject(version:SEnLK,reset:SEnLm,name:SEnLe):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=SEnLt,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=SEnLt,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:SEnLe,inject:SEnLm,reset:SEnLm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=SEnLt,default=SEnLs)
def cmd_cpvcs_list_pods(remote:SEnLm):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_versions(name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SEnLt,type=SEnLK)
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_version_info(version:SEnLK,name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=SEnLK,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_version_metamodel(version:SEnLK,name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=SEnLt,type=SEnLK,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=SEnLt,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=SEnLt,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=SEnLs,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_set_version(version:SEnLK,inject:SEnLm,reset:SEnLm,commit_before:SEnLm,name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_commits(version:SEnLK,name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=SEnLt)
@click.option("--commit","-c",required=SEnLt)
@click.option("-n","--name",help="Name of the cloud pod",required=SEnLt)
def cmd_cpvcs_commit_diff(version:SEnLK,commit:SEnLK,name:SEnLe):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(SEnLk)
  status.stop()
...
def print_table(columns:Dict[SEnLe,SEnLe],rows:List[Dict[SEnLe,Any]],formatters:Dict[SEnLe,Callable[[Any],SEnLe]]=SEnLk):
 from rich.table import Table
 if formatters is SEnLk:
  formatters=SEnLO()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  SEnLi(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->SEnLk:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->SEnLk:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->SEnLk:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->SEnLk:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
