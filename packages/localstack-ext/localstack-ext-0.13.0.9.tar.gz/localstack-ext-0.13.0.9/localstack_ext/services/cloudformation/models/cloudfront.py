...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or bJihI
 @bJihu
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return bJihO(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @bJihP
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if bJihB(origins,bJihx)else origins.get("Items")
  origins_to_create=bJihG([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if bJihB(aliases,bJihx)else aliases.get("Items")
  aliases_to_create=bJihG(aliases)
  for distr in distrs:
   origins=bJihG([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=bJihG(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",bJihC)
   config["Enabled"]=bJihz(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if bJihB(custom_errors,bJihM)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=bJihz(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":bJihC}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=bJihz(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=bJihI):
    parent=config if parent is bJihI else parent
    items=parent.get(key)or[]
    if bJihB(items,bJihM)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":bJihj(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if bJihB(cache_behavs,bJihM)else cache_behavs)
