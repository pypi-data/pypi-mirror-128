...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tjkmW(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tjkmJ
   return result
  except tjkmO:
   props=self.props
   ...
   if parts[0]in candidates and tjkmf(parts)>1:
