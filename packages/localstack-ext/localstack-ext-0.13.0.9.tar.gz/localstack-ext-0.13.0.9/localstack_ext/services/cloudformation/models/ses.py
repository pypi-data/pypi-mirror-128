...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[tfyRU])[0]
 def get_physical_resource_id(self,attribute=tfyRU,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if tfyRW(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return tfyRV(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @tfyRH
 def add_defaults(resource,stack_name:tfyRY):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return tfyRV(SESReceiptRule,self).get_cfn_attribute(attribute)
 @tfyRT
