...
from locan import Region, Region1D, Region2D, Region3D, RegionND, EmptyRegion, Interval, \
...
def test_Interval():
    ...
    assert region.bounds == (0, 1)
    assert region.intervals == ((0, 1),)
    ...
    assert region.subregion_measure is None
...
def test_Rectangle():
    ...
    assert region.intervals == (pytest.approx((-1, 0)), pytest.approx((0, 2)))
    assert region.extent == pytest.approx((1, 2))
    assert len(region.points) == 5
    assert np.allclose(region.points.astype(float), [[0., 0.], [-1., 0.], [-1., 2.], [0., 2.], [0., 0.]])
    assert region.centroid == (-0.5, 1)
    assert region.max_distance == np.sqrt(5)
    assert region.region_measure == 2
    assert region.subregion_measure == 6
    assert np.array_equal(region.contains([[0, 0], [-0.5, 0.5], [100, 100], [-1, 2]]), (1,))
    ...
    assert region.bounding_box.corner == (-1., 0.)
...
def test_Ellipse():
    ...
    assert np.array_equal(region.contains([[9, 8], [10.5, 10.5], [100, 100], [11, 12]]), (1,))
    ...
    assert isinstance(region.buffer(1), Polygon)
    assert region.bounding_box.corner == (9, 8)
...
def test_Polygon():
    ...
    assert region.dimension == 2
    assert np.array_equal(region.points, points)
    assert region.centroid == (pytest.approx(0.4444444444444444), pytest.approx(0.611111111111111))
    ...
    assert np.array_equal(region.contains([[0, 0], [0.2, 0.8], [100, 100], [1, 0.5]]), (1,))
    ...
    assert region.region_measure == pytest.approx(region.shapely_object.area, rel=10e-3)
    assert isinstance(region.buffer(1), Polygon)
    assert region.bounding_box.corner == (0, 0)
    assert region.bounding_box.width == pytest.approx(1)
...
def test_Polygon_with_holes():
    ...
    assert region.dimension == 2
    assert np.array_equal(region.points, points)
    assert region.centroid == (pytest.approx(0.446485260770975), pytest.approx(0.6162131519274376))
    assert region.max_distance == np.sqrt(2)
...
def test_MultiPolygon():
    ...
    assert region.centroid == (pytest.approx(1.4555555555555553), pytest.approx(1.6237373737373733))
    ...
    assert region.region_measure == pytest.approx(region.shapely_object.area, rel=10e-3)
    assert isinstance(region.buffer(1), Polygon)
    assert region.bounds == (0, 0, 3, 3)
    assert region.extent == (3, 3)
    assert region.bounding_box.corner == (0, 0)
...
def test_AxisOrientedCuboid():
    ...
    assert region.intervals == (pytest.approx((1, 10)), pytest.approx((1, 20)), pytest.approx((1, 30)))
...
def test_pickling_Region():
    regions = [EmptyRegion(), Interval(), Rectangle(), Ellipse(), Polygon(((0, 0), (0, 1), (2, 2))),
               MultiPolygon([Polygon(((10, 10), (10, 11), (12, 12))), Polygon(((0, 0), (0, 1), (2, 2)))])]
