...
class MasterRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_bind_host, master_bind_port):
        ...
        if self.rebalancing_enabled():
            self.rebalance_work_queue = gevent.queue.Queue()
            self.greenlet.spawn(self.rebalance_worker).link_exception(greenlet_exception_handler)
            self.greenlet.spawn(self.periodic_rebalance_scheduler).link_exception(greenlet_exception_handler)
    ...
    def start(self, user_count: int, spawn_rate: float, *, is_rebalance: bool = False, **kwargs) -> None:
        if not is_rebalance and self.rebalancing_enabled():
            logger.debug("Clearing rebalance work queue")
            self.rebalance_work_queue.queue.clear()
    ...
    def periodic_rebalance_scheduler(self):
        logger.debug("Starting periodic rebalance scheduler")
        while True:
            gevent.sleep(10)
            if (
                not self.spawning_completed
                or self.state != STATE_RUNNING
                or self._users_dispatcher is None
                or self._users_dispatcher.dispatch_in_progress
            ):
                continue
            logger.debug("Performing periodic rebalance")
            # Set spawn rate to target user count +1 so that it is done in one dispatch
            self._users_dispatcher.prepare_rebalance()
            self.rebalance_work_queue.put((self.target_user_count, self.target_user_count + 1))

    def rebalance_worker(self):
        logger.debug("Starting rebalance worker")
        while True:
            target_user_count, spawn_rate = self.rebalance_work_queue.get()
            common_msg = "rebalance with target_user_count = {} and spawn_rate = {}".format(
                target_user_count, spawn_rate
            )
            if (
                not self.spawning_completed
                or self.state != STATE_RUNNING
                ...
                logger.debug("Skipping {}".format(common_msg))
                continue
            logger.debug("Performing {}".format(common_msg))
            self.start(user_count=target_user_count, spawn_rate=spawn_rate, is_rebalance=True)
            logger.debug("Done performing {}".format(common_msg))
                ...
                if client.heartbeat < 0 and client.state != STATE_MISSING:
                    ...
                    if self._users_dispatcher is not None:
                        ...
                        if (
                            ...
                            and not self._users_dispatcher.dispatch_in_progress
    ...
    def client_listener(self):
        while True:
            ...
            if msg.node_id not in self.clients:
                logger.debug("Received message of type {} from unknown worker {}".format(msg.type, client_id))
            ...
            if msg.type == "client_ready":
                ...
                if msg.node_id in self.clients:
                    ...
                    if c.state == STATE_MISSING:
                        ...
                        if self._users_dispatcher is not None and not self._users_dispatcher.worker_exists(c):
                            logger.error("Expected worker {} to exist in user dispatcher".format(client_id))
                ...
                if msg.node_id in self.clients:
                    ...
                    logger.info(
                        ...
                        % (msg.node_id, len(self.clients.ready + self.clients.running + self.clients.spawning))
                    )
...
class WorkerRunner(DistributedRunner):
    ...
    def __init__(self, environment, master_host, master_port):
        ...
        self.client_id = os.getenv("LOCUST_WORKER_ID_OVERRIDE", socket.gethostname() + "_" + uuid4().hex)
