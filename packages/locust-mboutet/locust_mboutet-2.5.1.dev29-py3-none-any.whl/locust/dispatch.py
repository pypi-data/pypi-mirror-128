...
class UsersDispatcher(Iterator):
    ...
    def __init__(self, worker_nodes: "List[WorkerNode]", user_classes: List[Type[User]]):
        ...
        # Use set in order to easily check if a worker exists in the users dispatcher
        self._worker_nodes_inventory = set(map(attrgetter("id"), worker_nodes))
    ...
    def add_worker(self, worker_node: "WorkerNode") -> None:
        ...
        """
        if self.worker_exists(worker_node):
            logger.debug("Not adding worker {} because it already existed".format(worker_node.id))
            return
        logger.debug("Adding worker {}".format(worker_node.id))
        self._worker_nodes_inventory.add(worker_node.id)
    ...
    def remove_worker(self, worker_node: "WorkerNode") -> None:
        ...
        """
        if not self.worker_exists(worker_node):
            logger.debug("Not removing worker {} because it did not exist".format(worker_node.id))
            return
        logger.debug("Removing worker {}".format(worker_node.id))
        self._worker_nodes_inventory.remove(worker_node.id)
    ...
    def worker_exists(self, worker_node: "WorkerNode") -> bool:
        return worker_node.id in self._worker_nodes_inventory
