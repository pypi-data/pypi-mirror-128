...
import urllib3
import requests
from lrabbit_scrapy.all_excepiton import Excepiton403, Excepiton404, Excepiton500
...
class RequestSession:
    ...
    def __init__(self, proxies=None, timeout=9):
        self.proxies = proxies
        self.timeout = timeout
        self.session = requests.session()

    def send_request(self, method='GET', url=None, headers=None, data=None):
        if headers:
            headers = copy.copy(self.session.headers).update(headers)
        if method == 'GET':
    ...
    def download_file_by_url(self, out_file_path, url, headers=None):
        res = self.session.post(url=url, headers=headers, proxies=self.proxies)
