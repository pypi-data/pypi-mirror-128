...
class LrabbitSpider:
    ...
    def __init__(self):
        ...
        self.task_list = []
        self.current_task_key = f"list:current:task:{self.spider_name}"
        ...
        def self_loop_call_back():
            while True:
                ...
                if not task:
                    break
                try:
                    self.redis_client.redis_executor.sadd(self.current_task_key, task)
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                while True:
                    task = self.redis_client.redis_executor.spop(self.current_task_key)
                    if not task:
                        break
                    self.redis_client.redis_executor.sadd(self.task_list_redis_key, task)
