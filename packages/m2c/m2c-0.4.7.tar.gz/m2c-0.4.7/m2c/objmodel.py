...
class ObjNode(object):
    ...
    def genObjFuncParam(self, ingores = []):
        pm = ''
        for child in self.children:
            if child.fieldName in ingores:
                continue
    ...
    def objmodelV4(self):
        ...
        mgoIdField = ''
        for child in self.children:
            if child.fieldType == '_id':
                mgoIdField = child.fieldName 
                mgoIdNum += 1
        
        assert mgoIdNum <= 1, '只能有一个mgo _id'

        code += 'type %sModel struct {\n' % self.bigLongNamespace()
        code += '// auto bind\n'
        if mgoIdNum:
            code += '%s    interface{} `json:"%s" bson:"_id,omitempty"` // int or string \n' % (upperFirst(mgoIdField), mgoIdField)
        else:
        ...
        code += '// basic type\n'
        for child in self.children:
            if child.fieldName in ('ctime', 'mtime', mgoIdField):
                continue
        ...
        # 不需要了
        # code += 'func New%sModel (' % self.bigLongNamespace()
        # code += self.genObjFuncParam(ingores=[mgoIdField])
        # code += ')*%sModel {\n' % self.bigLongNamespace()
        # code += 'return &%sModel{\n' % self.bigLongNamespace()
        # for child in self.children:
        #     if child.fieldName in ('ctime', 'mtime', mgoIdField):
    ...
    def fromLine(cls, lineno, line, parent):
        ...
        _isList = False
        if pobjs.endswith('[]') or pobjs.startswith('[]'):
            _isList = True
