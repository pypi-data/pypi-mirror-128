...
Ext.ux.StartMenu = Ext.extend(Ext.menu.Menu, {
    ...
    initComponent: function(config) {
        ...
        this.useMenuSearch = this.initialConfig.useMenuSearch;
    },
    ...
    onRender : function (ct, position) {
        ...
        this.mainMenuPanel = this.menuBWrap.createChild({
            tag: "div",
            cls: "x-panel",
            style: {float: 'left'},
        });
        this.menuPanel = this.mainMenuPanel.createChild({
            tag: "div",
            cls: "x-panel ux-start-menu-apps-panel"
        });

        // Формирование поля для поиска и панели для него
        if (this.useMenuSearch) {
            this.menuSearchField = new Ext.app.form.SearchField({
                emptyText: 'Поиск',
                hideLabel: true,
                triggerClass: 'x-form-search-trigger',
                onTrigger1Click: function () {
                    // Очистка поля для поиска и возвращение исходного меню
                    this.setValue('');
                    this.triggers[0].hide();
                    AppDesktop.clearMenuSearchFilter();
                },
                onTrigger2Click: function () {
                    // Установка фильтрации элементов меню по тексту в поле
                    var filterText = this.getValue(),
                        trigger = this.triggers[0];
                    filterText ? trigger.show() : trigger.hide();
                    AppDesktop.showFilteredStartMenuItems(filterText);
                },
            });
            this.menuSearchPanel = new Ext.FormPanel({
                cls: 'x-menu-list-item',
                items: [this.menuSearchField],
                style: {'background-color': 'white'},
            });
            this.menuSearchPanel.render(this.mainMenuPanel);
        }

        this.toolsPanel = this.menuBWrap.createChild({
            tag: "div",
            cls: "x-panel ux-start-menu-tools-panel",
    ...
    show : function (el, pos, parentMenu) {
        ...
        var box = this.menuBWrap.getBox(),
            leftMenuWidth = box.width - this.toolsPanelWidth,
            menuPanelHeight = box.height;

        if (this.useMenuSearch) {
            this.menuSearchPanel.setWidth(leftMenuWidth);
            this.menuSearchField.itemCt.setStyle({margin: '2px'});
            menuPanelHeight -= this.menuSearchPanel.getHeight();
            // Нужно для установки всей ширины
            this.menuSearchField.wrap.setWidth('100%');
            this.menuSearchField.triggers[1].show();
        }
        this.menuPanel.setHeight(menuPanelHeight);
        this.menuPanel.setWidth(leftMenuWidth);
        this.toolsPanel.setWidth(this.toolsPanelWidth);
    },
...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
        this.startMenu = new Ext.ux.StartMenu(Ext.apply({
            ...
            title: 'Jack Slocum',
            width: 300,
            useMenuSearch: this.app.useMenuSearch,
...
Ext.app.App = Ext.extend(Ext.util.Observable, {
    ...
    constructor: function (desktopConfig, params) {
        ...
        this.useMenuSearch = params.useMenuSearch;
    ...
    initApp : function(){
        ...
        this.startMenuItems = this.launcher.items.items;
    ...
    /**
     * Устанавливает/отключает видимость элемента меню в соответствии
     * с текстом для фильтрации
     * @param filterText Текст для фильтрации
     * @param menuItem Пункт меню для проверки. Если он содержит дочерние
     *  элементы, они также проверяются.
     * @returns {boolean} Стал ли элемент видимым или нет
     */
    applyMenuFilter: function (filterText, menuItem) {
        var text = filterText.toLowerCase(),
            isVisible = menuItem.text.toLowerCase().indexOf(text) !== -1;
        if (menuItem.menu && menuItem.menu.items.length) {
            // Если у вложенного меню внутри есть хотя бы 1
            // видимый элемент, то вложенное меню тоже видно
            var innerVisibleCount = this.showFilteredStartMenuItems(
                filterText, menuItem.menu.items.items)
            if (!isVisible && innerVisibleCount > 0) isVisible = true;
        }
    ...
    /**
     * Отфильтровывает пункты меню в соответствии со строкой
     * @param filterText Текст для фильтрации. Если пустой, фильтр отключается
     * @param items Пункты меню для проверки (необязательный параметр). Если не
     *  задан, проверяются корневые и вложенные в них элементы меню.
     * @returns {number} Количество видимых элементов среди этих пунктов меню
     */
    showFilteredStartMenuItems: function (filterText, items) {
        var menuItems = items || this.startMenuItems;
        if (!filterText) {
            this.clearMenuSearchFilter();
            return menuItems.length;
        }
        var visibilityArray = menuItems.map(
            this.applyMenuFilter.bind(this, filterText));
        return visibilityArray.reduce((a, b) => a + b, 0);
    },

    /**
     * Делает пункт меню видимым, то есть убирает эффект фильтрации
     * @param menuItem Пункт меню. Если он содержит дочерние
     *  элементы, они также обрабатываются.
     */
    clearMenuItemSearchFilter: function (menuItem) {
        if (menuItem.menu && menuItem.menu.items.length) {
            this.clearMenuSearchFilter(menuItem.menu.items.items)
        }
        menuItem.setVisible(true);
    },

    /**
     * Делает пункты меню видимыми, то есть убирает эффект фильтрации
     * @param items Пункты меню для отключения фильтрации. Если не указан,
     *  обрабатываются все элементы меню.
     */
    clearMenuSearchFilter: function (items) {
        var menuItems = items || this.startMenuItems;
        menuItems.forEach(this.clearMenuItemSearchFilter.bind(this));
    },
