...
class RestlessBandit:
    ...
    def random_restless(cls, dim, struct='dense', seed=None):
        ...
        np.random.seed(seed)
        if struct=="dense":
            transition_matrices= random_matrices_dense(dim)
        elif struct=="sparse":
            transition_matrices= random_matrices_sparse(dim)
        elif struct=="tridiag":
            transition_matrices= random_matrices_tridiag(dim)
        elif len(struct)==2 and struct[0]=="ndiag":
            transition_matrices = random_matrices_ndiag(dim, struct[1])
        else:
            transition_matrices= random_matrices(dim)
    ...
    def get_P0P1R0R1(self):
        return self.transition_matrices[:,0,:], self.transition_matrices[:,1,:], self.reward_vector[:,0], self.reward_vector[:,1]
    ...
    def transition_matrix_from_policy(self, policy):
        ...
        for i in range(dim):
            transition_matrix[i,:] = self.transition_matrices[i,action,:]
        return transition_matrix

    def whittle_indices(self, check_indexability=True, discount=1, number_of_updates=5):
        """
        Returs the whittle indices (or gittins if discount < 1 and the bandit is rested)
        """
        return whittle.compute_whittle_indices(self.transition_matrices[:, 0, :], self.transition_matrices[:, 1, :],
                self.reward_vector[:, 0], self.reward_vector[:, 1],
                check_indexability=check_indexability, discount=discount, number_of_updates=number_of_updates)
    ...
    def gittins_indices(self, discount, number_of_updates=5):
        """
        ...
        return whittle.compute_whittle_indices(self.transition_matrices[:, 0, :], self.transition_matrices[:, 1, :],
                self.reward_vector[:, 0], self.reward_vector[:, 1],
                check_indexability=False, discount=discount, number_of_updates=number_of_updates)[1]

    def is_indexable(self, discount=1):
        return whittle.compute_whittle_indices(self.transition_matrices[:, 0, :], self.transition_matrices[:, 1, :],
                self.reward_vector[:, 0], self.reward_vector[:, 1],
                check_indexability=True, discount=discount, number_of_updates=5)[0]
