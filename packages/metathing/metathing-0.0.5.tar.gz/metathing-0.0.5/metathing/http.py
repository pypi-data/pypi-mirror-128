...
class ModelProc(RequestHandler):
...
class ModelProcID(RequestHandler):
    def initialize(self, sql):
        self.sql = sql
...
class ModelExist(RequestHandler):
    def initialize(self, sql):
        self.sql = sql
...
class Event(RequestHandler):
    def initialize(self, srv, mqtt):
        self.srv = srv
        self.mqtt = mqtt

        if not ('ecb' in self.srv):
            self.srv.ecb = {}

    def post(self, event_name: str):
        setup = self.get_argument('setup') # 0 = off, 1 = on
        ...
        print("Event : {0}, {1}".format(setup, pub_topic))

        def on_event_callback(content):
            if (setup > 0):
                self.mqtt.publish(pub_topic, json.dumps({'content': content}), qos)

        self.srv.ecb[event_name] = on_event_callback
        self.write("OK")

# MQTT 操作 (Flow)
class MqttOp(RequestHandler):
    def initialize(self, srv):
        self.srv = srv
        self.mqtt = srv.mqtt
