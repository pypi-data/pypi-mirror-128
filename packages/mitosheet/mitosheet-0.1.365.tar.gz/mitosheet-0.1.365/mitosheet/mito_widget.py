...
"""
import json
from mitosheet import step_performers
...
class MitoWidget(DOMWidget):
    ...
    def set_usage_triggered_feedback_id(self):
        ...
        """
        # If the current step type is not the same as the previous step type, then there is a .25% chance that we ask the user for feedback
        previous_step_type = self.steps_manager.steps[self.steps_manager.curr_step_idx - 1].step_type if self.steps_manager.curr_step_idx > 1 else None
