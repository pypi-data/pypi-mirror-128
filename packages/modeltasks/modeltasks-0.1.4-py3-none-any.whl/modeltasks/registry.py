...
from modeltasks.io import TaskInput, TaskOutput
...
class TaskRegistry(metaclass=ModelTaskMount):
    ...
    def __init__(self, task_path: Path = None):
        # Load from specified tasks path
        self.load_task_path(task_path=task_path)
    ...
    def _list_tasks(self):
        ...
        if logging.getLogger().level == logging.DEBUG:
            for t, d in self.__class__._tasks.items():
                logging.info(f'{(t + " ").ljust(30, "=")}> {d}')
                for i, ti in d.inputs.items():
                    logging.info(f'   {"|_ INPUT":15} {i} == {ti.dependency} ({ti.type})')
                for i, ti in d.outputs.items():
                    logging.info(f'   {"|_ OUTPUT":15} {i} == ? ({ti.type})')
                for i, ti in d.parameters.items():
                    logging.info(f'   {"|_ PARAMETER":15} {i} == {ti.key} ({ti.type})')
    ...
    def set_task_path(cls, task_path: Path):
        if task_path:
            if task_path and not Path(task_path).exists():
                logging.warning(f'Provided task path "{task_path}" does not exist or is not a file or directory')
            cls._task_path = task_path
        else:
            cwd_tasks = Path.cwd() / 'model'
            script_tasks = Path(sys.argv[0]).parent / 'model'
            if cwd_tasks.exists():
                logging.warning(f'Using default task path "{cwd_tasks}" (Specify other if not correct)')
                cls._task_path = cwd_tasks
            elif script_tasks.exists():
                logging.warning(f'Using default task path "{script_tasks}" (Specify other if not correct)')
        ...
        for p in modules:
            ...
            if module != '__init__.py':
                ...
                if module not in cls._modules:
                    logging.info(f'Loading model tasks from module "{module}"')
                    cls._modules.append(module)
                    spec = importlib.util.spec_from_file_location(p.stem, p)
                    try:
                        logging.debug(f'Importing from module "{p}"')
                        spec.loader.exec_module(importlib.util.module_from_spec(spec))
                    except Exception as e:
                        logging.warning(f'Failed to import from module "{p}" ({type(e).__name__}: {e})')
                else:
                    logging.debug(f'Module "{module}" already imported (Avoid recursive imports in your model)')
        if len(cls._tasks) == 0:
            logging.warning(f'Found 0 task from task source "{cls._task_path}" (Task path correctly set?)')
            sys.exit()
    ...
    def _check_io_match(cls):
        ...
        of a dependent task.
        """
        logging.info('Checking IO type match between tasks')
        for t, n, i in [(t.name, n, i) for t in cls._tasks.values() for n, i in t.inputs.items()]:
            ...
            try:
                ...
                logging.error(f'Task "{i.dependency}" does not have an output "{i.name}" as required by task "{t}" (Check task definition)')
                sys.exit()
            try:
                if input_class._type != output_class._type:
                    logging.error(f'Type of input "{n}" in task "{t}" does not match output "{i.name}" of task "{i.dependency}" ({input_class._type} != {output_class._type})')
                    sys.exit()
            except AttributeError:
                if not hasattr(input_class, '_type'):
                    logging.error(f'Input "{n}" class has no IO type defined (Update class "{type(output_class)}" with a "_type" attribute)')
                elif not hasattr(output_class, '_type'):
                    logging.error(f'Output "{i.name}" class has no IO type defined (Update class "{type(output_class)}" with a "_type" attribute)')
                sys.exit()
    ...
    def _resolve_missing_dependencies(cls):
        ...
        for t, n, i in [(t.name, n, i) for t in cls._tasks.values() for n, i in t.inputs.items() if not i.name or not i.dependency]:
            ...
            if not name:
                name = cls._lookup_output(task_instances.get(dependency or i.raw), t, n, i.module)
            if name:
                i.name = name
            if dependency:
        ...
        if len(unresolved) > 0:
            for t, n, i in unresolved:
                logging.error(f'Task dependency "{n}" is unresolved (Correct or specify non-ambiguous dependency in task "{t}")')
                sys.exit()

    @classmethod
    def _lookup_output(cls, output: str, task_name: str, task_input: str, task_module: str) -> str:
        try:
            if issubclass(output, ModelTask):
                outputs = output.get_outputs()
                if len(outputs) == 0:
                    logging.error(f'Task "{task_name}" depends on task "{output.get_task_name()}" which has no outputs (Properly annotated outputs in "{output.get_task_name()}"?)')
                    sys.exit()
                elif len(outputs) > 1:
                    logging.error(f'Task "{task_name}" requires ambiguous output from task "{output.get_task_name()}" (Specify exact task output for input "{task_input}")')
                    sys.exit()
                else:
                    return outputs[0]
        except TypeError:
            if isinstance(output, ModelTask):
                ...
                if len(outputs) == 0:
                    ...
                    logging.error(f'Task "{task_name}" requires ambiguous output from task "{output.name}" (Specify exact task output for input "{task_input}")')
                    sys.exit()
                else:
                    return outputs[0]
        if output:
            ...
            logging.warning(f'Required task input "{task_input}" is not properly specified. Update input in task "{task_name}"')
    ...
    def _lookup_dependency(cls, requirement: Union[str, ModelTask, type], task_name: str, task_module: str) -> str:
        ...
        if requirement:
            ...
            logging.warning(f'Found unknown task input dependency in module "{task_name}"')
            return 'Unknown'
    ...
    def register_task(cls, task):
        ...
        try:
            if (task_id := cls.get_task_id(task)) not in cls._tasks:
                logging.info(f'Registering model task "{task_id}"')
                ...
                try:
                    for a_name, a_type in task.__annotations__.items():
                        try:
                            ...
                            logging.error(f'Missing task dependency: Cannot find model task "{getattr(task, a_name)}" specified in "{task_id}"')
                            sys.exit()
                        except Exception as e:
                            logging.error(f'Failed to interpret properties of task "{task_id}" ({e})')
                            sys.exit()
            ...
            logging.warning(f'Failed to register model task "{cls.get_task_id(task)}" ({type(e).__name__}: {e})')

    @classmethod
    def unregister_task(cls, task):
        try:
            if (task_id := cls.get_task_id(task)) in cls._tasks:
                logging.info(f'Unregistering model task "{task_id}"')
                del cls._tasks[task_id]
        except Exception as e:
            logging.error(f'Failed to unregister task "{cls.get_task_id(task)}" ({type(e).__name__}: {e})')
