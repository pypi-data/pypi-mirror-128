...
step_tasks_option = OptionDefinition(
    ...
    doc=dedent("""\
        ...
        Step option Recipe/config option allowed
        ----------- ----------------------------
        *           Any dimensions allowed (e.g. "2", "2.2" etc)
        ...
        4.?         Two dimensions required, and the first must be "4" (e.g. "4.1", "4.2.3")
        4.*         At least two dimensions required, and the first must be "4" (e.g. "4.1", "4.2.3")
        4.4         A fixed dimension. Recipe/config option not required, but if provided, MUST equal "4.4"
    """),
...
def get_step_tasks_option(recipe: RecipeBundle, config: typing.Optional[ConfigBundle], step_id: str) \
    ...
    what is supported. The format for the definition is documented in the doc string of `step_tasks_option`
...
def get_task_id_lookup(task_ids: typing.Iterable[StepTaskId]) -> typing.Dict[str, StepTaskId]:
    """ Convert an iterable of StepTaskIds into a lookup table to convert a string representation of a
    task id to the StepTaskId
