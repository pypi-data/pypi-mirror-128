...
def to_numpy_dtype(datatype):
    ...
    try:
        ...
        if combiner == MPI.COMBINER_STRUCT:
            ...
            names = list(imap('f{}'.format, xrange(len(datatypes))))
            formats = list(imap(mpi2npy, datatypes, blocklengths))
            offsets = displacements
            itemsize = datatype.extent
            aligned &= all(imap(_is_aligned, datatypes, offsets))
            return np_dtype(
                {
                    'names': names,
                    'formats': formats,
        ...
        if combiner in combiner_vector:
            ...
            names = list(imap('f{0}'.format, xrange(count)))
            formats = [(dtype, (blocklength,))] * count
            offsets = [stride * i for i in xrange(count)]
            itemsize = datatype.extent
            return np_dtype(
                {
                    'names': names,
        ...
        if combiner in combiner_indexed:
            ...
            names = list(imap('f{}'.format, xrange(len(displacements))))
