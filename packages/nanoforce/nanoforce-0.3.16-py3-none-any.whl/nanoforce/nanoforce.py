...
class Forceboard:
    ...
    def __init__(
        ...
        self.dynamic_cof = []
        self.static_cof = []
        self.dynamic_cof_std = []
        self.static_cof_std = []
        self.dynamic_cof_raw = []
        self.static_cof_raw = []
        self.cof = []
    ...
    def file_search(
        ...
        for path, subdirs, file_name in os.walk(folder):
            for name in file_name:
                if fnmatch(name, '*.' + ext):
                    _files = np.append(_files, os.path.join(path, name))
    ...
    def name_samples(
        ...
        *args
        ...
        for sample_name in args:
            _samples.update({sample_name: []})

        _samples_dcof = dict()

        for sample_name in args:
            _samples_dcof.update({sample_name: []})

        _samples_scof = dict()

        for sample_name in args:
            _samples_scof.update({sample_name: []})

        _samples_dstd = dict()

        for sample_name in args:
            _samples_dstd.update({sample_name: []})

        _samples_sstd = dict()

        for sample_name in args:
            _samples_sstd.update({sample_name: []})

        _samples_dcof_raw = dict()

        for sample_name in args:
            _samples_dcof_raw.update({sample_name: []})

        _samples_scof_raw = dict()

        for sample_name in args:
            _samples_scof_raw.update({sample_name: []})

        self.samples = _samples
        self.dynamic_cof = _samples_dcof
        self.static_cof = _samples_scof
        self.dynamic_cof_std = _samples_dstd
        self.static_cof_std = _samples_sstd
        self.dynamic_cof_raw = _samples_dcof_raw
        self.static_cof_raw = _samples_scof_raw
        self.sample_names = list(fb.samples.keys())

    def import_data(
        self,
        delete_0th_loop = True
        ):

        for i in range(0, len(self.samples.keys())):
            self.samples[self.sample_names[i]] = pd.read_table(self.files[i], header = 7, sep = ';')

            if delete_0th_loop is True:
                s = fb.samples[self.sample_names[i]]['Loop number']
                sidx = s[s.diff() != 0].index.tolist()

                self.samples[self.sample_names[i]] = self.samples[self.sample_names[i]][sidx[1]:]
    ...
    def plot_raw(
        ...
        if primary_axis == 'cof':
            ...
            primary_legend = 'Coefficient of friction'
        elif primary_axis == 'displacement':
            primary = 'Displacement'
            primary_legend = 'Displacement (mm)'
        elif primary_axis == 'horizontal':
            primary = 'Horizontal Force'
            primary_legend = 'Horizontal Force (N)'
        elif primary_axis == 'vertical':
            primary = 'Vertical Force'
            primary_legend = 'Vertical Force (N)'
        else:
            raise ValueError('Enter cof, displacement, horizontal or vertical')
        ...
        if x_axis == 'time':
            ...
            raise ValueError('Enter time or displacement')

        if loop == None:
            plot_data = self.samples[sample_name]
        else:
            plot_data = self.samples[sample_name][self.samples[sample_name]['Loop number'] == loop]

            max_idx = plot_data['Displacement'].idxmax()

            if direction == 'positive':
                plot_data = plot_data[plot_data.index < plot_data['Displacement'].idxmax()]
            elif direction == 'negative':
                plot_data = plot_data[plot_data.index > plot_data['Displacement'].idxmax()]

            if trim is True:
                plot_data = plot_data[plot_data['Coefficient of friction'] > 0.01]

        fig = plt.figure()
        sns.lineplot(data = plot_data, x = x_ax, y = primary, color="b")
        plt.xlabel(x_ax_label)

        if secondary_axis != []:

            if secondary_axis == 'cof':
                secondary = 'Coefficient of friction'
                secondary_legend = 'Coefficient of friction'
            elif secondary_axis == 'displacement':
                secondary = 'Displacement'
                secondary_legend = 'Displacement (mm)'
            elif secondary_axis == 'horizontal':
                secondary = 'Horizontal Force'
                secondary_legend = 'Horizontal Force (N)'
            elif secondary_axis == 'vertical':
                secondary = 'Vertical Force'
                secondary_legend = 'Vertical Force (N)'
            else:
                raise ValueError('Enter cof, displacement, horizontal or vertical')

            ax2 = plt.twinx()
            sns.lineplot(data = plot_data, x = x_ax, y = secondary, ax = ax2, color="r")
            fig.legend([primary_legend, secondary_legend])
            
        else:
            plt.legend([primary_legend])  
    ...
    def calc_cof(
        ...
        self.cof = pd.DataFrame(columns=['Static COF','Static STD','Dynamic COF','Dynamic STD'])

        for sample in self.sample_names:

            self.dynamic_cof[sample] = pd.DataFrame(columns=['Positive','Negative'])
            self.dynamic_cof_std[sample] = pd.DataFrame(columns=['Positive','Negative'])
            self.static_cof[sample] = pd.DataFrame(columns=['Positive','Negative'])
            self.static_cof_std[sample] = pd.DataFrame(columns=['Positive','Negative'])

            for loop in range(0,5):

                calc_data = self.samples[sample][self.samples[sample]['Loop number'] == loop + 1]
                
                calc_data_positive = calc_data[calc_data.index < calc_data['Displacement'].idxmax()]
                calc_data_negative = calc_data[calc_data.index > calc_data['Displacement'].idxmax()]

                if trim is True:
                    calc_data_positive = calc_data_positive[calc_data_positive['Coefficient of friction'] > 0.01]
                    calc_data_negative = calc_data_negative[calc_data_negative['Coefficient of friction'] > 0.01]

                if method == 'median':
                    dynamic_cof_positive = np.median(calc_data_positive['Coefficient of friction'][int(len(calc_data_positive)*static_region):])
                    dynamic_cof_negative = np.median(calc_data_negative['Coefficient of friction'][int(len(calc_data_positive)*static_region):])

                if method == 'mean':
                    dynamic_cof_positive = np.mean(calc_data_positive['Coefficient of friction'][int(len(calc_data_positive)*static_region):])
                    dynamic_cof_negative = np.mean(calc_data_negative['Coefficient of friction'][int(len(calc_data_positive)*static_region):])
                
                dynamic_positive_std = np.std(calc_data_positive['Coefficient of friction'][int(len(calc_data_positive)*static_region):])
                dynamic_negative_std = np.std(calc_data_negative['Coefficient of friction'][int(len(calc_data_positive)*static_region):])

                if method == 'median':
                    static_cof_positive = np.median(calc_data_positive['Coefficient of friction'][:int(len(calc_data_positive)*static_region)])
                    static_cof_negative = np.median(calc_data_negative['Coefficient of friction'][:int(len(calc_data_negative)*static_region)])

                if method == 'mean':
                    static_cof_positive = np.mean(calc_data_positive['Coefficient of friction'][:int(len(calc_data_positive)*static_region)])
                    static_cof_negative = np.mean(calc_data_negative['Coefficient of friction'][:int(len(calc_data_negative)*static_region)])

                static_positive_std = np.std(calc_data_positive['Coefficient of friction'][:int(len(calc_data_positive)*static_region)])
                static_negative_std = np.std(calc_data_negative['Coefficient of friction'][:int(len(calc_data_negative)*static_region)])

                self.dynamic_cof[sample].loc[self.dynamic_cof[sample].shape[0]] = [dynamic_cof_positive, dynamic_cof_negative]
                self.dynamic_cof_std[sample].loc[self.dynamic_cof_std[sample].shape[0]] = [dynamic_positive_std, dynamic_negative_std]
                self.static_cof[sample].loc[self.dynamic_cof[sample].shape[0]] = [static_cof_positive, static_cof_negative]
                self.static_cof_std[sample].loc[self.dynamic_cof_std[sample].shape[0]] = [static_positive_std, static_negative_std]

                self.dynamic_cof_raw[sample] = np.append(self.dynamic_cof_raw[sample], calc_data_positive['Coefficient of friction'][int(len(calc_data_positive)*static_region):])
                self.static_cof_raw[sample] = np.append(self.static_cof_raw[sample], calc_data_positive['Coefficient of friction'][:int(len(calc_data_positive)*static_region)])
                
            if method == 'median':
                
                new_row = pd.Series(data={
                    'Static COF':np.median(self.static_cof_raw[sample]),
                    'Static STD':np.std(self.static_cof_raw[sample]),
                    'Dynamic COF':np.median(self.dynamic_cof_raw[sample]),
                    'Dynamic STD':np.std(self.dynamic_cof_raw[sample]),
                },
                name = sample
                )

                self.cof = self.cof.append(new_row)
            
            if method == 'mean':
                self.cof = self.cof.append({
                    'Sample':sample,
                    'Static COF':np.mean(self.static_cof_raw[sample]),
                    'Static STD':np.std(self.static_cof_raw[sample]),
                    'Dynamic COF':np.mean(self.dynamic_cof_raw[sample]),
                    'Dynamic STD':np.std(self.dynamic_cof_raw[sample]),
                },
                ignore_index=True
                )
