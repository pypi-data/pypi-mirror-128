...
def nano_groupreads(mapping, OUTPUT="Grouped_reads", groupfile=None, keepSubdir=False, changeName=True, changePrefix="", exist_OK = False):
    ...
    for k, v in GRPs.items():
        ...
        if not os.path.exists(k):
            ...
            os.makedirs(sam, exist_ok=exist_OK)
...
def nano_modiheader(sample_read_dict, filetype="fasta", out="Modified_header", forqiime2=False, exist_ok=True):
    ...
    Args:
        ...
        forqiime2: modify headers as specified in qiime2 sample-id_seq-id
    ...
    if forqiime2:
        for sample, reads in sample_read_dict.items():
            ...
            for read in reads:
                ...
                header1 = "_".join([sample_folder, newbarcode])
