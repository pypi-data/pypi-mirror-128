...
def parser_fispact_out_data(string):
    ...
    atoms, act, contact_dose, ci = map(float, [line_ele[0], line_ele[2], line_ele[6], line_ele[9]])
...
def get_fispact_out_line_info(line):
    ...
    atoms, act, contact_dose, ci, half_life, decay_heat = parser_fispact_out_data(line[12:])
        ...
        if found_interval_line(line, interval_list):  # found a interval
            ...
            while True:
                ...
                if 'TOTAL NUMBER' in line:  # found the end of this interval part
                    break
                nuc, atoms, act, contact_dose, ci, hf_lf, dh = get_fispact_out_line_info(line)
...
def read_material_composition(filename, interval):
    """
    Read the material compositon in fispact output file.
            ...
            if is_interval_in_line(interval, line):
                ...
                while True:
                    ...
                    if ("TOTAL NUMBER OF NUCLIDES PRINTED" in line):
                        break
                    if line == '':
                        raise ValueError(f"inventory information in {filename} interval {interval} unexpect end")
                    nuc, atoms, act, contact_dose, ci, hf_lf, dh = get_fispact_out_line_info(line)
                    nucs.append(nuc)
...
def get_material_after_irradiation(filename, mid=10001):
    """
    The the updated material compositon after irradiation. The material
    ...
    density, nucs, nuc_atoms = read_material_composition(filename, interval_irrads[-1])
