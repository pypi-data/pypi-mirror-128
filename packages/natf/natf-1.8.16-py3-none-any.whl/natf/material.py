...
class Material(object):
    ...
    def fispact_material_atom_kilogram(self, value):
        ...
        if len(self._fispact_material_nuclide) != 0 and len(value) != len(self._fispact_material_nuclide):
            raise ValueError(
                'the lenght of fispact_material_atom_kilogram should equals to the length of fispact_material_nuclide!')
    ...
    def mcnp2fispact(self):
        ...
        for i in range(len(self._mcnp_material_nuclide)):
            ...
            if a != 0:
                self._fispact_material_nuclide.append(compose_fispact_nuc(z, a, m))
    ...
    def fispact2mcnp(self, ntrans_avail_nucs):
        """
        Convert the mcnp material to fispact material
        ...
        for i, nuc_m in enumerate(mcnp_nuclides):
            ...
            mcnp_atom_fraction.append(self.fispact_material_atom_kilogram[nidx]/total_atoms)
        # assign the value
    ...
    def treat_fispact_nuc_composition(self, nuc, level):
        ...
        #    nuc = nuc_mcnp2fispact(nuc_m)
        #    nidx = self.fispact_material_nuclide.index(nuc)
        #    self.mcnp_material_atom_fractionl[i] = self.fispact_material_atom_kilogram[nidx]/total_atoms
...
def get_nucs_from_xsdir(filename):
    ...
    with open(filename, 'r') as fin:
        while True:
            ...
            if line == '':
                break
            if line[0:6] == '      ' or utils.is_blank_line(line): # continue line
                continue
...
def get_neutron_library_nuclides(filenames=[fendl31c_xsdir, tendl19c_xsdir]):
    """
    ...
    filenames: list of str
        The given nuclear libraries:
        - fendl31c_xsdir: FENDL31d library
        - tendl19c_xsdir: TENDL19 library
    """
