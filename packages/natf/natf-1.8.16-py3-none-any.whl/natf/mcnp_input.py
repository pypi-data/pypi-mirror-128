...
def cell_title_change_mat(cell_title, mid_new=0, den_new=0.0, atom_den_new=0.0):
    ...
    if mid_new > 0:
        if den_new > 0:
            ...
            raise ValueError("Wrong den_new:{den_new} and atom_den_new:{atom_den_new}")
        ...
        while True:
            ...
            if is_cell_title(line):
                ...
                if cell.is_item_cell(cid, cells):
                    ...
                    new_line = cell_title_change_mat(line, mid_new, atom_den_new=cells[cidx].mat.atom_density)
                    fo.write(new_line+"\n")
                    continue
...
def update_mcnp_input_tallies(filename, cells, tid_start=10000, write_file=True):
    ...
    for i, cell in enumerate(cells):
        ...
        tally_string = compose_fn_tally_single(tid=current_tid, cid=cell.id, mid=cell.mid, mt=205)
        if write_file:
