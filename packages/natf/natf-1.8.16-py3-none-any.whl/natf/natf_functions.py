        ...
        if 'component nuclide, atom fraction' in line:  # read atom fraction
            ...
            while True:
                ...
                if 'material' in line:  # end of the material atom fraction part
                    if is_mat_used(materials, mid): # some materials used only for a perturbation or tally, do not add here
...
def treat_nuc_responses(cells, parts, NUC_TREATMENT):
    ...
    bar = Bar("treating nuclides responses", max=len(item_nuc_treatments), suffix='%(percent).1f%% - %(eta)ds')
    for i, cnt in enumerate(item_nuc_treatments):
...
def natf_cell_mat_evo(config):
    ...
    irrad_blocks, operation_times = split_irradiation_scenario(IRRADIATION_SCENARIO, POWER_TIME_STEP)
    ...
    if not os.path.isfile(tbr_file):
        with open(tbr_file, 'w') as fo:
            fo.write("Operation time (MW*year),TBR\n")
    # ============= PREPARE end ================
    ...
    if stat == 'PRE':
        ...
        if current_step > 0:
            ...
            tbr_tallies = update_mcnp_input_tallies(MCNP_INPUT, cells_need_cal, tid_start=max(tally_numbers), write_file=False)
            tbr = get_tbr_from_mcnp_output(MCNP_OUTPUT, tbr_tallies)
            update_tbr_file(tbr_file, operation_times[current_step], tbr)
    ...
    if stat == 'POST' and current_step < total_steps -1:
        ...
        tally_numbers = get_tally_numbers(MCNP_INPUT)
        update_mcnp_input_tallies(ofname, cells_need_cal, tid_start=max(tally_numbers))
        # update status
