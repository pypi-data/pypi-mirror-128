...
def plot_parts_flux(parts, work_dir=None, figname="example.png", dpi=600,
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel(xlabel='Neutron energy (MeV)', fontsize='x-large', fontweight='bold')
...
def plot_rwcs_time_evo(parts, key='rwc_chn2018',
                       work_dir=None, ofname='rwc_time_evo_mass.png', labels=None):
    """
    ...
    #
    fig, ax = plt.subplots()
    ax.set_xlabel('Cooling time (years)', fontsize='x-large', fontweight='bold')
    ax.set_ylabel(r'Mass ($\times 10^{3}$ kg)', fontsize='x-large', fontweight='bold')
    # get the mass
...
def plot_rwcs_compare(parts, keys=['rwc_chn2018', 'rwc_usnrc', 'rwc_usnrc_fetter', 'rwc_uk'],
                      cooling_time_str='1 s', work_dir=None, ofname='rwc_compare_1s.png', labels=None):
    """
    ...
    rects0 = ax.bar(
        x - 1.5 * witdh, rwcs_masses[:][0][:3], witdh, label=labels[0], color=colors[0], hatch='//')
    rects1 = ax.bar(
        x - 0.5 * witdh, rwcs_masses[:][1][:3], witdh, label=labels[1], color=colors[1], hatch='--')
    rects2 = ax.bar(
        x + 0.5 * witdh, rwcs_masses[:][2][:3], witdh, label=labels[2], color=colors[2], hatch='xx')
    rects3 = ax.bar(
        x + 1.5 * witdh, rwcs_masses[:][3][:3], witdh, label=labels[3], color=colors[3], hatch='++')
...
def calc_recycle_cooling_requirement(parts, key='cdt', rh='CRH', work_dir=None, out_unit='a', ofname=None):
    """
    Calculate the cooling time requirement for CRH (conservative remote handling, 0.01 Sv/h) and ARH (advanced remote handling, 10000 Sv/h).
    """
    ctrs = []
    for i, p in enumerate(parts):
...
def get_rwcs_by_cooling_times(parts, cooling_times_str=['1 s', '1 a', '10 a', '100 a'],
    ...
    if len(cooling_times) == 0:
        ...
        for i, ct in enumerate(cooling_times_str):
            cooling_times[i] = cooling_times_str_to_float(cooling_times_str)
