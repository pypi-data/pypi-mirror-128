...
class ArgParserNode(anytree.NodeMixin):
    ...
    def _create_subparser_factory(self):
        ...
        except TypeError as e:  # Python 3.6 support: 'required=True' argument may cause TypeError
            # Re-Raise if error is something other than '... unexpected argument 'required''.
            if "got an unexpected keyword argument 'required'" not in str(e):
                raise e
            self._subparser_factory = self._parser.add_subparsers(
                dest="subcommand", 
                parser_class=configargparse.ArgParser
