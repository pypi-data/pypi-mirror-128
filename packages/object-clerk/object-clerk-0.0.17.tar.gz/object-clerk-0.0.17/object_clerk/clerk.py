...
from object_clerk.exceptions import ArchiveCopyNotSupportedException
from object_clerk.exceptions import ArchiveMoveNotSupportedException
...
__all__ = ["ObjectClerk", "ArchiveClerk"]
...
class ObjectClerk:
    ...
    @mutate_client_exceptions
    def _upload_object(
        self, bytes_obj: bytes, bucket: str, object_key: str, content_type: str, metadata: dict
    ) -> None:
        """
        ...
        :param content_type: Content type of the file to be uploaded.  Will be added to the ExtraArgs as Content-Type
        :param metadata: metadata for the file to be uploaded.  Will be added to the ExtraArgs as Metadata
        :return: None
        """
...
class ArchiveClerk(ObjectClerk):
    """
    The ObjectClerk intended for archiving operations
    ...
    def __init__(
        ...
        retry_tries: int = -1,
        use_ssl: bool = False,
        archive_storage_class: str = None,
    ):
        """
        Initialize the Archive clerk
        :param host: Host name or ip for an s3 Gateway
        :param port: Post the s3 gateway listens on
        :param access_key: Access Key for the gateway
        :param secret_key: Secret Key for the gateway
        ...
        :param retry_tries: Number of time to retry connecting ot hte gateway. -1 for indefinite retries
        :param use_ssl: True for https and False for http
        :param archive_storage_class: Archive to AWS Glacier Deep Archive when DEEP_ARCHIVE.  Archive to Ceph otherwise.
        """
        super().__init__(
            host,
            port,
        ...
        if isinstance(archive_storage_class, str):
            self.archive_storage_class = archive_storage_class.upper()
        else:
            self.archive_storage_class = "NOT_PROVIDED"

    def __repr__(self):
        return (
            f"ArchiveClerk(host={self.host}, port={self.port}, access_key={self.access_key}, "
            f"secret_key=<SECRET KEY>, retry_delay={self.retry_delay}, retry_backoff={self.retry_backoff}, "
            f"retry_jitter={self.retry_jitter}, retry_max_delay={self.retry_max_delay}, "
            f"retry_tries={self.retry_tries}, use_ssl={self.use_ssl})"
        )
    ...
    def __str__(self):
        return f"ArchiveClerk connected to {self.endpoint_url} retrying connection errors with {self.retry_config}"
    ...
    def copy_object(
        ...
        """
        Always raise ArchiveCopyNotSupportedException to prevent copy within archive-store
        :param source_bucket: Bucket of the object being copied
        :param source_object_key: Object key of the object being copied
        :param destination_bucket: Bucket to copy to
        :param destination_object_key: Object key of the copied object
        ...
        copied file matches the original
        :return: None
        :raises: ArchiveCopyNotSupportedException
        """
        raise ArchiveCopyNotSupportedException(
            "Copy not supported for objects in the archive store"
        )
    ...
    def move_object(
        ...
        """
        Always raise ArchiveMoveNotSupportedException to prevent move within archive-store
        :param source_bucket: Bucket of the object being copied
        :param source_object_key: Object key of the object being copied and will be deleted
        :param destination_bucket: Bucket to copy to
        :param destination_object_key: Object key of the copied object
        ...
        copied file matches the original
        :return: None
        :raises: ArchiveMoveNotSupportedException
        """
        self.copy_object(
            source_bucket,
            source_object_key,
        ...
        raise ArchiveMoveNotSupportedException(
            "Move not supported for objects in the archive store"
        )
    ...
    def upload_object(
        ...
        :param object_key: Object Key in the bucket for the object after upload
        :param verify_checksum: Boolean indicator of whether to verify the checksum of the uploaded file matches
        what is in the S3 bucket when archive_storage_class is not DEEP_ARCHIVE. The checksum is never verified when
        archive_storage_class is DEEP_ARCHIVE.
        :param content_type: Content type of the file to be uploaded.  Will be added to the ExtraArgs as Content-Type
        :param metadata: metadata for the file to be uploaded.  Will be added to the ExtraArgs as Metadata
        :return: None
        :raises: ObjectVerificationException, ObjectSaveException, ObjectClerkException, ObjectClerkServerAuthException
        ...
        if checksum == md5(b"").hexdigest():
            raise ObjectSaveException("Attempt to upload an empty file not allowed.")
        # proactively retain checksum for all archive objects
        metadata["dkist-checksum"] = checksum
        retry_call(
            self._upload_object,
            fkwargs={
                "bytes_obj": bytes_obj,
        ...
        # GET request will fail for DEEP_ARCHIVE storage class, so skip when archive_storage_class is DEEP_ARCHIVE
        if verify_checksum and self.archive_storage_class != "DEEP_ARCHIVE":
            etag = self.get_object_info(bucket, object_key).get("etag")
            try:
                self._verify_checksum(checksum, etag)
            except ObjectVerificationException as e:
    ...
    def _upload_object(
        ...
        if self.archive_storage_class == "DEEP_ARCHIVE":
