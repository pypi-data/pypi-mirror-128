...
class smp_reservation_compute(models.Model):
  ...
  cs_carconfig_id = fields.Many2one('sm_carsharing_structure.cs_carconfig', string=_("Associated carConfig (CS Structure)"),
    compute="_get_cs_carconfig_id",store=False)
      ...
      if 'active_ids' in self.env.context:
        ...
        if computes.exists():
          for compute in computes:
            if not compute.ignore_update:
              compute.fetch_update_reservation_data()

  def fetch_update_reservation_data(self):
    if self.startTime and self.endTime:
      self.env["smp.sm_reservation_wizard"].compute_reservations(
        parent=self, from_q=self.startTime, till_q=self.endTime, comp=self)
