        ...
        for q_order in queue:
            try:
                ...
                if order is not None:
                    ...
                    logger.error(current_job + f' | Não foi possível obter detalhes do pedido entre em contato com o suporte')
...
def protocol_order(db_config: DatabaseConfig, order: Order, order_erp_id: str = '', client_erp_id: str = '') -> None:
    ...
    try:
        ...
        cursor.execute(queries.get_client_protocol_command(db_config.db_type), queries.get_command_parameter(db_config.db_type, [order_erp_id, order.order_id]))
        cursor.close()
        conn.commit()
        conn.close()
    except Exception as e:
