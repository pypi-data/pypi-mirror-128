    ...
    if p_size > 0:
        ...
        for stock in stocks:
            try:
                ...
                logger.warning(job_config_dict.get('job_name') + f' | Erro ao atualizar estoque para o sku {stock.get("codigo_erp")}: {str(e)}')
...
def stock_ud_dict(produtos):
    ...
    for row in produtos:
        pdict = {
            'unidade_distribuicao': row['CODIGO_EXTERNO_UNIDADE_DISTRIBUICAO'],
            'codigo_erp': row['CODIGO_ERP'],
