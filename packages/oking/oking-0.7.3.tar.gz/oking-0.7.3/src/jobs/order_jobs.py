...
def insert_temp_order(order: Order, db_config: DatabaseConfig) -> bool:
    ...
    try:
        ...
        cursor.execute(queries.get_insert_order_command(db_config.db_type), queries.get_command_parameter(db_config.db_type, [
            order.order_id,
            order.order_code,
            str(datetime.strptime(order.date.replace('T', ' '), '%Y-%m-%d %H:%M:%S') if order.date is not None else None),
            order.status,
            client_id[0],
            order.total_amount,
            ...
            str(datetime.strptime(order.paid_date.replace('T', ' '), '%Y-%m-%d %H:%M:%S') if order.paid_date is not None else None),
            order.payment_type,
            order.flag,
            order.parcels,
            order.erp_payment_condition,
            order.tracking_code,
            str(datetime.strptime(order.delivery_forecast.replace('T', ' '), '%Y-%m-%d %H:%M:%S') if order.delivery_forecast is not None else None),
            order.carrier,
            order.shipping_mode]))
...
def call_order_procedures(db_config: DatabaseConfig, order_id: int) -> (bool, str, str):
    ...
    try:
        ...
        if client_erp_id is not None:
            ...
            logger.info(current_job + f' | Atualizando codigo ERP do cliente no banco semaforo')
            cursor.callproc('OPENK_SEMAFORO.SP_PROCESSA_PEDIDO', [order_id, client_erp_id, order_out_value])
