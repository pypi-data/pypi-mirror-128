...
from metadata.utils.helpers import get_database_service_or_create, get_pipeline_service_or_create
...
class GlueSource(Source):
    def __init__(
        ...
        self.service = get_database_service_or_create(config, metadata_config, self.config.db_service_name)
    ...
    def ingest_tables(self, next_tables_token = None) -> Iterable[OMetaDatabaseAndTable]:
        try:
            if next_tables_token is not None:
                glue_resp = self.glue.get_tables(DatabaseName=self.database_name,NextToken=next_tables_token)
            else: 
                glue_resp = self.glue.get_tables(DatabaseName=self.database_name)
            for tables in glue_resp[
                "TableList"
            ]:
                ...
                database_entity = Database(
                    ...
                    service=EntityReference(
                        id=self.service.id, type="databaseService"
                    ),
                )
                fqn = f"{self.config.service_name}.{self.database_name}.{tables['Name']}"
