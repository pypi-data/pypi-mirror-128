...
def make_plugin(
    ...
    One and only one of `channel`, `generator`, `provider`, and
    `providers` must be specified.
    ...
    if len([_ for _ in (channel, generator, provider, providers) if _]) != 1:
        raise ValueError(
            'One and only one of channel, generator, provider, or providers is required.'
        )
    if manifest is None:
        try:
