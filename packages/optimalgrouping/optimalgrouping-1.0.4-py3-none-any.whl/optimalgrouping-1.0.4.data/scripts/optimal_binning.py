...
help+='the input spectrum file is grouped using grppha and the output has the _rbn.pi extension added to the root ' \
	  'file name\n '
...
parser.add_argument("--systematic_fraction",
					help="energy dependence of resolution (power-law index) for custom input (keV)", type=float, default=0)
...
res = args.res
en_dep = args.en_dep
sys_fract = args.systematic_fraction

execute_binning(spectrum, bkg, rmf, arf, mine, maxe, min_counts, eref, res, en_dep, systematic_fraction=sys_fract)