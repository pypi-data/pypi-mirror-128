...
class DataLabeling(OCIWorkRequestMixin):
    ...
    def __init__(
        ...
        if self.compartment_id is None:
            self.compartment_id = NB_SESSION_COMPARTMENT_OCID
    ...
    def export(self, dataset_id: str, path: str, include_unlabeled=False) -> str:
        ...
        if not re.match(r"oci://*@*", path):
            raise InvalidObjectStoragePath(
                "The `path` is not valid. It must follow the pattern 'oci://<bucket_name>@<namespace>/path'."
            )
        ...
        if not bucketname:
            raise InvalidObjectStoragePath(
                "The `path` is not valid. The `<bucket_name>` is not found. It must follow the pattern 'oci://<bucket_name>@<namespace>/path'."
            )
        if not namespace:
            raise InvalidObjectStoragePath(
                "`path` is not valid. The `<namespace>` is not found. It must follow the pattern 'oci://<bucket_name>@<namespace>/path'."
            )
        ...
        try:
            ...
            if dataset_id not in self.list_dataset().index:
                raise ValueError(
                    "Invalid `dataset_id`. Call `.list_dataset()` method to get all available datasets."
                )
            raise error
