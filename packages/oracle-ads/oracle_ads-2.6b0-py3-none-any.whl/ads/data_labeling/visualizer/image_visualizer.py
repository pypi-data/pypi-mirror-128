...
class WrongEntityFormat(ValueError):
    def __init__(self):
        super().__init__("Wrong entity format. It must be `BoundingBoxItem`.")
...
class LabeledImageItem:
    ...
    def _validate(self):
        ...
        """
        if self.img is None:
            raise ValueError("The `img` object must be provided.")

        if not isinstance(self.img, ImageFile):
            raise ValueError(
                "The input `img` object type must be `PIL.ImageFile.ImageFile`."
            )
...
class ImageLabeledDataFormatter:
    ...
    def render_item(
        ...
        if not isinstance(item, LabeledImageItem):
            raise TypeError(
                "Wrong format for the item. The item must be `LabeledImageItem`."
            )
        ...
        if path:
            if os.path.isdir(path):
                ...
                img_format_str = ", ".join(IMG_FORMAT)
                raise ValueError(
...
def _df_to_bbox_items(
    df: pd.DataFrame, content_column="Content", annotations_column: str = "Annotations"
    ...
    for item in df.T.to_dict().values():
        if item[annotations_column] and not isinstance(item[annotations_column], list):
            raise ValueError(
                f"Wrong format of the `{annotations_column}` column. "
                "It must be List[`BoundingBoxItem`]."
            )
