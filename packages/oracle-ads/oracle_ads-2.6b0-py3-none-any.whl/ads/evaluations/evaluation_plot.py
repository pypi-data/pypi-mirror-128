...
class EvaluationPlot:
    ...
    def get_legend_labels(cls, legend_labels):
        ...
        def render_legend_labels(label_dict):
            ...
            try:
                ...
                utils._log_missing_module("ipywidgets", "oracle-ads[notebook]")
                raise e
    ...
    def plot(
        ...
        protected_features_labels=None,
        perfect=False,
        baseline=True,
        legend_labels=None,
        plot_pdf="report.pdf",
        ...
        if plots is None:
            ...
            logger.info(
                "Showing plot types: "
                + ", ".join(
                    [str(EvaluationPlot._pretty_titles_map[str(p)]) for p in plots]
                )
                + ", ".join([str(x) for x in map(str, plots)]),
            )
        ...
        if cls.prob_type == "_bin":
            ...
            # for ftr in protected_features:
            #     EvaluationPlot.
            #     plots.insert("protected_ftr_tpr")
        ...
        try:
            for i, plot_type in enumerate(plots):
                ...
                if plot_pdf is not None:
                    pp.savefig(fig)
            for ftr in protected_features:
                cls.legend_labels = protected_features_labels[ftr]
                fig_title, fig = None, None
                fig_title, ax_title = plt.subplots(1, 1, figsize=(18, 0.5), dpi=144)
                ax_title.text(
                    x=0.5,
                ...
                if plot_pdf is not None:
                    pp.savefig(fig_title)
                for fn in [
                    cls._protected_ftr_tpr,
                    cls._protected_ftr_fpr,
                    cls._protected_ftr_num_pp,
                ]:
                    nrows = math.ceil(len(evaluation.columns) / MAX_PLOTS_PER_ROW)
                    fig, ax = plt.subplots(
                        nrows, MAX_PLOTS_PER_ROW, figsize=(10, 4 * nrows), dpi=144
                    ...
                    ax = ax.flatten()
                    fn(axs=ax, evaluation=evaluation, ftr=ftr)
                    fig.tight_layout()
                    if plot_pdf is not None:
                        pp.savefig(fig)
    ...
    def _normalized_confusion_matrix(cls, axs, evaluation):
        for model_num, ax in enumerate(axs):
            ...
            raw_cm = np.asarray(mod["raw_confusion_matrix"])
    ...
    @classmethod
    def _protected_ftr_tpr(cls, axs, evaluation, ftr):
        n_models = len(evaluation.columns)
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(
                    ax,
                    evaluation[col][f"_{ftr}_classes"],
                    evaluation[col][f"_{ftr}_tpr"],
                    axis_lim=[0, 1],
                    axis_labels=[f"True Positive Rate in {ftr}", None],
                    title=col,
                )
            else:
                ax.axis("off")
    ...
    @classmethod
    def _protected_ftr_fpr(cls, axs, evaluation, ftr):
        n_models = len(evaluation.columns)
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(
                    ax,
                    evaluation[col][f"_{ftr}_classes"],
                    evaluation[col][f"_{ftr}_fpr"],
                    axis_lim=[0, 1],
                    axis_labels=[f"False Positive Rate in {ftr}", None],
                    title=col,
                )
            else:
                ax.axis("off")
    ...
    @classmethod
    def _protected_ftr_num_pp(cls, axs, evaluation, ftr):
        n_models = len(evaluation.columns)
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(
                    ax,
                    evaluation[col][f"_{ftr}_classes"],
                    evaluation[col][f"_{ftr}_positive_pred"],
                    axis_labels=[f"Number of Positive Predictions by {ftr}", None],
                    title=col,
                )
            else:
                ax.axis("off")
