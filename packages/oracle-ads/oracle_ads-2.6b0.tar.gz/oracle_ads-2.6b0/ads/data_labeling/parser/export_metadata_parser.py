    ...
    def _validate(json_data: Dict[Any, Any]) -> None:
        ...
        for k in MetadataParser.EXPECTED_KEYS:
            if k not in json_data:
                raise ValueError(
                    f"Invalid format of the dataset metadata. The {k} field not found."
                )
        expected_list_format = ["labelsSet", "recordFiles"]
        for k in expected_list_format:
            if not isinstance(json_data[k], list):
                raise ValueError(
                    f"Invalid format of the dataset metadata. The {k} field should be a list."
                )
        ...
        for k in expected_dict_format:
            if not isinstance(json_data[k], dict):
                raise ValueError(
                    f"Invalid format of the dataset metadata. The {k} field should be a dictionary."
                )
