...
class LabeledTextItem:
    ...
    def _validate(self):
        ...
        """
        if not self.txt:
            raise ValueError("The `txt` field cannot be empty.")

        if not isinstance(self.ents, List):
            raise ValueError(
                "Wrong format for the entities. The entities must be List[NERItem]."
            )

        if any([entity.offset + entity.length > len(self.txt) for entity in self.ents]):
            raise ValueError(
                "At least one of the entities exceeds the length of the text. Check the start index and offset of the entities."
            )
        ...
        for i in range(len(self.ents) - 1):
            if self.ents[i].offset + self.ents[i].length >= self.ents[i + 1].offset:
                raise AssertionError(
                    ...
                    "so make sure the entities you are setting do not overlap."
                )
...
class TextLabeledDataFormatter:
    ...
    def render(items: List[LabeledTextItem], options: Dict = None) -> str:
        ...
        """
        if not items:
            raise ValueError("Items not provided.")
...
def _df_to_ner_items(
    ...
    if content_column not in list(df.columns) or annotations_column not in list(
        df.columns
    ):
        raise ValueError(
            "The wrong format of input dataframe. It must have "
            f"`{content_column}` and `{annotations_column}` columns."
        )
    ...
    for item in df.T.to_dict().values():
        if item[annotations_column] and not isinstance(item[annotations_column], list):
            raise ValueError(
                f"The wrong format of the `{annotations_column}` column. "
                "It must be `List[NERItem]`."
            )
