...
class ModelEvaluator:
    ...
    def _get_general_metrics(self):
        ...
        self.safe_metrics_call(scoring_functions, **kwargs)
        self.safe_metrics_call(scoring_functions_without_labs)
    ...
    def _get_fairness_metrics(self):
        for ftr in self.protected_features:
            ftr_arr = self.X[ftr]
            tot = type_of_target(ftr_arr.sample(n=min(1000, ftr_arr.shape[0])))
            if tot == "binary":
                self._get_fairness_metrics_binary(ftr, ftr_arr)
            elif tot == "multiclass":
                self._get_fairness_metrics_discrete()
            elif tot == "continuous":
                self._get_fairness_metrics_continuous()
            else:
                pass

    def _get_fairness_metrics_binary(self, ftr, ftr_arr):
        # Stat Parity
        stat_parity_name = f"{ftr}_statistical_parity"
        self.fairness_mets.add(stat_parity_name)
        protected_classes = ftr_arr.unique()
        self.metrics[f"_{ftr}_classes"] = protected_classes

        assert len(protected_classes) == 2, (
            f"This feature is not binary: {ftr}," f"it has classes: {protected_classes}"
        )
        y_pred_a = self.y_pred[ftr_arr == protected_classes[0]]
        y_pred_b = self.y_pred[ftr_arr == protected_classes[1]]
        prob_pos_pred_a = np.sum(y_pred_a == self.positive_class)
        prob_pos_pred_b = np.sum(y_pred_b == self.positive_class)
        try:
            stat_parity = prob_pos_pred_a / prob_pos_pred_b
        except:
            stat_parity = f"{prob_pos_pred_a} / {prob_pos_pred_b}"

        self.metrics[f"_{ftr}_positive_pred"] = [prob_pos_pred_a, prob_pos_pred_b]
        self.metrics[stat_parity_name] = stat_parity

        # Equal Oppurtunity (TPR)
        equal_opp_name = f"{ftr}_equal_oppurtunity(TPR)"
        self.fairness_mets.add(equal_opp_name)
        y_true_a = self.y_true[ftr_arr == protected_classes[0]]
        y_true_b = self.y_true[ftr_arr == protected_classes[1]]

        y_pos_a = y_pred_a[y_true_a == self.positive_class]
        y_pos_b = y_pred_b[y_true_b == self.positive_class]

        tp_a = y_pos_a[y_pos_a == self.positive_class]
        tp_b = y_pos_b[y_pos_b == self.positive_class]

        tpr_a = len(tp_a) / len(y_pos_a)
        tpr_b = len(tp_b) / len(y_pos_b)
        try:
            equal_opp = tpr_a / tpr_b
        except:
            equal_opp = f"{tpr_a} / {tpr_b}"
        self.metrics[f"_{ftr}_tpr"] = [tpr_a, tpr_b]
        self.metrics[equal_opp_name] = equal_opp

        # Predictive Equality (FPR)
        pred_equal_name = f"{ftr}_predictive_equality(FPR)"
        self.fairness_mets.add(pred_equal_name)
        y_false_a = y_pred_a[y_true_a != self.positive_class]
        y_false_b = y_pred_b[y_true_b != self.positive_class]

        fp_a = y_false_a[y_false_a == self.positive_class]
        fp_b = y_false_b[y_false_b == self.positive_class]

        fpr_a = len(fp_a) / len(y_false_a)
        fpr_b = len(fp_b) / len(y_false_b)

        try:
            pred_equal = fpr_a / fpr_b
        except:
            pred_equal = f"{fpr_a} / {fpr_b}"
        self.metrics[f"_{ftr}_fpr"] = [fpr_a, fpr_b]
        self.metrics[pred_equal_name] = pred_equal

    def _get_fairness_metrics_discrete(self):
        pass

    def _get_fairness_metrics_continuous(self):
        pass
    ...
    def _get_regression_metrics(self):
        ...
        self.safe_metrics_call(scoring_functions)
    ...
    def safe_metrics_call(self, scoring_functions, **kwargs):
        ...
        for name, fn in scoring_functions.items():
            try:
                self.metrics[name] = getattr(metrics, fn)(
                    self.y_true, self.y_pred, **kwargs
                )
