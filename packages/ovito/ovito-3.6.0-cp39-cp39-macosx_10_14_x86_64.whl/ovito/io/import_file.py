...
def import_file(location, **params):
    """ Imports data from an external file.
        ...
        **Explicit file format specification**
        ...
        ``"gaussian/cube"``, ``"gro"``, ``"gsd/hoomd"``, ``"imd"``, ``"lammps/data"``, ``"lammps/dump"``, ``"lammps/dump/bin"``, ``"lammps/dump/local"``, 
    ...
    if 'input_format' in params:
        ...
        if not format in import_file._formatTable:
            raise ValueError("Unknown input format: '{}'. The supported formats are: {}".format(format, sorted(list(import_file._formatTable.keys()))))

        # Create an instance of the importer class. It will be configured below.
        importer = import_file._formatTable[format]()
    else:
    ...
    for key in params:
        if not hasattr(importer, key):
            raise RuntimeError("Invalid keyword parameter. File reader object %s doesn't have a parameter named '%s'." % (repr(importer), key))
