...
const dateEditor = function (cell, onRendered, success, cancel) {
    ...
    const cellValue = window.luxon.DateTime.fromMillis(rawValue, opts).toFormat("yyyy-MM-dd");
...
const datetimeEditor = function (cell, onRendered, success, cancel) {
    ...
    const rawValue = cell.getValue();
    const opts = { zone: new window.luxon.IANAZone('UTC') };
    const cellValue = window.luxon.DateTime.fromMillis(rawValue, opts).toFormat("yyyy-MM-dd'T'T");
    const input = document.createElement("input");
