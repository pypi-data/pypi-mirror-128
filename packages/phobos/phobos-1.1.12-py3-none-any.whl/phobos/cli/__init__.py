...
from phobos.cli.utils import datalab, get_all_tasks_eu, get_task_details_eu, get_annotations_eu, DataLab
    ...
    with open(path, 'r') as fp:
        ...
        if not distributed:
            if context == "datalab" and len(node_pool) > 0:
                data['run']['enviornment']['nodeSelector']['polyaxon'] = node_pool
            if len(image) > 0:
            ...
            if context == "datalab" and len(node_pool) > 0:
                data['run']['master']['enviornment']['nodeSelector']['polyaxon'] = node_pool
                data['run']['worker']['enviornment']['nodeSelector']['polyaxon'] = node_pool
            if len(image) > 0:
...
@click.option('--path', required=False, default=None, help="destination path for annotations")
def get(all, details, email, passwd, task, path):
    '''
    phobos get
    ...
    path    :   destination path for annotations (optional)
    '''
    if email == "" or passwd == "":
        ...
        if path is not None:
            if not os.path.exists(path):
                try:
                    ...
                    print('directory {} cannot be created'.format(path))
                    return
            get_annotations_eu(id=task, path=path, email=email, passwd=passwd)
        else:
            print('please provide valid destination path for annotations')
