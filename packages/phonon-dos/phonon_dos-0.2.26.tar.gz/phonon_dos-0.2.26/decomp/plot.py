...
def save_proj(x,y,ytot,convolution, k,freq,n,namedir, params, max_Z):
    pgf_with_latex["figure.figsize"] = figsize(1.4)
...
def plot_k2(kk, kk_many, frequencies_disp, frequencies_ft, f, data, data_projected, indexes, max_Z, branches):
    ...
    plt.ion()
    dk = kk_many[1] - kk_many[0]
    kk_many2 = np.concatenate((kk_many, [kk_many[-1]+dk])) - dk/2
    kk_many2 = kk_many2 / kk_many[-1]
    X, Y = np.meshgrid(kk_many2,f)
    kk_many = kk_many/kk_many[-1]
    kk = kk/kk[-1]
    ...
    if(len(branches) == tot_branches): #plot whole spectrum
        ...
        ax.pcolormesh(X,Y,data, vmax=max_Z, cmap='Blues')
        print('finished')
    else:
        print('Drawing branches ', branches)
        ax.pcolormesh(X,Y,np.sum(data_projected[:,branches,:], axis=1), vmax=max_Z, cmap='Blues')
        print('finished')
    ...
    ax2.set_ylabel('[Thz]')
    for j in branches:
        if(len(kk_many) > len(kk)): # then you are interpolating
            ...
            graph_disp = ax2.scatter(kk_many, frequencies_disp[j,:], color='black', label='0K dispersion', linewidth=1)
            # graph_ft, = ax2.plot(kk_many, frequencies_ft[j,:], color='red', label='interp finite dispersion', linewidth=1)

    ax2.get_shared_y_axes().join(ax, ax2)
    plt.ylim([f[0], f[-1]])
    plt.xlim([kk_many[0], kk_many[-1]])
    ...
    plt.show()
    plt.title('Spectrum branches '+str(branches))
    plt.tight_layout()
    fig.savefig('plot5_'+str(branches)+'.pdf', backend='pgf')
    return 


def plot1(freq,ZQS, labels, max_Z):
    pgf_with_latex["figure.figsize"] = figsize(1.4)[::-1]
    mpl.rcParams.update(pgf_with_latex)
    n = len(ZQS[0])-1

    fig, axs = plt.subplots(n, sharex=True, gridspec_kw={'hspace': 0})
    if not (type(axs) == np.ndarray):
        axs = [axs]
    for i in range(n):
        if(max_Z > np.max(ZQS[:,i+1]) or max_Z < 0.0):
            max_Z_used = np.max(ZQS[:,i+1])*1.15
            min_Z_used = -0.05*max_Z_used
        ...
        axs[i].set_ylim(bottom=min_Z_used, top=max_Z_used)
        axs[i].set_yticks([])
...
def plot2(freq,ZQS,modes, labels, freqs_from_disp, max_Z):
    pgf_with_latex["figure.figsize"] = figsize(1.4)[::-1]
    mpl.rcParams.update(pgf_with_latex)
    n_modes = len(modes)
    n_kpoints = len(ZQS[:,0,0])
    
    if(max_Z > np.max(ZQS) or max_Z < 0.0):
        max_Z = np.max(ZQS)

    for n in range(n_kpoints): #iterating over the k points
        fig, axs = plt.subplots(n_modes, sharex=True,  gridspec_kw={'hspace': 0})
        if not (type(axs) == np.ndarray):
            axs = [axs]
        for i in range(n_modes): #iterating over the modes
            mode = modes[i]
            if(max_Z > np.max(ZQS[n,:,mode+1]) or max_Z < 0.0):
                max_Z_used = np.max(ZQS[n,:,mode+1])*1.15
                min_Z_used = -0.05*max_Z_used
