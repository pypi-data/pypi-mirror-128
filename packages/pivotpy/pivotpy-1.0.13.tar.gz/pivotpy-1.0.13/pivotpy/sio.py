...
__all__ = ['Arrow3D', 'fancy_quiver3d', 'export_poscar', 'InvokeMaterialsProject', 'get_kpath', 'read_ticks',
           'str2kpath', 'get_kmesh', 'order', 'rotation', 'get_bz', 'splot_bz', 'iplot_bz', 'to_R3', 'kpoints2bz',
           'fix_sites', 'get_pairs', 'iplot_lat', 'splot_lat', 'join_poscars', 'write_poscar', 'scale_poscar',
           'rotate_poscar']
...
class InvokeMaterialsProject:
    ...
    def request(self,formula,mp_id=None,max_sites = None,min_sites=None):
        ...
        for res in self.__response:
            ...
            crs = dict(mp_id = mp_id,symbol = symbol, crystal = crystal, unit = unit, content = cif)
            cifs.append(vp.Dict2Data(crs))
        self.__cifs = cifs

        # get poscars
        poscars = []
        for cr in self.__cifs:
            cif = cr.content
            lines = cif.split('\n')
            if '' in lines.copy():
                lines.remove('')
            ...
            # =======================
            net_out = dict(system = system, symbol = symbol, crystal =crystal,mp_id = mp_id,content = content)
            poscars.append(vp.Dict2Data(net_out))
        self.__poscars = poscars

        # Add write method to cifs and poscars
        def write(dd_instance,filename):
            "Write current poscar/cif string to file"
            with open(filename,'w') as f:
                f.write(dd_instance.content)
...
def str2kpath(kpath_str,n = 5, weight = None, ibzkpt = None, outfile = None):
    ...
    for a,b in zip(where_blanks[:-1], where_blanks[1:]):
        if b - a < 2:
            return print(f"There should be at least two points in a patch of path at line {a+1}!")
