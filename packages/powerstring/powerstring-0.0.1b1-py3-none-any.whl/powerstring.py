...
class Powerstring:
    """Create a Powerstring.
    
    :param string: The text to style
    :type string: str
    
    :param start_line: The anchor where to start the line. Set to `False` if line should start at position 0.
    :type start_line: Union[str, bool], optional
    
    :param escape: The character to use for escaping
    :type escape: str, optional
    
    :param join: The string which joins between the lines
    ...
    def to_markdown(self) -> str:
        """Generate markdown from Powerstring Object"""
        beta_error()
    
    def __str__(self) -> str:
        """Turns Powerstring object into a string"""
        return self.syntax.process(self)
    
    def __invert__(self) -> str:
        """Returns styled string without ansi sequences"""
        return _remove_ansi(self.syntax.process(self))
    
    def __iadd__(self, other) -> Powerstring:
        """Adds text to the Powerstring"""
        if isinstance(other, Powerstring):
            string = other.text
        elif isinstance(other, str):
            string = other
        else:
            self += other # raise error
        self.lines.append(string.split("\n"))
        self.text += "\n" + string
        return self
...
def _remove_ansi(string: str) -> str:
    ansipattern = r"\x1b\[.+?m"
    return re.sub(ansipattern, "", string)
