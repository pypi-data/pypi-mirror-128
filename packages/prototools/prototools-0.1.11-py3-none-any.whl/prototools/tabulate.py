...
class _Tabulate:
    ...
    def _calculate_widths(self) -> None:
        ...
        self.cols_list = [
            max([len(str(e)) for e in i]) for i in self.data_transposed
        ]
        if self.headers is not None:
            self.h_list = [len(str(e)) for e in self.headers]
            self.cols_list = list(
    ...
    def _draw_line(self, pos: str) -> str:
        ...
        line = " "*self.style.margin_left + D[pos]["L"]
        for w in self.cols_list:
            line += u"{content}".format(
                content=(
                    self.border.horizontal * (w+self.style.paddings()) +
                    D[pos]["C"]
                    ),
            )
        return line[:-1] + D[pos]["R"]

    def _draw_inner(self):
        line = " "*self.style.margin_left + self.border.vertical_left
        for w in self.cols_list:
            line += u"{content}".format(
                content=(
                    self.border.horizontal * (w+self.style.paddings()) +
                    self.border.intersection
                    ),
            )
        return line[:-1] + self.border.vertical_right
...
def _generate_columns(data: List[float], c: str = u"\u2589") -> List[str]:
    """
...
def show_cols(data: List[float], chars: Optional[str] = None) -> str:
    ...
    if chars is None:
        chars = u"\u2588"
    t = _show_columns(data, _generate_columns(data, chars))
    return "\n".join(t)