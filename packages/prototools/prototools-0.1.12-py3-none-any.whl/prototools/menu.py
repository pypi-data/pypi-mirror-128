...
def get_option(options: Sequence, menu) -> int:
    ...
    event = ""
    position = 0
    sys.stdout.write(str(options[position])+"\r")
    while True:
        size = len(str(options[position]))
        k = ord(msvcrt.getch())
        ...
        print(" " + str(options[position]) + " "*(width-size), end="\r")
    return position
...
class Builder:
    ...
    Args:
        max_dimension (Dimension, optional): Max dimension of the menu.
    """
    ...
    def __init__(
        ...
        self._style = Style()
        self._header = Header(self._style, max_dimension)
        self._prologue = Text(self._style, max_dimension)
        self._items = Items(self._style, max_dimension)
        self._epilogue = Text(self._style, max_dimension)
        self._footer = Footer(self._style, max_dimension)
        self._prompt = Prompt(self._style, max_dimension)
    ...
    def set_style(self, border_style: str):
        ...
        self._style.border.set_style(border_style)
        return self
...
class Menu:
    ...
    def __init__(
        ...
        self.parent = None
        self.exit_item = Exit(menu=self, text=exit_option_text)
...
class Item:
    ...
    def show(self, index: int) -> str:
        ...
        Default is::
            ...
            2 - Another Item
        """
        return f"{index + 1} - {self.get_text()}"
...
class Exit(Item):
    ...
    def __init__(
        ...
        super().__init__(text, menu=menu, should_exit=True)
