...
# 根据操作系统读取对应的DLL库
def Get_Reader_Dll() -> WinDLL:
    import platform
    import os
    dir = os.path.dirname(os.path.abspath(__file__))
    try:
        return WinDLL(f'{dir}/dlls/MasterRD.dll')
    except:
        return Get_Reader_Dll_custom()


# 根据操作系统读取对应的DLL库
def Get_Reader_Dll_custom() -> WinDLL:
    import platform
    import os
    dir = os.path.dirname(os.path.abspath(__file__))
    os_bit, os_name = platform.architecture()
    if os_bit == '32bit':
        return WinDLL(f'{dir}/32/MasterRD.dll')
    elif os_bit == '64bit':
        return WinDLL(f'{dir}/64/MasterRD.dll')
    else:
