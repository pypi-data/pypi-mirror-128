...
class push():
    ...
    def get_deleate(self):
        try:
            requests.get('http://www.pushplus.plus/send?token={}&title={}&content={}&template={}'.format(self.token,self.title,self.content,self.template))
            print('请求成功')
        except:
...
if __name__=='__main__':
    qingqiu=push('自己token',title='成功',content='成功')
    qingqiu.post_put()
