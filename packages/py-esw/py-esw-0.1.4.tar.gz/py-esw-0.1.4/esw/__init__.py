...
class ESWrapper:
    ...
    def get_current_block(self):
        ts = int(time.time())
        url = f'https://api.etherscan.io/api?module=block&action=getblocknobytime&timestamp={ts}&closest=before&apikey={self.api}'
        return int(requests.get(url).json()['result'])

    def get_logs(self, contract, topic, origin=None, function=None):
        start = origin if origin else self.get_origin_block(contract)
        call = self._call(module='logs', action='getLogs', start=start, end=99999999999,
                          account=contract, sort='asc', max_results=1000, function=function, other=f'&topic0={topic}')
        return call
