...
'''
        lasthemisph = hemisph
        twi = alma_ephem.twilight(dfl, i, hemisph, True)      # True = round to seconds
        moon, moon2 = alma_ephem.moonrise_set2(dfl,i)
...
'''.format(datestr)
    ...
    p = alma_ephem.planetstransit(dfl, True)      # True = round to seconds
    m = m + r'''Venus & {} & {} \\
'''.format(p[0],p[1])
    m = m + r'''Mars & {} & {} \\
'''.format(p[2],p[3])
...
def maketables(first_day, dtp, dfA4):
