...
class Mqtt:
    ...
    def __init__(
        ...
        asyncio.create_task(self.setup_topics())
    ...
    async def setup_topics(self):
        # Setup Gateway Topics
        self.setup_gateway_topics()
        # Gateway parsing needs to be complete before we start setting up the device topics
        await self.gateway_parse_complete.wait()
        # Setup Device Topics
        self.setup_device_topics()
    ...
    async def update_gateway_topics(self, params: typing.Dict):
        """
    ...
    async def gateway_heartbeat(self):
        ...
        while True:
            ...
            try:
                ...
                if connections != connections_save_state:
    ...
    async def update_gateway_state(self, data: dict):
        ...
        state = {"network": data.get("network", {})}
        events.publish(
            f"trigger_send",
            topic_type="state",
            data=state,
    ...
    async def clean_up_setup_tasks(self):
        ...
        exceptions = {
            cancelled_tasks[index].get_coro().__name__: type(result).__name__
            for index, result in enumerate(results)
            if isinstance(result, BaseException)
        }
        if exceptions:
            log.warning(
                f"The following MQTT setup tasks cancelled with errors: {exceptions}"
            )
    ...
    async def send(self, *, data: dict, topic_type: str, **kwargs):
        ...
        if self.config.parsers.get(topic_type, {}).get("cache", True):
            ...
            fut = store.disk_storage.mqtt.insert_delayed(
                topic=topic,
                payload=payload,
    ...
    async def handle_publish_flow(
        ...
        try:
            ...
            if self.parser.get(topic_type, {}).get("device_message", True):
                await asyncio.wait_for(self.evt_setup.wait(), 60)
...
class BacklogManager:
    ...
    async def start(
        ...
        while True:
            ...
            msg_cnt = 0
            try:
                async for row in store.disk_storage.mqtt.get_all_unsent(
                    self.client, 1000
                ):
                    if start is None:
                        start = time.time()
