    ...
    def transform(self) -> None:
        ...
        if self.gridder is not None:
            ...
            for ms in dataset.ms_list:
                ...
                original_weight_sum.append(da.apply_along_axis(np.sum, 0, weight.data))
                unique_idx = da.unique(idx).compute()
                gridded_weights = bin_count[unique_idx]  # Gridded weight for each u,v coordinate on the grid
                gridded_weightsquared_sum.append(da.apply_along_axis(np.sum, 0, gridded_weights * gridded_weights))

            original_weight_sum_array = da.array(original_weight_sum)
            gridded_weightsquared_sum_array = da.array(gridded_weightsquared_sum)
            original_weight_sum = da.apply_along_axis(np.sum, 0, original_weight_sum_array)
            gridded_weightsquared_sum = da.apply_along_axis(np.sum, 0, gridded_weightsquared_sum_array)
