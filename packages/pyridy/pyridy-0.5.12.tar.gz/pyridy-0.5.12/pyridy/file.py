...
class RDYFile:
    ...
    def create_map(self, t_lim: Tuple[np.datetime64, np.datetime64] = None) -> Map:
        ...
        if coords == [[]]:
            ...
            if t_lim:
                ...
                gps_series = gps_series[mask]
                coords = [c for i, c in enumerate(coords) if mask[i]]
                time = [c for i, c in enumerate(gps_series.time) if mask[i]]
            ...
            m = Map(center=self.determine_track_center(gps_series)[::-1],
                    zoom=12,
