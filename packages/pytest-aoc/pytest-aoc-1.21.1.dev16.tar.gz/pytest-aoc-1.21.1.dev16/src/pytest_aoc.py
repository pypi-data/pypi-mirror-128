...
def create_fixture(name, fn):
    def with_file(fn, read):
        with open(fn, 'r') as f:
            return read(f)
    globals().update({
        name: (lambda fn, read: pytest.fixture(lambda: with_file(fn, read)))(fn, read)
        for (name, read) in {
            f"{name}_raw"           : lambda f: f.read(),
            f"{name}_text"          : lambda f: f.read().strip(),
            f"{name}_lines"         : lambda f: [line.strip() for line in f],
            f"{name}_numbers"       : lambda f: [int(line) for line in f],
            f"{name}_number"        : lambda f: int(f.read()),
            f"{name}_grid"          : lambda f: [row.split() for row in f],
            f"{name}_number_grid"   : lambda f: [[int(cell) for cell in row.split()] for row in f]
        }.items()})
    globals().update({
        name: (lambda fn, read: pytest.fixture(lambda request: with_file(fn, lambda fn: read(fn, getattr(request.module, 'parse')))))(fn, read)
        for (name, read) in {
            f"{name}_parsed_raw"    : lambda f, p: p(f.read()),
            f"{name}_parsed_text"   : lambda f, p: p(f.read().strip()),
            f"{name}_parsed_lines"  : lambda f, p: [p(line.strip()) for line in f]
        }.items()})
