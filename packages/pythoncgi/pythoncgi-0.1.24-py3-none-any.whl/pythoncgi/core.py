from .utils import obj_to_bytes, _should_return_304, _basic_authorization, _cache_control, parse_range, parse_authorization, set_authenticate_response, _write_to_cache_file
from omnitools import dt2yyyymmddhhmmss, HeadersDict, ApacheHeadersDict, str2html, encodeURIComponent, rfc822gmt2dt
...
__RESPONSE["headers"].update(__SETTINGS["default_content_type"])
__METHODS = {}


def init(*, keep_blank_values: bool = True, session: str = None, headers_filter: list = None):
    ...
    arguments = FieldStorage(environ=__SERVER, keep_blank_values=keep_blank_values)
    ...
    _HEADERS.update({k: v for k, v in _SERVER.items() if k not in __SETTINGS["headers_filter"]})
...
def log(obj, fp: str = None):
    ...
    open(fp or __SETTINGS["log_fp"], "ab").write(__SETTINGS["log_format"](now, obj)+b"\n")
...
def flush():
    ...
    __SETTINGS["stdout"].flush()


def _print(obj):
    __SETTINGS["stdout"].write(obj_to_bytes(obj))


def print(obj = b"", end=__SETTINGS["print_end"]):
...
def print_flush(c: bytes, buf: int = __SETTINGS["buf_size"]):
    def w(c):
        while c:
            yield c[:buf]
            c = c[buf:]
...
def print_file(fo, buf_size: int = __SETTINGS["buf_size"], range = None):
...
def _generate_headers():
    if not __PRINTED["STATUS"]:
        ...
        __PRINTED["STATUS"] = True
    if not __PRINTED["HEADERS"]:
        ...
        _print(session)
        _print("\n")
        __PRINTED["HEADERS"] = True


def _generate_response():
    _generate_headers()
    flush()
    print_flush(__RESPONSE.pop("content"))
    ...
    if not __RESPONSE["cache"] and status_code >= 500 and status_code <= 599:
        if status_code in http.client.responses:
            ...
            status_message = "<h1>{} {}</h1><br/>{}".format(
                status_code,
                msg,
                __SETTINGS["default_5xx_response"]
            )
...
def execute(
        ...
        cache_ctrl = lambda: _cache_control(max_age=1, must_revalidate=True),
        cache_norm = lambda: _should_return_304(_HEADERS),
