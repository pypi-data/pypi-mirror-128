...
class pulsar_qrm_scpi_ifc(ieee488_2):
    ...
    def __init__(self, transport_inst, debug = 0):
        ...
        if self._debug == 0:
            try:
                ...
                build_ref = "fwVersion=0.6.2 fwBuild=09/10/2021-17:43:37 fwHash=0x2368969B fwDirty=0 kmodVersion=0.6.2 kmodBuild=09/10/2021-17:43:37 kmodHash=0x2368969B kmodDirty=0 swVersion=0.6.2 swBuild=09/10/2021-17:43:37 swHash=0x2368969B swDirty=0"
                if build != build_ref:
    ...
    def _set_dac_offset_0(self, offset):
        ...
        ----------
        offset : float
            Offset as a fraction of fullscale in a range of -1.0 to 1.0.
    ...
    def _set_sequencer_channel_map(self, sequencer, channel_map):
        """
        Set channel map of the indexed sequencer. The channel map consists of list of associated output channels for the given sequencer index.
    ...
    def _get_sequencer_channel_map(self, sequencer):
        """
        Get channel map of the indexed sequencer. The channel map consists of list of associated output channels for the given sequencer index.
        If an invalid sequencer index is given or the channel map is not valid, an error is set in
        system error.
    ...
    def _get_awg_waveforms(self, sequencer):
        ...
        #SCPI call
        try:
            var0 = self._read_bin('SEQuencer{}:AWG:WLISt?'.format(sequencer))
    ...
    def _add_acq_weight(self, sequencer, name, size, is_integer):
        """
        Add new weight to weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if the weight causes the weight
        memory limit to be exceeded, an error is set in system error. The weight names 'all' and 'ALL' are reserved and adding those will also result in an
        error being set in system error.
    ...
    def _delete_acq_weight(self, sequencer, name):
        """
        Delete weight from weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing weight name is given,
        an error is set in system error. The weight names 'all' and 'ALL' are reserved and deleting those will cause a purge of the entire weight list.
    ...
    #---------------------------------------------------------------------------
    def _set_acq_weight_data(self, sequencer, name, waveform):
        """
        Set weight data of weight in weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing weight
        name is given, an error is set in system error. If the wavefrom size does not match the size specified while adding the weight or if the wavefrom
        samples are out-of-range, an error is set in the system error.
        ...
        #SCPI call
        try:
            self._write_bin('SEQuencer{}:ACQ:WLISt:WEIght:DATA "{}",'.format(sequencer, name), struct.pack('f'*len(waveform), *waveform))
        except Exception as err:
            self._check_error_queue(err)
        finally:
            self._check_error_queue()
    ...
    def _get_acq_weight_data(self, sequencer, name, start, size):
        """
        Get weight data of weight in weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing weight
        name is given, an error is set in system error. The start and size arguments can be used to return a section of the weight. If the start argument
        is not given, zero is used as the default start sample. If the size argument is not given, all samples starting from the start sample are returned.
    ...
    def _set_acq_weight_index(self, sequencer, name, index):
        """
        Set weight index of weight in weight list of indexed sequencer's acquisition path. The index is used by the sequencer Q1ASM program to refer to the weight.
        If an invalid sequencer or weight index is given or if a non-existing weight name is given, an error is set in system error.
    ...
    def _get_acq_weight_index(self, sequencer, name):
        """
        Get weight index of weight in weight list of indexed sequencer's acquisition path. The index is used by the sequencer Q1ASM program to refer to the weight.
        If an invalid sequencer index is given or if a non-existing weight name is given, an error is set in system error.
    ...
    def _get_acq_weight_length(self, sequencer, name):
        """
        Get length of weight in weight list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing weight name is given,
        an error is set in system error.
    ...
    def _get_acq_weight_name(self, sequencer, index):
        """
        Get name of weight in weight list of indexed sequencer's acquisition path based on its weight index. If an invalid sequencer index is given or if a non-existing
        weight index is given, an error is set in system error.
    ...
    def _get_acq_weights(self, sequencer):
        ...
        #SCPI call
        try:
            var0 = self._read_bin('SEQuencer{}:ACQ:WLISt?'.format(sequencer))
    ...
    def _delete_acq_acquisition(self, sequencer, name):
        """
        Delete acquisition from acquisition list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing acquisition name is given,
        an error is set in system error. The acquisition names 'all' and 'ALL' are reserved and deleting those will cause a purge of the entire acquisition list.
    ...
    def _set_acq_acquisition_data(self, sequencer, name):
        """
        Move scope mode (raw) acquisition data into acquisition in acquisition list of indexed sequencer's acquisition path. If an invalid sequencer index is given or
    ...
    def _get_acq_acquisition_data(self, sequencer, name):
        """
        Get acquisition data of acquisition in acquisition list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing acquisition
        name is given, an error is set in system error.
        ...
        ----------
        list
            Binary structure containing the acquistition.
        ...
        #SCPI call
        try:
            var0 = self._read_bin('SEQuencer{}:ACQ:ALISt:ACQuisition:DATA? "{}"'.format(sequencer, name))
    ...
    def _set_acq_acquisition_index(self, sequencer, name, index):
        """
        Set acquisition index of acquisition in acquisition list of indexed sequencer's acquisition path. The index is used by the sequencer Q1ASM program to refer to the acquisition.
        If an invalid sequencer or acquisition index is given or if a non-existing acquisition name is given, an error is set in system error.
    ...
    def _get_acq_acquisition_index(self, sequencer, name):
        """
        Get acquisition index of acquisition in acquisition list of indexed sequencer's acquisition path. The index is used by the sequencer Q1ASM program to refer to the acquisition.
        If an invalid sequencer index is given or if a non-existing acquisition name is given, an error is set in system error.
    ...
    def _get_acq_acquisition_num_bins(self, sequencer, name):
        """
        Get number of bins in acquisition in acquisition list of indexed sequencer's acquisition path. If an invalid sequencer index is given or if a non-existing acquisition name is given,
        an error is set in system error.
    ...
    def _get_acq_num_acquisitions(self, sequencer):
        """
        Get name of acquisition in acquisition list of indexed sequencer's acquisition path based on its acquisition index. If an invalid sequencer index is given or if a non-existing
        acquisition index is given, an error is set in system error.
    ...
    def _get_acq_acquisitions(self, sequencer):
        ...
        #SCPI call
        try:
            var0 = self._read_bin('SEQuencer{}:ACQ:ALISt?'.format(sequencer))
