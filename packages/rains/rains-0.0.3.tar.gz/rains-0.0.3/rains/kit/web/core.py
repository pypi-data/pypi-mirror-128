...
from rains.kit.web.const import BrowserType
...
class WebCore(ICore):
    ...
    * 另外，它代理着一个 WebEngine 引擎对象，并在工作流中管理其生命周期。
    ...
    def _running_task_code(self, task_instruction: TaskInstruction, log: bool):
        ...
        try:
            ...
            db_task_id = self._db.write(Sql.tasks_add({
                ...
                ConstDatabaseTaskNaming.EXECUTE_DATE: time.strftime("%Y-%m-%d")
            }))
    ...
    def _running_case(self, case_id, task_class, case_function_object) -> bool:
        ...
        case_db_dict[ConstDatabaseCaseNaming.START_TIME] = time.strftime("%H:%M:%S")
        ...
        # 如果用例执行过程发生异常则判断用例失败
            ...
            case_db_dict[ConstDatabaseCaseNaming.RUN_STEP] = self._case_recorder.read()
