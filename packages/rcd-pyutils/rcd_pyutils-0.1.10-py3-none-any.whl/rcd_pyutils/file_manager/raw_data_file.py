...
from ..database_manager.mysql_operator import MysqlOperator
from ..database_manager import download_from_gcloud, upload_to_gcloud
...
class RawDataFile(ABC):
    ...
    def check_source_uuid(cls) -> None:
        ...
        mo = MysqlOperator()
        df_source = pd.read_sql_query("SELECT * FROM oip.sources", mo.conn)
    ...
    def sync_gcloud(self) -> None:
        ...
        upload_to_gcloud(local_folder_path=self.file_path, uuid=self.source_uuid)
        download_from_gcloud(local_folder_path=self.file_path, uuid=self.source_uuid)
