...
def unzip_file(zip_path, dst_path, clean_zip_file=False):
    """Unzip a file
    Args:
        zip_path (str): Zip file.
...
def _extract_movielens(zip_path, clean_zip_file=False):
    root_path = os.path.dirname(zip_path)
    unzip_file(zip_path, root_path, clean_zip_file)
    dst_path = os.path.join(root_path, "ml-1m")
    return dst_path
...
def load_seq_movielens(file_path):
    ...
    with open(train_path, 'w') as f1, open(val_path, 'w') as f2, open(test_path, 'w') as f3:
        for user, hist in history.items():
            ...
            f3.write(str(user) + "\t" + ' '.join(hist[:-1]) + "\t" + hist[-1] + '\n')
            f2.write(str(user) + "\t" + ' '.join(hist[:-2]) + "\t" + hist[-2] + '\n')
            f1.write(str(user) + "\t" + ' '.join(hist[:-3]) + '\n')
    return max(users) + 1, max(items) + 1, train_path, val_path, test_path


def load_zip_seq_movielens(zip_path, clean_zip_file=False):
    dst_path = _extract_movielens(zip_path, clean_zip_file)
    file_path = os.path.join(dst_path, "rating.dat")
    return load_seq_movielens(file_path)
...
def create_ml_1m_dataset(file, trans_score=1, seq_len=40, test_neg_num=100):
    """
