...
class ClaimLineFocus:
    ...
    def _procedure_code_fallback(self) -> str:
        ...
        try:
            ...
            if n < 1:
                raise ClaimError('Field procedureCode was not found for this claim line.')
            if n > 1:
                raise ClaimError('Too many possible values for procedureCode were found found for this claim line')
    ...
    def line_charge_amt(self) -> Decimal:
        self._fields_looked_at.add('line_charge_amt')
        try:
            value = cast(
                Money,
        ...
        raise ClaimError("Field lineChargeAmt was not found on this claim line.")
    ...
    def program_indicator(self) -> str:
        ...
        try:
            ...
            n = len(indicators)
            if n > 1:
                raise ClaimError(f'Too many candidates for field programIndicator.')
            if n == 1:
                return next(iter(indicators))
        except (AttributeError, TypeError):
            pass

        raise ClaimError(f'Field programIndicator was not found on this claim line.')
    ...
    def emergency_indicator(self) -> str:
        ...
        raise ClaimError('Field emergencyInd not found on this claim line.')
    ...
    def _rendering_practitioner_role(self) -> PractitionerRole:
        ...
        if not hasattr(self.claim_line, 'careTeamSequence') or not \
            ...
            raise ClaimError('No rendering provider was found in this claim line.')
        for i, index in enumerate(self.claim_line.careTeamSequence):
            ...
            try:
                ...
                msg = f'Malformed claim: claim.item[{seq}].careTeamSequence[{i}] is {index + 1}, ' \
            ...
            if practitioner_role is not None:
                raise ClaimError('Too many rendering providers for this claim line.')
    ...
    def rend_prov_npi(self) -> str:
        ...
        try:
            ...
            try:
                ...
                raise ClaimError(f'Resource with id {ref} not found in claim.contained.')

            if provider.resource_type != 'Practitioner':
                raise ClaimError(f'Resource with id {ref} was expected to be a Practitioner.')
            ...
            for identifier in cast(List[Identifier], provider.identifier):
                try:
                    ...
                    if npi_number is not None:
                        raise ClaimError('Too many candidate values for rendProvNPI in this claim line.')
            ...
            if npi_number is None:
                raise ClaimError('Field rendProvNPI was not found in this claim line.')
    ...
    def rend_prov_taxonomy(self) -> str:
        ...
        try:
            ...
            raise ClaimError(f'Field rendProvTaxonomy was not found in this claim line.')
