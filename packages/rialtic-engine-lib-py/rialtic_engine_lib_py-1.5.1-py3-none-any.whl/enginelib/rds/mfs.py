...
def mfs_field_for(field: str, clf: ClaimLineFocus) -> str:
...
def global_days(clf: ClaimLineFocus) -> str:
    return mfs_field_for('GLOBDAYS', clf)


def pctc_indicator(clf: ClaimLineFocus) -> str:
    return mfs_field_for('PCTCIND', clf)


def facility_total(clf: ClaimLineFocus) -> float:
    ft = mfs_field_for('FACILITYTOTAL', clf)
...
def mult_proc(clf: ClaimLineFocus) -> str:
    return mfs_field_for('MULTPROC', clf)


def status_code(clf: ClaimLineFocus) -> str:
    return mfs_field_for('STATUSCODE', clf).strip().upper()
