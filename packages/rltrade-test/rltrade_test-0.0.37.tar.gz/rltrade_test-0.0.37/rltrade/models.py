...
from rltrade.environments import SmartPortfolioEnv
from stable_baselines3.common.vec_env import DummyVecEnv
from rltrade.ibkr import api_connect, buy_stock,sell_stock
...
class SmartDRLAgent:
    ...
    def get_trade_actions(self,trade_period):
        ...
        self.data = YahooDownloader(start_date=trade_period[0],
    ...
    def make_trade(self,actions,demo=True):
        self.prev_actions = self.ext_data['prev_actions'].tolist()
        self.stock_number = self.ext_data['stock_number'].tolist()
        print(self.price_today)
        app = api_connect(demo=demo)
        trade_log = pd.DataFrame({"tic":[],"action":[],"quantity":[]})
        for i,tic in enumerate(actions.index):
            num_stock = (self.env_kwargs['initial_amount'] * actions[i].item())/self.price_today[i]
            quantity = int(num_stock-self.stock_number[i])
            if quantity > 0:
                print("buying {} stocks of {}".format(quantity,tic))
                trade_log.loc[len(trade_log.index)] = [tic,"buy",quantity]
                buy_stock(app,tic,quantity)
                self.stock_number[i] += quantity
            elif quantity < 0:
                sell_quantity = min(self.stock_number[i],abs(quantity))
                print("selling {} stocks of {}".format(sell_quantity,tic))
                trade_log.loc[len(trade_log.index)]  = [tic,"sell",sell_quantity]
                sell_stock(app,tic,sell_quantity)
                self.stock_number[i] -= sell_quantity
            else:
                print("Holding ticker {}".format(tic))
                trade_log.loc[len(trade_log.index)]  = [tic,"hold","-"]
            self.prev_actions[i] = actions[i]
        self.ext_data['prev_action'] = self.prev_actions
        self.ext_data['prev_action'] = self.prev_actions
        print("Trade Log")
        print(trade_log)
        app.disconnect()
    ...
    def save_model(self,path:str):
        ...
        ext_data = pd.DataFrame({'ticker_list':self.ticker_list,
                                'prev_actions':self.prev_actions,
                                'stock_number':self.stock_number})
        ext_data.to_csv(path+'/ext_data.csv')
    ...
    def train_model(self):
        ...
        self.model_kwargs['n_steps'] = self.train.index.nunique()

        print("Setting up environment...")
        self.env_kwargs['ticker_list'] = self.ticker_list
        environment = SmartPortfolioEnv(df=self.train,**self.env_kwargs)
        model = self.get_model(environment)
        model = model.learn(total_timesteps=self.total_timesteps,tb_log_name=self.tb_log_name)
    ...
    def train_model_filter(self):
        print("Ticker list is",self.ticker_list)
        self.train_model()
