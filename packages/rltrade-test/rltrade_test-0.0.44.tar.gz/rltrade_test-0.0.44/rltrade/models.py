...
class SmartDRLAgent:
    ...
    def make_trade(self,actions,accountid,demo=True):
        ...
        for i,tic in enumerate(ticker_list):
            ...
            quantity =  min(stock_data[tic][1],abs(quantity)) if quantity < 0 else quantity
            if quantity > 0:
                print("buying {} stocks of {}".format(quantity,tic))
                trade_log.loc[len(trade_log.index)] = [tic,"buy",quantity]
                buy_stock(app,tic,quantity)
            elif quantity < 0:
                print("selling {} stocks of {}".format(quantity,tic))
                trade_log.loc[len(trade_log.index)]  = [tic,"sell",quantity]
                sell_stock(app,tic,quantity)
            else:
