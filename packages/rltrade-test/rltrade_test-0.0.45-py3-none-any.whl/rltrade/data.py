...
class FeatureEngineer:
    ...
    def remove_corrupt_ticker(self,df:pd.DataFrame):
        ...
        df = df.reset_index(drop=True)
        print("Deleted tickers",invalid_ticker)
...
class DatTradeFeatureEngineer:

    def __init__(self,stock_indicator_list = [],additional_indicators=[]):
        self.additional_indicators = additional_indicators
        self.stock_indicator_list = stock_indicator_list
    ...
    def create_data(self,df):
        ...
        df.loc[:,self.additional_indicators] = df[self.additional_indicators].replace([np.inf, -np.inf], np.nan)
        df = df.fillna(method="ffill").fillna(method="bfill")
        df = df.sort_values(["tic","tick_no","time"], ignore_index=True)
        df = df.reset_index(drop=True)
        return df
    
    def time_series_split(self,df, start, end, target_date_col="time"):
        df = df.copy()
    ...
    def add_hurst_exponent(self,data,max_lag=20):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp['hurst_exp'] = temp['price'].rolling(max_lag*2).apply(lambda x:self.get_hurst_exponent(x.values))
            indicator_df = indicator_df.append(temp, ignore_index=True )
        df = df.merge(indicator_df[["tick_no","tic", "time", 'hurst_exp']], on=["tick_no","tic", "time"], how="left")
        return df
    ...
    def add_sharpe(self,data,n):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp['daily_return'] = temp['price'].pct_change(1)
            temp['daily_return'].fillna(0,inplace=True)
            temp[f'sharpe_{n}'] = temp['daily_return'].rolling(n,min_periods=1).mean() / temp['daily_return'].rolling(n,min_periods=1).std()
            indicator_df = indicator_df.append(temp, ignore_index=True )
        df = df.merge(indicator_df[["tick_no","tic", "time", f'sharpe_{n}']], on=["tick_no","tic", "time"], how="left")
        return df
    ...
    def add_sortino(self,data,n):
        ...
        for ticker in unique_ticker:
            ...
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(indicator_df[["tick_no","tic", "time", f'sortino_{n}']], on=["tick_no","tic", "time"], how="left")
        return df
    ...
    def add_clamar(self,data,n):
        ...
        for ticker in unique_ticker:
            ...
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(indicator_df[["tick_no","tic", "time", f'calamar_{n}']], on=["tick_no","tic", "time"], how="left")
        return df
    ...
    def add_vix_fix(self,data,n):
        ...
        for ticker in unique_ticker:
            temp = df[(df['tic'] == ticker)].copy()
            temp[f'vix_fix_{n}'] = ((temp['price'].rolling(n,min_periods=1).max() \
                                         - temp['price'].rolling(n,min_periods=1).max())/temp['price'].rolling(n,min_periods=1).max()) * 100
            indicator_df = indicator_df.append(temp, ignore_index=True)
        df = df.merge(indicator_df[["tick_no","tic", "time", f'vix_fix_{n}']], on=["tick_no","tic", "time"], how="left")
        return df
