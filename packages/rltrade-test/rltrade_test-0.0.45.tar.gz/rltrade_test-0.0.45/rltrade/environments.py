    ...
    def step(self,action):
        ...
        if self.terminal:
            ...
            if action < 0: 
                ...
                if self.state[2]==0:
                    self.state = ([self.data['price']]
                            + [last_tick_memory['price'],action] + self.data[self.tech_indicator_list].tolist()) 
                ...
                if self.state[2]>0:
                    ...
                    self.reward = (last_tick_memory['price']-self.state[1]) * self.state[2]
                    self.state = ([self.data['price']]
                            + [last_tick_memory['price'],action] + self.data[self.tech_indicator_list].tolist()) 
                ...
                if self.state[2]>0:
                    ...
                    #get reward for prev short
                    self.reward = (self.state[1] - self.state[0])

            #action is 0
            else:
                #change to current price
                self.state[0] = self.data['price']
                #reward for current long/short 
                self.reward = (self.state[1] - self.state[0]) * self.state[2]
