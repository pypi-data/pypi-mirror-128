...
class ruinews():
    ...
    def mysql_conn(self,ip,user,pwd,db_name):
        self.db = pymysql.connect(ip, user, pwd, db_name)
        self.cursor = self.db.cursor()
        return self.db,self.cursor

    def insert_db(self,product_info,tb_name):
        values = ', '.join(['%s'] * len(product_info))
        fields = product_info.keys()
        try:
            qry = 'Insert Into '+ tb_name +' (%s) Values (%s);' % (','.join(fields), values)

            self.cursor.execute(qry, [product_info[x] for x in fields])
            self.db.commit()
    ...
    def get_request(self,url,hds,charset='utf-8'):

        r=requests.get(url,headers=hds,timeout=self.TIMEOUT)
        text=r.content.decode(charset)
        r.close()
        return etree.HTML(text)

    def get_api_content(self,url,hds,charset='utf-8'):
        ...
        return html
    def post_request(self,url,data,hds,charset='utf-8'):
        r=requests.post(url,headers=hds,data=data)
        text=r.content.decode(charset)
        r.close()
        html=json.loads(text)
        return html
    def get_url_html(self,url,charset='utf-8'):
        s = requests.session()
        s.keep_alive = False
        s.proxies = None
        headers = {
    ...
    def element_convert(self,s):
        ...
        return s
    def stamp_convent(self,timeStamp):
        """
        时间戳转换格式
        """
    ...
    def download_file(self,_dnurl,_filename):
        """
        下载文件,filename全路径
        """
    ...
    def trans_word(self,text,from_lan,to_lan):
    ...
    @staticmethod
    def pro_dict_art(self,art_content,art_title):
    ...
    def pic_process(self,art_content, art_url):
        ...
        for url in pic_url:
            ...
            try:
                ...
                r = requests.post(self.pic_api, data=m.read(), headers=headers)
    ...
    def get_sups(self,authors, au_split):
        sups = []
    ...
    def str_between(self,s,s_front,s_end):
        s_list=[]
    ...
    def get_string(self,s, s_split, s_front, s_end,is_split=0):
        sups = []
    ...
    def transApi(self,q):
        ...
        art_date=translator.translate(q)
        return art_date
    # def transAPI(self,q):
    ...
    def transAPI_lan(self,q,from_lan,to_lan):
        appid = '20210125000680783'
        secretKey = 'kBG1C0TMC9rBwYwpsNBY'
        httpClient = None
        myurl = '/api/trans/vip/translate'
