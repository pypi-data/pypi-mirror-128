...
class ruinewsV2():
    ...
    def filter_json_url(self,news_ls):
        ...
        if os.path.exists(csv):
            df_csv = pd.read_csv(csv).tail(200)
        else:
        ...
        if len(csv_ls) > 0:
            ...
            return
    def get_news_list(self,url,links_xpath):
        #get list方式 返回列表
        ...
        if len(csv_ls) > 0:
            ...
            return
    def get_news_detail(self,url,content_xpath='',
                title_xpath='',
                author_xpath='',
                source_xpath='',
                publish_time_xpath=''):
        ...
        if not url.startswith('http'):
            domain = self.domain_url[0:self.domain_url.find('/', self.domain_url.find('://') + 3)]
            if url.startswith('/'):
                url = domain + url
            else:
                url = domain + '/' + url
        ...
        art_html = requests.get(url, headers=self.hd, verify=False).content.decode(self.web_code)
        rst = art_ex.extract(art_html, title_xpath=title_xpath,
                             body_xpath=content_xpath, with_body_html=True)
        ...
        catalogId = self.CATALOG['catalogName']
        ...
        country = rtn_dict[0]
        inds = rtn_dict[1]
        art_content = pic_process(art_content, art_url)
