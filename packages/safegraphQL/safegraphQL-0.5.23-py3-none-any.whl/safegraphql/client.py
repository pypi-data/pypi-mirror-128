...
class HTTP_Client:
    ...
    async def main_runner(self, query, timeout=200): # on default waits 2 mins before timeout error 
        self.ct+=1
        transport = AIOHTTPTransport(url=self.url, headers=self.headers, timeout=timeout)
 ...
 search(first: {first} after: {after+after_result_number} filter: {{{params}   
        ...
        {dataset}
                ...
                try:
                    ...
                    if type(e) == TypeError:
                        # print("result","result", result, e, e == "'TimeoutError' object is not subscriptable")
                        if e == "'TimeoutError' object is not subscriptable":
                            print(408, "Timeout on request")
                            continue
                        # try:
                        if result.code == 408:
                            print(result.code, "Timeout on request")
                            continue
                        if result.code == 429:
                            print(result.code, "Too many requests")
                        continue
                        # except AttributeError:
                        #     print("AttributeError", e, result)
