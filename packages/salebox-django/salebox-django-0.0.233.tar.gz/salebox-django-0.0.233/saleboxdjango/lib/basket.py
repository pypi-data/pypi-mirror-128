...
class SaleboxBasket:
            ...
            try:
                if request.COOKIES["psessionid"] != key:
                    self._migrate_anonymous_basket(
                        request.COOKIES["psessionid"], request.user
                    )
                    self.cookie = "remove"
                    self.data = None
            except:
    ...
    def get_data(self, request, results, variant_id=None):
        ...
        if "basket_html_button" in results and variant_id:
            ...
            try:
                stock_total = self.data["basket"]["lookup"][str(variant_id)]["variant"][
                    "stock_total"
                ]
            except:
            ...
            try:
                preorder_flag = self.data["basket"]["lookup"][str(variant_id)][
                    "variant"
                ]["preorder_flag"]
            except:
                preorder_flag = False
        ...
        if "wishlist_html_button" in results and variant_id:
            ...
            o["wishlist_html_button"] = render_to_string(
                ...
                {
                    "pv": {"id": variant_id, "in_wishlist": in_wishlist},
                    "request": request,
                },
                request,
            )
    ...
    def migrate_basket_wishlist(self, request, variant, to_basket):
        ...
        bwl = self._filter_basket_queryset(
            request,
            BasketWishlist.objects.filter(variant=variant).exclude(
                basket_flag=to_basket
            ),
        )
    ...
    def reset_basket(self, request):
        ...
        self._filter_basket_queryset(
            request, BasketWishlist.objects.filter(basket_flag=True)
        ).delete()
        self._init_basket(request)
    ...
    def _calculate_price(self, request):
        ...
        for i in self.data["basket"]["items"]:
            ...
            if i["variant"]["preorder_flag"]:
                self.data["basket"]["orig_price_preorder"] += (
                    i["variant"]["price"] * i["qty"]
                )
                self.data["basket"]["sale_price_preorder"] += (
                    i["variant"]["sale_price"] * i["qty"]
                )
            else:
                self.data["basket"]["orig_price_onshelf"] += (
                    i["variant"]["price"] * i["qty"]
                )
                self.data["basket"]["sale_price_onshelf"] += (
                    i["variant"]["sale_price"] * i["qty"]
                )

        self.data["basket"]["saving"] = (
            self.data["basket"]["orig_price"] - self.data["basket"]["sale_price"]
        )
    ...
    def _filter_basket_queryset(self, request, qs):
        ...
        if request.user.is_authenticated:
            return qs.filter(user=request.user).filter(session__isnull=True)
        else:
            return qs.filter(user__isnull=True).filter(
    ...
    def _get_raw_basket(self, request):
        while True:
            ...
            for b in basket:
                if (
                    not b.variant.preorder_flag
                    and b.variant.product.inventory_type == "T"
                ):
            ...
            # get items from db
            qs = self._filter_basket_queryset(request, BasketWishlist.objects.all())
    ...
    def _get_variants(self, request, variant):
        ...
        basket = self._filter_basket_queryset(
            request,
            BasketWishlist.objects.filter(basket_flag=True).filter(variant=variant),
        )

        wishlist = self._filter_basket_queryset(
            request,
            BasketWishlist.objects.filter(basket_flag=False).filter(variant=variant),
        )
    ...
    def _init_basket_attribute(self, attribute_item):
        items = attribute_item.all()
        output = []

        for item in items:
            output.append(
                {
                    "attribute": item.attribute.code if item.attribute else None,
                    "id": item.id,
                    "slug": item.slug,
                    "value": item.value,
                }
    ...
    def _init_basket(self, request):
        ...
        for q in qs:
            pv = {
                "attribute_1": self._init_basket_attribute(q.variant.attribute_1),
                "attribute_2": self._init_basket_attribute(q.variant.attribute_2),
                "attribute_3": self._init_basket_attribute(q.variant.attribute_3),
                "attribute_4": self._init_basket_attribute(q.variant.attribute_4),
                "attribute_5": self._init_basket_attribute(q.variant.attribute_5),
                "attribute_6": self._init_basket_attribute(q.variant.attribute_6),
                "attribute_7": self._init_basket_attribute(q.variant.attribute_7),
                "attribute_8": self._init_basket_attribute(q.variant.attribute_8),
                "attribute_9": self._init_basket_attribute(q.variant.attribute_9),
                "attribute_10": self._init_basket_attribute(q.variant.attribute_10),
                "boolean_1": q.variant.boolean_1,
                "boolean_2": q.variant.boolean_2,
                "boolean_3": q.variant.boolean_3,
                "boolean_4": q.variant.boolean_4,
                "boolean_5": q.variant.boolean_5,
                "date_1": q.variant.date_1,
                "date_2": q.variant.date_2,
                ...
                "product": {
                    "attribute_1": self._init_basket_attribute(
                        q.variant.product.attribute_1
                    ),
                    "attribute_2": self._init_basket_attribute(
                        q.variant.product.attribute_2
                    ),
                    "attribute_3": self._init_basket_attribute(
                        q.variant.product.attribute_3
                    ),
                    "attribute_4": self._init_basket_attribute(
                        q.variant.product.attribute_4
                    ),
                    "attribute_5": self._init_basket_attribute(
                        q.variant.product.attribute_5
                    ),
                    "attribute_6": self._init_basket_attribute(
                        q.variant.product.attribute_6
                    ),
                    "attribute_7": self._init_basket_attribute(
                        q.variant.product.attribute_7
                    ),
                    "attribute_8": self._init_basket_attribute(
                        q.variant.product.attribute_8
                    ),
                    "attribute_9": self._init_basket_attribute(
                        q.variant.product.attribute_9
                    ),
                    "attribute_10": self._init_basket_attribute(
                        q.variant.product.attribute_10
                    ),
                    "category": {
        ...
        for i in self.data["wishlist"]["order"]:
            self.data["wishlist"]["items"].append(
                self.data["wishlist"]["lookup"][str(i)]
            )
    ...
    def _migrate_anonymous_basket(self, key, user):
        ...
        basket = BasketWishlist.objects.filter(user__isnull=True).filter(session=key)
        for b in basket:
            b.user = user
            b.session = None
            b.save()
