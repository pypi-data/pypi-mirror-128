...
class GetConnectorAsync:
    ...
    def get_data(self, connector: Union[str, List], fields=None, values=None, operators=None, orderbyfields=None, batch_size: int = 8, take: int = 40000) -> dict:
        ...
        :param fields: dict with per connectorobject that needs to be filtered, string of filters. each item is one filter. example: 'naam,woonplaats'
        ...
        if fields is not None:
            parameters = {**{connector: {"filterfieldids": fields[connector], "filtervalues": values[connector], "operatortypes": operators[connector]} for connector in fields.keys()}}
        else:
            parameters = {}

        if orderbyfields is not None:
            for orderbyfields_conn in orderbyfields.keys():
                parameters[orderbyfields_conn]["orderbyfieldids"] = f"-{orderbyfields[orderbyfields_conn]}"
...
class UpdateConnector:
    ...
    def update_applicant(self, data: dict, method: str, overload_fields={}):
        """
        :param data: Fields that are allowed are listed in allowed fields array. Update this whenever necessary
        ...
        for field in data.keys():
            if field not in all_fields:
                raise ValueError(f'Field {field} is not allowed. Allowed fields are: {tuple(all_fields)}')
        ...
        if method == 'DELETE':
            ...
            fields_to_update.update({"Ad": data['street']}) if 'street' in data else fields_to_update
            fields_to_update.update({"HmNr": data['housenumber']}) if 'house_number' in data else fields_to_update
