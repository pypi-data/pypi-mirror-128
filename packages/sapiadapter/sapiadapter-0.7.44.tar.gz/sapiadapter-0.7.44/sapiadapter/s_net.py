...
def check_connectivity( ) :
    ...
    if( flag_networkok and get_request_response( {"X-Omni-Action": "mwget" } ) == False ) :
...
def get_request_response( request_headers = { }  , payload = "" ) :
    ...
    # Careful - don't want infinite recursion here!
    if( "X-Omni-Action" in request_headers ) and ( request_headers[ "X-Omni-Action" ] != "mwget" ) :
        if not "X-Omni-Mw" in request_headers :
            mw = mw_get( )
            if( not mw ) : return( False )
            request_headers[ "X-Omni-Mw" ] = mw
