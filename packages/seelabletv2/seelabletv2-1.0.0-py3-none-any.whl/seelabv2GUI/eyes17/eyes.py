	...
	* loads calibration values.
	...
	def __runInitSequence__(self,**kwargs):
		...
		if self.H.connected:
			...
			self.load_equation('W1','sine')
			self.load_equation('W2','sine')
			self.hexid=hex(self.device_id())
		
		self.DAC = PWMDAC(self.H,3.3,0)
		print('dac ready', self.DAC)
...
#-------------------- Passive Time Interval Measurements ----------------------------------
	...
	def set_sine(self,freq, chan='W1'):
		'''
		helper function for forward compatibility with expeyes17. defaults to sine1
		'''
		if chan=='W1':
			return self.set_w1(freq)
		elif chan=='W2':
			return self.set_w2(freq)

	def set_wave(self,freq, waveType=None, chan='W1'):
		"""
		...
		**Arguments** 
		...
		if chan=='W1':
			return self.set_w1(freq,waveType)
		elif chan=='W2':
			return self.set_w2(freq,waveType)
		else: return 0
	...
	def set_sqr1(self,freq,duty_cycle=0.5,onlyPrepare=False):
		if freq== -1: #Turn off SQR1
			self.set_state(SQR1=0)
			return
		self.sqr1(freq,duty_cycle,onlyPrepare)

	def set_sqr2(self,freq,duty_cycle=0.5):
		if freq== -1: #Turn off SQR1
			self.set_state(SQR2=0)
			return
		self.sqr2(freq,duty_cycle=0.5)
