...
class Expt(QtWidgets.QWidget, ui_scope_layout.Ui_Form):
	...
	def update(self):
		...
		try:
			s = self.p.get_states()
			self.ID1.setChecked(s['ID1'])
			self.ID2.setChecked(s['ID2'])
			self.ID3.setChecked(s['ID3'])
			self.ID4.setChecked(s['ID4'])
		except Exception as e:
			pass
	...
	def set_wave(self):
		if self.waveindex <= 1:
			res = self.p.set_wave(self.AWGval, self.Waves[self.waveindex],'W1')
			ss = '%6.2f'%res
	...
	def set_phase(self,phase):
		res = self.p.set_waves(self.AWGval,phase,self.AWG2val)
		ss = '%6.2f'%res
		self.msg('Freq1: %.2f , Phase %d °, Freq2: %.2f '%(res,phase,self.AWG2val))

	def set_wave2(self):
		try:
			res = self.p.set_wave(self.AWG2val,'sine','W2')
			ss = '%6.2f'%res
			self.msg(self.tr('AWG2 set to ') + ss + self.tr(' Hz'))
		except Exception as e:
			print('what',e)
			self.comerr()
	...
	def awg2_text(self):
		text = self.AWG2text.value()
		try:
			val = float(text)
			if self.AWGmin <= val <= self.AWGmax:
				self.AWG2val = val
				self.AWG2slider.setValue(self.AWG2val)
				self.set_wave2()
		except:
			return

	def awg2_slider(self, val):
		if self.AWGmin <= val <= self.AWGmax:
			self.AWG2val = val
			self.AWG2text.setValue(val)
			self.set_wave2()


	def dout_changed(self):
		try:
			self.p.set_state(SQR1 = self.SQR1.isChecked(),SQR2=self.SQR2.isChecked(),SQR3=self.SQR3.isChecked(),SQR4=self.SQR4.isChecked())
		except:
			self.comerr()
