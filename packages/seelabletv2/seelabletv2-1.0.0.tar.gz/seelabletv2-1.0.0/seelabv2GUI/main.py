...
class MainWindow(QMainWindow):
	...
	def scope_help(self,e):
		...
		if self.expName != 'scope':
			explib = importlib.import_module('.scope',package='seelabv2GUI')
			try:
	...
	def callExpt(self, e):
		...
		explib = importlib.import_module('.'+module_name,package='seelabv2GUI')
		try:
			if self.expWidget != None:
				self.expWidget.timer.stop()	 # Stop the timer loop of current widget			
			self.hwin = None
	...
	def screenshot(self, tmpFileName=None):
		...
		if path or tmpFileName:
			...
			d=datetime.now()
			generator.setTitle(d.strftime("SEELab Screenshot--%Y-%m-%d--%H-%M"))
	...
	def screenshotPlot(self, tmpFileName=None):
		...
		if path or tmpFileName:
			...
			if path[-4:] == '.png':
				...
				d=datetime.now()
				generator.setTitle(d.strftime("SEELab Smaller Screenshot--%Y-%m-%d--%H-%M"))
				generator.setDescription(self.title)
				p = QtGui.QPainter()
				p.begin(generator)
				self.render(p)
