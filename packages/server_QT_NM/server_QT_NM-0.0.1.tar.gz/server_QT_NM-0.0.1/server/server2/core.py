...
from server.common.descrptrs import Port, Host
...
class MessageProcessor(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                logger.info('Установлено соедение с ПК %s', client_address)
                client.settimeout(5)
            ...
            try:
                ...
                logger.error('Ошибка работы с сокетами: %s', err.errno)
    ...
    def remove_client(self, client):
        ...
        '''
        logger.info('Клиент %s ', client.getpeername(), 'отключился от сервера.')
    ...
    def init_socket(self):
        ...
        logger.info(
            'Запущен сервер, порт для подключений: %s', self.port,
            'адрес с которого принимаются подключения: %s', self.addr,
    ...
    def process_message(self, message):
        ...
        if message[DESTINATION] in self.names and self.names[message[DESTINATION]
            ...
            try:
                ...
                logger.info(
                    'Отправлено сообщение пользователю %s', message[DESTINATION], 'от пользователя',
                    message[SENDER])
            except OSError:
            ...
            logger.error(
                'Связь с клиентом %s', message[DESTINATION],
                'была потеряна. Соединение закрыто, доставка невозможна.')
            self.remove_client(self.names[message[DESTINATION]])
        else:
            logger.error(
                'Пользователь', message[DESTINATION],
                'не зарегистрирован на сервере, отправка сообщения невозможна.')

    @login_required
    def process_client_message(self, message, client):
        '''Метод отбработчик поступающих сообщений.'''
        logger.debug('Разбор сообщения от клиента : %s', message)
    ...
    def autorize_user(self, message, sock):
        ...
        logger.debug('Start auth process for %s', message[USER])
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            response = RESPONSE_400
            response[ERROR] = 'Имя пользователя уже занято.'
            try:
                logger.debug('Username busy, sending', response)
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            try:
                logger.debug('Unknown username, sending', response)
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            _hash = hmac.new(
                ...
                'MD5')
            digest = _hash.digest()
            logger.debug('Auth message %s', message_auth)
            try:
                # Обмен с клиентом
