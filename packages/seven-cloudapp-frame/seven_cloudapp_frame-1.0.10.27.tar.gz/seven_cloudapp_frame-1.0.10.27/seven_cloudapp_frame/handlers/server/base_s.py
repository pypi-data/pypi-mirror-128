...
@LastEditTime: 2021-11-25 17:39:53
...
class PlaceInfoListHandler(ClientBaseHandler):
    """
    :description: 位置信息列表
    """
    def get_async(self):
        ...
        cms_info_model = CmsInfoModel(context=self)
        condition = "is_release=1"
        params = []
        if place_id > 0:
            condition += "place_id=%s"
            params.append(place_id)
        place_info_list = cms_info_model.get_cache_list(condition, params=params)
        return self.response_json_success(place_info_list)
