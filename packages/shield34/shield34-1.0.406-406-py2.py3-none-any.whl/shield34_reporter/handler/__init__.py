...
from shield34_reporter.dynamic_locator import Locator
from shield34_reporter.dynamic_locator.element_dynamic_locator import locate_element_by_element_descriptor, \
    web_element_found
...
class SeleniumMethodHandler(HandlerAPI):
    ...
    def __init__(self, action_name, action_csv_row):
        ...
        self.timeout = Shield34Properties.wait_timeout
        self.polling = 0.2
    ...
    @staticmethod
    def _add_web_element_descriptor(web_element, by, locator):
        try:
            web_element_found(web_element, by, locator)
            RunReportContainer.get_current_block_run_holder().web_element_descriptors_map[web_element.id] \
                = Locator(by, locator)
        ...
        try:
            ...
            while not success and self.retry_counter < self.retry_count and not failed_action:
                try:
                    ...
                    RunReportContainer.add_report_csv_row(
                        ExceptionCaughtCsvRow(exception=e, exception_type=ExceptionType.MAIN_ACTION))
    ...
    @staticmethod
    def _handle_element_exception(exception, driver, element, timeout, polling):
        try:
    ...
    @staticmethod
    def _handle_lookup_exception(exception, driver, by, locator):
        try:
            if isinstance(exception, InvalidSelectorException) or isinstance(exception, NoSuchElementException):
                # try dynamic locator
                web_element, current_by, current_value = locate_element_by_element_descriptor(driver, by, locator)
