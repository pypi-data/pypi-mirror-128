...
from shield34_reporter.dynamic_locator.element_dynamic_locator import locate_elements_by_element_descriptor, \
    locate_element_by_element_descriptor
...
class DriverFindElements(SeleniumMethodHandler):
    ...
    def action_pre_defenses(self):
        ...
        try:
            ...
            after = time.time()
            RunReportContainer.add_report_csv_row(WaitForNumberOfElementToBeMoreThanCsvRow(
                self.timeout, True, 0, (after-before)))
        except Exception as e:
            after = time.time()
            RunReportContainer.add_report_csv_row(WaitForNumberOfElementToBeMoreThanCsvRow(
                self.timeout, False, 0, (after-before)))
    ...
    def handle_unsuccessful_action(self):
        web_elements, current_by, current_value = locate_elements_by_element_descriptor(self.driver,
                                                                                        self.orig_by,
                                                                                        self.orig_locator)
...
class DriverFindElement(SeleniumMethodHandler):
    ...
    def on_successful_action(self):
        ...
        RunReportContainer.add_report_csv_row(PageHtml(DriverUtils.get_page_html()))
        self._add_web_element_descriptor(self.located_web_element, self.orig_by, self.orig_locator)
        RunReportContainer.get_current_block_run_holder().add_web_element(self.located_web_element.id,
                                                                          self.by + ": " + self.locator)

    def handle_unsuccessful_action(self):
        try:
            web_element, current_by, current_value = locate_element_by_element_descriptor(self.driver, self.orig_by, self.orig_locator)
...
class DriverQuit(SeleniumMethodHandler):
    ...
    def start_handle_hook(self):
        try:
            RunReportContainer.get_current_block_run_holder().screen_shot_file_before_quit = self.driver.get_screenshot_as_png()
        except:
            pass
