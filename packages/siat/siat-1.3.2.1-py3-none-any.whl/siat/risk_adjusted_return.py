...
def get_ff_factors(start,end,scope='US',factor='FF3',freq='daily'):
    ...
    #如果未找到匹配的模式，显示信息后返回
    if len(ss)==0:
        print("  #Warning(get_ff_factors): No data item available for",scope,factor,freq)
        return None
    ...
    try:
        ...
        print("  #Error(get_ff_factors): Server did not respond")        
        return None
    
    #提取希望的资产定价因子
    factor_df=ds[seq]
    if len(factor_df)==0:
        print("  #Error(get_ff_factors): Server returned empty data for",start,end,scope,factor,freq)
        return None    
...
def rar_ratio_rolling(portfolio,start,end,ratio_name='treynor', \
    ...
    #第3步：获得无风险收益率/市场收益率序列
    #获得期间的日无风险收益率(抓取的RF为百分比) 
    rf_df=get_rf(startdate1,enddate,scope=scope,freq='daily')  
    if rf_df is None:
        message="  #Warning("+func_name+"): "+"rf data not available for "+scope
        print(message)
        #以美国的rf数据替代
        rf_df=get_rf(startdate1,enddate,scope='US',freq='daily')
...
def draw_rar_ratio(rars,portfolio,ratio_name):
    ...
    import datetime as dt; today=dt.date.today() 
    footnote="数据来源：新浪财经/雅虎财经，"+str(today)
    plt.xlabel(footnote)
    plt.show()
...
def sharpe_ratio_portfolio(portfolio,start,end,printout=True):
    ...
    if round(sum(portionlist),1) != 1.0:
        print("  #Error(sharpe_ratio): Incorrect total of portions")
        return None,None
    """
    ...
    try:
        ...
        print("  #Error(sharpe_ratio): incorrect start/end date(s)")
        return None,None       
    ...
    try:
        ...
        print("  #Error(sharpe_ratio): Server did not respond")
        return None,None
    
    if len(price) == 0:
        print("  #Error(sharpe_ratio): Server returned empty data")
        return None,None
    ...
    try:
        ...
        print("  #Error(sharpe_ratio): RF server did not respond")
        return None,None        
    if len(rf_df) == 0:
        print("  #Error(sharpe_ratio): RF server returned empty data")
        return None,None
...
def sortino_ratio_portfolio(portfolio,start,end,printout=True):
    ...
    """
    if round(sum(portionlist),1) != 1.0:
        print("  #Error(sortino_ratio): Incorrect total of portions")
        return None,None
    """
    ...
    try:
        ...
        print("  #Error(sortino_ratio): Incorrect start/end date(s)")
        return None,None
    ...
    try:
        ...
        print("  #Error(sortino_ratio): Server did not respond")
        return None,None
    
    if len(price) == 0:
        print("  #Error(sortino_ratio): Server returned empty data")
        return None,None
    ...
    if rf_df is None:
        print("  #Error(sortino_ratio): RF server did not respond")
        return None,None        
    if len(rf_df) == 0:
...
def jensen_alpha_portfolio(portfolio,start,end,printout=True):
    ...
    """
    if round(sum(portionlist),1) != 1.0:
        print("  #Error(jensen_alpha): Incorrect total of portions.")
        return None,None
    """
    ...
    try:
        ...
        print("  #Error(jensen_alpha): Incorrect start/end date(s).")
        return None,None        
    ...
    try:
        ...
        print("  #Error(jensen_alpha): Server did not respond.")
        return None,None
    
    if len(price) == 0:
        print("  #Error(jensen_alpha): Server returned empty data.")
        return None,None
    ...
    if rf_df is None:
        print("  #Error(jensen_alpha): RF server did not respond.")
        return None,None        
    if len(rf_df) == 0:
        print("  #Error(jensen_alpha): RF server returned empty data.")
        return None,None 
    ...
    reg=reg.dropna()
    if len(reg) == 0:
        print("  #Error(jensen_alpha): Empty data for regression.")
        return None,None     
    ret_rf_mean=reg['Ret-RF'].mean()
...
def plot_rar_monthly(portfolio,start,end,rar_type):
    ...
    try:
        ...
        print("  #Error(plot_rar_monthly): Incorrect start/end date(s)")
        return None
    if startdate >= enddate:
        print("  #Error(plot_rar_monthly): start date should be earlier")
        return None        

    #检查3：支持的rar_type
    rar_list=['treynor_ratio','sharpe_ratio','sortino_ratio','jensen_alpha']
    if rar_type not in rar_list:
        print("  #Error(draw_rar_monthly): not supported rar type")
...
def plot_rar_annual(portfolio,start,end,rar_type):
    ...
    try:
        ...
        print("  #Error(plot_rar_annual): Incorrect start/end date(s)")
        return None
    if startdate >= enddate:
        print("  #Error(plot_rar_annual): start date should be earlier")
        return None        
