...
def get_shibor_rates(start,end):
    """
    功能：计算标的物价格的年化标准差
    underlying：标的证券代码
    date：当前日期
    days：历史期间长度，日历日天数，默认365天
    """
...
if __name__=='__main__': 
    rates=get_shibor_rates('2021-10-1','2021-11-19')   
    print(rates[-1:])
...
def shibor_rate(date,rate_period='ON',daysahead=30):
    """
    获取shibor指定日期的指定期间利率
    若无最新利率，则取最近日期的利率替代
    """    
    ...
    df=get_shibor_rates(start,date) 
    
    try:
        rate=float(df[-1:]['shibor'+rate_period].values[0])/100
    except:
        #使用最短期的隔夜利率
        rate=float(df[-1:]['shiborON'].values[0])/100
...
def treasury_yield_china(today,term='3M',daysahead=10):
    """
    功能：抓取指定日期和期限的国债收益率, treasury yield rate
    """
    ...
    import akshare as ak
    df = ak.bond_china_yield(start_date=start, end_date=today)
    
    df1=df[df['曲线名称']=='中债国债收益率曲线']
    
    collist=['curve','date','3M','6M','1Y','3Y','5Y','7Y','10Y','30Y']
    df1.columns=collist
    
    termlist=['date','3M','6M','1Y','3Y','5Y','7Y','10Y','30Y']
    df2=df1[termlist]
    
    df2.sort_values(by=['date'],ascending=['True'],inplace=True)
    
    try:
        rate=df2[-1:][term].values[0]
    except:
        #使用最短期的3个月利率
        rate=df2[-1:]['3M'].values[0]
        
    return rate/100
        
if __name__=='__main__':
    treasury_yield_china('2021-11-20',term='1Y',daysahead=7)  
...
def option_fin_pricing_china(option,end_month,contract,today,direction='call', \
                             sample_days=30,rate_type='shibor',rate_period='ON',daysahead=30,printout=True):
    """
    ...
    if ua is None:
        ...
        print("  Possible reasons: either")
        print("    Option not found for",option,'\b, or')
        print("    Maturity year-month not found for",end_month,'\b, or')
        print("    Contract not found for",contract,'as',direction.lower(),'\b, or')
    ...
    if rate_type=='SHIBOR':
        ...
        if rf is None:
            rf=shibor_rate(today,rate_period,daysahead*2)
            
    elif rate_type=='TREASURY':
        rf=treasury_yield_china(today,rate_period,daysahead)
        
    else:
        print("  #Warning(option_fin_pricing_china): invalid rate type",rate_type.lower())
        print("  Use shibor rate instead")
        rate_type='SHIBOR'
        rf=shibor_rate(today,rate_period)
    ...
    if printout:
        ...
        print("    距离到期:",days,'\b天')
        print("    标的样本历史期间:",sample_days,'\b天')
        print("    历史波动率:",round(sigma,5))
        
        print("    无风险利率种类:",rate_type.lower())
        print("    无风险利率("+rate_period+"):",round(rf*100,4),'\b%')
        
        print("*** 定价结果：")
        print("    定价模型: Black-Scholes")        
        print("    预期价格:",round(expected_price,5))
        
        import datetime; pgm_date = datetime.date.today()         
        print("\n来源: 新浪/沪深所/中金所,",pgm_date)        
...
if __name__=='__main__': 
    ...
    contract='510300C2206M04500'
    option_fin_pricing_china(option,end_month,contract,today='2021-11-19', \
                             direction='call', \
    ...
    option_fin_pricing_china(option,end_month,contract,today='2021-11-19', \
                             direction='call', \
                             sample_days=90,rate_type='treasury',rate_period='1Y') 
    ...
    option_fin_pricing_china(option,end_month,contract,today='2021-11-19', \
                             direction='put', \
                             sample_days=365,rate_type='treasury',rate_period='1Y')
