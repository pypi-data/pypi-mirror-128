...
def get_prices(ticker,fromdate,todate,adj=False,retry_count=3,pause=1):
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving failed from Sina for",ticker)
        return None 
    else:
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving failed from stooq for",ticker)
        return None                 
...
def get_index_fred(ticker,start,end):
    """
    功能：替代新浪/stooq不能用的临时解决方案，获取标普500、道琼斯等市场指数
    """
    ...
    if ticker in yahoolist:
        ...
        try:
            ...
            print("  #Warning(get_index_fred): connection failed, trying hard to recover ...")
            import time
            time.sleep(5) # 暂停 5秒
            try:
                df = web.DataReader([id], 'fred', start, end)
