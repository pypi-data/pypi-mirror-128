...
def prepare_fin_stmt_ak(ticker,start,end,period_type='all'):
    """
    ...
    if not valid:
        print("  #Warning(prepare_fin_stmt_ak): invalid period",start,end)
        return None

    #是否中国股票
    result,prefix,suffix=split_prefix_suffix(ticker)
    if not (suffix in ['SS','SZ']):
        print("  #Error(prepare_fin_stmt_ak): not a stock in China",ticker)
        return None        
    ...
    try:
        ...
        print("  #Error(prepare_fin_stmt_ak): no income stmt available for",ticker)
        return None        
    
    #若无资产负债表，则返回
    if len(fbs)==0:
        print("  #Warning(prepare_fin_stmt_ak): no balance statement available for",ticker)
        return None        
    ...
    #合成：左连接
    fs1=pd.merge(fbs1,fis1,how='left',left_index=True,right_index=True)
    fs2=pd.merge(fs1,fcf1,how='left',left_index=True,right_index=True)

    #删除全为空、全为0的列
    nan_col_list=[]
    for i in fs2.columns:
        if fs2[i].count() == 0:
            nan_col_list=nan_col_list+[i]
    fs2.drop(labels=nan_col_list, axis=1, inplace=True)
    ...
    #发现因合并产生的重复列
    dup_col_list=[]
    for c in fs3.columns:
        if ('_x' in c) or ('_y' in c): 
            dup_col_list=dup_col_list+[c]
    fs3.drop(labels= dup_col_list, axis=1, inplace=True)
    ...
    for i in fs3.columns:
        fs3i=fs3[fs3[i]=='0']
        if len(fs3i) == len(fs3):
            zero_col_list=zero_col_list+[i]
    fs3.drop(labels=zero_col_list, axis=1, inplace=True)

    #过滤日期
    fs3a=fs3[fs3.index >=start1]
    fs4=fs3a[fs3a.index <=end1]
    
    #区分年报季报
    fs4['Year']=fs4.index.year
    fs4['Month']=fs4.index.month
    period_type=period_type.lower()
    if period_type=='annual':
        fs5=fs4[fs4['Month']==12]
    else:
        fs5=fs4
    
    num5=len(fs5)
    if num5==0:
        print("  #Warning(prepare_fin_stmt_ak): zero reports found for",ticker,'from',start,'to',end)
        return None
    else:
        print("  Successfully retrieved",num5,"reports for",ticker)
    
    return fs5

if __name__=='__main__':
    prepare_fin_stmt_ak('600519.SS','2018-1-1','2020-12-31')    
    prepare_fin_stmt_ak('600519.SS','2010-1-1','2020-12-31',period_type='annual')
