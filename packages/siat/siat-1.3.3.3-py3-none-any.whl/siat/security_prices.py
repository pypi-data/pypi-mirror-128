...
def get_prices(ticker,fromdate,todate,adj=False,retry_count=3,pause=1):
    ...
    pause：每次重试前的间隔秒数
    """
    print("  Searching security prices, it may take time depending on network performance ...")
    ...
    #尝试AkShare+Sina（对中国内地股票、港股和美股有效，但不包括国外市场指数）
    #printmsg=str(ticker)+" from "+fromdate+' to '+todate
    print("  Trying to capture info from Sina for",ticker)
    try:
        #prices=get_prices_ak(ticker,fromdate,todate,adjust=adj)
        prices=get_prices_ak(ticker,fromdate,todate)
    except:
        print("  #Warning(get_prices): info retrieving failed from Sina for",ticker)
        #return None 
    else:
        if prices is None: 
            print("  #Warning(get_prices): info not found from sina for",ticker)
        else:
            num=len(prices)
            if num==0:
    ...
    try:
        ...
        if prices is None: 
            print("  #Warning(get_prices): info not found fromstooq for",ticker)
        else:
            num=len(prices)
            if num==0:
                print("  #Warning(get_prices): zero record found for",ticker)
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving using non-threads failed from yahoo for",ticker)
    else:
        if prices is None: 
    ...
    try:
        ...
        print("  #Warning(get_prices): price retrieving using threads failed from yahoo for",ticker)
    else:
        if prices is None: 
            print("  #Warning(get_prices): info not found using non-threads failed from yahoo for",ticker)
        else:
...
def cvt_stooq_suffix(symbol):
    """
    映射新浪/stooq后缀符号至stooq后缀符号
    输入：新浪/stooq后缀符号。输出：stooq后缀符号
    """
    import pandas as pd
    suffix=pd.DataFrame([
        ['SS','CN'], ['SZ','CN'], ['T','JP'], 
...
def cvt_stooq_symbol(symbol):
    """
    映射新浪/stooq证券符号至stooq证券符号
    输入：新浪/stooq证券符号。输出：stooq证券符号
    """
    import pandas as pd
    suffix=pd.DataFrame([
        ['^GSPC','^SPX'], ['^IXIC','^NDQ'], ['000001.SS','^SHC'],  
...
def get_price_stooq(ticker,start,end):
    ...
    try:
        ...
        print("  #Error(get_price_stooq): inaccessible for",ticker,"from",start,'to',end)
        return None
...
def get_portfolio_prices(portfolio,fromdate,todate,adj=False):
    ...
    #抓取股票价格
    print("  Searching prices for",tickerlist,'from',fromdate,'to',todate)
    p=get_prices(tickerlist,fromdate,todate,adj=adj)
    if p is None:
        print("  #Error(get_portfolio_prices): information inaccessible for",tickerlist)
        return None  

    print("  Retrieved",len(p),'records of portfolio prices')
    import pandas as pd
    if len(sharelist) > 0:    
        #计算投资组合的开盘价
