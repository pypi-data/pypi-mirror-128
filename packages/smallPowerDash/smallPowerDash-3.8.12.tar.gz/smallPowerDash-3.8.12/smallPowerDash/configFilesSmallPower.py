...
class SmallPowerMaster():
    ...
    def getModeHub(self,timeRange_Window):
        ...
        if isinstance(timeRange_Window,list) :
            dfmodeHUB = self.DF_loadTimeRangeTags(timeRange_Window,[modeSystem],rs='raw')
        else :
            dfmodeHUB   = self.realtimeTagsDF([modeSystem],timeWindow=timeRange_Window,rs='raw',**kwargs)
        ...
        dfmodeHUB['mode hub']=dfmodeHUB['value'].apply(lambda x:dictmode[x])
        return dfmodeHUB
