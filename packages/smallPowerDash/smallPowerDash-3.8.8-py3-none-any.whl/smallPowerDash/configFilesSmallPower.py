...
class SmallPowerMaster():
    ...
    def fuiteAir(self,timeRange_Window,**kwargs):
        ...
        dfFuitesAir=pd.concat([df,dfCalculated])
    ...
    def fuitesFuel(self,timeRange_Window,**kwargs):
        ...
        vannes = [vanne26,vanne27,vanne29,vanne35,vanne40]
        Istacks = self.getTagsTU('STK.*IT.*HM05')

        fuel303=self.getTagsTU('l303.*FT')[0]
        fuel41=self.getTagsTU('l041.*FT')[0]
        fuel32=self.getTagsTU('l032.*FT')[0]
        fuel25=self.getTagsTU('l025.*FT')[0]
        debits =[fuel303,fuel41,fuel32,fuel25]
        listTags = vannes+Istacks+debits
        ...
        # Boucle fermée = L035 ECV2 fermée, L027 H2 ouverte, L029 H2 fermée, L026 H2 ECV2 fermée
        # Boucle ouverte = L035 ouverte OU L027 H2 fermée OU L029 H2 ouverte OU L026 H2 ouverte
        dfFuel = pd.DataFrame()
        dfFuel['modeBF'] = (1-df[vanne40])*df[vanne27]*(1-df[vanne29])*(1-df[vanne26])

        # BF = L303 + L041 + Somme i x 25 / 2F
        dfFuel['fuiteBF'] = df[fuel32] + df[fuel41] + PH2Nlmin
        dfFuel['fuel025'] = df[fuel25]
        # Vérif débitmètres ligne fuel BF = L032 FT – L303 – L025
        verifDebitmetre = df[fuel32]-df[fuel303]-df[fuel25]

        fuiteFuelBF = dfFuel.apply(lambda x: x[1] if x[0]==True else 0,axis=1)
        # BO = BF – L025
        fuiteFuelBO = dfFuel.apply(lambda x: x[1]-x[2] if x[0]==False else 0,axis=1)
        ...
        newUnits=['A','mol/s','Nl/min','Nl/min','Nl/min','Nl/min','ETAT Pile BF']
        unitGroups={t:self.getUnitofTag(t) for t in df.columns}
        unitGroups.update({t:v for t,v in zip(dfCalculated.columns,newUnits)})
        dfFuitesFuel=pd.concat([df,dfCalculated])
