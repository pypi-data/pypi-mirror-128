...
class BoostingMachine:
    ...
    def _import_model(self, input_file, input_type):
        ...
        out_ = libsnapml.booster_import(input_file, input_type)
    ...
    def compress_trees(self, X=None):
        ...
        if X is not None:
            ...
            if type(X).__name__ != "ndarray":
                ...
                Xsample = X[0 : min(Lmax, X.shape[0])]
                num_ex = Xsample.shape[0]
                num_ft = Xsample.shape[1]
                data = np.ascontiguousarray(
                    Xsample, dtype=np.float32
                )  # enforce row-major format
