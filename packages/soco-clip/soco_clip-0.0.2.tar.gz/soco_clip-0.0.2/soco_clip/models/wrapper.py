...
class CustomCLIPWrapper(CLIPWrapper):
    def __init__(self,
        ...
        with open('soco_clip/models/configs/RN.yaml') as fin:
        #with open('soco_clip/models/configs/ViT.yaml') as fin:
        ...
        self.change_text_emb = nn.Linear(768,512)
    ...
    def training_step(self, train_batch, idx):
        ...
        with torch.no_grad():
            ...
            txt = [F.normalize(self.change_text_emb(self.encode_text(t)), dim=1) for t in text_mbs]
            # gather from all GPUs
            ims = self.all_gather(torch.cat(ims))
            txt = self.all_gather(torch.cat(txt))
            ...
            teacher_txt = [F.normalize(self.change_text_emb(self.encode_text(t, teacher=True)), dim=1) for t in text_mbs]
        ...
        for j, mb in enumerate(text_mbs):
            text_tmp = copy.deepcopy(txt)
            text_tmp[self.global_rank][j*self.minibatch_size:(j+1)*self.minibatch_size] = F.normalize(self.change_text_emb(self.encode_text(mb)), dim=1)
    ...
    def forward(self, images, text):
        #logits = self.image_encoder.encode_image(images) @ F.normalize(self.change_text_emb(self.encode_text(text)), dim=1).t() * self.model.logit_scale.exp()
        logits = F.normalize(self.image_encoder.encode_image(images), dim=1) @ F.normalize(self.change_text_emb(self.encode_text(text)), dim=1).t() * self.model.logit_scale.exp()
