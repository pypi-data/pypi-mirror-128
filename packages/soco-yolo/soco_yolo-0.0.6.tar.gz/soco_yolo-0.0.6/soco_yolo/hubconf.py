...
"""
PyTorch Hub yolo_models https://pytorch.org/hub/ultralytics_yolov5/
Usage:
	import torch
	model = torch.hub.load('ultralytics/models', 'yolov5s')
"""
...
def _create(name, pretrained=True, channels=3, classes=80, autoshape=True, verbose=True, device=None):
	...
	import os
	from soco_yolo.yolo import Model
	from soco_yolo.experimental import attempt_load
	from soco_yolo.utils.general import check_requirements, set_logging
	from soco_yolo.utils.downloads import attempt_download
	from soco_yolo.utils.torch_utils import select_device
	from soco_yolo.config import EnvVar

	check_requirements(exclude=('tensorboard', 'thop', 'opencv-python'))
	set_logging(verbose=verbose)

	# save_dir = Path('') if str(name).endswith('.pt') else file.parent
	root_dir = os.path.dirname(os.path.abspath(__file__)).replace('soco_yolo', EnvVar.MODEL_DIR)
	save_dir = Path(root_dir).resolve()
	path = (save_dir / name).with_suffix('.pt')  # checkpoint path
	try:
		device = EnvVar.DEVICE
                print (classes)
		if pretrained and channels == 3 and classes == 80:
			#model = attempt_load(path, map_location=device)  # download/load FP32 model
			model = attempt_load(name, map_location=device)  # download/load FP32 model
		else:
			cfg = list((Path(__file__).parent / 'yolo_models').rglob(f'{name}.yaml'))[0]  # model.yaml path
