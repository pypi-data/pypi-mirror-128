...
from models.utils.downloads import gsutil_getsize
from models.utils.metrics import box_iou, fitness
...
def xyxy2xywhn(x, w=640, h=640, clip=False, eps=0.0):
    ...
    if clip:
        clip_coords(x, (h - eps, w - eps))  # warning: inplace itc
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
...
def non_max_suppression(prediction, conf_thres=0.01, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_det=300):
