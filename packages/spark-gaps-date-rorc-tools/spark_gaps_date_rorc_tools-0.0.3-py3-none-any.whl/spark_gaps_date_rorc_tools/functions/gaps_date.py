...
def show_gaps_date(spark=None, path_name=None, hdfs_uri=None,
                   filter_date_initial="202101",
                   filter_date_final="202112"):
    if not path_name:
        raise Exception(f'require file .yaml: {path_name} ')
    ...
    data = load_config(path_name=f"{path_name}")
    conf_table_job_dict = data["conf-rorc"]
    ...
    for ind, table in enumerate(table_list):
        table_cdd_dict = data["conf-rorc"][table]
        ...
        for index, table_supplies_path in enumerate(table_supplies_list):
            ...
            try:
                ...
                for hdfs_file in hdfs_files:
                    supplies_input_dict = dict(table_rorc="", table_rorc_path="",
                    ...
                    if len(table_supplies_partition) == 2:
                        ...
                        supplies_input_dict["table_rorc"] = table
                        supplies_input_dict["table_rorc_path"] = table_rorc_path
                ...
                df2 = df.groupby(["table_rorc", "table_rorc_path", "table_supplies_path", "year_month"]) \
    ...
    df_pivot = pd.pivot_table(data=df_detail2,
                              index=["table_rorc", "table_rorc_path", "table_supplies_path"],
                              columns="year_month",
