...
class ConfigHandler:
    ...
    def load_config_from_file(self, loader=yaml.safe_load):
        ...
        try:
            ...
            raise ConfigException("error reading config file {}: {}".format(self.config_file_path, e))
        if not isinstance(conf, dict) or "type" not in conf:
            raise ConfigException(
                "error reading config file {}: could not identify a 'type'".format(self.config_file_path)
            )
        class_ = self.get_config_class(conf["type"])
        if class_ is None:
            raise ConfigException(
                "error reading config file {}: invalid value for type: '{}'".format(self.config_file_path, conf["type"])
            )

        valid, error = class_.is_valid_dict(conf)
        if not valid:
            raise ConfigException("{} config file {} not valid: {}.".format(conf["type"], self.config_file_path, error))
    ...
    def write(self, writer=partial(yaml.safe_dump, default_flow_style=False)):
        ...
        if not os.path.isdir(dir_):
            self._log("creating directory to write {} config file: {}".format(self.config.type, dir_))
            try:
                os.makedirs(dir_)
            except OSError as e:
                raise ConfigException(
        ...
        try:
            with open(self.config_file_path, "w") as f:
                self._log("writing {} config file to disk at {}".format(self.config.type, self.config_file_path))
                writer(self.config.to_dict(), f)
        except (yaml.YAMLError, IOError) as e:
            raise ConfigException(
                "Could not write {} config file to disk at {}: {}".format(self.config.type, self.config_file_path, e)
            )
