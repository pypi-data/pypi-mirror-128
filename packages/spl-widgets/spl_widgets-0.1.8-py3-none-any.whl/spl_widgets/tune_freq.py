...
def tune_cols(filepath, interval, scale):
    tune_freqs=True
    ...
    args=['Tuning Parameters:', '*'*20, f'Interval: {10*interval}ms (setting: {interval})', f'Scale: {num_scale_to_strs(scale)}']
    with open(f'{out_dir_filepath}/params.txt','w') as writer: writer.write('\n'.join(args))

    run(['open', out_dir_filepath], capture_output=True)
    return