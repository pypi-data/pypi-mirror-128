...
class StdFindBoards(QWidget):
    ...
    def is_stduino_plat(self):
        try:
            ...
            for i in range(0,len(stdinit.stduino_platforms)):
                if stdinit.pio_boards[self.board.currentIndex()]['platform']==stdinit.stduino_platforms[i]["name"]:
