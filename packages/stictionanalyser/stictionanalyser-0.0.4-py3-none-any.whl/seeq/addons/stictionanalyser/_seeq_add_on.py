...
class HamburgerMenu(v.Menu):
    def __init__(self, **kwargs):
        ...
        self.help_button = v.ListItem(value='help',
                                      ripple=True,
                                      href='mailto: applied.research@seeq.com?subject=Stiction Analyser Feedback',
...
def seeq_add_on(sdl_notebook_url):
    ...
    def on_click(widget, event, data):
        ...
        if select_error.v_model=='' or select_op.v_model=='':
            btn_continue.loading=False
            on_click_open_snackbar_no_signals()
        else:
            overall_results,overall_results_oscillation=ExecutePullSignal(df)
            if overall_results==[] or overall_results_oscillation==[]:
                on_click_open_snackbar()
            btn_continue.loading=False
            #Reset disable 
    ...
    def on_click_close_snackbar_signals(widget, event, data):
        no_signals.v_model = False
        
    def on_click_close_snackbar_events(widget, event, data):
        no_oscillation.v_model = False

    def on_click_open_snackbar():
        no_oscillation.v_model = True

    def on_click_open_snackbar_no_signals():
        no_signals.v_model=True
    ...
    global dataframe_empty

    #pre-defined UI componetnts
    container_3=CustomData()
    # Snackbar
    close_snackbar_signals = v.Btn(color='white', icon=True, children=[v.Icon(children=['mdi-window-close'])])
    close_snackbar_signals.on_event('click',on_click_close_snackbar_signals)

    no_signals = v.Snackbar(absolute=True,timeout=2500,v_model=False, color='#BF360C',
                                        children=['No Signals to Pull.',
                                                close_snackbar_signals])  
    ...
    appBar = v.AppBar(
        ...
        dense=True,
        dark=True,
        children=[v.ToolbarTitle(children=['Stiction Analyser']),
                    v.Spacer(),
    ...
    select_capsule=v.Select(v_on='tooltip.on',items=items_cond,class_='mt-3',disabled=False,label='Select Condition',v_model='')

    #Tooltip
    tooltip=v.Tooltip(top=True, v_slots=[{
        'name': 'activator',
        'variable': 'tooltip',
        'children': select_capsule,
    }], children=['Optional'])

    # Snackbar
    close_snackbar = v.Btn(color='white', icon=True, children=[v.Icon(children=['mdi-window-close'])])
    close_snackbar.on_event('click',on_click_close_snackbar_events)

    no_oscillation = v.Snackbar(absolute=True,timeout=2500,v_model=False, color='#BF360C',
                                        children=['No Oscillation found.',
                                                close_snackbar]) 
    ...
    #Card
    container_results=v.Card(class_='',color='grey lighten-3',height='100%',children=[no_oscillation,no_signals,
