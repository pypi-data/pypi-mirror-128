...
class FieldDeserializer:
    ...
    def get_deserialized_value(self):
        ...
        if is_dataclass(field_type):
            return self.deserialize_dataclass()
        elif type(field_type) == EnumMeta:
            return self.deserialize_enum()
        elif field_type in (re.Pattern, Pattern):
            return self.deserialize_pattern()
        elif type(field_type) == _UnionGenericAlias:
            return self.deserialize_union()
        elif type(field_type) in (_GenericAlias, GenericAlias):
            builtin_type = str(field_type).split("[")[0].lower()

            if builtin_type == "dict":
                return self.deserialize_dict()
            elif builtin_type == "frozenset":
                return self.deserialize_frozenset()
            elif builtin_type == "list":
                return self.deserialize_list()
            elif builtin_type == "set":
                return self.deserialize_set()
            elif builtin_type == "tuple":
                return self.deserialize_tuple()

        return value

    def deserialize_item(self, item: Any, dc: dataclass) -> Any:
    ...
    def deserialize_dict(self) -> None:
    ...
    def deserialize_frozenset(self) -> None:
    ...
    def deserialize_pattern(self) -> None:
        """Retruns a compiled regex pattern with value as the expression."""
        return re.compile(rf"{self.value}")

    def deserialize_set(self) -> None:
        """Returns a set with resolved element types."""
        return set(self.deserialize_list())

    def deserialize_tuple(self) -> None:
