...
def save_l_svg(domTree, transed_points,indx_zi,color_zi, hz_zi, mode='random',file_path=None,n=10):
    '''
    :param domTree:
...
def main(svgpath=None,sample_n=10,filetype='path',c_svg=None,csvg_filetype='path',arch_per=0.3,pos='up',fix='top', mode='arch',file_path = None):
    '''
    ...
    if 'arch' in mode:
        ...
        s_points, target_tensor = simpleget_points_insvg(c_svg,filetype=csvg_filetype)
        transed_points = trans_points_anycon(read_points, target_tensor, s_points, scale_sf=1.01)
    else:
        raise ValueError("mode not in 'arch' 'single_arch' 'polygon' 'circle','random'")
    trt = time.time()
    # print('transed time:',trt-rt)
    svg, xml = save_l_svg(domTree,transed_points, bh_indx, bh_color, bh_hz, mode=mode, file_path=file_path)
    sat = time.time()
...
if __name__ == "__main__":
    ...
    svg,xml = main(svgpath=r"C:\Users\25790\Downloads\1637228907.184102923296_path.svg",c_svg='https://oss.looodesign.com/model_test/bg/templateSvg621_2.svg',file_path=r'C:\Users\25790\Downloads\result.svg', filetype='path',csvg_filetype='url',mode='arch')
    # print(time.time()-st1)
    # print(xml)
