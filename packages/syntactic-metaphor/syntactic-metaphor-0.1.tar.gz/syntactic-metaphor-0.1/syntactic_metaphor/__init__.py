...
def pirmyn():
    ...
    if app.text.index('%s lineend' % position) == position:
        ...
        app.text.mark_set(INSERT, '%d.%d' % (line, offset + 1))
    app.store_cursor()
...
def pažymėta(kas):
    app.ensure()
    return app.text.selection_get() == kas
...
def kopijuoti():
    app.ensure()
    app.clipboard = app.text.selection_get()
...
def pažymėta_spaudos_ženklų(kiek):
    app.ensure()
    return len(app.text.selection_get()) == kiek

def pažymėtas_tekstas_prasideda(kuo):
    app.ensure()
    return app.text.selection_get().startswith(kuo)

def pažymėtas_tekstas_baigiasi(kuo):
    app.ensure()
    return app.text.selection_get().endswith(kuo)

def pažymėtame_tekste_yra(kas):
    app.ensure()
    return kas in app.text.selection_get()

def pažymėtas_tekstas_yra_tekste(kokiame):
    app.ensure()
    return app.text.selection_get() in kokiame

def pažymėtas_tekstas_lygus_nukopijuotam():
    app.ensure()
    return app.text.selection_get() == app.clipboard
