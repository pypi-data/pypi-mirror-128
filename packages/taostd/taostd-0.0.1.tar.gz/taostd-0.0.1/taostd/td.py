...
def get(sql: str):
    """ execute select SQL and return unique result.
        ...
        or
        select ts from meters where tag = 'xxx' and ts = 'xxx' limit 1
        :return: only value
    """
    result = _query(sql)
    try:
...
def select_one(sql: str):
    """ execute select SQL and return unique result.
        select * from meters where tag = 'xxx' and ts = 'xxx' limit 1;
        :return: {k:v}
    """
    result = _select(sql)
    if result:
...
def insert_one(table: str, **kwargs):
    """
    往已存在的表里插入一行数据，时间可以是字符串，也可以是datetime
    ...
    if cache_stable is None:
        raise TDError(f"'{table}'表还未创建，请使用insert_one_with_stable方法插入.")
...
def insert_one_with_stable(table: str, stable: str, **kwargs):
    """
    往表插入一行数据，如果表不存在，就自动建表，时间可以是字符串，也可以是datetime
    :param table: table name
    ...
    if cache_stable is None:
        ...
        raise TDError(f"存在的超级表是'{cache_stable}'，而传入的是'{stable}', 请检查是否正确.")
    else:
        return _insert_one(table, cache_stable, **kwargs)


@with_connection
def insert_many(table: str, args: list, batch_size=1000):
    """
    时间可以是字符串，也可以是datetime
    :param table:
    :param args: [{"ts": xxx, ...}, {}]
    :return:
    """
    cache_stable = _get_table_cache(table)
    if cache_stable is None:
        raise TDError(f"'{table}'表还未创建，请使用insert_many_rows_with_stable方法插入.")

    return _insert_many(table, cache_stable, args, batch_size=batch_size)


@with_connection
def insert_many_with_stable(table: str, stable: str, args: list, batch_size=1000):
    """时间只能是datetime"""
    cache_stable = _get_table_cache(table)
...
@with_connection
def insert_many_tables(args: list, batch_size=1000):
    """
    时间可以是字符串，也可以是datetime
...
def _init_table_cache():
    ...
    for stbl in stables:
        desc = _select(f"describe {stbl['name']}")
        desc = [_del_field_length(f) for f in desc]
        cache.set(stbl['name'], {'columns': stbl['columns'], 'tags': stbl['tags'], 'fields': desc})
...
def _get_stable_cache(stable: str):
    ...
    raise TDError(f"请检查超级表'{stable}'是否存在.")
...
def _insert_many_tables_with_stable(args: list):
    ...
    for arg in args:
        ...
        if cache_stable:
            ...
            raise TDError(f"'{table}'表还未创建, 请指定stable")
