...
from tecton._internals.errors import FV_INVALID_ARG_COMBO
...
from tecton_proto.args.feature_package_pb2 import FeatureAggregation as FeatureAggregationProto
    ...
    def run(
        ...
        **mock_inputs: DataFrame,
        ...
        :param aggregation_level: This only works with aggregate FeatureView types. Select the level of aggregation
            ...
            1) "full" - Fully aggregate the features. The output rows for each of the time_windows specified in
            FeatureAggregation(s) under the FeatureView config will be aggregated together.

            2) "tile" - Aggregate the output rows under the same fix-size sliding aggregate window. The size of the
            aggregate window is specified as aggregation_slide_period in the FeatureView config.

            3) None - No aggregation operation performed.
        ...
        if aggregate_tiles is not None:
            if aggregation_level:
                raise FV_INVALID_ARG_COMBO(["aggregate_tiles", "aggregation_level"])
            else:
                logger.warning("aggregate_tiles is deprecated - please use aggregation_level instead.")
            aggregation_level = "tile"

        if aggregation_level and not self._args.HasField("temporal_aggregate_args"):
            raise FV_UNSUPPORTED_ARG("aggregate_tiles, aggregation_level")
