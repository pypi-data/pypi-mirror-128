...
graph = tfg.Graph(
    ...
    edge_index=np.random.randint(0, num_nodes, [2, 1008]),  # 4 undirected edges
    # edge_weight=[
)
...
adj = SparseAdj(graph.edge_index, graph.edge_weight, shape=[graph.num_nodes, graph.num_nodes])
adj.index = np.array(adj.index)
adj.value = np.array(adj.value)
...
forward_ = forward.get_concrete_function(graph.x)
