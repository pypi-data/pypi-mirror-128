...
class PrometheusMiddleware(metaclass=SingletonMeta):
    ...
    def add_fastapi_middleware(self):
        ...
        def fastapi_after_request(response, request):
            ...
            request_path = request.scope.get("raw_path")
            request_path = (
                request_path.decode("utf-8") if request_path else request.url.path
            )

            path_format = get_path_format(request.app.routes, request_path)
