...
def monitor_project(task_id, silent=False):
    ...
    task_id : str
        Task identification string.
    silent: bool
        Set to ``True`` to suppress display of current status.
    """
    status = ""
...
def download_results_file(task_id, src, target=None):
    ...
    try:
        client = get_s3_client()
        bucket = Config.STUDIO_BUCKET
        key = "users/{0}/{1}/{2}".format(keys["UserId"], task_id, src)
...
def download_results(task_id, target_folder=""):
    ...
    for fname in ["monitor_data.hdf5", "tidy3d.log"]:
        download_results_file(
            task_id, src=os.path.join("output", fname), target=os.path.join(target_folder, fname)
        )
    download_results_file(
        task_id, "simulation.json", target=os.path.join(target_folder, "simulation.json")
    )
