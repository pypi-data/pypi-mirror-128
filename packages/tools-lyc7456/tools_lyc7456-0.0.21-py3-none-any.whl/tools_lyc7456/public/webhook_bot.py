...
class Webhook_bot:
    ...
    def send_webhook_msg(self, content_text):
        ...
        if self.webhook_type == 'wechat':
            webhook_api_url="https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key={}".format(self.token)
        elif self.webhook_type == 'dingding':
            webhook_api_url="https://oapi.dingtalk.com/robot/send?access_token={}".format(self.token)
        else:
