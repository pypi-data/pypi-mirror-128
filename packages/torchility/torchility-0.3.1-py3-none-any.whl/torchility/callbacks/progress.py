...
class SimpleBar(ProgressBarBase, ProgressMix):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        progress = f"E:{c_epoch:>3d}/{num_epoch:<3d} B:{c_batch:>4d}/{num_batch:<4d}"
        stage = 'TRAIN >'
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        progress = f"E:{train_c_epoch:>3d}/{train_num_epoch:<3d} B:{train_num_batch:>4d} {val_num_batch:<4d}"
        train_stage = 'TRAIN >'
...
class Progress(Callback, ProgressMix):
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        val_info = {
            metric: [e['val_epoch'][metric] for e in self.progress_info]
            for metric in metrics
        }
