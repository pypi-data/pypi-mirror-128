...
class UCUCCluster(ChangeLoggedModel):
    UCUCCluster = models.CharField(max_length=200)
    description = models.CharField(max_length=200)
    tags = TaggableManager(through=TaggedItem)
    tenant = models.ForeignKey(
        to='tenancy.Tenant',
    ...
    csv_headers = ['UCUCCluster', 'tenant', 'description', 'publisher', 'subscriber1', 'subscriber2', 'subscriber3', 'subscriber4', 'tftp1', 'tftp2']

    def __str__(self):
        return str(self.UCUCCluster)

    def get_absolute_url(self):
        return reverse("plugins:ucbox_plugin:UCUCCluster_view", kwargs={"pk": self.pk})

    class Meta:
        unique_together = ("UCUCCluster", "tenant",)#ermoe enant here
...
class DevicePool(ChangeLoggedModel):
    ...
    csv_headers = ['devicepool', 'tenant', 'description', 'region', 'datetimegroup', 'localroutegroup']
