...
class UCUCClusterTable(BaseTable):

    pk = ToggleColumn()
    UCUCCluster = tables.LinkColumn()
    tenant = tables.LinkColumn()
    ...
    class Meta(BaseTable.Meta):
        model = UCUCCluster
        fields = ('pk', 'UCUCCluster', 'tenant', 'description', 'publisher', 'subscriber1', 'subscriber2', 'subscriber3', 'subscriber4', 'tftp1', 'tftp2')
...
class DevicePoolTable(BaseTable):
    ...
    class Meta(BaseTable.Meta):
        model = DevicePool
        fields = ('pk', 'devicepool', 'tenant', 'description', 'region', 'datetimegroup', 'localroutegroup')
