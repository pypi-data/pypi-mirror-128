...
class UCUCClusterFilterForm(BootstrapMixin, forms.Form):
...
class UCUCClusterEditForm(BootstrapMixin, forms.ModelForm):

    UCUCCluster = forms.CharField(
        required=True,
        widget=forms.TextInput(
            attrs={
                'class': 'form-control',
                'autocomplete': 'off',
                'pattern': r'^\+?[0-9A-D\*\#]+$',
                'title': 'Enter the UCUCCluster Name'
            }
        )
    )
    tags = DynamicModelMultipleChoiceField(
    ...
    class Meta:
        model = UCUCCluster
        fields = ('UCUCCluster', 'tenant', 'description', 'publisher', 'subscriber1', 'subscriber2', 'subscriber3', 'subscriber4', 'tftp1', 'tftp2')


class UCUCClusterBulkEditForm(BootstrapMixin, AddRemoveTagsForm, BulkEditForm):

    pk = forms.ModelMultipleChoiceField(
        queryset=UCUCCluster.objects.all(),
        widget=forms.MultipleHiddenInput()
    )
    tenant = DynamicModelChoiceField(
        queryset=Tenant.objects.all(),
...
class UCUCClusterCSVForm(CSVModelForm):
    tenant = CSVModelChoiceField(
        queryset=Tenant.objects.all(),
        required=True,
        to_field_name='name',
    ...
    class Meta:
        model = UCUCCluster
        fields = UCUCCluster.csv_headers
...
class DevicePoolEditForm(BootstrapMixin, forms.ModelForm):
    ...
    class Meta:
        model = DevicePool
        fields = ('devicepool', 'tenant', 'region', 'description', 'datetimegroup', 'localroutegroup', 'tags')
...
class DevicePoolBulkEditForm(BootstrapMixin, AddRemoveTagsForm, BulkEditForm):
    ...
    class Meta:
        nullable_fields = ('region', 'description', 'datetimegroup', 'localroutegroup')
