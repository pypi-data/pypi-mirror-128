...
def add_telegram_bot(path, api_hash, api_id):
    """Used to add a bot api info to the database so it can be distributed"""
    try:
        conn = sqlite3.connect(path)
        cur = conn.cursor()
        cur.execute("INSERT INTO telegram_bots(api_hash, api_id) VALUES(?,?)", (api_hash, api_id))
        conn.commit()
    except sqlite3.IntegrityError as e:
        print(e)
...
def list_telegram_bots(path):
    ...
    try:
        conn = sqlite3.connect(path)
        cur = conn.cursor()
        data = cur.execute("SELECT api_hash, api_id FROM telegram_bots")
        return data.fetchall()
    except sqlite3.DataError as e:
        print(e)
