...
def fromjwk(jwk: dict) -> Union[PrivateKey, PublicKey]:
    """Parse the JSON Web Key `jwk` and return the corresponding
    key instance.
    """
    kty = jwk.get('kty')
    if kty != 'RSA':
    ...
    return RSAPublicKey.fromjwk(DTO.fromdict(jwk))
...
def frompem(
    ...
    """
    key = None
    if bytes.startswith(content, b'-----'):
        if b'RSA PRIVATE KEY' in content:
            from .pkcs import RSAPrivateKey
            key = RSAPrivateKey({'content': content})
    if key is None:
        raise NotImplementedError("Unsupported key format.")
    if not key.is_public() and public:
