...
class ECKeyTrustTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.t1 = jose.jwt.sync({'iss': 'self'}, signer=self.s1)
        self.t2 = jose.jwt.sync({'iss': 'self'}, signer=self.s2)
        self.t3 = jose.jwt.sync({'iss': 'self'}, signer=self.s3)
        self.t4 = jose.jwt.sync({'iss': 'self'}, signer=self.s4)
