...
class UserMetricCollector(Collector):
    ...
    def _read_distributed_info(self) -> Dict[str, int]:
        distributed_info = {}
        distributed_number = safe_cast(
            os.environ.get("VESSL_DISTRIBUTED_NUMBER", None), int
        )
    ...
    def build_metric_payload(self, payload: Dict[str, Any]):
        ts = time.time()
        fields = payload
    ...
    def build_image_payload(self, payload: Dict[str, Any]):
        ts = time.time()
        fields = payload
        tags = {"step": str(self.step)}
    ...
    def log_images(self, payloads: List[ExperimentMetricEntry]) -> int:
        self.add(payloads)
        return self.step
