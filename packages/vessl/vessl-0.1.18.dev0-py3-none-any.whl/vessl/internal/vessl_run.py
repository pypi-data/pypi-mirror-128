...
class VesslRun(object):
    ...
    def log(self, payload: Dict[str, Any], step: Optional[int] = None):
        ...
        for k, v in payload.items():
            if isinstance(v, list):
                if all(isinstance(i, Image) for i in v):
                    image_dict[k] = v
                elif all(isinstance(i, Audio) for i in v):
                    audio_dict = v
        ...
        if audio_dict:
            self._update_audios(audio_dict)
    ...
    def _update_audios(self, payload: Dict[str, List[Audio]]):
        path_to_caption = {}
        for audios in payload.values():
            for audio in audios:
                path_to_caption[audio.path] = audio.caption

        source_path = os.path.join(VESSL_AUDIO_PATH, "")
        assert self._experiment
        dest_volume_id = self._experiment.experiment_plot_volume
        dest_path = "/"
