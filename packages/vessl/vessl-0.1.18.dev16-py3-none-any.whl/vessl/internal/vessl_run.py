...
class VesslRun(object):
    ...
    def _update_media(self, payload: Dict[str, Union[List[Image], List[Audio]]]):
        ...
        print(f'  is image:{media_type == Image.__name__}')
        print(f'  is audio:{media_type == Audio.__name__}')

        # Source image path: vessl-media/images/
        # Source audio path: vessl-media/audio/
        if media_type == Image.__name__:
            source_path = os.path.join(VESSL_MEDIA_PATH, VESSL_IMAGE_PATH, "")
        elif media_type == Audio.__name__:
            source_path = os.path.join(VESSL_MEDIA_PATH, VESSL_AUDIO_PATH, "")

        assert self._experiment
        dest_volume_id = self._experiment.experiment_plot_volume
        dest_path = "/" + VESSL_MEDIA_PATH
        ...
        print('path_to_caption')
        for k, v in path_to_caption.items():
            print(f'k:{k}, v: {v}')
        ...
        for f in plot_files:
            ...
            payloads.append(self._user_metric_collector.build_media_payload(payload))
