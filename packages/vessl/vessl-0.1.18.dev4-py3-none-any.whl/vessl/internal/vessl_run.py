...
class VesslRun(object):
    ...
    def _update_media(self, payload: Dict[str, Union[List[Image], List[Audio]]]):
        # Source image path: media/images/
        # Source audio path: media/audios/
        source_path, set_source_path = VESSL_MEDIA_PATH, False
        path_to_caption = {}
        for media in payload.values():
            for medium in media:
                path_to_caption[medium.path] = medium.caption
                if not set_source_path:
                    if type(medium) == Image:
                        source_path = os.path.join(source_path, VESSL_IMAGE_PATH, "")
                        set_source_path = True
                    elif type(medium) == Audio:
                        source_path = os.path.join(source_path, VESSL_AUDIO_PATH, "")
                        set_source_path = True
                    else:
        ...
        for f in plot_files:
            payload = {VESSL_PLOTS_FILETYPE_MEDIA: f}
