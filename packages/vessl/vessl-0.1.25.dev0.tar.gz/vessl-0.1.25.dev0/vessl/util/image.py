...
from vessl.util.constant import VESSL_IMAGE_PATH, VESSL_MEDIA_PATH
...
class Image:
    ...
    def _init_meta(self, caption, mode):
        ...
        image_root = os.path.join(VESSL_MEDIA_PATH, VESSL_IMAGE_PATH)
        os.makedirs(image_root, exist_ok=True)
        self._path = os.path.join(image_root, Image.generate_uuid() + ".png")

    def _init_image_from_path(self, data):
        pil_image = get_module(
            "PIL.Image", required='Pillow package is required.Run "pip install Pillow"'
        )
