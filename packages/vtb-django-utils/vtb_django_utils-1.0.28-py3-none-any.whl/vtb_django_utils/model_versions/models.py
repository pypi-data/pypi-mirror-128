...
from vtb_django_utils.utils.db import CreatedMixin, get_model_fields, get_rel_model_fields
...
from .utils.consts import START_VERSION, THERE_IS_NO_VERSION_DATA, DOES_NOT_EXIST_VERSION, REL_VERSION_FIELD_END, \
    ...
    VERSION_PATTERN_REGEX, VERSION_MODEL_SUFFIX, REL_VERSION_CALCULATED_FIELD_END
...
class ModelWithVersionedRelations(ModelVersionedRelFieldsInfoMixin, ModelJsonMixin, models.Model):
    ...
    def json(self):
        result = super(ModelWithVersionedRelations, self).json()
        for rel_field_name in self.rel_versioned_fields:
            if rel_model_version_str := self.get_rel_model_version_str:
                result[f'{rel_field_name}{REL_VERSION_CALCULATED_FIELD_END}'] = rel_model_version_str
...
def create_version_model_class(
    ...
    class_version = class_factory(module_name, f'{capitalize_name}{VERSION_MODEL_SUFFIX}', (version_parent_model,), {
