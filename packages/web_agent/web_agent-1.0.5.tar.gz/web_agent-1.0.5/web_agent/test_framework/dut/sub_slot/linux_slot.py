...
from test_framework.database.dut_database import update_slot, delete_removed_slot
from tool.device.linux_nvme import LinuxNvme


class LinuxSlot(object):

    def __init__(self, db_slots, agent_name):
    ...
    def refresh(self, device):
        dev_info = self.nvme.get_info(device["ctrl_id"])
    ...
    @delete_removed_slot
    def is_slot_removed(self, slot):
        if slot[0] not in self.live_slots:
            return slot[0]
        else:
            return None

    def delete_removed_slots(self):
        for item in self.db_slots:
            self.is_slot_removed(item)

    def _format_results(self, results, dev):
        format_result = {
            "slot": dev["name"],
            "vendor": results["ssd_config_dic"]["drive_pn"],
            ...
            "cap": results["ssd_config_dic"]["drive_tnvmcap"],
            "bb": "{}/{}/{}".format(results["drive_life_info_dic"]["life_cycle_state"],
                                    results["drive_life_info_dic"]["count_grown_defects"],
                                    results["drive_life_info_dic"]["count_primary_defects"]),
