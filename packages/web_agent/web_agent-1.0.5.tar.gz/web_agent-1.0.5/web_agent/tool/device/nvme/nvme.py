...
from tool.device.nvme.ioctl import IO, ADMIN, IOCTL
from tool.device.nvme.struct import *
from tool.device.nvme.struct.hmb import HMB
...
class Tahoe(NVMe):
    ...
    def check_security(self):
        return self.vu_cmd.get_drive_security_state()
    ...
    def get_info(self):
        ...
        security_type = "ISE" if self.check_security()[2] else "SED"
        security_status = "security enable" if self.check_security()[3] else "security disable"
        int_type = self.check_data(self.check_int_type())
        life_cycle_state = self.check_data(self.check_security()[4])
        count_grown_defects = self.check_data(self.get_vendor_uniquelog()[1][144])
        count_primary_defects = self.check_data(self.get_vendor_uniquelog()[1][152])
        nand_max_erase_count = self.check_data(self.get_vendor_uniquelog()[1][112])
