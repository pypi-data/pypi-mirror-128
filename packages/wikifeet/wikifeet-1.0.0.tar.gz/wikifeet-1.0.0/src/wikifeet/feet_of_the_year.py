...
PATTERN_RANKS_LIST = r"<h2>#(.*?): (.*?)</h2><div class='round8 celebbox'>\s*<div class=boxcont><a href=\"/(.*?)\">"
...
def rank_list():
    url = http('/feetoftheyear')
    if url is not None:
        collection = re.findall(PATTERN_RANKS_LIST, url)
        data_list = []
        for i in collection:
            model_id = i[0]
            model_name = i[1]
            model_user_name = re.sub(r'_| ', '-', i[2])
            model_url = f'https://www.wikifeet.com/{i[2]}'
            data_list.append(
                f"{{\"id\": \"{model_id}\", \"name\": \"{model_name}\", \"username\": \"{model_user_name}\", \"url\": \"{model_url}\"}}")
        return json_object.dumps(data_list)
    else:
        return None
...
class FeetOfTheYear:
    ...
    def __init__(self, model_id: int, index: int = 0):
        """
        Initialize FeetOfTheYear.
        :param model_id: The rank of the model.
        :param index: The position of the model.
        """
        self.__model_id = model_id
        self.__index = index
        self.__model_info = self.__model_info()

    def __rank_info(self):
        try:
            if rank_list() is not None:
                collection = json_object.loads(rank_list())
                data_list = []
                for i in collection:
                    if json(i, 'id') == str(self.__model_id):
                        model_id = json(i, 'id')
                        model_name = json(i, 'name')
                        model_user_name = json(i, 'username')
                        model_url = json(i, 'url')
                        data_list.append(
                            f"{{\"id\": \"{model_id}\", \"name\": \"{model_name}\", \"username\": \"{model_user_name}\", "
                            f"\"url\": \"{model_url}\"}}")
                return json_object.dumps(data_list)
            else:
                return None
    ...
    def __model_info(self):
        global i
        try:
            if self.__rank_info() is not None:
                i = json_object.loads(self.__rank_info())
                if self.__index == 1:
                    self.__index = 0
                elif self.__index == 2:
                    self.__index = 1
                model_id = json(i[self.__index], 'id')
                model_name = json(i[self.__index], 'name')
                model_user_name = json(i[self.__index], 'username')
                model_url = json(i[self.__index], 'url')
                return f"{{\"id\": \"{model_id}\", \"name\": \"{model_name}\", \"username\": \"{model_user_name}\", " \
                       f"\"url\": \"{model_url}\"}} "
        except:
            try:
                model_id = json(i[0], 'id')
                model_name = json(i[0], 'name')
                model_user_name = json(i[0], 'username')
                model_url = json(i[0], 'url')
                return f"{{\"id\": \"{model_id}\", \"name\": \"{model_name}\", \"username\": \"{model_user_name}\", " \
                       f"\"url\": \"{model_url}\"}} "
            except:
                return None
