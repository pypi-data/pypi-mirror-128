...
PATTERN_MODEL_INFO = r".value='(.*?)';parent.location='/' \+ encodeURI\('(.*?)'\)"
...
PATTERN_ADULT = r"WARNING: CONTAINS ADULT CONTENT(.*?)Switch to wikiFeet X"
...
def is_adult_content(url):
    matcher = re.search(PATTERN_ADULT, url)
    if matcher:
        return True
    else:
        return False
...
class AdultContentException(Exception):
    """
    Thrown when the model contains adult content.
    """

    def __init__(self, message):
        super().__init__(message + " Contains Adult Content")


class WikiFeet:
    """
    Gets model by specifying the name when it's initialized.
    """
    ...
    def __model_info(self):
        try:
            url = http(f"/perl/ajax.fpl?req=suggest&gender=undefined&value={quote_plus(self.__model_name)}")
            if url is not None:
                replacement = re.sub(r'\\\\', '', url)
                if replacement is not None:
                    matcher = re.search(PATTERN_MODEL_INFO, replacement)
                    if matcher:
        ...
        if data_url is not None and data_name is not None:
            ...
            if url is not None:
                ...
                if is_adult_content(url):
                    raise AdultContentException(data_name)
    ...
    def model_page(self):
        ...
        :raises AdultContentException: Thrown if it contains adult content.
        """
        data_url = json(self.__model_info, "url")
        data_name = json(self.__model_info, "name")
        if data_url is not None and data_name is not None:
