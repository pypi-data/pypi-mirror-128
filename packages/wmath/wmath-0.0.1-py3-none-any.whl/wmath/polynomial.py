...
class Polynomial:
    ...
    def __init__(self, coefficient: list):
        ...
        for _i in coefficient:
            assert type(_i) == fraction.Fraction
    ...
    def __add__(self, other):
        ...
        _coefficient = [fraction.Fraction(0, 1) for _i in range(_degree + 1)]
    ...
    def __truediv__(self, other):
        ...
        if self.degree < other.degree:
            return Polynomial([fraction.Fraction(0, 1)]), _self
        if other.degree == 0:
            return _self.times(other.coefficient[0]), Polynomial([fraction.Fraction(0, 1)])
    ...
    def monic(self):
        ...
        """
        if self.coefficient[-1] == fraction.Fraction(1, 1) or self.coefficient[-1].moleuole == 0:
            return self
        else:
            for _i in range(self.degree + 1):
                self.coefficient[_i] /= self.coefficient[-1]
            return self
    ...
    def primitive(self):
        ...
        _molecule = number_theory.least_common_multiple_in_list(_denominator_list)
        _denominator = number_theory.greatest_common_divisor_in_list(_molecule_list)
        _times = fraction.Fraction(_molecule, _denominator)
        for _i in range(self.degree + 1):
            self.coefficient[_i] *= _times
        return self

    def times(self, n: fraction.Fraction, degree: int = 0):
        """
        ...
        """
        assert degree >= 0
        _coefficient = [fraction.Fraction(0, 1) for _i in range(degree)]
        for _i in self.coefficient:
    ...
    def rational_roots(self):
        ...
        """
        _self = copy.deepcopy(self).primitive()
        if _self.degree == 0:
            return []
        if _self.degree == 1:
            return [-_self.coefficient[0] / _self.coefficient[1]]
        _molecule_list = number_theory.factor_without_exp(_self.coefficient[0].molecule)
        _denominator_list = number_theory.factor_without_exp(_self.coefficient[-1].molecule)
        _rational_roots = []
        for _molecule in _molecule_list:
            for _denominator in _denominator_list:
                _fraction = fraction.Fraction(_molecule, _denominator)
    ...
    def is_irreducible_according_eisenstein(self):
        ...
        _prime_list = number_theory.factor_without_exp(
            number_theory.greatest_common_divisor_in_list(
                _part_coefficient_list))
        _prime_list.remove(1)
        if _prime_list:
...
def greatest_common_divisor_with_exp_in_polynomial(a: Polynomial, b: Polynomial):
    """
    ...
    if (a % b).degree == 0:
        ...
        return b.monic(), Polynomial([fraction.Fraction(0, 1)]), _y
    else:
        __greatest_common_divisor, __x, __y = greatest_common_divisor_with_exp_in_polynomial(b, a % b)
        _x = __y
        _y = __x - (a // b) * _x
