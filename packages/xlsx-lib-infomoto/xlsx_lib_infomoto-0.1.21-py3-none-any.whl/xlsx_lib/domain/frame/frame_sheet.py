...
from xlsx_lib.domain.xlsx_elements.errors.empty_value_error import EmptyValueError
from xlsx_lib.domain.xlsx_elements.errors.sheet_end_error import SheetEndError
...
class FrameSheet(Sheet):
    ...
    def __init__(
        ...
        new_frame_elements: List[FrameElement] = []

        for element in self.frame_elements:
            if element.name not in [x.name for x in new_frame_elements]:
                new_frame_elements.append(element)

        self.frame_elements = new_frame_elements
