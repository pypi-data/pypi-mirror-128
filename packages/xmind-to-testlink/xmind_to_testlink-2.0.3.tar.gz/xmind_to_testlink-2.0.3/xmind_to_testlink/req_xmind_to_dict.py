...
class XmindToDict(object):
    ...
    def create_case(self, in_data, pre_dict, num, req_spec, req, save_custom = '', repeat_argv = '', is_case = 1, father = '', tf_name = ''):
        ...
        if "title" in in_data.keys():
            ...
            if self.repeat_req == '':
                self.repeat_req = father
            if self.repeat_req != father:
                self.repeat_req = father
                self.case_count = 1
            if father == '':
                case['title'] = req.split('/')[-1] + '_用例' + str(self.case_count)
            else:
                case['title'] = father + '_用例' + str(self.case_count)
            self.case_count += 1
            #case['summary'] = title_msg
            case['summary'] = self._add_summary(in_data)
            new_tf_name = self._get_tf_name(case['summary'])
            if not new_tf_name:
                #print(tf_name)
                #case['summary'] = '<strong>' + tf_name.split(':')[0] + '</strong>'  + '\n' +\
                #        tf_name.split(':')[1] + '\n' + case['summary']
    ...
    def get_dict(self, in_data, out_dict, start = 0, pre_dict = {}, count = 0, req_spec = '', req = '', save_custom = [], repeat_argv = '', father = '', tf_name = ''):
        ...
        elif self.type == "case_req":
            if not self.have_case(in_data) and start == 0:
                return 0
            else:
                self.parse_req_xmind(in_data, out_dict, start, pre_dict, count, req_spec, req, save_custom, father = father)
    ...
    def get_case_req_dict(self, xmind_file):
        if self.type == "case_req":
            ...
            root_name = self.get_root(js_dict)
            root_dict = self.create_suite(root_name)
            self.parse_req_xmind(js_dict[0]['rootTopic'], root_dict)
            return root_dict
